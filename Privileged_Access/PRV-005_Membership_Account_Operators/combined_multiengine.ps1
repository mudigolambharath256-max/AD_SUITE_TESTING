...
Write-Host "=== Multi-Engine Execution: Membership: Account Operators ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        $ldapFilter = '(&(objectCategory=group)(cn=Account Operators))'
...
public class Program
{
  ...
  {
    string filter = @"(&(objectCategory=group)(cn=Account Operators))";
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'PRV-005'
                adSuiteCheckName  = 'Membership_Account_Operators'
    ...
    $bhFile = Join-Path $bhDir "PRV-005_$bhTs.json"
    @{
