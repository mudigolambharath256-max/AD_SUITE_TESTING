...
Write-Host "=== Multi-Engine Execution: Privileged Computers (adminCount=1) ===" -ForegroundColor Cyan
Write-Host ""
...
public class Program
{
  ...
  {
    string filter = @"(&(objectCategory=computer)(adminCount=1))";
    string[] props = new string[] { "name", "distinguishedName", "samAccountName", "adminCount", "operatingSystem" };
...
try {
    ...
    $bhFile = Join-Path $bhDir "PRV-016_$bhTs.json"
    @{
