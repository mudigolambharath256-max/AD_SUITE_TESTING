...
Write-Host "=== Multi-Engine Execution: Membership: Enterprise Key Admins ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        $ldapFilter = '(&(objectCategory=group)(cn=Enterprise Key Admins))'
...
public class Program
{
  ...
  {
    string filter = @"(&(objectCategory=group)(cn=Enterprise Key Admins))";
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'PRV-014'
                adSuiteCheckName  = 'Membership_Enterprise_Key_Admins'
    ...
    $bhFile = Join-Path $bhDir "PRV-014_$bhTs.json"
    @{
