...
Write-Host "=== Multi-Engine Execution: Membership: Cert Publishers ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        $ldapFilter = '(&(objectCategory=group)(cn=Cert Publishers))'
...
public class Program
{
  ...
  {
    string filter = @"(&(objectCategory=group)(cn=Cert Publishers))";
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'PRV-011'
                adSuiteCheckName  = 'Membership_Cert_Publishers'
    ...
    $bhFile = Join-Path $bhDir "PRV-011_$bhTs.json"
    @{
