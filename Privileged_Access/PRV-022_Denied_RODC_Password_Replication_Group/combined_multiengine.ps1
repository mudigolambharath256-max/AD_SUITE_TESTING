...
Write-Host "=== Multi-Engine Execution: Denied RODC Password Replication Group ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        $ldapFilter = '(&(objectCategory=group)(cn=Denied RODC Password Replication Group))'
...
public class Program
{
  ...
  {
    string filter = @"(&(objectCategory=group)(cn=Denied RODC Password Replication Group))";
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'PRV-022'
                adSuiteCheckName  = 'Denied_RODC_Password_Replication_Group'
    ...
    $bhFile = Join-Path $bhDir "PRV-022_$bhTs.json"
    @{
