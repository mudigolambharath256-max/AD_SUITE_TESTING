...
Write-Host "Found $($results.Count) objects for check: PAW Compliance Check" -ForegroundColor Cyan

$results | ForEach-Object {
    $props = $_.Properties
    [PSCustomObject]@{
        CheckID = 'PRV-031'
        CheckName = 'PAW Compliance Check'
...
try {
    ...
    if ($results -and $results.Count -gt 0) {
        ...
        $bhFile = Join-Path $bhDir "PRV-031_nodes.json"
