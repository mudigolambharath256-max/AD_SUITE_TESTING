...
# Check: Privileged Access Check 34
...
Write-Host "=== Privileged Access Check 34 ===" -ForegroundColor Cyan
...
try {
    ...
    Get-ADObject -LDAPFilter '(&(objectClass=user)(adminCount=1))' `
                 -Properties @('name','distinguishedName','samAccountName','adminCount', 'objectSid') `
                 -SearchBase $targetNC `
                 -ErrorAction Stop |
    ForEach-Object {
    ...
    Write-Warning "PRV-034 PowerShell engine: $_"   # Fix R10: no silent catch
}
...
try {
    ...
    $adsiS.Filter   = '(&(objectClass=user)(adminCount=1))'
    $adsiS.PageSize = 1000
    $adsiS.PropertiesToLoad.Clear()
    @('name','distinguishedName','samAccountName','adminCount', 'objectSid') | ForEach-Object { [void]$adsiS.PropertiesToLoad.Add($_) }
    ...
    Write-Warning "PRV-034 ADSI engine: $_"          # Fix R10
}
...
try {
    $csFilter  = '(&(objectClass=user)(adminCount=1))'
    $csProps   = @("name","distinguishedName","samAccountName","adminCount")   # PS array of double-quoted strings
    $csCheckId = 'PRV-034'
    ...
    Write-Warning "PRV-034 C# engine: $_"             # Fix R10
}
...
Write-Host ""
Write-Host "=== PRV-034: $($uniqueResults.Count) unique findings ===" -ForegroundColor Cyan
$uniqueResults | Format-List

# Fix R14: Fixed 5-field schema enforced before CSV export
$csvPath = Join-Path $env:TEMP "PRV-034_results.csv"
$uniqueResults |
