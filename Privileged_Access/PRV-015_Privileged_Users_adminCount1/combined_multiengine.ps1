...
Write-Host "=== Multi-Engine Execution: Privileged Users (adminCount=1) ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        $props = @('name', 'distinguishedName', 'samAccountName', 'adminCount', 'memberOf', 'pwdLastSet')

        Get-ADObject -LDAPFilter $ldapFilter -Properties $props,userAccountControl,samAccountName -ErrorAction Stop |
          Select-Object name, distinguishedName, samAccountName, adminCount, memberOf, pwdLastSet |
          Sort-Object name |
          ForEach-Object { $_ }
...
try {
    $adsiResults = @(
        ...
        (@('name', 'distinguishedName', 'samAccountName', 'adminCount', 'memberOf', 'pwdLastSet', 'userAccountControl') | ForEach-Object { [void]$searcher.PropertiesToLoad.Add($_) }
...
public class Program
{
  public static void Run()
  {
    ...
    string[] props = new string[] { "name", "distinguishedName", "samAccountName", "adminCount", "memberOf", "pwdLastSet" };
...
try {
    ...
    $bhFile = Join-Path $bhDir "PRV-015_$bhTs.json"
    @{
