...
Write-Host "=== Multi-Engine Execution: Membership: DnsAdmins ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        $ldapFilter = '(&(objectCategory=group)(cn=DnsAdmins))'
...
public class Program
{
  ...
  {
    string filter = @"(&(objectCategory=group)(cn=DnsAdmins))";
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'PRV-009'
                adSuiteCheckName  = 'Membership_DnsAdmins'
    ...
    $bhFile = Join-Path $bhDir "PRV-009_$bhTs.json"
    @{
