...
Write-Host "=== Multi-Engine Execution: AdminSDHolder Object ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        $ldapFilter = '(&(objectClass=container)(cn=AdminSDHolder))'
        $props = @('name', 'distinguishedName', 'cn', 'nTSecurityDescriptor')

        Get-ADObject -LDAPFilter $ldapFilter -Properties $props -ErrorAction Stop |
          Select-Object name, distinguishedName, cn, nTSecurityDescriptor |
          Sort-Object name |
          ForEach-Object { $_ }
...
public class Program
{
  ...
  {
    string filter = @"(&(objectClass=container)(cn=AdminSDHolder))";
    string[] props = new string[] { "name", "distinguishedName", "cn", "nTSecurityDescriptor" };
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'PRV-026'
                adSuiteCheckName  = 'AdminSDHolder_Object'
    ...
    $bhFile = Join-Path $bhDir "PRV-026_$bhTs.json"
    @{
