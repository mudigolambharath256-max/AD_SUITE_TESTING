...
Write-Host "=== Multi-Engine Execution: Membership: Server Operators ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        $ldapFilter = '(&(objectCategory=group)(cn=Server Operators))'
...
public class Program
{
  ...
  {
    string filter = @"(&(objectCategory=group)(cn=Server Operators))";
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'PRV-007'
                adSuiteCheckName  = 'Membership_Server_Operators'
    ...
    $bhFile = Join-Path $bhDir "PRV-007_$bhTs.json"
    @{
