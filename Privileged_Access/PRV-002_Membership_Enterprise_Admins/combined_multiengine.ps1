...
Write-Host "=== Multi-Engine Execution: Membership: Enterprise Admins ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        $ldapFilter = '(&(objectCategory=group)(cn=Enterprise Admins))'
...
public class Program
{
  ...
  {
    string filter = @"(&(objectCategory=group)(cn=Enterprise Admins))";
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'PRV-002'
                adSuiteCheckName  = 'Membership_Enterprise_Admins'
    ...
    $bhFile = Join-Path $bhDir "PRV-002_$bhTs.json"
    @{
