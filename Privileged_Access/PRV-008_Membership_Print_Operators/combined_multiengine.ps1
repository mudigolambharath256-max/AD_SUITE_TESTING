...
Write-Host "=== Multi-Engine Execution: Membership: Print Operators ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        $ldapFilter = '(&(objectCategory=group)(cn=Print Operators))'
...
public class Program
{
  ...
  {
    string filter = @"(&(objectCategory=group)(cn=Print Operators))";
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'PRV-008'
                adSuiteCheckName  = 'Membership_Print_Operators'
    ...
    $bhFile = Join-Path $bhDir "PRV-008_$bhTs.json"
    @{
