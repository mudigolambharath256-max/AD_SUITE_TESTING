...
# Check: Admin Accounts with SPNs (Kerberoasting Risk)
...
Write-Host "=== Multi-Engine Execution: Admin Accounts with SPNs (Kerberoasting Risk) ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        $ldapFilter = '(&(&(objectCategory=person)(objectClass=user)(!(userAccountControl:1.2.840.113556.1.4.803:=2)))(adminCount=1)(servicePrincipalName=*))'
        $props = @('name', 'distinguishedName', 'samAccountName', 'servicePrincipalName', 'pwdLastSet')

        Get-ADObject -LDAPFilter $ldapFilter -Properties $props,adminCount,userAccountControl,servicePrincipalName,samAccountName -ErrorAction Stop |
          Select-Object name, distinguishedName, samAccountName, servicePrincipalName, pwdLastSet |
          Sort-Object name |
          ForEach-Object { $_ }
...
try {
    $adsiResults = @(
        ...
        $searcher = [ADSISearcher]'(&(&(objectCategory=person)(objectClass=user)(!(userAccountControl:1.2.840.113556.1.4.803:=2)))(adminCount=1)(servicePrincipalName=*))'
        $searcher.PageSize = 1000
        $searcher.PropertiesToLoad.Clear()
        (@('name', 'distinguishedName', 'samAccountName', 'servicePrincipalName', 'pwdLastSet', 'adminCount', 'userAccountControl') | ForEach-Object { [void]$searcher.PropertiesToLoad.Add($_) }

        $searcher.FindAll() | ForEach-Object {
          $p = $_.Properties
          [PSCustomObject]@{
            Label = 'Admin Accounts with SPNs (Kerberoasting Risk)'
            Name = $p['name'][0]
...
public class Program
{
  ...
  {
    string filter = @"(&(&(objectCategory=person)(objectClass=user)(!(userAccountControl:1.2.840.113556.1.4.803:=2)))(adminCount=1)(servicePrincipalName=*))";
    string[] props = new string[] { "name", "distinguishedName", "samAccountName", "servicePrincipalName", "pwdLastSet" };
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'PRV-018'
                adSuiteCheckName  = 'Admin_Accounts_with_SPNs_Kerberoasting_Risk'
    ...
    $bhFile = Join-Path $bhDir "PRV-018_$bhTs.json"
    @{
