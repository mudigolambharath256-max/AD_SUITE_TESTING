...
Write-Host "=== Multi-Engine Execution: Membership: Schema Admins ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        $ldapFilter = '(&(objectCategory=group)(cn=Schema Admins))'
...
public class Program
{
  ...
  {
    string filter = @"(&(objectCategory=group)(cn=Schema Admins))";
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'PRV-003'
                adSuiteCheckName  = 'Membership_Schema_Admins'
    ...
    $bhFile = Join-Path $bhDir "PRV-003_$bhTs.json"
    @{
