...
Write-Host "=== Multi-Engine Execution: Exchange Windows Permissions Group ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        $ldapFilter = '(&(objectCategory=group)(cn=Exchange Windows Permissions))'
...
public class Program
{
  ...
  {
    string filter = @"(&(objectCategory=group)(cn=Exchange Windows Permissions))";
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'PRV-027'
                adSuiteCheckName  = 'Exchange_Windows_Permissions_Group'
    ...
    $bhFile = Join-Path $bhDir "PRV-027_$bhTs.json"
    @{
