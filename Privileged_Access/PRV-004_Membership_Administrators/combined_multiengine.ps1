...
Write-Host "=== Multi-Engine Execution: Membership: Administrators ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        $ldapFilter = '(&(objectCategory=group)(cn=Administrators))'
...
public class Program
{
  ...
  {
    string filter = @"(&(objectCategory=group)(cn=Administrators))";
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'PRV-004'
                adSuiteCheckName  = 'Membership_Administrators'
    ...
    $bhFile = Join-Path $bhDir "PRV-004_$bhTs.json"
    @{
