...
# Check: Admin Accounts with Non-Expiring Password
...
Write-Host "=== Multi-Engine Execution: Admin Accounts with Non-Expiring Password ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        $ldapFilter = '(&(&(objectCategory=person)(objectClass=user)(!(userAccountControl:1.2.840.113556.1.4.803:=2)))(adminCount=1)(userAccountControl:1.2.840.113556.1.4.803:=65536))'
        $props = @('name', 'distinguishedName', 'samAccountName', 'userAccountControl', 'pwdLastSet')

        Get-ADObject -LDAPFilter $ldapFilter -Properties $props,adminCount,userAccountControl,samAccountName -ErrorAction Stop |
          Select-Object name, distinguishedName, samAccountName, userAccountControl, pwdLastSet |
          Sort-Object name |
          ForEach-Object { $_ }
...
try {
    $adsiResults = @(
        ...
        $searcher = [ADSISearcher]'(&(&(objectCategory=person)(objectClass=user)(!(userAccountControl:1.2.840.113556.1.4.803:=2)))(adminCount=1)(userAccountControl:1.2.840.113556.1.4.803:=65536))'
        $searcher.PageSize = 1000
        $searcher.PropertiesToLoad.Clear()
        (@('name', 'distinguishedName', 'samAccountName', 'userAccountControl', 'pwdLastSet', 'adminCount') | ForEach-Object { [void]$searcher.PropertiesToLoad.Add($_) }

        $searcher.FindAll() | ForEach-Object {
          $p = $_.Properties
          [PSCustomObject]@{
            Label = 'Admin Accounts with Non-Expiring Password'
            Name = $p['name'][0]
...
public class Program
{
  ...
  {
    string filter = @"(&(&(objectCategory=person)(objectClass=user)(!(userAccountControl:1.2.840.113556.1.4.803:=2)))(adminCount=1)(userAccountControl:1.2.840.113556.1.4.803:=65536))";
    string[] props = new string[] { "name", "distinguishedName", "samAccountName", "userAccountControl", "pwdLastSet" };
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'PRV-017'
                adSuiteCheckName  = 'Admin_Accounts_with_Non-Expiring_Password'
    ...
    $bhFile = Join-Path $bhDir "PRV-017_$bhTs.json"
    @{
