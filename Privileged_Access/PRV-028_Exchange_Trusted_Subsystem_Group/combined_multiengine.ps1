...
Write-Host "=== Multi-Engine Execution: Exchange Trusted Subsystem Group ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        $ldapFilter = '(&(objectCategory=group)(cn=Exchange Trusted Subsystem))'
...
public class Program
{
  ...
  {
    string filter = @"(&(objectCategory=group)(cn=Exchange Trusted Subsystem))";
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'PRV-028'
                adSuiteCheckName  = 'Exchange_Trusted_Subsystem_Group'
    ...
    $bhFile = Join-Path $bhDir "PRV-028_$bhTs.json"
    @{
