...
# Check: Membership: Domain Admins
...
Write-Host "=== Multi-Engine Execution: Membership: Domain Admins ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        $ldapFilter = '(&(objectCategory=group)(cn=Domain Admins))'
...
try {
    $adsiResults = @(
        ...
        $searcher.FindAll() | ForEach-Object {
          $p = $_.Properties
          [PSCustomObject]@{
            Label = 'Membership: Domain Admins'
            Name = $p['name'][0]
...
public class Program
{
  ...
  {
    string filter = @"(&(objectCategory=group)(cn=Domain Admins))";
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'PRV-001'
                adSuiteCheckName  = 'Membership_Domain_Admins'
    ...
    $bhFile = Join-Path $bhDir "PRV-001_$bhTs.json"
    @{
