...
class Program
{
    ...
    static void Main()
    {
        string filter = @"(&(objectClass=user)(adminCount=1))";
        string[] props = new string[] { "name","distinguishedName","samAccountName","adminCount" };
        ...
        try
        {
            using (DirectoryEntry rootEntry = new DirectoryEntry("LDAP://RootDSE"))
            {
                ...
                using (DirectorySearcher searcher = new DirectorySearcher(searchEntry))
                {
                    ...
                    {
                        Console.WriteLine("PRV-035: found " + results.Count + " objects");
        ...
        {
            Console.Error.WriteLine("PRV-035 error: " + ex.Message);
            Environment.Exit(1);
        }
    }
}
