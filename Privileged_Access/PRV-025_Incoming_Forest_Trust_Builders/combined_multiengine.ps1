...
Write-Host "=== Multi-Engine Execution: Incoming Forest Trust Builders ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        $ldapFilter = '(&(objectCategory=group)(cn=Incoming Forest Trust Builders))'
...
public class Program
{
  ...
  {
    string filter = @"(&(objectCategory=group)(cn=Incoming Forest Trust Builders))";
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'PRV-025'
                adSuiteCheckName  = 'Incoming_Forest_Trust_Builders'
    ...
    $bhFile = Join-Path $bhDir "PRV-025_$bhTs.json"
    @{
