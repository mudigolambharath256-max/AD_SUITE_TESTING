...
Write-Host "=== Multi-Engine Execution: Membership: Backup Operators ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        $ldapFilter = '(&(objectCategory=group)(cn=Backup Operators))'
...
public class Program
{
  ...
  {
    string filter = @"(&(objectCategory=group)(cn=Backup Operators))";
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'PRV-006'
                adSuiteCheckName  = 'Membership_Backup_Operators'
    ...
    $bhFile = Join-Path $bhDir "PRV-006_$bhTs.json"
    @{
