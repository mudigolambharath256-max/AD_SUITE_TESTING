...
Write-Host "=== Multi-Engine Execution: Allowed RODC Password Replication Group ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        $ldapFilter = '(&(objectCategory=group)(cn=Allowed RODC Password Replication Group))'
        $props = @('name', 'distinguishedName', 'cn', 'member')
...
try {
    $adsiResults = @(
        ...
        $searcher = [ADSISearcher]'(&(objectCategory=group)(cn=Allowed RODC Password Replication Group))'
        ...
        $searcher.FindAll() | ForEach-Object {
          $p = $_.Properties
          [PSCustomObject]@{
            Label = 'Allowed RODC Password Replication Group'
            Name = $p['name'][0]
...
public class Program
{
  ...
  {
    string filter = @"(&(objectCategory=group)(cn=Allowed RODC Password Replication Group))";
    string[] props = new string[] { "name", "distinguishedName", "cn", "member" };
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'PRV-023'
                adSuiteCheckName  = 'Allowed_RODC_Password_Replication_Group'
    ...
    $bhFile = Join-Path $bhDir "PRV-023_$bhTs.json"
    @{
