...
Write-Host "=== Multi-Engine Execution: Membership: Key Admins ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        $ldapFilter = '(&(objectCategory=group)(cn=Key Admins))'
...
public class Program
{
  ...
  {
    string filter = @"(&(objectCategory=group)(cn=Key Admins))";
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'PRV-013'
                adSuiteCheckName  = 'Membership_Key_Admins'
    ...
    $bhFile = Join-Path $bhDir "PRV-013_$bhTs.json"
    @{
