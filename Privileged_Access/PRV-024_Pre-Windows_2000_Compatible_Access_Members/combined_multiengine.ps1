...
Write-Host "=== Multi-Engine Execution: Pre-Windows 2000 Compatible Access Members ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        $ldapFilter = '(&(objectCategory=group)(cn=Pre-Windows 2000 Compatible Access))'
...
public class Program
{
  ...
  {
    string filter = @"(&(objectCategory=group)(cn=Pre-Windows 2000 Compatible Access))";
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'PRV-024'
                adSuiteCheckName  = 'Pre-Windows_2000_Compatible_Access_Members'
    ...
    $bhFile = Join-Path $bhDir "PRV-024_$bhTs.json"
    @{
