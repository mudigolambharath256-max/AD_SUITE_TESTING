...
Write-Host "PRV-039: found $($results.Count) objects"
...
try {
    ...
    $bhS.Filter   = '(&(objectClass=user)(adminCount=1))'
    $bhS.PageSize = 1000
    ...
    foreach ($r in $bhRaw) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                isdeleted         = $false
                adSuiteCheckId    = 'PRV-039'
                adSuiteCheckName  = 'Privileged Access Check 39'
                adSuiteSeverity   = 'high'
                adSuiteCategory   = 'Privileged_Access'
                adSuiteFlag       = $true
            }
            Aces           = @()
            IsDeleted      = $false
    ...
    @{ data = $bhNodes.ToArray()
       meta = @{ type = 'users'; count = $bhNodes.Count; version = 5; methods = 0 }
        ...
        Out-File -FilePath (Join-Path $bhDir "PRV-039_$bhTs.json") -Encoding UTF8 -Force

# Fix R10: No silent catch — warn on failure
} catch { Write-Warning "PRV-039 BloodHound export error: $_" }
