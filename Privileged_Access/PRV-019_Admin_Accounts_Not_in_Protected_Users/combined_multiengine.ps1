...
# Check: Admin Accounts Not in Protected Users
...
Write-Host "=== Multi-Engine Execution: Admin Accounts Not in Protected Users ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    $adsiResults = @(
        ...
        (@('name', 'distinguishedName', 'samAccountName', 'memberOf', 'adminCount', 'userAccountControl') | ForEach-Object { [void]$searcher.PropertiesToLoad.Add($_) }

        $searcher.FindAll() | ForEach-Object {
          $p = $_.Properties
          [PSCustomObject]@{
            Label = 'Admin Accounts Not in Protected Users'
            Name = $p['name'][0]
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'PRV-019'
                adSuiteCheckName  = 'Admin_Accounts_Not_in_Protected_Users'
    ...
    $bhFile = Join-Path $bhDir "PRV-019_$bhTs.json"
    @{
