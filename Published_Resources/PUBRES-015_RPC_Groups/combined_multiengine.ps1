...
Write-Host "=== PUBRES-015: RPC Groups ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        # Execute query
        $adObjects = Get-ADObject -LDAPFilter '(objectClass=rpcGroup)' `
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'PUBRES-015'}}, `
            @{N='CheckName'; E={'RPC Groups'}}, `
...
if (-not $useADModule) {
    ...
    try {
        ...
        $searcher.Filter = '(objectClass=rpcGroup)'
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'PUBRES-015'
                adSuiteCheckName  = 'RPC_Groups'
    ...
    $bhFile = Join-Path $bhDir "PUBRES-015_$bhTs.json"
    @{
