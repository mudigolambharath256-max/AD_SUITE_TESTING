# ============================================================================
# PUBRES-030: Published Resources Inventory
# ============================================================================
...
Write-Host "=== PUBRES-030: Published Resources Inventory ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        # Execute query
        $adObjects = Get-ADObject -LDAPFilter '(|(objectClass=volume)(objectClass=printQueue)(objectClass=serviceConnectionPoint))' `
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'PUBRES-030'}}, `
            @{N='CheckName'; E={'Published Resources Inventory'}}, `
...
if (-not $useADModule) {
    ...
    try {
        ...
        $searcher.Filter = '(|(objectClass=volume)(objectClass=printQueue)(objectClass=serviceConnectionPoint))'
        ...
        $results = $adsiResults | ForEach-Object {
            ...
            [PSCustomObject]@{
                CheckID           = 'PUBRES-030'
                CheckName         = 'Published Resources Inventory'
                Method            = 'ADSI'
                Domain            = $domainNC
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'PUBRES-030'
                adSuiteCheckName  = 'Published_Resources_Inventory'
                adSuiteMEDIUM   = 'MEDIUM'
                adSuitePublished_Resources   = 'Published_Resources'
                adSuiteFlag       = $true
            }
            Aces      = @()
            IsDeleted = $false
    ...
    $bhFile = Join-Path $bhDir "PUBRES-030_$bhTs.json"
    @{
