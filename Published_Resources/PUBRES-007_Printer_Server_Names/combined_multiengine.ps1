...
Write-Host "=== PUBRES-007: Printer Server Names ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'PUBRES-007'}}, `
            @{N='CheckName'; E={'Printer Server Names'}}, `
...
if (-not $useADModule) {
    ...
    try {
        ...
        $results = $adsiResults | ForEach-Object {
            ...
            [PSCustomObject]@{
                CheckID           = 'PUBRES-007'
                CheckName         = 'Printer Server Names'
                Method            = 'ADSI'
                Domain            = $domainNC
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'PUBRES-007'
                adSuiteCheckName  = 'Printer_Server_Names'
    ...
    $bhFile = Join-Path $bhDir "PUBRES-007_$bhTs.json"
    @{
