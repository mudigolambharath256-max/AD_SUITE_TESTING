...
Write-Host "=== PUBRES-016: RPC Servers ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        # Execute query
        $adObjects = Get-ADObject -LDAPFilter '(objectClass=rpcServer)' `
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'PUBRES-016'}}, `
            @{N='CheckName'; E={'RPC Servers'}}, `
...
if (-not $useADModule) {
    ...
    try {
        ...
        $searcher.Filter = '(objectClass=rpcServer)'
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'PUBRES-016'
                adSuiteCheckName  = 'RPC_Servers'
    ...
    $bhFile = Join-Path $bhDir "PUBRES-016_$bhTs.json"
    @{
