# ============================================================================
# PUBRES-011: RPC Services Published
# ============================================================================
...
Write-Host "=== PUBRES-011: RPC Services Published ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        # Execute query
        $adObjects = Get-ADObject -LDAPFilter '(objectClass=rpcEntry)' `
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'PUBRES-011'}}, `
            @{N='CheckName'; E={'RPC Services Published'}}, `
...
if (-not $useADModule) {
    ...
    try {
        ...
        $searcher.Filter = '(objectClass=rpcEntry)'
        ...
        $results = $adsiResults | ForEach-Object {
            ...
            [PSCustomObject]@{
                CheckID           = 'PUBRES-011'
                CheckName         = 'RPC Services Published'
                Method            = 'ADSI'
                Domain            = $domainNC
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'PUBRES-011'
                adSuiteCheckName  = 'RPC_Services_Published'
    ...
    $bhFile = Join-Path $bhDir "PUBRES-011_$bhTs.json"
    @{
