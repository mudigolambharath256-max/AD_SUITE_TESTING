...
Write-Host "=== PUBRES-025: MSMQ Settings ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        # Execute query
        $adObjects = Get-ADObject -LDAPFilter '(objectClass=mSMQSettings)' `
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'PUBRES-025'}}, `
            @{N='CheckName'; E={'MSMQ Settings'}}, `
...
if (-not $useADModule) {
    ...
    try {
        ...
        $searcher.Filter = '(objectClass=mSMQSettings)'
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'PUBRES-025'
                adSuiteCheckName  = 'MSMQ_Settings'
    ...
    $bhFile = Join-Path $bhDir "PUBRES-025_$bhTs.json"
    @{
