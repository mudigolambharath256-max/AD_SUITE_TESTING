...
Write-Host "=== PUBRES-001: Printers Published in AD ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'PUBRES-001'}}, `
            @{N='CheckName'; E={'Printers Published in AD'}}, `
...
if (-not $useADModule) {
    ...
    try {
        ...
        $results = $adsiResults | ForEach-Object {
            ...
            [PSCustomObject]@{
                CheckID           = 'PUBRES-001'
                CheckName         = 'Printers Published in AD'
                Method            = 'ADSI'
                Domain            = $domainNC
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'PUBRES-001'
                adSuiteCheckName  = 'Printers_Published_in_AD'
    ...
    $bhFile = Join-Path $bhDir "PUBRES-001_$bhTs.json"
    @{
