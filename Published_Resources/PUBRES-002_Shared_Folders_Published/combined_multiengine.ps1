...
Write-Host "=== PUBRES-002: Shared Folders Published ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        # Execute query
        $adObjects = Get-ADObject -LDAPFilter '(objectClass=volume)' `
...
if (-not $useADModule) {
    ...
    try {
        ...
        $results = $adsiResults | ForEach-Object {
            ...
            [PSCustomObject]@{
                CheckID           = 'PUBRES-002'
                CheckName         = 'Shared Folders Published'
                Method            = 'ADSI'
                Domain            = $domainNC
