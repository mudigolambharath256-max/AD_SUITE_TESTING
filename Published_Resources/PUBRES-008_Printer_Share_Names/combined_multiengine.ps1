...
Write-Host "=== PUBRES-008: Printer Share Names ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'PUBRES-008'}}, `
            @{N='CheckName'; E={'Printer Share Names'}}, `
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'PUBRES-008'
                adSuiteCheckName  = 'Printer_Share_Names'
    ...
    $bhFile = Join-Path $bhDir "PUBRES-008_$bhTs.json"
    @{
