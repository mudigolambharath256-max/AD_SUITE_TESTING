...
Write-Host "=== PUBRES-013: RPC Profiles ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        # Execute query
        $adObjects = Get-ADObject -LDAPFilter '(objectClass=rpcProfile)' `
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'PUBRES-013'}}, `
            @{N='CheckName'; E={'RPC Profiles'}}, `
...
if (-not $useADModule) {
    ...
    try {
        ...
        $searcher.Filter = '(objectClass=rpcProfile)'
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'PUBRES-013'
                adSuiteCheckName  = 'RPC_Profiles'
    ...
    $bhFile = Join-Path $bhDir "PUBRES-013_$bhTs.json"
    @{
