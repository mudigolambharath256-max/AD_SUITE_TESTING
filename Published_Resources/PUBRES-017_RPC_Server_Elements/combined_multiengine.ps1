# ============================================================================
# PUBRES-017: RPC Server Elements
# ============================================================================
...
Write-Host "=== PUBRES-017: RPC Server Elements ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        # Execute query
        $adObjects = Get-ADObject -LDAPFilter '(objectClass=rpcServerElement)' `
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'PUBRES-017'}}, `
            @{N='CheckName'; E={'RPC Server Elements'}}, `
...
if (-not $useADModule) {
    ...
    try {
        ...
        $searcher.Filter = '(objectClass=rpcServerElement)'
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'PUBRES-017'
                adSuiteCheckName  = 'RPC_Server_Elements'
    ...
    $bhFile = Join-Path $bhDir "PUBRES-017_$bhTs.json"
    @{
