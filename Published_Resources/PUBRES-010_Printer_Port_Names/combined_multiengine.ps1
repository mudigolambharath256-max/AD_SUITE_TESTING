...
Write-Host "=== PUBRES-010: Printer Port Names ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'PUBRES-010'}}, `
            @{N='CheckName'; E={'Printer Port Names'}}, `
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'PUBRES-010'
                adSuiteCheckName  = 'Printer_Port_Names'
    ...
    $bhFile = Join-Path $bhDir "PUBRES-010_$bhTs.json"
    @{
