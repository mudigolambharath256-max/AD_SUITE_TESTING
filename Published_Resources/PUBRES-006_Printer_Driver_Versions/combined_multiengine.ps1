...
Write-Host "=== PUBRES-006: Printer Driver Versions ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'PUBRES-006'}}, `
            @{N='CheckName'; E={'Printer Driver Versions'}}, `
...
if (-not $useADModule) {
    ...
    try {
        ...
        $results = $adsiResults | ForEach-Object {
            ...
            [PSCustomObject]@{
                CheckID           = 'PUBRES-006'
                CheckName         = 'Printer Driver Versions'
                Method            = 'ADSI'
                Domain            = $domainNC
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'PUBRES-006'
                adSuiteCheckName  = 'Printer_Driver_Versions'
    ...
    $bhFile = Join-Path $bhDir "PUBRES-006_$bhTs.json"
    @{
