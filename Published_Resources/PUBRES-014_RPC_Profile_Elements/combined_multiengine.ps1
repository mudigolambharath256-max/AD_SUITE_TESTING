# ============================================================================
# PUBRES-014: RPC Profile Elements
# ============================================================================
...
Write-Host "=== PUBRES-014: RPC Profile Elements ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        # Execute query
        $adObjects = Get-ADObject -LDAPFilter '(objectClass=rpcProfileElement)' `
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'PUBRES-014'}}, `
            @{N='CheckName'; E={'RPC Profile Elements'}}, `
...
if (-not $useADModule) {
    ...
    try {
        ...
        $searcher.Filter = '(objectClass=rpcProfileElement)'
        ...
        $results = $adsiResults | ForEach-Object {
            ...
            [PSCustomObject]@{
                CheckID           = 'PUBRES-014'
                CheckName         = 'RPC Profile Elements'
                Method            = 'ADSI'
                Domain            = $domainNC
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'PUBRES-014'
                adSuiteCheckName  = 'RPC_Profile_Elements'
    ...
    $bhFile = Join-Path $bhDir "PUBRES-014_$bhTs.json"
    @{
