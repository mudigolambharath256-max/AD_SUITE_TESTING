...
Write-Host "=== PUBRES-024: MSMQ Queues ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        # Execute query
        $adObjects = Get-ADObject -LDAPFilter '(objectClass=mSMQQueue)' `
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'PUBRES-024'}}, `
            @{N='CheckName'; E={'MSMQ Queues'}}, `
...
if (-not $useADModule) {
    ...
    try {
        ...
        $searcher.Filter = '(objectClass=mSMQQueue)'
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'PUBRES-024'
                adSuiteCheckName  = 'MSMQ_Queues'
    ...
    $bhFile = Join-Path $bhDir "PUBRES-024_$bhTs.json"
    @{
