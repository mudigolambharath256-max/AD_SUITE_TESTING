...
Write-Host "=== PUBRES-005: Printer Queue Permissions ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        # Execute query
        $adObjects = Get-ADObject -LDAPFilter '(objectClass=printQueue)' `
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'PUBRES-005'}}, `
            @{N='CheckName'; E={'Printer Queue Permissions'}}, `
...
if (-not $useADModule) {
    ...
    try {
        ...
        $results = $adsiResults | ForEach-Object {
            ...
            [PSCustomObject]@{
                CheckID           = 'PUBRES-005'
                CheckName         = 'Printer Queue Permissions'
                Method            = 'ADSI'
                Domain            = $domainNC
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'PUBRES-005'
                adSuiteCheckName  = 'Printer_Queue_Permissions'
    ...
    $bhFile = Join-Path $bhDir "PUBRES-005_$bhTs.json"
    @{
