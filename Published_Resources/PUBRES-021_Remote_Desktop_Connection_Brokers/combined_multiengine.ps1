# ============================================================================
# PUBRES-021: Remote Desktop Connection Brokers
# ============================================================================
...
Write-Host "=== PUBRES-021: Remote Desktop Connection Brokers ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        # Execute query
        $adObjects = Get-ADObject -LDAPFilter '(objectClass=serviceConnectionPoint)' `
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'PUBRES-021'}}, `
            @{N='CheckName'; E={'Remote Desktop Connection Brokers'}}, `
...
if (-not $useADModule) {
    ...
    try {
        ...
        $results = $adsiResults | ForEach-Object {
            ...
            [PSCustomObject]@{
                CheckID           = 'PUBRES-021'
                CheckName         = 'Remote Desktop Connection Brokers'
                Method            = 'ADSI'
                Domain            = $domainNC
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'PUBRES-021'
                adSuiteCheckName  = 'Remote_Desktop_Connection_Brokers'
    ...
    $bhFile = Join-Path $bhDir "PUBRES-021_$bhTs.json"
    @{
