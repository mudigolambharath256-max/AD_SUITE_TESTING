...
Write-Host "=== PUBRES-020: SCP Keywords ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'PUBRES-020'}}, `
            @{N='CheckName'; E={'SCP Keywords'}}, `
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'PUBRES-020'
                adSuiteCheckName  = 'SCP_Keywords'
    ...
    $bhFile = Join-Path $bhDir "PUBRES-020_$bhTs.json"
    @{
