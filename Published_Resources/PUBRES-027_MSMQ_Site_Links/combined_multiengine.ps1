...
Write-Host "=== PUBRES-027: MSMQ Site Links ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        # Execute query
        $adObjects = Get-ADObject -LDAPFilter '(objectClass=mSMQSiteLink)' `
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'PUBRES-027'}}, `
            @{N='CheckName'; E={'MSMQ Site Links'}}, `
...
if (-not $useADModule) {
    ...
    try {
        ...
        $searcher.Filter = '(objectClass=mSMQSiteLink)'
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'PUBRES-027'
                adSuiteCheckName  = 'MSMQ_Site_Links'
    ...
    $bhFile = Join-Path $bhDir "PUBRES-027_$bhTs.json"
    @{
