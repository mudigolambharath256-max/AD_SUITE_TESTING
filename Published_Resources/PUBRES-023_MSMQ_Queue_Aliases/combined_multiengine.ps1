...
Write-Host "=== PUBRES-023: MSMQ Queue Aliases ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        # Execute query
        $adObjects = Get-ADObject -LDAPFilter '(objectClass=mSMQQueueAlias)' `
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'PUBRES-023'}}, `
            @{N='CheckName'; E={'MSMQ Queue Aliases'}}, `
...
if (-not $useADModule) {
    ...
    try {
        ...
        $searcher.Filter = '(objectClass=mSMQQueueAlias)'
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'PUBRES-023'
                adSuiteCheckName  = 'MSMQ_Queue_Aliases'
    ...
    $bhFile = Join-Path $bhDir "PUBRES-023_$bhTs.json"
    @{
