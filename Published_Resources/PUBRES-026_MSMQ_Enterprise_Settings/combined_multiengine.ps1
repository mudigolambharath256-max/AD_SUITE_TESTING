# ============================================================================
# PUBRES-026: MSMQ Enterprise Settings
# ============================================================================
...
Write-Host "=== PUBRES-026: MSMQ Enterprise Settings ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        # Execute query
        $adObjects = Get-ADObject -LDAPFilter '(objectClass=mSMQEnterpriseSettings)' `
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'PUBRES-026'}}, `
            @{N='CheckName'; E={'MSMQ Enterprise Settings'}}, `
...
if (-not $useADModule) {
    ...
    try {
        ...
        $searcher.Filter = '(objectClass=mSMQEnterpriseSettings)'
        ...
        $results = $adsiResults | ForEach-Object {
            ...
            [PSCustomObject]@{
                CheckID           = 'PUBRES-026'
                CheckName         = 'MSMQ Enterprise Settings'
                Method            = 'ADSI'
                Domain            = $domainNC
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'PUBRES-026'
                adSuiteCheckName  = 'MSMQ_Enterprise_Settings'
    ...
    $bhFile = Join-Path $bhDir "PUBRES-026_$bhTs.json"
    @{
