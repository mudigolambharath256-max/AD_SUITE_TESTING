...
Write-Host "=== Multi-Engine Execution: Firewall GPO Check ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        $ldapFilter = '(&(objectClass=groupPolicyContainer)(|(displayName=*firewall*)(displayName=*Firewall*)))'
...
public class Program
{
  ...
  {
    string filter = @"(&(objectClass=groupPolicyContainer)(|(displayName=*firewall*)(displayName=*Firewall*)))";
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'GPO-025'
                adSuiteCheckName  = 'Firewall_GPO_Check'
    ...
    $bhFile = Join-Path $bhDir "GPO-025_$bhTs.json"
    @{
