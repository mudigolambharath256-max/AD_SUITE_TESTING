...
Write-Host "=== Multi-Engine Execution: GPO Links on Domain Root ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        $ldapFilter = '(&(objectClass=domainDNS)(gPLink=*))'
        $props = @('name', 'distinguishedName', 'gPLink')
...
public class Program
{
  ...
  {
    string filter = @"(&(objectClass=domainDNS)(gPLink=*))";
    string[] props = new string[] { "name", "distinguishedName", "gPLink" };
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'GPO-018'
                adSuiteCheckName  = 'GPO_Links_on_Domain_Root'
    ...
    $bhFile = Join-Path $bhDir "GPO-018_$bhTs.json"
    @{
