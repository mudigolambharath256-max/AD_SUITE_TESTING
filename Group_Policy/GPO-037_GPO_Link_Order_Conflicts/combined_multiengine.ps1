# ============================================================================
# GPO-037: GPO Link Order Conflicts - Multi-Engine
# ============================================================================

[CmdletBinding()]
param([Parameter()][string]$SearchBase, [Parameter()][string]$ExportPath)

Write-Host "[GPO-037] GPO Link Order Conflicts" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    $results = Get-ADObject -LDAPFilter '(objectClass=organizationalUnit)' -Properties name,distinguishedName -SearchBase $SearchBase -SearchScope Subtree -ResultSetSize $null -ErrorAction SilentlyContinue
} else {
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'GPO-037'
                adSuiteCheckName  = 'GPO_Link_Order_Conflicts'
    ...
    $bhFile = Join-Path $bhDir "GPO-037_$bhTs.json"
    @{
