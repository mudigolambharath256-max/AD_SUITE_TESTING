...
Write-Host "Found $($results.Count) objects for check: GPO Modification History" -ForegroundColor Cyan

$results | ForEach-Object {
    $props = $_.Properties
    [PSCustomObject]@{
        CheckID = 'GPO-031'
        CheckName = 'GPO Modification History'
...
try {
    ...
    if ($results -and $results.Count -gt 0) {
        ...
        $bhFile = Join-Path $bhDir "GPO-031_nodes.json"
