...
Write-Host "=== Group Policy Check 35 ===" -ForegroundColor Cyan
...
try {
    ...
    Write-Warning "GPO-035 PowerShell engine: $_"   # Fix R10: no silent catch
}
...
try {
    ...
    Write-Warning "GPO-035 ADSI engine: $_"          # Fix R10
}
...
try {
    ...
    Write-Warning "GPO-035 C# engine: $_"             # Fix R10
}
...
Write-Host ""
Write-Host "=== GPO-035: $($uniqueResults.Count) unique findings ===" -ForegroundColor Cyan
$uniqueResults | Format-List

# Fix R14: Fixed 5-field schema enforced before CSV export
$csvPath = Join-Path $env:TEMP "GPO-035_results.csv"
$uniqueResults |
