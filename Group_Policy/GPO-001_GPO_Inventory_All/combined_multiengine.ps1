...
Write-Host "=== Multi-Engine Execution: GPO Inventory (All) ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        $props = @('name', 'distinguishedName', 'displayName', 'gPCFileSysPath', 'versionNumber', 'flags', 'whenCreated')

        Get-ADObject -LDAPFilter $ldapFilter -Properties $props -ErrorAction Stop |
          Select-Object name, distinguishedName, displayName, gPCFileSysPath, versionNumber, flags, whenCreated |
          Sort-Object name |
          ForEach-Object { $_ }
...
public class Program
{
  ...
  {
    string filter = @"(objectClass=groupPolicyContainer)";
    string[] props = new string[] { "name", "distinguishedName", "displayName", "gPCFileSysPath", "versionNumber", "flags", "whenCreated" };
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'GPO-001'
                adSuiteCheckName  = 'GPO_Inventory_All'
    ...
    $bhFile = Join-Path $bhDir "GPO-001_$bhTs.json"
    @{
