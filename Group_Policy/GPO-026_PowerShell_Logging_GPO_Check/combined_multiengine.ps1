...
Write-Host "=== Multi-Engine Execution: PowerShell Logging GPO Check ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        $ldapFilter = '(&(objectClass=groupPolicyContainer)(|(displayName=*powershell*)(displayName=*PowerShell*)(displayName=*script*)))'
...
public class Program
{
  ...
  {
    string filter = @"(&(objectClass=groupPolicyContainer)(|(displayName=*powershell*)(displayName=*PowerShell*)(displayName=*script*)))";
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'GPO-026'
                adSuiteCheckName  = 'PowerShell_Logging_GPO_Check'
    ...
    $bhFile = Join-Path $bhDir "GPO-026_$bhTs.json"
    @{
