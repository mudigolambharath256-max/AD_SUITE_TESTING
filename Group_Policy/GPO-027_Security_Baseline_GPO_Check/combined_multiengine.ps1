...
Write-Host "=== Multi-Engine Execution: Security Baseline GPO Check ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        $ldapFilter = '(&(objectClass=groupPolicyContainer)(|(displayName=*baseline*)(displayName=*Baseline*)(displayName=*security*)(displayName=*hardening*)))'
        $props = @('name', 'distinguishedName', 'displayName', 'gPCFileSysPath')

        Get-ADObject -LDAPFilter $ldapFilter -Properties $props -ErrorAction Stop |
          Select-Object name, distinguishedName, displayName, gPCFileSysPath |
          Sort-Object name |
          ForEach-Object { $_ }
...
public class Program
{
  ...
  {
    string filter = @"(&(objectClass=groupPolicyContainer)(|(displayName=*baseline*)(displayName=*Baseline*)(displayName=*security*)(displayName=*hardening*)))";
    string[] props = new string[] { "name", "distinguishedName", "displayName", "gPCFileSysPath" };
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'GPO-027'
                adSuiteCheckName  = 'Security_Baseline_GPO_Check'
    ...
    $bhFile = Join-Path $bhDir "GPO-027_$bhTs.json"
    @{
