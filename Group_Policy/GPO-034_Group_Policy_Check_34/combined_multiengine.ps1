...
Write-Host "=== Group Policy Check 34 ===" -ForegroundColor Cyan
...
try {
    ...
    Write-Warning "GPO-034 PowerShell engine: $_"   # Fix R10: no silent catch
}
...
try {
    ...
    Write-Warning "GPO-034 ADSI engine: $_"          # Fix R10
}
...
try {
    ...
    Write-Warning "GPO-034 C# engine: $_"             # Fix R10
}
...
Write-Host ""
Write-Host "=== GPO-034: $($uniqueResults.Count) unique findings ===" -ForegroundColor Cyan
$uniqueResults | Format-List

# Fix R14: Fixed 5-field schema enforced before CSV export
$csvPath = Join-Path $env:TEMP "GPO-034_results.csv"
$uniqueResults |
