...
Write-Host "=== Group Policy Check 2 ===" -ForegroundColor Cyan
...
try {
    ...
    Write-Warning "GPO-002 PowerShell engine: $_"   # Fix R10: no silent catch
}
...
try {
    ...
    Write-Warning "GPO-002 ADSI engine: $_"          # Fix R10
}
...
try {
    ...
    Write-Warning "GPO-002 C# engine: $_"             # Fix R10
}
...
Write-Host ""
Write-Host "=== GPO-002: $($uniqueResults.Count) unique findings ===" -ForegroundColor Cyan
$uniqueResults | Format-List

# Fix R14: Fixed 5-field schema enforced before CSV export
$csvPath = Join-Path $env:TEMP "GPO-002_results.csv"
$uniqueResults |
