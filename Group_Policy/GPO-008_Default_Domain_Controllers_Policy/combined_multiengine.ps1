...
Write-Host "=== Multi-Engine Execution: Default Domain Controllers Policy ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        $ldapFilter = '(&(objectClass=groupPolicyContainer)(cn={6AC1786C-016F-11D2-945F-00C04fB984F9}))'
        $props = @('name', 'distinguishedName', 'displayName', 'versionNumber', 'gPCFileSysPath')

        Get-ADObject -LDAPFilter $ldapFilter -Properties $props -ErrorAction Stop |
          Select-Object name, distinguishedName, displayName, versionNumber, gPCFileSysPath |
          Sort-Object name |
          ForEach-Object { $_ }
...
public class Program
{
  ...
  {
    string filter = @"(&(objectClass=groupPolicyContainer)(cn={6AC1786C-016F-11D2-945F-00C04fB984F9}))";
    string[] props = new string[] { "name", "distinguishedName", "displayName", "versionNumber", "gPCFileSysPath" };
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'GPO-008'
                adSuiteCheckName  = 'Default_Domain_Controllers_Policy'
    ...
    $bhFile = Join-Path $bhDir "GPO-008_$bhTs.json"
    @{
