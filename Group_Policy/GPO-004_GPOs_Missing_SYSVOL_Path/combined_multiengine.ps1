...
Write-Host "=== Multi-Engine Execution: GPOs Missing SYSVOL Path ===" -ForegroundColor Cyan
Write-Host ""
...
public class Program
{
  ...
  {
    string filter = @"(&(objectClass=groupPolicyContainer)(!(gPCFileSysPath=*)))";
    string[] props = new string[] { "name", "distinguishedName", "displayName" };
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'GPO-004'
                adSuiteCheckName  = 'GPOs_Missing_SYSVOL_Path'
    ...
    $bhFile = Join-Path $bhDir "GPO-004_$bhTs.json"
    @{
