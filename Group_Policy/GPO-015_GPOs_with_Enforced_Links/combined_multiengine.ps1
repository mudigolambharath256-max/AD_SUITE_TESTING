...
Write-Host "=== Multi-Engine Execution: GPOs with Enforced Links ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        $ldapFilter = '(&(objectClass=organizationalUnit)(gPLink=*2*))'
...
public class Program
{
  ...
  {
    string filter = @"(&(objectClass=organizationalUnit)(gPLink=*2*))";
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'GPO-015'
                adSuiteCheckName  = 'GPOs_with_Enforced_Links'
    ...
    $bhFile = Join-Path $bhDir "GPO-015_$bhTs.json"
    @{
