...
Write-Host "Found $($results.Count) objects for check: GPOs with Weak Permissions" -ForegroundColor Cyan

$results | ForEach-Object {
    $props = $_.Properties
    [PSCustomObject]@{
        CheckID = 'GPO-032'
        CheckName = 'GPOs with Weak Permissions'
...
try {
    ...
    if ($results -and $results.Count -gt 0) {
        ...
        $bhFile = Join-Path $bhDir "GPO-032_nodes.json"
