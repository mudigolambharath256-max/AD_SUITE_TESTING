...
Write-Host "=== GPO-032: GPOs with Weak Permissions ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    $results = Get-ADObject -LDAPFilter '(objectClass=groupPolicyContainer)' -Properties name,distinguishedName -SearchBase $SearchBase -SearchScope Subtree -ResultSetSize $null
} else {
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'GPO-032'
                adSuiteCheckName  = 'GPOs_with_Weak_Permissions'
    ...
    $bhFile = Join-Path $bhDir "GPO-032_$bhTs.json"
    @{
