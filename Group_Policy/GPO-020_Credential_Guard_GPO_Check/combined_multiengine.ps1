...
Write-Host "=== Multi-Engine Execution: Credential Guard GPO Check ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        $ldapFilter = '(&(objectClass=groupPolicyContainer)(|(displayName=*credential*)(displayName=*guard*)))'
...
public class Program
{
  ...
  {
    string filter = @"(&(objectClass=groupPolicyContainer)(|(displayName=*credential*)(displayName=*guard*)))";
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'GPO-020'
                adSuiteCheckName  = 'Credential_Guard_GPO_Check'
    ...
    $bhFile = Join-Path $bhDir "GPO-020_$bhTs.json"
    @{
