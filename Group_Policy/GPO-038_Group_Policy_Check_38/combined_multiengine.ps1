...
Write-Host "=== Group Policy Check 38 ===" -ForegroundColor Cyan
...
try {
    ...
    Write-Warning "GPO-038 PowerShell engine: $_"   # Fix R10: no silent catch
}
...
try {
    ...
    Write-Warning "GPO-038 ADSI engine: $_"          # Fix R10
}
...
try {
    ...
    Write-Warning "GPO-038 C# engine: $_"             # Fix R10
}
...
Write-Host ""
Write-Host "=== GPO-038: $($uniqueResults.Count) unique findings ===" -ForegroundColor Cyan
$uniqueResults | Format-List

# Fix R14: Fixed 5-field schema enforced before CSV export
$csvPath = Join-Path $env:TEMP "GPO-038_results.csv"
$uniqueResults |
