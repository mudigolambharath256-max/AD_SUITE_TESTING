...
Write-Host "GPO-039: found $($results.Count) objects"
...
try {
    ...
    foreach ($r in $bhRaw) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                adSuiteCheckName  = 'Group Policy Check 39'
    ...
    @{ data = $bhNodes.ToArray()
       meta = @{ type = 'gpos'; count = $bhNodes.Count; version = 5; methods = 0 }
        ...
        Out-File -FilePath (Join-Path $bhDir "GPO-039_$bhTs.json") -Encoding UTF8 -Force

# Fix R10: No silent catch — warn on failure
} catch { Write-Warning "GPO-039 BloodHound export error: $_" }
