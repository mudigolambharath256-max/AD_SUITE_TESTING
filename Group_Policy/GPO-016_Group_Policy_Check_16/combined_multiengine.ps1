...
Write-Host "=== Group Policy Check 16 ===" -ForegroundColor Cyan
...
$targetNC = "CN=Policies,CN=System,$domainNC"
...
try {
    ...
    Get-ADObject -LDAPFilter '(&(objectClass=groupPolicyContainer))' `
                 -Properties @('name','distinguishedName','displayName') `
                 -SearchBase $targetNC `
                 -ErrorAction Stop |
    ForEach-Object {
    ...
    Write-Warning "GPO-016 PowerShell engine: $_"   # Fix R10: no silent catch
}
...
try {
    ...
    $adsiS.Filter   = '(&(objectClass=groupPolicyContainer))'
    $adsiS.PageSize = 1000
    $adsiS.PropertiesToLoad.Clear()
    @('name','distinguishedName','displayName') | ForEach-Object { [void]$adsiS.PropertiesToLoad.Add($_) }
    ...
    Write-Warning "GPO-016 ADSI engine: $_"          # Fix R10
}
...
try {
    $csFilter  = '(&(objectClass=groupPolicyContainer))'
    $csProps   = @("name","distinguishedName","displayName")   # PS array of double-quoted strings
    $csCheckId = 'GPO-016'
    ...
    Write-Warning "GPO-016 C# engine: $_"             # Fix R10
}
...
Write-Host ""
Write-Host "=== GPO-016: $($uniqueResults.Count) unique findings ===" -ForegroundColor Cyan
$uniqueResults | Format-List

# Fix R14: Fixed 5-field schema enforced before CSV export
$csvPath = Join-Path $env:TEMP "GPO-016_results.csv"
$uniqueResults |
