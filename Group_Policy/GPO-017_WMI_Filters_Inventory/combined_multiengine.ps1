...
Write-Host "=== Multi-Engine Execution: WMI Filters Inventory ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        $props = @('name', 'distinguishedName', 'msWMI-Name', 'msWMI-Author', 'msWMI-Parm2')

        Get-ADObject -LDAPFilter $ldapFilter -Properties $props -ErrorAction Stop |
          Select-Object name, distinguishedName, msWMI-Name, msWMI-Author, msWMI-Parm2 |
          Sort-Object name |
          ForEach-Object { $_ }
...
public class Program
{
  ...
  {
    string filter = @"(objectClass=msWMI-Som)";
    string[] props = new string[] { "name", "distinguishedName", "msWMI-Name", "msWMI-Author", "msWMI-Parm2" };
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'GPO-017'
                adSuiteCheckName  = 'WMI_Filters_Inventory'
    ...
    $bhFile = Join-Path $bhDir "GPO-017_$bhTs.json"
    @{
