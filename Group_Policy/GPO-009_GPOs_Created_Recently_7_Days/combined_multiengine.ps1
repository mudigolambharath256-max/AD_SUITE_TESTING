...
Write-Host "=== Multi-Engine Execution: GPOs Created Recently (7 Days) ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        $props = @('name', 'distinguishedName', 'displayName', 'whenCreated', 'gPCFileSysPath')

        Get-ADObject -LDAPFilter $ldapFilter -Properties $props -ErrorAction Stop |
          Select-Object name, distinguishedName, displayName, whenCreated, gPCFileSysPath |
          Sort-Object name |
          ForEach-Object { $_ }
...
public class Program
{
  public static void Run()
  {
    ...
    string[] props = new string[] { "name", "distinguishedName", "displayName", "whenCreated", "gPCFileSysPath" };
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'GPO-009'
                adSuiteCheckName  = 'GPOs_Created_Recently_7_Days'
    ...
    $bhFile = Join-Path $bhDir "GPO-009_$bhTs.json"
    @{
