...
Write-Host "=== Multi-Engine Execution: BitLocker GPO Check ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        $ldapFilter = '(&(objectClass=groupPolicyContainer)(|(displayName=*bitlocker*)(displayName=*BitLocker*)(displayName=*encryption*)))'
...
public class Program
{
  ...
  {
    string filter = @"(&(objectClass=groupPolicyContainer)(|(displayName=*bitlocker*)(displayName=*BitLocker*)(displayName=*encryption*)))";
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'GPO-022'
                adSuiteCheckName  = 'BitLocker_GPO_Check'
    ...
    $bhFile = Join-Path $bhDir "GPO-022_$bhTs.json"
    @{
