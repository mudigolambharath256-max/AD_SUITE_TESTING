...
Write-Host "=== Multi-Engine Execution: Fine-Grained Password Policies ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        $props = @('name', 'distinguishedName', 'cn', 'msDS-MinimumPasswordLength', 'msDS-PasswordComplexityEnabled', 'msDS-LockoutThreshold', 'msDS-PasswordSettingsPrecedence', 'msDS-PSOAppliesTo')

        Get-ADObject -LDAPFilter $ldapFilter -Properties $props -ErrorAction Stop |
          Select-Object name, distinguishedName, cn, msDS-MinimumPasswordLength, msDS-PasswordComplexityEnabled, msDS-LockoutThreshold, msDS-PasswordSettingsPrecedence, msDS-PSOAppliesTo |
          Sort-Object name |
          ForEach-Object { $_ }
...
try {
    $adsiResults = @(
        ...
        @('name', 'distinguishedName', 'cn', 'msDS-MinimumPasswordLength', 'msDS-PasswordComplexityEnabled', 'msDS-LockoutThreshold', 'msDS-PasswordSettingsPrecedence', 'msDS-PSOAppliesTo') | ForEach-Object { [void]$searcher.PropertiesToLoad.Add($_) }
...
public class Program
{
  public static void Run()
  {
    ...
    string[] props = new string[] { "name", "distinguishedName", "cn", "msDS-MinimumPasswordLength", "msDS-PasswordComplexityEnabled", "msDS-LockoutThreshold", "msDS-PasswordSettingsPrecedence", "msDS-PSOAppliesTo" };
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'GPO-012'
                adSuiteCheckName  = 'Fine-Grained_Password_Policies'
    ...
    $bhFile = Join-Path $bhDir "GPO-012_$bhTs.json"
    @{
