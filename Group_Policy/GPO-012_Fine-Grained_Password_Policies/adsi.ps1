...
@('name', 'distinguishedName', 'cn', 'msDS-MinimumPasswordLength', 'msDS-PasswordComplexityEnabled', 'msDS-LockoutThreshold', 'msDS-PasswordSettingsPrecedence', 'msDS-PSOAppliesTo', 'objectSid', 'samAccountName') | ForEach-Object { [void]$searcher.PropertiesToLoad.Add($_) }
...
try {
    ...
    if ($results -and $results.Count -gt 0) {
        ...
        $bhFile = Join-Path $bhDir "GPO-012_nodes.json"
    ...
    Write-Warning "[BloodHound] Export failed: # Check: Fine-Grained Password Policies
# Category: Group Policy
# Severity: info
# ID: GPO-012
# Requirements: None
