...
Write-Host "=== Multi-Engine Execution: OUs Blocking Inheritance ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        $props = @('name', 'distinguishedName', 'ou', 'gPOptions', 'gPLink')

        Get-ADObject -LDAPFilter $ldapFilter -Properties $props -ErrorAction Stop |
          Select-Object name, distinguishedName, ou, gPOptions, gPLink |
          Sort-Object name |
          ForEach-Object { $_ }
...
public class Program
{
  ...
  {
    string filter = @"(&(objectClass=organizationalUnit)(gPOptions=1))";
    string[] props = new string[] { "name", "distinguishedName", "ou", "gPOptions", "gPLink" };
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'GPO-014'
                adSuiteCheckName  = 'OUs_Blocking_Inheritance'
    ...
    $bhFile = Join-Path $bhDir "GPO-014_$bhTs.json"
    @{
