# ============================================================================
# GPO-038: Loopback Processing Misconfigurations - Multi-Engine
# ============================================================================

[CmdletBinding()]
param([Parameter()][string]$SearchBase, [Parameter()][string]$ExportPath)

Write-Host "[GPO-038] Loopback Processing Misconfigurations" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    $results = Get-ADObject -LDAPFilter '(objectClass=groupPolicyContainer)' -Properties name,distinguishedName -SearchBase $SearchBase -SearchScope Subtree -ResultSetSize $null -ErrorAction SilentlyContinue
} else {
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'GPO-038'
                adSuiteCheckName  = 'Loopback_Processing_Misconfigurations'
    ...
    $bhFile = Join-Path $bhDir "GPO-038_$bhTs.json"
    @{
