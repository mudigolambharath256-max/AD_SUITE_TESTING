...
Write-Host "=== Group Policy Check 20 ===" -ForegroundColor Cyan
...
try {
    ...
    Write-Warning "GPO-020 PowerShell engine: $_"   # Fix R10: no silent catch
}
...
try {
    ...
    Write-Warning "GPO-020 ADSI engine: $_"          # Fix R10
}
...
try {
    ...
    Write-Warning "GPO-020 C# engine: $_"             # Fix R10
}
...
Write-Host ""
Write-Host "=== GPO-020: $($uniqueResults.Count) unique findings ===" -ForegroundColor Cyan
$uniqueResults | Format-List

# Fix R14: Fixed 5-field schema enforced before CSV export
$csvPath = Join-Path $env:TEMP "GPO-020_results.csv"
$uniqueResults |
