...
Write-Host "Found $($results.Count) objects for check: GPOs with Scripts" -ForegroundColor Cyan

$results | ForEach-Object {
    $props = $_.Properties
    [PSCustomObject]@{
        CheckID = 'GPO-034'
        CheckName = 'GPOs with Scripts'
...
try {
    ...
    if ($results -and $results.Count -gt 0) {
        ...
        $bhFile = Join-Path $bhDir "GPO-034_nodes.json"
