...
Write-Host "=== Multi-Engine Execution: GPOs with Both Settings Disabled ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        $ldapFilter = '(&(objectClass=groupPolicyContainer)(flags=3))'
        $props = @('name', 'distinguishedName', 'displayName', 'flags', 'whenChanged')

        Get-ADObject -LDAPFilter $ldapFilter -Properties $props -ErrorAction Stop |
          Select-Object name, distinguishedName, displayName, flags, whenChanged |
          Sort-Object name |
          ForEach-Object { $_ }
...
public class Program
{
  ...
  {
    string filter = @"(&(objectClass=groupPolicyContainer)(flags=3))";
    string[] props = new string[] { "name", "distinguishedName", "displayName", "flags", "whenChanged" };
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'GPO-003'
                adSuiteCheckName  = 'GPOs_with_Both_Settings_Disabled'
    ...
    $bhFile = Join-Path $bhDir "GPO-003_$bhTs.json"
    @{
