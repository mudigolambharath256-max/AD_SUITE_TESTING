...
Write-Host "=== Group Policy Check 12 ===" -ForegroundColor Cyan
...
try {
    ...
    Write-Warning "GPO-012 PowerShell engine: $_"   # Fix R10: no silent catch
}
...
try {
    ...
    Write-Warning "GPO-012 ADSI engine: $_"          # Fix R10
}
...
try {
    ...
    Write-Warning "GPO-012 C# engine: $_"             # Fix R10
}
...
Write-Host ""
Write-Host "=== GPO-012: $($uniqueResults.Count) unique findings ===" -ForegroundColor Cyan
$uniqueResults | Format-List

# Fix R14: Fixed 5-field schema enforced before CSV export
$csvPath = Join-Path $env:TEMP "GPO-012_results.csv"
$uniqueResults |
