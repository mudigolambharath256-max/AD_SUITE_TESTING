...
Write-Host "=== Multi-Engine Execution: OUs Without GPO Links ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        $props = @('name', 'distinguishedName', 'ou')

        Get-ADObject -LDAPFilter $ldapFilter -Properties $props -ErrorAction Stop |
          Select-Object name, distinguishedName, ou |
          Sort-Object name |
          ForEach-Object { $_ }
...
public class Program
{
  public static void Run()
  {
    ...
    string[] props = new string[] { "name", "distinguishedName", "ou" };
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'GPO-013'
                adSuiteCheckName  = 'OUs_Without_GPO_Links'
    ...
    $bhFile = Join-Path $bhDir "GPO-013_$bhTs.json"
    @{
