...
Write-Host "=== Multi-Engine Execution: GPOs with WMI Filters ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        $props = @('name', 'distinguishedName', 'displayName', 'gPCWQLFilter')

        Get-ADObject -LDAPFilter $ldapFilter -Properties $props -ErrorAction Stop |
          Select-Object name, distinguishedName, displayName, gPCWQLFilter |
          Sort-Object name |
          ForEach-Object { $_ }
...
public class Program
{
  ...
  {
    string filter = @"(&(objectClass=groupPolicyContainer)(gPCWQLFilter=*))";
    string[] props = new string[] { "name", "distinguishedName", "displayName", "gPCWQLFilter" };
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'GPO-005'
                adSuiteCheckName  = 'GPOs_with_WMI_Filters'
    ...
    $bhFile = Join-Path $bhDir "GPO-005_$bhTs.json"
    @{
