...
Write-Host "=== Multi-Engine Execution: Audit Policy GPO Check ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        $ldapFilter = '(&(objectClass=groupPolicyContainer)(|(displayName=*audit*)(displayName=*Audit*)(displayName=*logging*)))'
...
public class Program
{
  ...
  {
    string filter = @"(&(objectClass=groupPolicyContainer)(|(displayName=*audit*)(displayName=*Audit*)(displayName=*logging*)))";
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'GPO-024'
                adSuiteCheckName  = 'Audit_Policy_GPO_Check'
    ...
    $bhFile = Join-Path $bhDir "GPO-024_$bhTs.json"
    @{
