...
Write-Host "=== Multi-Engine Execution: Domain Password Policy (from Domain Object) ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        $props = @('name', 'distinguishedName', 'minPwdLength', 'pwdHistoryLength', 'lockoutThreshold', 'lockoutDuration', 'maxPwdAge', 'minPwdAge', 'pwdProperties')

        Get-ADObject -LDAPFilter $ldapFilter -Properties $props -ErrorAction Stop |
          Select-Object name, distinguishedName, minPwdLength, pwdHistoryLength, lockoutThreshold, lockoutDuration, maxPwdAge, minPwdAge, pwdProperties |
          Sort-Object name |
          ForEach-Object { $_ }
...
public class Program
{
  public static void Run()
  {
    ...
    string[] props = new string[] { "name", "distinguishedName", "minPwdLength", "pwdHistoryLength", "lockoutThreshold", "lockoutDuration", "maxPwdAge", "minPwdAge", "pwdProperties" };
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'GPO-011'
                adSuiteCheckName  = 'Domain_Password_Policy_from_Domain_Object'
    ...
    $bhFile = Join-Path $bhDir "GPO-011_$bhTs.json"
    @{
