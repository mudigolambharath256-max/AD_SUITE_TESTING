...
@('name', 'distinguishedName', 'minPwdLength', 'pwdHistoryLength', 'lockoutThreshold', 'lockoutDuration', 'maxPwdAge', 'minPwdAge', 'pwdProperties', 'objectSid', 'samAccountName') | ForEach-Object { [void]$searcher.PropertiesToLoad.Add($_) }
...
try {
    ...
    if ($results -and $results.Count -gt 0) {
        ...
        $bhFile = Join-Path $bhDir "GPO-011_nodes.json"
    ...
    Write-Warning "[BloodHound] Export failed: # Check: Domain Password Policy (from Domain Object)
# Category: Group Policy
# Severity: info
# ID: GPO-011
# Requirements: None
