...
Write-Host "=== Multi-Engine Execution: GPOs Modified Recently ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        $props = @('name', 'distinguishedName', 'displayName', 'whenChanged', 'versionNumber')

        Get-ADObject -LDAPFilter $ldapFilter -Properties $props -ErrorAction Stop |
          Select-Object name, distinguishedName, displayName, whenChanged, versionNumber |
          Sort-Object name |
          ForEach-Object { $_ }
...
public class Program
{
  public static void Run()
  {
    ...
    string[] props = new string[] { "name", "distinguishedName", "displayName", "whenChanged", "versionNumber" };
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'GPO-010'
                adSuiteCheckName  = 'GPOs_Modified_Recently'
    ...
    $bhFile = Join-Path $bhDir "GPO-010_$bhTs.json"
    @{
