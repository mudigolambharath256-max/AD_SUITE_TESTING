...
Write-Host "=== Group Policy Check 37 ===" -ForegroundColor Cyan
...
try {
    ...
    Write-Warning "GPO-037 PowerShell engine: $_"   # Fix R10: no silent catch
}
...
try {
    ...
    Write-Warning "GPO-037 ADSI engine: $_"          # Fix R10
}
...
try {
    ...
    Write-Warning "GPO-037 C# engine: $_"             # Fix R10
}
...
Write-Host ""
Write-Host "=== GPO-037: $($uniqueResults.Count) unique findings ===" -ForegroundColor Cyan
$uniqueResults | Format-List

# Fix R14: Fixed 5-field schema enforced before CSV export
$csvPath = Join-Path $env:TEMP "GPO-037_results.csv"
$uniqueResults |
