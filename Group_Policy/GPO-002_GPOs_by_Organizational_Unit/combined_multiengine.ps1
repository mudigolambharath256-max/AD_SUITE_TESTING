...
Write-Host "=== Multi-Engine Execution: GPOs by Organizational Unit ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        $ldapFilter = '(&(objectClass=organizationalUnit)(gPLink=*))'
        $props = @('name', 'distinguishedName', 'ou', 'gPLink')

        Get-ADObject -LDAPFilter $ldapFilter -Properties $props -ErrorAction Stop |
          Select-Object name, distinguishedName, ou, gPLink |
          Sort-Object name |
          ForEach-Object { $_ }
...
public class Program
{
  ...
  {
    string filter = @"(&(objectClass=organizationalUnit)(gPLink=*))";
    string[] props = new string[] { "name", "distinguishedName", "ou", "gPLink" };
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'GPO-002'
                adSuiteCheckName  = 'GPOs_by_Organizational_Unit'
    ...
    $bhFile = Join-Path $bhDir "GPO-002_$bhTs.json"
    @{
