...
Write-Host "=== Multi-Engine Execution: Starter GPOs ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        $ldapFilter = '(objectClass=msGPOsForConfiguration)'
...
public class Program
{
  ...
  {
    string filter = @"(objectClass=msGPOsForConfiguration)";
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'GPO-016'
                adSuiteCheckName  = 'Starter_GPOs'
    ...
    $bhFile = Join-Path $bhDir "GPO-016_$bhTs.json"
    @{
