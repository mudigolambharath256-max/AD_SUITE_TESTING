...
Write-Host "=== Multi-Engine Execution: GPO Links on Sites ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        $ldapFilter = '(&(objectClass=site)(gPLink=*))'
        $props = @('name', 'distinguishedName', 'cn', 'gPLink')

        Get-ADObject -LDAPFilter $ldapFilter -Properties $props -ErrorAction Stop |
          Select-Object name, distinguishedName, cn, gPLink |
          Sort-Object name |
          ForEach-Object { $_ }
...
public class Program
{
  ...
  {
    string filter = @"(&(objectClass=site)(gPLink=*))";
    string[] props = new string[] { "name", "distinguishedName", "cn", "gPLink" };
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'GPO-019'
                adSuiteCheckName  = 'GPO_Links_on_Sites'
    ...
    $bhFile = Join-Path $bhDir "GPO-019_$bhTs.json"
    @{
