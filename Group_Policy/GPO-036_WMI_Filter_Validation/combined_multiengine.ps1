# ============================================================================
# GPO-036: WMI Filter Validation - Multi-Engine
# ============================================================================

[CmdletBinding()]
param([Parameter()][string]$SearchBase, [Parameter()][string]$ExportPath)

Write-Host "[GPO-036] WMI Filter Validation" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    $results = Get-ADObject -LDAPFilter '(objectClass=msWMI-Som)' -Properties name,distinguishedName -SearchBase $SearchBase -SearchScope Subtree -ResultSetSize $null -ErrorAction SilentlyContinue
} else {
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'GPO-036'
                adSuiteCheckName  = 'WMI_Filter_Validation'
    ...
    $bhFile = Join-Path $bhDir "GPO-036_$bhTs.json"
    @{
