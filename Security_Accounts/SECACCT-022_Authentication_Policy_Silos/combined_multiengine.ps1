...
Write-Host "=== SECACCT-022: Authentication Policy Silos ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        # Execute query
        $adObjects = Get-ADObject -LDAPFilter '(objectClass=msDS-AuthNPolicySilo)' `
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'SECACCT-022'}}, `
            @{N='CheckName'; E={'Authentication Policy Silos'}}, `
...
if (-not $useADModule) {
    ...
    try {
        ...
        $searcher.Filter = '(objectClass=msDS-AuthNPolicySilo)'
        ...
        $results = $adsiResults | ForEach-Object {
            ...
            [PSCustomObject]@{
                CheckID           = 'SECACCT-022'
                CheckName         = 'Authentication Policy Silos'
                Method            = 'ADSI'
                Domain            = $domainNC
...
try {
    ...
    $bhFile = Join-Path $bhDir "SECACCT-022_$bhTs.json"
    @{
