...
Write-Host "=== SECACCT-028: Claim Types ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        # Execute query
        $adObjects = Get-ADObject -LDAPFilter '(objectClass=msDS-ClaimType)' `
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'SECACCT-028'}}, `
            @{N='CheckName'; E={'Claim Types'}}, `
...
if (-not $useADModule) {
    ...
    try {
        ...
        $searcher.Filter = '(objectClass=msDS-ClaimType)'
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'SECACCT-028'
                adSuiteCheckName  = 'Claim_Types'
    ...
    $bhFile = Join-Path $bhDir "SECACCT-028_$bhTs.json"
    @{
