...
Write-Host "=== SECACCT-015: Managed Service Accounts ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        # Execute query
        $adObjects = Get-ADObject -LDAPFilter '(objectClass=msDS-ManagedServiceAccount)' `
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'SECACCT-015'}}, `
            @{N='CheckName'; E={'Managed Service Accounts'}}, `
...
if (-not $useADModule) {
    ...
    try {
        ...
        $searcher.Filter = '(objectClass=msDS-ManagedServiceAccount)'
        ...
        $results = $adsiResults | ForEach-Object {
            ...
            [PSCustomObject]@{
                CheckID           = 'SECACCT-015'
                CheckName         = 'Managed Service Accounts'
                Method            = 'ADSI'
                Domain            = $domainNC
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'SECACCT-015'
                adSuiteCheckName  = 'Managed_Service_Accounts'
    ...
    $bhFile = Join-Path $bhDir "SECACCT-015_$bhTs.json"
    @{
