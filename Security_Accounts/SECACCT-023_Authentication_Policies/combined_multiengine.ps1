...
Write-Host "=== SECACCT-023: Authentication Policies ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        # Execute query
        $adObjects = Get-ADObject -LDAPFilter '(objectClass=msDS-AuthNPolicy)' `
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'SECACCT-023'}}, `
            @{N='CheckName'; E={'Authentication Policies'}}, `
...
if (-not $useADModule) {
    ...
    try {
        ...
        $searcher.Filter = '(objectClass=msDS-AuthNPolicy)'
        ...
        $results = $adsiResults | ForEach-Object {
            ...
            [PSCustomObject]@{
                CheckID           = 'SECACCT-023'
                CheckName         = 'Authentication Policies'
                Method            = 'ADSI'
                Domain            = $domainNC
...
try {
    ...
    $bhFile = Join-Path $bhDir "SECACCT-023_$bhTs.json"
    @{
