...
Write-Host "=== SECACCT-010: PSO Lockout Duration ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'SECACCT-010'}}, `
            @{N='CheckName'; E={'PSO Lockout Duration'}}, `
...
if (-not $useADModule) {
    ...
    try {
        ...
        $results = $adsiResults | ForEach-Object {
            ...
            [PSCustomObject]@{
                CheckID           = 'SECACCT-010'
                CheckName         = 'PSO Lockout Duration'
                Method            = 'ADSI'
                Domain            = $domainNC
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'SECACCT-010'
                adSuiteCheckName  = 'PSO_Lockout_Duration'
    ...
    $bhFile = Join-Path $bhDir "SECACCT-010_$bhTs.json"
    @{
