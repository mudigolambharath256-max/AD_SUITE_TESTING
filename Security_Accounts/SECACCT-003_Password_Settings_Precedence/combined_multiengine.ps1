...
Write-Host "=== SECACCT-003: Password Settings Precedence ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'SECACCT-003'}}, `
            @{N='CheckName'; E={'Password Settings Precedence'}}, `
...
if (-not $useADModule) {
    ...
    try {
        ...
        $results = $adsiResults | ForEach-Object {
            ...
            [PSCustomObject]@{
                CheckID           = 'SECACCT-003'
                CheckName         = 'Password Settings Precedence'
                Method            = 'ADSI'
                Domain            = $domainNC
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'SECACCT-003'
                adSuiteCheckName  = 'Password_Settings_Precedence'
    ...
    $bhFile = Join-Path $bhDir "SECACCT-003_$bhTs.json"
    @{
