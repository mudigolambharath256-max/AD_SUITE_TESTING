...
Write-Host "=== SECACCT-029: Claims Transformation Policies ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        # Execute query
        $adObjects = Get-ADObject -LDAPFilter '(objectClass=msDS-ClaimsTransformationPolicyType)' `
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'SECACCT-029'}}, `
            @{N='CheckName'; E={'Claims Transformation Policies'}}, `
...
if (-not $useADModule) {
    ...
    try {
        ...
        $searcher.Filter = '(objectClass=msDS-ClaimsTransformationPolicyType)'
        ...
        $results = $adsiResults | ForEach-Object {
            ...
            [PSCustomObject]@{
                CheckID           = 'SECACCT-029'
                CheckName         = 'Claims Transformation Policies'
                Method            = 'ADSI'
                Domain            = $domainNC
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'SECACCT-029'
                adSuiteCheckName  = 'Claims_Transformation_Policies'
    ...
    $bhFile = Join-Path $bhDir "SECACCT-029_$bhTs.json"
    @{
