...
Write-Host "=== SECACCT-014: PSO Applied To Users ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'SECACCT-014'}}, `
            @{N='CheckName'; E={'PSO Applied To Users'}}, `
...
if (-not $useADModule) {
    ...
    try {
        ...
        $results = $adsiResults | ForEach-Object {
            ...
            [PSCustomObject]@{
                CheckID           = 'SECACCT-014'
                CheckName         = 'PSO Applied To Users'
                Method            = 'ADSI'
                Domain            = $domainNC
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'SECACCT-014'
                adSuiteCheckName  = 'PSO_Applied_To_Users'
    ...
    $bhFile = Join-Path $bhDir "SECACCT-014_$bhTs.json"
    @{
