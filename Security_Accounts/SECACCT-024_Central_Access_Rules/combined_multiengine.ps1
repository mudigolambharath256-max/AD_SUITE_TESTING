...
Write-Host "=== SECACCT-024: Central Access Rules ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        # Execute query
        $adObjects = Get-ADObject -LDAPFilter '(objectClass=msAuthz-CentralAccessRule)' `
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'SECACCT-024'}}, `
            @{N='CheckName'; E={'Central Access Rules'}}, `
...
if (-not $useADModule) {
    ...
    try {
        ...
        $searcher.Filter = '(objectClass=msAuthz-CentralAccessRule)'
        ...
        $results = $adsiResults | ForEach-Object {
            ...
            [PSCustomObject]@{
                CheckID           = 'SECACCT-024'
                CheckName         = 'Central Access Rules'
                Method            = 'ADSI'
                Domain            = $domainNC
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'SECACCT-024'
                adSuiteCheckName  = 'Central_Access_Rules'
    ...
    $bhFile = Join-Path $bhDir "SECACCT-024_$bhTs.json"
    @{
