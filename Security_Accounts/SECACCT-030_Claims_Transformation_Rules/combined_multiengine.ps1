...
Write-Host "=== SECACCT-030: Claims Transformation Rules ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        # Execute query
        $adObjects = Get-ADObject -LDAPFilter '(objectClass=msDS-ClaimsTransformationPolicy)' `
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'SECACCT-030'}}, `
            @{N='CheckName'; E={'Claims Transformation Rules'}}, `
...
if (-not $useADModule) {
    ...
    try {
        ...
        $searcher.Filter = '(objectClass=msDS-ClaimsTransformationPolicy)'
        ...
        $results = $adsiResults | ForEach-Object {
            ...
            [PSCustomObject]@{
                CheckID           = 'SECACCT-030'
                CheckName         = 'Claims Transformation Rules'
                Method            = 'ADSI'
                Domain            = $domainNC
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'SECACCT-030'
                adSuiteCheckName  = 'Claims_Transformation_Rules'
    ...
    $bhFile = Join-Path $bhDir "SECACCT-030_$bhTs.json"
    @{
