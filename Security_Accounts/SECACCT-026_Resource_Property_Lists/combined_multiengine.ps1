...
Write-Host "=== SECACCT-026: Resource Property Lists ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        # Execute query
        $adObjects = Get-ADObject -LDAPFilter '(objectClass=msAuthz-ResourcePropertyList)' `
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'SECACCT-026'}}, `
            @{N='CheckName'; E={'Resource Property Lists'}}, `
...
if (-not $useADModule) {
    ...
    try {
        ...
        $searcher.Filter = '(objectClass=msAuthz-ResourcePropertyList)'
        ...
        $results = $adsiResults | ForEach-Object {
            ...
            [PSCustomObject]@{
                CheckID           = 'SECACCT-026'
                CheckName         = 'Resource Property Lists'
                Method            = 'ADSI'
                Domain            = $domainNC
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'SECACCT-026'
                adSuiteCheckName  = 'Resource_Property_Lists'
    ...
    $bhFile = Join-Path $bhDir "SECACCT-026_$bhTs.json"
    @{
