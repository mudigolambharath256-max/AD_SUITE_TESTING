...
Write-Host "=== SECACCT-019: gMSA Principals Allowed to Retrieve ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        # Execute query
        $adObjects = Get-ADObject -LDAPFilter '(objectClass=msDS-GroupManagedServiceAccount)' `
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'SECACCT-019'}}, `
            @{N='CheckName'; E={'gMSA Principals Allowed to Retrieve'}}, `
...
if (-not $useADModule) {
    ...
    try {
        ...
        $results = $adsiResults | ForEach-Object {
            ...
            [PSCustomObject]@{
                CheckID           = 'SECACCT-019'
                CheckName         = 'gMSA Principals Allowed to Retrieve'
                Method            = 'ADSI'
                Domain            = $domainNC
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'SECACCT-019'
                adSuiteCheckName  = 'gMSA_Principals_Allowed_to_Retrieve'
    ...
    $bhFile = Join-Path $bhDir "SECACCT-019_$bhTs.json"
    @{
