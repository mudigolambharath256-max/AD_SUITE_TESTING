...
Write-Host "=== SECACCT-027: Resource Properties ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        # Execute query
        $adObjects = Get-ADObject -LDAPFilter '(objectClass=msDS-ResourceProperty)' `
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'SECACCT-027'}}, `
            @{N='CheckName'; E={'Resource Properties'}}, `
...
if (-not $useADModule) {
    ...
    try {
        ...
        $searcher.Filter = '(objectClass=msDS-ResourceProperty)'
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'SECACCT-027'
                adSuiteCheckName  = 'Resource_Properties'
    ...
    $bhFile = Join-Path $bhDir "SECACCT-027_$bhTs.json"
    @{
