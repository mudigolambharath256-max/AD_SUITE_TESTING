...
Write-Host "=== SECACCT-025: Central Access Policies ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        # Execute query
        $adObjects = Get-ADObject -LDAPFilter '(objectClass=msAuthz-CentralAccessPolicy)' `
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'SECACCT-025'}}, `
            @{N='CheckName'; E={'Central Access Policies'}}, `
...
if (-not $useADModule) {
    ...
    try {
        ...
        $searcher.Filter = '(objectClass=msAuthz-CentralAccessPolicy)'
        ...
        $results = $adsiResults | ForEach-Object {
            ...
            [PSCustomObject]@{
                CheckID           = 'SECACCT-025'
                CheckName         = 'Central Access Policies'
                Method            = 'ADSI'
                Domain            = $domainNC
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'SECACCT-025'
                adSuiteCheckName  = 'Central_Access_Policies'
    ...
    $bhFile = Join-Path $bhDir "SECACCT-025_$bhTs.json"
    @{
