...
Write-Host "=== SECACCT-008: PSO Minimum Password Age ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'SECACCT-008'}}, `
            @{N='CheckName'; E={'PSO Minimum Password Age'}}, `
...
if (-not $useADModule) {
    ...
    try {
        ...
        $results = $adsiResults | ForEach-Object {
            ...
            [PSCustomObject]@{
                CheckID           = 'SECACCT-008'
                CheckName         = 'PSO Minimum Password Age'
                Method            = 'ADSI'
                Domain            = $domainNC
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'SECACCT-008'
                adSuiteCheckName  = 'PSO_Minimum_Password_Age'
    ...
    $bhFile = Join-Path $bhDir "SECACCT-008_$bhTs.json"
    @{
