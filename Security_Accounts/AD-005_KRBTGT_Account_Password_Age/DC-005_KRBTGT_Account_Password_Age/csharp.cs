...
class Program
{
    static void Main()
    {
        ...
        string[] props = { "name", "distinguishedName", "samAccountName", "pwdLastSet" };

        using (DirectoryEntry root = new DirectoryEntry("LDAP://RootDSE"))
        using (DirectoryEntry domain = new DirectoryEntry("LDAP://" + root.Properties["defaultNamingContext"].Value))
        using (DirectorySearcher searcher = new DirectorySearcher(domain))
        {
            ...
            foreach (SearchResult r in searcher.FindAll())
            {
                ...
                string pwdLastSet = r.Properties.Contains("pwdLastSet") ? r.Properties["pwdLastSet"][0].ToString() : "";

                Console.WriteLine("Label: KRBTGT Account Password Age");
                Console.WriteLine("Name: " + name);
                Console.WriteLine("DistinguishedName: " + dn);
                Console.WriteLine("SamAccountName: " + sam);
                Console.WriteLine("PwdLastSet: " + pwdLastSet);
                Console.WriteLine("-------------------------------------------");
            }
        }
    }
