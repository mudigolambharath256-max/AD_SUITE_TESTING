...
Write-Host "=== SECACCT-006: PSO Password History ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'SECACCT-006'}}, `
            @{N='CheckName'; E={'PSO Password History'}}, `
...
if (-not $useADModule) {
    ...
    try {
        ...
        $results = $adsiResults | ForEach-Object {
            ...
            [PSCustomObject]@{
                CheckID           = 'SECACCT-006'
                CheckName         = 'PSO Password History'
                Method            = 'ADSI'
                Domain            = $domainNC
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'SECACCT-006'
                adSuiteCheckName  = 'PSO_Password_History'
    ...
    $bhFile = Join-Path $bhDir "SECACCT-006_$bhTs.json"
    @{
