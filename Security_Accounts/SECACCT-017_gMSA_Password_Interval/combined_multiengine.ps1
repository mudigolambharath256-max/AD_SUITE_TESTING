...
Write-Host "=== SECACCT-017: gMSA Password Interval ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'SECACCT-017'}}, `
            @{N='CheckName'; E={'gMSA Password Interval'}}, `
...
if (-not $useADModule) {
    ...
    try {
        ...
        $results = $adsiResults | ForEach-Object {
            ...
            [PSCustomObject]@{
                CheckID           = 'SECACCT-017'
                CheckName         = 'gMSA Password Interval'
                Method            = 'ADSI'
                Domain            = $domainNC
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'SECACCT-017'
                adSuiteCheckName  = 'gMSA_Password_Interval'
    ...
    $bhFile = Join-Path $bhDir "SECACCT-017_$bhTs.json"
    @{
