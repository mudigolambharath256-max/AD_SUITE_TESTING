...
Write-Host "=== SECACCT-001: Password Settings Objects Fine-Grained ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        # Execute query
        $adObjects = Get-ADObject -LDAPFilter '(objectClass=msDS-PasswordSettings)' `
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'SECACCT-001'}}, `
            @{N='CheckName'; E={'Password Settings Objects Fine-Grained'}}, `
...
if (-not $useADModule) {
    ...
    try {
        ...
        $results = $adsiResults | ForEach-Object {
            ...
            [PSCustomObject]@{
                CheckID           = 'SECACCT-001'
                CheckName         = 'Password Settings Objects Fine-Grained'
                Method            = 'ADSI'
                Domain            = $domainNC
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'SECACCT-001'
                adSuiteCheckName  = 'Password_Settings_Objects_Fine-Grained'
    ...
    $bhFile = Join-Path $bhDir "SECACCT-001_$bhTs.json"
    @{
