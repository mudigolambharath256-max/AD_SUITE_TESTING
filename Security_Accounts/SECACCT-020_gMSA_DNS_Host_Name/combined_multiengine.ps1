...
Write-Host "=== SECACCT-020: gMSA DNS Host Name ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'SECACCT-020'}}, `
            @{N='CheckName'; E={'gMSA DNS Host Name'}}, `
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'SECACCT-020'
                adSuiteCheckName  = 'gMSA_DNS_Host_Name'
    ...
    $bhFile = Join-Path $bhDir "SECACCT-020_$bhTs.json"
    @{
