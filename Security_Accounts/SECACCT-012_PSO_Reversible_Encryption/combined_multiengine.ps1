...
Write-Host "=== SECACCT-012: PSO Reversible Encryption ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'SECACCT-012'}}, `
            @{N='CheckName'; E={'PSO Reversible Encryption'}}, `
...
if (-not $useADModule) {
    ...
    try {
        ...
        $results = $adsiResults | ForEach-Object {
            ...
            [PSCustomObject]@{
                CheckID           = 'SECACCT-012'
                CheckName         = 'PSO Reversible Encryption'
                Method            = 'ADSI'
                Domain            = $domainNC
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'SECACCT-012'
                adSuiteCheckName  = 'PSO_Reversible_Encryption'
    ...
    $bhFile = Join-Path $bhDir "SECACCT-012_$bhTs.json"
    @{
