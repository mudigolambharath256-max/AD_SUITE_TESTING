...
Write-Host "=== SECACCT-007: PSO Maximum Password Age ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'SECACCT-007'}}, `
            @{N='CheckName'; E={'PSO Maximum Password Age'}}, `
...
if (-not $useADModule) {
    ...
    try {
        ...
        $results = $adsiResults | ForEach-Object {
            ...
            [PSCustomObject]@{
                CheckID           = 'SECACCT-007'
                CheckName         = 'PSO Maximum Password Age'
                Method            = 'ADSI'
                Domain            = $domainNC
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'SECACCT-007'
                adSuiteCheckName  = 'PSO_Maximum_Password_Age'
    ...
    $bhFile = Join-Path $bhDir "SECACCT-007_$bhTs.json"
    @{
