...
Write-Host "=== SECACCT-021: gMSA Service Principal Names ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'SECACCT-021'}}, `
            @{N='CheckName'; E={'gMSA Service Principal Names'}}, `
...
if (-not $useADModule) {
    ...
    try {
        ...
        $results = $adsiResults | ForEach-Object {
            ...
            [PSCustomObject]@{
                CheckID           = 'SECACCT-021'
                CheckName         = 'gMSA Service Principal Names'
                Method            = 'ADSI'
                Domain            = $domainNC
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'SECACCT-021'
                adSuiteCheckName  = 'gMSA_Service_Principal_Names'
    ...
    $bhFile = Join-Path $bhDir "SECACCT-021_$bhTs.json"
    @{
