...
Write-Host "=== SECACCT-005: PSO Password Complexity ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'SECACCT-005'}}, `
            @{N='CheckName'; E={'PSO Password Complexity'}}, `
...
if (-not $useADModule) {
    ...
    try {
        ...
        $results = $adsiResults | ForEach-Object {
            ...
            [PSCustomObject]@{
                CheckID           = 'SECACCT-005'
                CheckName         = 'PSO Password Complexity'
                Method            = 'ADSI'
                Domain            = $domainNC
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'SECACCT-005'
                adSuiteCheckName  = 'PSO_Password_Complexity'
    ...
    $bhFile = Join-Path $bhDir "SECACCT-005_$bhTs.json"
    @{
