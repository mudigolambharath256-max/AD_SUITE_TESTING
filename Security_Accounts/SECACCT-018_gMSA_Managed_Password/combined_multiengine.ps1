...
Write-Host "=== SECACCT-018: gMSA Managed Password ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'SECACCT-018'}}, `
            @{N='CheckName'; E={'gMSA Managed Password'}}, `
...
if (-not $useADModule) {
    ...
    try {
        ...
        $results = $adsiResults | ForEach-Object {
            ...
            [PSCustomObject]@{
                CheckID           = 'SECACCT-018'
                CheckName         = 'gMSA Managed Password'
                Method            = 'ADSI'
                Domain            = $domainNC
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'SECACCT-018'
                adSuiteCheckName  = 'gMSA_Managed_Password'
    ...
    $bhFile = Join-Path $bhDir "SECACCT-018_$bhTs.json"
    @{
