...
Write-Host "=== SECACCT-013: PSO Applied To Groups ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'SECACCT-013'}}, `
            @{N='CheckName'; E={'PSO Applied To Groups'}}, `
...
if (-not $useADModule) {
    ...
    try {
        ...
        $results = $adsiResults | ForEach-Object {
            ...
            [PSCustomObject]@{
                CheckID           = 'SECACCT-013'
                CheckName         = 'PSO Applied To Groups'
                Method            = 'ADSI'
                Domain            = $domainNC
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'SECACCT-013'
                adSuiteCheckName  = 'PSO_Applied_To_Groups'
    ...
    $bhFile = Join-Path $bhDir "SECACCT-013_$bhTs.json"
    @{
