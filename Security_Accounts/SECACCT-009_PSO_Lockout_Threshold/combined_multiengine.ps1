...
Write-Host "=== SECACCT-009: PSO Lockout Threshold ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'SECACCT-009'}}, `
            @{N='CheckName'; E={'PSO Lockout Threshold'}}, `
...
if (-not $useADModule) {
    ...
    try {
        ...
        $results = $adsiResults | ForEach-Object {
            ...
            [PSCustomObject]@{
                CheckID           = 'SECACCT-009'
                CheckName         = 'PSO Lockout Threshold'
                Method            = 'ADSI'
                Domain            = $domainNC
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'SECACCT-009'
                adSuiteCheckName  = 'PSO_Lockout_Threshold'
    ...
    $bhFile = Join-Path $bhDir "SECACCT-009_$bhTs.json"
    @{
