...
Write-Host "=== SECACCT-011: PSO Lockout Observation Window ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'SECACCT-011'}}, `
            @{N='CheckName'; E={'PSO Lockout Observation Window'}}, `
...
if (-not $useADModule) {
    ...
    try {
        ...
        $results = $adsiResults | ForEach-Object {
            ...
            [PSCustomObject]@{
                CheckID           = 'SECACCT-011'
                CheckName         = 'PSO Lockout Observation Window'
                Method            = 'ADSI'
                Domain            = $domainNC
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'SECACCT-011'
                adSuiteCheckName  = 'PSO_Lockout_Observation_Window'
    ...
    $bhFile = Join-Path $bhDir "SECACCT-011_$bhTs.json"
    @{
