...
Write-Host "=== SECACCT-004: PSO Minimum Password Length ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'SECACCT-004'}}, `
            @{N='CheckName'; E={'PSO Minimum Password Length'}}, `
...
if (-not $useADModule) {
    ...
    try {
        ...
        $results = $adsiResults | ForEach-Object {
            ...
            [PSCustomObject]@{
                CheckID           = 'SECACCT-004'
                CheckName         = 'PSO Minimum Password Length'
                Method            = 'ADSI'
                Domain            = $domainNC
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'SECACCT-004'
                adSuiteCheckName  = 'PSO_Minimum_Password_Length'
    ...
    $bhFile = Join-Path $bhDir "SECACCT-004_$bhTs.json"
    @{
