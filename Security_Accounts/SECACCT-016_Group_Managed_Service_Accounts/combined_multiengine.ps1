...
Write-Host "=== SECACCT-016: Group Managed Service Accounts ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'SECACCT-016'}}, `
            @{N='CheckName'; E={'Group Managed Service Accounts'}}, `
...
if (-not $useADModule) {
    ...
    try {
        ...
        $results = $adsiResults | ForEach-Object {
            ...
            [PSCustomObject]@{
                CheckID           = 'SECACCT-016'
                CheckName         = 'Group Managed Service Accounts'
                Method            = 'ADSI'
                Domain            = $domainNC
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'SECACCT-016'
                adSuiteCheckName  = 'Group_Managed_Service_Accounts'
    ...
    $bhFile = Join-Path $bhDir "SECACCT-016_$bhTs.json"
    @{
