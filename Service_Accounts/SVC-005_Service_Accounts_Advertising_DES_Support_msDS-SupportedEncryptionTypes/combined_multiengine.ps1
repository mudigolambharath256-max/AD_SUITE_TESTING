...
# Check: Service Accounts Advertising DES Support (msDS-SupportedEncryptionTypes)
...
Write-Host "=== Multi-Engine Execution: Service Accounts Advertising DES Support (msDS-SupportedEncryptionTypes) ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        $ldapFilter = '(&(objectCategory=person)(objectClass=user)(servicePrincipalName=*)(!(userAccountControl:1.2.840.113556.1.4.803:=2)))(msDS-SupportedEncryptionTypes:1.2.840.113556.1.4.803:=3))'
        $props = @('name', 'distinguishedName', 'samAccountName', 'msDS-SupportedEncryptionTypes', 'servicePrincipalName')

        Get-ADObject -LDAPFilter $ldapFilter -Properties $props,userAccountControl,servicePrincipalName,msDS-SupportedEncryptionTypes,samAccountName -ErrorAction Stop |
          Select-Object name, distinguishedName, samAccountName, msDS-SupportedEncryptionTypes, servicePrincipalName |
          Sort-Object name |
          ForEach-Object { $_ }
...
try {
    $adsiResults = @(
        ...
        $searcher = [ADSISearcher]'(&(objectCategory=person)(objectClass=user)(servicePrincipalName=*)(!(userAccountControl:1.2.840.113556.1.4.803:=2)))(msDS-SupportedEncryptionTypes:1.2.840.113556.1.4.803:=3))'
        $searcher.PageSize = 1000
        $searcher.PropertiesToLoad.Clear()
        (@('name', 'distinguishedName', 'samAccountName', 'msDS-SupportedEncryptionTypes', 'servicePrincipalName', 'userAccountControl', 'objectSid') | ForEach-Object { [void]$searcher.PropertiesToLoad.Add($_) }

        $searcher.FindAll() | ForEach-Object {
          $p = $_.Properties
          [PSCustomObject]@{
            Label = 'Service Accounts Advertising DES Support (msDS-SupportedEncryptionTypes)'
            Name = $p['name'][0]
...
public class Program
{
  ...
  {
    string filter = @"(&(objectCategory=person)(objectClass=user)(servicePrincipalName=*)(!(userAccountControl:1.2.840.113556.1.4.803:=2)))(msDS-SupportedEncryptionTypes:1.2.840.113556.1.4.803:=3))";
    string[] props = new string[] { "name", "distinguishedName", "samAccountName", "msDS-SupportedEncryptionTypes", "servicePrincipalName" };
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'SVC-005'
                adSuiteCheckName  = 'Service_Accounts_Advertising_DES_Support_msDS-SupportedEncryptionTypes'
                adSuiteMEDIUM   = 'MEDIUM'
                adSuiteService_Accounts   = 'Service_Accounts'
                adSuiteFlag       = $true
            }
            Aces      = @()
            IsDeleted = $false
    ...
    $bhFile = Join-Path $bhDir "SVC-005_$bhTs.json"
    @{
