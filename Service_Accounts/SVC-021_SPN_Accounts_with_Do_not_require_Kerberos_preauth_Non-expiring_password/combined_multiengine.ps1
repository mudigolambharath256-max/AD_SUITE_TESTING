...
# Check: SPN Accounts with “Do not require Kerberos preauth” + Non-expiring password
...
Write-Host "=== Multi-Engine Execution: SPN Accounts with “Do not require Kerberos preauth” + Non-expiring password ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        $ldapFilter = '(&(objectCategory=person)(objectClass=user)(servicePrincipalName=*)(!(userAccountControl:1.2.840.113556.1.4.803:=2)))(userAccountControl:1.2.840.113556.1.4.803:=4194304)(userAccountControl:1.2.840.113556.1.4.803:=65536))'
        $props = @('name', 'distinguishedName', 'samAccountName', 'userAccountControl', 'servicePrincipalName')

        Get-ADObject -LDAPFilter $ldapFilter -Properties $props,servicePrincipalName,userAccountControl,samAccountName -ErrorAction Stop |
          Select-Object name, distinguishedName, samAccountName, userAccountControl, servicePrincipalName |
          Sort-Object name |
          ForEach-Object { $_ }
...
try {
    $adsiResults = @(
        ...
        $searcher = [ADSISearcher]'(&(objectCategory=person)(objectClass=user)(servicePrincipalName=*)(!(userAccountControl:1.2.840.113556.1.4.803:=2)))(userAccountControl:1.2.840.113556.1.4.803:=4194304)(userAccountControl:1.2.840.113556.1.4.803:=65536))'
        ...
        $searcher.FindAll() | ForEach-Object {
          $p = $_.Properties
          [PSCustomObject]@{
            Label = 'SPN Accounts with “Do not require Kerberos preauth” + Non-expiring password'
            Name = $p['name'][0]
...
public class Program
{
  ...
  {
    string filter = @"(&(objectCategory=person)(objectClass=user)(servicePrincipalName=*)(!(userAccountControl:1.2.840.113556.1.4.803:=2)))(userAccountControl:1.2.840.113556.1.4.803:=4194304)(userAccountControl:1.2.840.113556.1.4.803:=65536))";
    string[] props = new string[] { "name", "distinguishedName", "samAccountName", "userAccountControl", "servicePrincipalName" };
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'SVC-021'
                adSuiteCheckName  = 'SPN_Accounts_with_Do_not_require_Kerberos_preauth_Non-expiring_password'
    ...
    $bhFile = Join-Path $bhDir "SVC-021_$bhTs.json"
    @{
