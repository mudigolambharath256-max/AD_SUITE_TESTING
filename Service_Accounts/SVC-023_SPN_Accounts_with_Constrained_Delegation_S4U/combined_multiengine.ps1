...
Write-Host "=== Multi-Engine Execution: SPN Accounts with Constrained Delegation + S4U ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        $ldapFilter = '(&(objectCategory=person)(objectClass=user)(servicePrincipalName=*)(!(userAccountControl:1.2.840.113556.1.4.803:=2)))(msDS-AllowedToDelegateTo=*)(userAccountControl:1.2.840.113556.1.4.803:=16777216))'
        $props = @('name', 'distinguishedName', 'samAccountName', 'msDS-AllowedToDelegateTo', 'userAccountControl', 'servicePrincipalName')

        Get-ADObject -LDAPFilter $ldapFilter -Properties $props,servicePrincipalName,userAccountControl,msDS-AllowedToDelegateTo,samAccountName -ErrorAction Stop |
          Select-Object name, distinguishedName, samAccountName, msDS-AllowedToDelegateTo, userAccountControl, servicePrincipalName |
          Sort-Object name |
          ForEach-Object { $_ }
...
public class Program
{
  ...
  {
    string filter = @"(&(objectCategory=person)(objectClass=user)(servicePrincipalName=*)(!(userAccountControl:1.2.840.113556.1.4.803:=2)))(msDS-AllowedToDelegateTo=*)(userAccountControl:1.2.840.113556.1.4.803:=16777216))";
    string[] props = new string[] { "name", "distinguishedName", "samAccountName", "msDS-AllowedToDelegateTo", "userAccountControl", "servicePrincipalName" };
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'SVC-023'
                adSuiteCheckName  = 'SPN_Accounts_with_Constrained_Delegation_S4U'
    ...
    $bhFile = Join-Path $bhDir "SVC-023_$bhTs.json"
    @{
