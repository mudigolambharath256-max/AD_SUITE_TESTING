...
# Check: Service Accounts with LogonWorkstations Restriction
...
Write-Host "=== Multi-Engine Execution: Service Accounts with LogonWorkstations Restriction ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        $ldapFilter = '(&(objectCategory=person)(objectClass=user)(servicePrincipalName=*)(!(userAccountControl:1.2.840.113556.1.4.803:=2)))(userWorkstations=*))'
        $props = @('name', 'distinguishedName', 'samAccountName', 'userWorkstations', 'servicePrincipalName')
        ...
        Get-ADObject -LDAPFilter $ldapFilter -Properties $props,userAccountControl,servicePrincipalName,samAccountName -ErrorAction Stop |
          Select-Object name, distinguishedName, samAccountName, userWorkstations, servicePrincipalName |
          Sort-Object name |
          ForEach-Object { $_ }
...
try {
    $adsiResults = @(
        ...
        $searcher = [ADSISearcher]'(&(objectCategory=person)(objectClass=user)(servicePrincipalName=*)(!(userAccountControl:1.2.840.113556.1.4.803:=2)))(userWorkstations=*))'
        $searcher.PageSize = 1000
        $searcher.PropertiesToLoad.Clear()
        (@('name', 'distinguishedName', 'samAccountName', 'userWorkstations', 'servicePrincipalName', 'userAccountControl', 'objectSid') | ForEach-Object { [void]$searcher.PropertiesToLoad.Add($_) }

        $searcher.FindAll() | ForEach-Object {
          $p = $_.Properties
          [PSCustomObject]@{
            Label = 'Service Accounts with LogonWorkstations Restriction'
            Name = $p['name'][0]
...
public class Program
{
  ...
  {
    string filter = @"(&(objectCategory=person)(objectClass=user)(servicePrincipalName=*)(!(userAccountControl:1.2.840.113556.1.4.803:=2)))(userWorkstations=*))";
    string[] props = new string[] { "name", "distinguishedName", "samAccountName", "userWorkstations", "servicePrincipalName" };
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'SVC-025'
                adSuiteCheckName  = 'Service_Accounts_with_LogonWorkstations_Restriction'
    ...
    $bhFile = Join-Path $bhDir "SVC-025_$bhTs.json"
    @{
