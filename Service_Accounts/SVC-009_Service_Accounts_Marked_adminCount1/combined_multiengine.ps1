...
# Check: Service Accounts Marked adminCount=1
...
Write-Host "=== Multi-Engine Execution: Service Accounts Marked adminCount=1 ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        $ldapFilter = '(&(objectCategory=person)(objectClass=user)(servicePrincipalName=*)(!(userAccountControl:1.2.840.113556.1.4.803:=2)))(adminCount=1))'
        $props = @('name', 'distinguishedName', 'samAccountName', 'adminCount', 'memberOf', 'servicePrincipalName')
        ...
        Get-ADObject -LDAPFilter $ldapFilter -Properties $props,userAccountControl,servicePrincipalName,samAccountName -ErrorAction Stop |
          Select-Object name, distinguishedName, samAccountName, adminCount, memberOf, servicePrincipalName |
          Sort-Object name |
          ForEach-Object { $_ }
...
try {
    $adsiResults = @(
        ...
        $searcher = [ADSISearcher]'(&(objectCategory=person)(objectClass=user)(servicePrincipalName=*)(!(userAccountControl:1.2.840.113556.1.4.803:=2)))(adminCount=1))'
        $searcher.PageSize = 1000
        $searcher.PropertiesToLoad.Clear()
        (@('name', 'distinguishedName', 'samAccountName', 'adminCount', 'memberOf', 'servicePrincipalName', 'userAccountControl', 'objectSid') | ForEach-Object { [void]$searcher.PropertiesToLoad.Add($_) }

        $searcher.FindAll() | ForEach-Object {
          $p = $_.Properties
          [PSCustomObject]@{
            Label = 'Service Accounts Marked adminCount=1'
            Name = $p['name'][0]
...
public class Program
{
  ...
  {
    string filter = @"(&(objectCategory=person)(objectClass=user)(servicePrincipalName=*)(!(userAccountControl:1.2.840.113556.1.4.803:=2)))(adminCount=1))";
    string[] props = new string[] { "name", "distinguishedName", "samAccountName", "adminCount", "memberOf", "servicePrincipalName" };
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'SVC-009'
                adSuiteCheckName  = 'Service_Accounts_Marked_adminCount1'
    ...
    $bhFile = Join-Path $bhDir "SVC-009_$bhTs.json"
    @{
