...
# Check: Service Accounts not Marked “Sensitive and Cannot be Delegated”
...
Write-Host "=== Multi-Engine Execution: Service Accounts not Marked “Sensitive and Cannot be Delegated” ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        $ldapFilter = '(&(objectCategory=person)(objectClass=user)(servicePrincipalName=*)(!(userAccountControl:1.2.840.113556.1.4.803:=2)))(!(userAccountControl:1.2.840.113556.1.4.803:=1048576)))'
...
try {
    $adsiResults = @(
        ...
        $searcher = [ADSISearcher]'(&(objectCategory=person)(objectClass=user)(servicePrincipalName=*)(!(userAccountControl:1.2.840.113556.1.4.803:=2)))(!(userAccountControl:1.2.840.113556.1.4.803:=1048576)))'
        ...
        $searcher.FindAll() | ForEach-Object {
          $p = $_.Properties
          [PSCustomObject]@{
            Label = 'Service Accounts not Marked “Sensitive and Cannot be Delegated”'
            Name = $p['name'][0]
...
public class Program
{
  ...
  {
    string filter = @"(&(objectCategory=person)(objectClass=user)(servicePrincipalName=*)(!(userAccountControl:1.2.840.113556.1.4.803:=2)))(!(userAccountControl:1.2.840.113556.1.4.803:=1048576)))";
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'SVC-018'
                adSuiteCheckName  = 'Service_Accounts_not_Marked_Sensitive_and_Cannot_be_Delegated'
    ...
    $bhFile = Join-Path $bhDir "SVC-018_$bhTs.json"
    @{
