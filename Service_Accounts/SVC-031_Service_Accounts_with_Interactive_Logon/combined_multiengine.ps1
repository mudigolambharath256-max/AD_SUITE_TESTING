...
Write-Host "=== SVC-031: Service Accounts with Interactive Logon ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    $results = Get-ADObject -LDAPFilter '(&(objectCategory=person)(objectClass=user)(servicePrincipalName=*))' -Properties name,distinguishedName,servicePrincipalName,samAccountName -SearchBase $SearchBase -SearchScope Subtree -ResultSetSize $null
} else {
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                adSuiteCheckName  = 'Service_Accounts_with_Interactive_Logon'
    ...
    $bhFile = Join-Path $bhDir "SVC-031_$bhTs.json"
    @{
