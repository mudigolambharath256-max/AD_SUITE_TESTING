...
(@('name', 'distinguishedName', 'whenCreated', 'whenChanged', 'servicePrincipalName', 'samAccountName', 'objectSid') | ForEach-Object { [void]$searcher.PropertiesToLoad.Add($_)
}
...
Write-Host "Found $($results.Count) objects for check: Service Accounts with Interactive Logon" -ForegroundColor Cyan

$results | ForEach-Object {
    $props = $_.Properties
    [PSCustomObject]@{
        CheckID = 'SVC-031'
        CheckName = 'Service Accounts with Interactive Logon'
...
try {
    ...
    if ($results -and $results.Count -gt 0) {
        ...
        $bhFile = Join-Path $bhDir "SVC-031_nodes.json"
