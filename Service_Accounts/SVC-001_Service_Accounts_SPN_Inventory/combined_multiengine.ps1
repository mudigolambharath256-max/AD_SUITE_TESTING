...
# Check: Service Accounts (SPN Inventory)
...
Write-Host "=== Multi-Engine Execution: Service Accounts (SPN Inventory) ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        $ldapFilter = '(&(objectCategory=person)(objectClass=user)(servicePrincipalName=*)(!(userAccountControl:1.2.840.113556.1.4.803:=2))))'
        $props = @('name', 'distinguishedName', 'samAccountName', 'servicePrincipalName', 'description')
        ...
        Get-ADObject -LDAPFilter $ldapFilter -Properties $props,userAccountControl,servicePrincipalName,samAccountName -ErrorAction Stop |
          Select-Object name, distinguishedName, samAccountName, servicePrincipalName, description |
          Sort-Object name |
          ForEach-Object { $_ }
...
try {
    $adsiResults = @(
        ...
        (@('name', 'distinguishedName', 'samAccountName', 'servicePrincipalName', 'description', 'userAccountControl', 'objectSid') | ForEach-Object { [void]$searcher.PropertiesToLoad.Add($_) }

        $searcher.FindAll() | ForEach-Object {
          $p = $_.Properties
          [PSCustomObject]@{
            Label = 'Service Accounts (SPN Inventory)'
            Name = $p['name'][0]
...
public class Program
{
  ...
  {
    string filter = @"(&(objectCategory=person)(objectClass=user)(servicePrincipalName=*)(!(userAccountControl:1.2.840.113556.1.4.803:=2))))";
    string[] props = new string[] { "name", "distinguishedName", "samAccountName", "servicePrincipalName", "description" };
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'SVC-001'
                adSuiteCheckName  = 'Service_Accounts_SPN_Inventory'
    ...
    $bhFile = Join-Path $bhDir "SVC-001_$bhTs.json"
    @{
