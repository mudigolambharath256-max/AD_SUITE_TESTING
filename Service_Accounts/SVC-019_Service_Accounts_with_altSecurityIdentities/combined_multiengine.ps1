...
# Check: Service Accounts with altSecurityIdentities
...
Write-Host "=== Multi-Engine Execution: Service Accounts with altSecurityIdentities ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        $ldapFilter = '(&(objectCategory=person)(objectClass=user)(servicePrincipalName=*)(!(userAccountControl:1.2.840.113556.1.4.803:=2)))(altSecurityIdentities=*))'
        $props = @('name', 'distinguishedName', 'samAccountName', 'altSecurityIdentities', 'servicePrincipalName')

        Get-ADObject -LDAPFilter $ldapFilter -Properties $props,userAccountControl,servicePrincipalName,samAccountName -ErrorAction Stop |
          Select-Object name, distinguishedName, samAccountName, altSecurityIdentities, servicePrincipalName |
          Sort-Object name |
          ForEach-Object { $_ }
...
try {
    $adsiResults = @(
        ...
        $searcher = [ADSISearcher]'(&(objectCategory=person)(objectClass=user)(servicePrincipalName=*)(!(userAccountControl:1.2.840.113556.1.4.803:=2)))(altSecurityIdentities=*))'
        $searcher.PageSize = 1000
        $searcher.PropertiesToLoad.Clear()
        (@('name', 'distinguishedName', 'samAccountName', 'altSecurityIdentities', 'servicePrincipalName', 'userAccountControl', 'objectSid') | ForEach-Object { [void]$searcher.PropertiesToLoad.Add($_) }

        $searcher.FindAll() | ForEach-Object {
          $p = $_.Properties
          [PSCustomObject]@{
            Label = 'Service Accounts with altSecurityIdentities'
            Name = $p['name'][0]
...
public class Program
{
  ...
  {
    string filter = @"(&(objectCategory=person)(objectClass=user)(servicePrincipalName=*)(!(userAccountControl:1.2.840.113556.1.4.803:=2)))(altSecurityIdentities=*))";
    string[] props = new string[] { "name", "distinguishedName", "samAccountName", "altSecurityIdentities", "servicePrincipalName" };
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'SVC-019'
                adSuiteCheckName  = 'Service_Accounts_with_altSecurityIdentities'
    ...
    $bhFile = Join-Path $bhDir "SVC-019_$bhTs.json"
    @{
