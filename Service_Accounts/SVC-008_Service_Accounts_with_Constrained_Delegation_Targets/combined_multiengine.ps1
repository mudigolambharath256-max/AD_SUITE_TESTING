...
# Check: Service Accounts with Constrained Delegation Targets
...
Write-Host "=== Multi-Engine Execution: Service Accounts with Constrained Delegation Targets ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        $ldapFilter = '(&(objectCategory=person)(objectClass=user)(servicePrincipalName=*)(!(userAccountControl:1.2.840.113556.1.4.803:=2)))(msDS-AllowedToDelegateTo=*))'
        $props = @('name', 'distinguishedName', 'samAccountName', 'msDS-AllowedToDelegateTo', 'servicePrincipalName')

        Get-ADObject -LDAPFilter $ldapFilter -Properties $props,userAccountControl,servicePrincipalName,msDS-AllowedToDelegateTo,samAccountName -ErrorAction Stop |
          Select-Object name, distinguishedName, samAccountName, msDS-AllowedToDelegateTo, servicePrincipalName |
          Sort-Object name |
          ForEach-Object { $_ }
...
try {
    $adsiResults = @(
        ...
        $searcher = [ADSISearcher]'(&(objectCategory=person)(objectClass=user)(servicePrincipalName=*)(!(userAccountControl:1.2.840.113556.1.4.803:=2)))(msDS-AllowedToDelegateTo=*))'
        $searcher.PageSize = 1000
        $searcher.PropertiesToLoad.Clear()
        (@('name', 'distinguishedName', 'samAccountName', 'msDS-AllowedToDelegateTo', 'servicePrincipalName', 'userAccountControl', 'objectSid') | ForEach-Object { [void]$searcher.PropertiesToLoad.Add($_) }

        $searcher.FindAll() | ForEach-Object {
          $p = $_.Properties
          [PSCustomObject]@{
            Label = 'Service Accounts with Constrained Delegation Targets'
            Name = $p['name'][0]
...
public class Program
{
  ...
  {
    string filter = @"(&(objectCategory=person)(objectClass=user)(servicePrincipalName=*)(!(userAccountControl:1.2.840.113556.1.4.803:=2)))(msDS-AllowedToDelegateTo=*))";
    string[] props = new string[] { "name", "distinguishedName", "samAccountName", "msDS-AllowedToDelegateTo", "servicePrincipalName" };
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'SVC-008'
                adSuiteCheckName  = 'Service_Accounts_with_Constrained_Delegation_Targets'
    ...
    $bhFile = Join-Path $bhDir "SVC-008_$bhTs.json"
    @{
