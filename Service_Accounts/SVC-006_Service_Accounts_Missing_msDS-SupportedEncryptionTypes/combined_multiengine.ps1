...
Write-Host "=== Multi-Engine Execution: Service Accounts Missing msDS-SupportedEncryptionTypes ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        $ldapFilter = '(&(objectCategory=person)(objectClass=user)(servicePrincipalName=*)(!(userAccountControl:1.2.840.113556.1.4.803:=2)))(!(msDS-SupportedEncryptionTypes=*)))'
        $props = @('name', 'distinguishedName', 'samAccountName', 'servicePrincipalName')

        Get-ADObject -LDAPFilter $ldapFilter -Properties $props,userAccountControl,msDS-SupportedEncryptionTypes,servicePrincipalName,samAccountName -ErrorAction Stop |
...
public class Program
{
  ...
  {
    string filter = @"(&(objectCategory=person)(objectClass=user)(servicePrincipalName=*)(!(userAccountControl:1.2.840.113556.1.4.803:=2)))(!(msDS-SupportedEncryptionTypes=*)))";
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'SVC-006'
                adSuiteCheckName  = 'Service_Accounts_Missing_msDS-SupportedEncryptionTypes'
    ...
    $bhFile = Join-Path $bhDir "SVC-006_$bhTs.json"
    @{
