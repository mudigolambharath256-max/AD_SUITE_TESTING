...
Write-Host "=== Multi-Engine Execution: Trust Attributes - Forest Transitive ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        $ldapFilter = '(&(objectClass=trustedDomain)(trustAttributes:1.2.840.113556.1.4.803:=8))'
...
public class Program
{
  ...
  {
    string filter = @"(&(objectClass=trustedDomain)(trustAttributes:1.2.840.113556.1.4.803:=8))";
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'TRST-026'
                adSuiteCheckName  = 'Trust_Attributes_-_Forest_Transitive'
    ...
    $bhFile = Join-Path $bhDir "TRST-026_$bhTs.json"
    @{
