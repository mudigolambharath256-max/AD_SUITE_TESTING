...
Write-Host "=== Multi-Engine Execution: Inbound Trusts ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        $ldapFilter = '(&(objectClass=trustedDomain)(trustDirection=1))'
...
public class Program
{
  ...
  {
    string filter = @"(&(objectClass=trustedDomain)(trustDirection=1))";
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'TRST-003'
                adSuiteCheckName  = 'Inbound_Trusts'
    ...
    $bhFile = Join-Path $bhDir "TRST-003_$bhTs.json"
    @{
