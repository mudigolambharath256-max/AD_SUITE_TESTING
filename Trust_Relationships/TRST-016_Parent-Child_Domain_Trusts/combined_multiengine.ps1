...
Write-Host "=== Multi-Engine Execution: Parent-Child Domain Trusts ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        $ldapFilter = '(&(objectClass=trustedDomain)(trustType=2)(trustAttributes:1.2.840.113556.1.4.803:=4))'
        $props = @('name', 'distinguishedName', 'cn', 'flatName', 'trustType', 'trustAttributes')

        Get-ADObject -LDAPFilter $ldapFilter -Properties $props -ErrorAction Stop |
          Select-Object name, distinguishedName, cn, flatName, trustType, trustAttributes |
          Sort-Object name |
          ForEach-Object { $_ }
...
public class Program
{
  ...
  {
    string filter = @"(&(objectClass=trustedDomain)(trustType=2)(trustAttributes:1.2.840.113556.1.4.803:=4))";
    string[] props = new string[] { "name", "distinguishedName", "cn", "flatName", "trustType", "trustAttributes" };
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'TRST-016'
                adSuiteCheckName  = 'Parent-Child_Domain_Trusts'
    ...
    $bhFile = Join-Path $bhDir "TRST-016_$bhTs.json"
    @{
