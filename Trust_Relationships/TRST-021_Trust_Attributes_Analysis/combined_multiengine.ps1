...
Write-Host "=== Multi-Engine Execution: Trust Attributes Analysis ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        $props = @('name', 'distinguishedName', 'cn', 'flatName', 'trustAttributes', 'trustDirection', 'trustType')

        Get-ADObject -LDAPFilter $ldapFilter -Properties $props -ErrorAction Stop |
          Select-Object name, distinguishedName, cn, flatName, trustAttributes, trustDirection, trustType |
          Sort-Object name |
          ForEach-Object { $_ }
...
public class Program
{
  public static void Run()
  {
    ...
    string[] props = new string[] { "name", "distinguishedName", "cn", "flatName", "trustAttributes", "trustDirection", "trustType" };
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'TRST-021'
                adSuiteCheckName  = 'Trust_Attributes_Analysis'
    ...
    $bhFile = Join-Path $bhDir "TRST-021_$bhTs.json"
    @{
