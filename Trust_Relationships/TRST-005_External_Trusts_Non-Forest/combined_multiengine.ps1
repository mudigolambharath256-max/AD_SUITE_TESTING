...
Write-Host "=== Multi-Engine Execution: External Trusts (Non-Forest) ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        $ldapFilter = '(&(objectClass=trustedDomain)(trustType=2))'
...
public class Program
{
  ...
  {
    string filter = @"(&(objectClass=trustedDomain)(trustType=2))";
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'TRST-005'
                adSuiteCheckName  = 'External_Trusts_Non-Forest'
    ...
    $bhFile = Join-Path $bhDir "TRST-005_$bhTs.json"
    @{
