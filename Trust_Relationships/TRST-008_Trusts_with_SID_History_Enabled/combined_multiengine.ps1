...
Write-Host "=== Multi-Engine Execution: Trusts with SID History Enabled ===" -ForegroundColor Cyan
Write-Host ""
...
public class Program
{
  ...
  {
    string filter = @"(&(objectClass=trustedDomain)(trustAttributes:1.2.840.113556.1.4.803:=32))";
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'TRST-008'
                adSuiteCheckName  = 'Trusts_with_SID_History_Enabled'
    ...
    $bhFile = Join-Path $bhDir "TRST-008_$bhTs.json"
    @{
