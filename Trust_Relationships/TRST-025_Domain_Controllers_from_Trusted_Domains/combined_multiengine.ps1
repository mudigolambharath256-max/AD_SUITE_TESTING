...
Write-Host "=== Multi-Engine Execution: Domain Controllers from Trusted Domains ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'TRST-025'
                adSuiteCheckName  = 'Domain_Controllers_from_Trusted_Domains'
    ...
    $bhFile = Join-Path $bhDir "TRST-025_$bhTs.json"
    @{
