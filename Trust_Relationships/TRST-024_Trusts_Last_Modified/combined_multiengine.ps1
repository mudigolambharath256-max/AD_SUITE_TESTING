...
Write-Host "=== Multi-Engine Execution: Trusts Last Modified ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        $props = @('name', 'distinguishedName', 'cn', 'flatName', 'whenChanged', 'modifyTimeStamp')

        Get-ADObject -LDAPFilter $ldapFilter -Properties $props -ErrorAction Stop |
          Select-Object name, distinguishedName, cn, flatName, whenChanged, modifyTimeStamp |
          Sort-Object name |
          ForEach-Object { $_ }
...
public class Program
{
  public static void Run()
  {
    ...
    string[] props = new string[] { "name", "distinguishedName", "cn", "flatName", "whenChanged", "modifyTimeStamp" };
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'TRST-024'
                adSuiteCheckName  = 'Trusts_Last_Modified'
    ...
    $bhFile = Join-Path $bhDir "TRST-024_$bhTs.json"
    @{
