...
Write-Host "=== Multi-Engine Execution: Trusts Without SID Filtering ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        $ldapFilter = '(&(objectClass=trustedDomain)(trustAttributes:1.2.840.113556.1.4.803:=64))'
        $props = @('name', 'distinguishedName', 'cn', 'flatName', 'trustDirection', 'trustAttributes')

        Get-ADObject -LDAPFilter $ldapFilter -Properties $props -ErrorAction Stop |
          Select-Object name, distinguishedName, cn, flatName, trustDirection, trustAttributes |
          Sort-Object name |
          ForEach-Object { $_ }
...
public class Program
{
  ...
  {
    string filter = @"(&(objectClass=trustedDomain)(trustAttributes:1.2.840.113556.1.4.803:=64))";
    string[] props = new string[] { "name", "distinguishedName", "cn", "flatName", "trustDirection", "trustAttributes" };
...
try {
    ...
    $bhFile = Join-Path $bhDir "TRST-007_$bhTs.json"
    @{
