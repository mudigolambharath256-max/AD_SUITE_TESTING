...
Write-Host "=== Multi-Engine Execution: Realm/MIT Kerberos Trusts ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        $ldapFilter = '(&(objectClass=trustedDomain)(trustType=3))'
        $props = @('name', 'distinguishedName', 'cn', 'flatName', 'trustType')

        Get-ADObject -LDAPFilter $ldapFilter -Properties $props -ErrorAction Stop |
          Select-Object name, distinguishedName, cn, flatName, trustType |
          Sort-Object name |
          ForEach-Object { $_ }
...
public class Program
{
  ...
  {
    string filter = @"(&(objectClass=trustedDomain)(trustType=3))";
    string[] props = new string[] { "name", "distinguishedName", "cn", "flatName", "trustType" };
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'TRST-011'
                adSuiteCheckName  = 'RealmMIT_Kerberos_Trusts'
    ...
    $bhFile = Join-Path $bhDir "TRST-011_$bhTs.json"
    @{
