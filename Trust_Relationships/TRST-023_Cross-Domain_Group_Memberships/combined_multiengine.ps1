...
Write-Host "=== Multi-Engine Execution: Cross-Domain Group Memberships ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        $ldapFilter = '(&(objectCategory=group)(member=*CN=ForeignSecurityPrincipals*))'
...
public class Program
{
  ...
  {
    string filter = @"(&(objectCategory=group)(member=*CN=ForeignSecurityPrincipals*))";
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'TRST-023'
                adSuiteCheckName  = 'Cross-Domain_Group_Memberships'
    ...
    $bhFile = Join-Path $bhDir "TRST-023_$bhTs.json"
    @{
