...
Write-Host "=== Multi-Engine Execution: Trust Objects Without FlatName ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        $ldapFilter = '(&(objectClass=trustedDomain)(!(flatName=*)))'
...
public class Program
{
  ...
  {
    string filter = @"(&(objectClass=trustedDomain)(!(flatName=*)))";
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'TRST-029'
                adSuiteCheckName  = 'Trust_Objects_Without_FlatName'
    ...
    $bhFile = Join-Path $bhDir "TRST-029_$bhTs.json"
    @{
