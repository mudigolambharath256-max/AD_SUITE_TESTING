...
Write-Host "=== Multi-Engine Execution: Foreign Security Principals ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        $props = @('name', 'distinguishedName', 'cn', 'objectSid')

        Get-ADObject -LDAPFilter $ldapFilter -Properties $props -ErrorAction Stop |
          Select-Object name, distinguishedName, cn, objectSid |
          Sort-Object name |
          ForEach-Object { $_ }
...
public class Program
{
  ...
  {
    string filter = @"(objectClass=foreignSecurityPrincipal)";
    string[] props = new string[] { "name", "distinguishedName", "cn", "objectSid" };
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'TRST-018'
                adSuiteCheckName  = 'Foreign_Security_Principals'
    ...
    $bhFile = Join-Path $bhDir "TRST-018_$bhTs.json"
    @{
