...
Write-Host "=== Multi-Engine Execution: Forest Trusts ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        $ldapFilter = '(&(objectClass=trustedDomain)(trustType=2)(trustAttributes:1.2.840.113556.1.4.803:=8))'
...
public class Program
{
  ...
  {
    string filter = @"(&(objectClass=trustedDomain)(trustType=2)(trustAttributes:1.2.840.113556.1.4.803:=8))";
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'TRST-006'
                adSuiteCheckName  = 'Forest_Trusts'
    ...
    $bhFile = Join-Path $bhDir "TRST-006_$bhTs.json"
    @{
