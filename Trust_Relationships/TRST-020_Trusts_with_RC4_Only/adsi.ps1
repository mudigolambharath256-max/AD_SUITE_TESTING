...
(@('name', 'distinguishedName', 'trustAttributes', 'trustDirection', 'trustType', 'trustPartner', 'flatName', 'whenCreated', 'whenChanged', 'msDS-SupportedEncryptionTypes', 'objectSid', 'samAccountName') | ForEach-Object { [void]$searcher.PropertiesToLoad.Add($_)
}.Add($_) }
...
try {
    ...
    if ($results -and $results.Count -gt 0) {
        ...
        $bhFile = Join-Path $bhDir "TRST-020_nodes.json"
    ...
    Write-Warning "[BloodHound] Export failed: # Check: Trusts with RC4 Only
# Category: Trust Relationships
# Severity: high
# ID: TRST-020
# Requirements: None
