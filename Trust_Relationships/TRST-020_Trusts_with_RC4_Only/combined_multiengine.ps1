...
Write-Host "=== Multi-Engine Execution: Trusts with RC4 Only ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        $ldapFilter = '(&(objectClass=trustedDomain)(!(msDS-SupportedEncryptionTypes:1.2.840.113556.1.4.803:=24)))'
        $props = @('name', 'distinguishedName', 'cn', 'flatName', 'msDS-SupportedEncryptionTypes')

        Get-ADObject -LDAPFilter $ldapFilter -Properties $props,msDS-SupportedEncryptionTypes -ErrorAction Stop |
          Select-Object name, distinguishedName, cn, flatName, msDS-SupportedEncryptionTypes |
          Sort-Object name |
          ForEach-Object { $_ }
...
public class Program
{
  ...
  {
    string filter = @"(&(objectClass=trustedDomain)(!(msDS-SupportedEncryptionTypes:1.2.840.113556.1.4.803:=24)))";
    string[] props = new string[] { "name", "distinguishedName", "cn", "flatName", "msDS-SupportedEncryptionTypes" };
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'TRST-020'
                adSuiteCheckName  = 'Trusts_with_RC4_Only'
    ...
    $bhFile = Join-Path $bhDir "TRST-020_$bhTs.json"
    @{
