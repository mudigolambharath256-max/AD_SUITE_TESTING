...
# Check: Trusts with Selective Authentication
...
Write-Host "=== Multi-Engine Execution: Trusts with Selective Authentication ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        $ldapFilter = '(&(objectClass=trustedDomain)(trustAttributes:1.2.840.113556.1.4.803:=16))'
        $props = @('name', 'distinguishedName', 'cn', 'flatName', 'trustAttributes')
...
try {
    $adsiResults = @(
        ...
        $searcher = [ADSISearcher]'(&(objectClass=trustedDomain)(trustAttributes:1.2.840.113556.1.4.803:=16))'
        ...
        $searcher.FindAll() | ForEach-Object {
          $p = $_.Properties
          [PSCustomObject]@{
            Label = 'Trusts with Selective Authentication'
            Name = $p['name'][0]
...
public class Program
{
  ...
  {
    string filter = @"(&(objectClass=trustedDomain)(trustAttributes:1.2.840.113556.1.4.803:=16))";
    string[] props = new string[] { "name", "distinguishedName", "cn", "flatName", "trustAttributes" };
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'TRST-010'
                adSuiteCheckName  = 'Trusts_with_Selective_Authentication'
    ...
    $bhFile = Join-Path $bhDir "TRST-010_$bhTs.json"
    @{
