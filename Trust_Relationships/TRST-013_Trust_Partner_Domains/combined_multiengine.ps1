...
Write-Host "=== Multi-Engine Execution: Trust Partner Domains ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        $props = @('name', 'distinguishedName', 'cn', 'trustPartner', 'flatName')

        Get-ADObject -LDAPFilter $ldapFilter -Properties $props -ErrorAction Stop |
          Select-Object name, distinguishedName, cn, trustPartner, flatName |
          Sort-Object name |
          ForEach-Object { $_ }
...
public class Program
{
  public static void Run()
  {
    ...
    string[] props = new string[] { "name", "distinguishedName", "cn", "trustPartner", "flatName" };
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'TRST-013'
                adSuiteCheckName  = 'Trust_Partner_Domains'
    ...
    $bhFile = Join-Path $bhDir "TRST-013_$bhTs.json"
    @{
