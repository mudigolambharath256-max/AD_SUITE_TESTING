...
Write-Host "=== Multi-Engine Execution: Orphaned Foreign Security Principals ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        $ldapFilter = '(&(objectClass=foreignSecurityPrincipal)(cn=S-1-5-21-*))'
        $props = @('name', 'distinguishedName', 'cn', 'objectSid', 'memberOf')

        Get-ADObject -LDAPFilter $ldapFilter -Properties $props -ErrorAction Stop |
          Select-Object name, distinguishedName, cn, objectSid, memberOf |
          Sort-Object name |
          ForEach-Object { $_ }
...
public class Program
{
  ...
  {
    string filter = @"(&(objectClass=foreignSecurityPrincipal)(cn=S-1-5-21-*))";
    string[] props = new string[] { "name", "distinguishedName", "cn", "objectSid", "memberOf" };
...
try {
    ...
    $bhFile = Join-Path $bhDir "TRST-019_$bhTs.json"
    @{
