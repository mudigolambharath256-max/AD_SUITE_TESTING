...
Write-Host "=== Multi-Engine Execution: Trust Security Identifiers ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        $props = @('name', 'distinguishedName', 'cn', 'flatName', 'securityIdentifier')

        Get-ADObject -LDAPFilter $ldapFilter -Properties $props -ErrorAction Stop |
          Select-Object name, distinguishedName, cn, flatName, securityIdentifier |
          Sort-Object name |
          ForEach-Object { $_ }
...
public class Program
{
  public static void Run()
  {
    ...
    string[] props = new string[] { "name", "distinguishedName", "cn", "flatName", "securityIdentifier" };
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'TRST-015'
                adSuiteCheckName  = 'Trust_Security_Identifiers'
    ...
    $bhFile = Join-Path $bhDir "TRST-015_$bhTs.json"
    @{
