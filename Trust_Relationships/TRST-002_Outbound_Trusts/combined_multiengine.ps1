...
Write-Host "=== Multi-Engine Execution: Outbound Trusts ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        $ldapFilter = '(&(objectClass=trustedDomain)(trustDirection=2))'
...
public class Program
{
  ...
  {
    string filter = @"(&(objectClass=trustedDomain)(trustDirection=2))";
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'TRST-002'
                adSuiteCheckName  = 'Outbound_Trusts'
    ...
    $bhFile = Join-Path $bhDir "TRST-002_$bhTs.json"
    @{
