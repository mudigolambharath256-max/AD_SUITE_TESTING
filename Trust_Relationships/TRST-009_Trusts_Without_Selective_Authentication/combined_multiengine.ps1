...
Write-Host "=== Multi-Engine Execution: Trusts Without Selective Authentication ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        $ldapFilter = '(&(objectClass=trustedDomain)(trustType=2)(!(trustAttributes:1.2.840.113556.1.4.803:=16)))'
...
public class Program
{
  ...
  {
    string filter = @"(&(objectClass=trustedDomain)(trustType=2)(!(trustAttributes:1.2.840.113556.1.4.803:=16)))";
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'TRST-009'
                adSuiteCheckName  = 'Trusts_Without_Selective_Authentication'
    ...
    $bhFile = Join-Path $bhDir "TRST-009_$bhTs.json"
    @{
