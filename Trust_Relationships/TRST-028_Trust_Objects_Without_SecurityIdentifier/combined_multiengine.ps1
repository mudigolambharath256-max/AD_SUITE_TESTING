...
Write-Host "=== Multi-Engine Execution: Trust Objects Without SecurityIdentifier ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        $ldapFilter = '(&(objectClass=trustedDomain)(!(securityIdentifier=*)))'
        $props = @('name', 'distinguishedName', 'cn', 'flatName')

        Get-ADObject -LDAPFilter $ldapFilter -Properties $props -ErrorAction Stop |
          Select-Object name, distinguishedName, cn, flatName |
          Sort-Object name |
          ForEach-Object { $_ }
...
public class Program
{
  ...
  {
    string filter = @"(&(objectClass=trustedDomain)(!(securityIdentifier=*)))";
    string[] props = new string[] { "name", "distinguishedName", "cn", "flatName" };
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'TRST-028'
                adSuiteCheckName  = 'Trust_Objects_Without_SecurityIdentifier'
    ...
    $bhFile = Join-Path $bhDir "TRST-028_$bhTs.json"
    @{
