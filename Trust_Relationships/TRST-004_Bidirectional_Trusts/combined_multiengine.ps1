...
Write-Host "=== Multi-Engine Execution: Bidirectional Trusts ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        $ldapFilter = '(&(objectClass=trustedDomain)(trustDirection=3))'
...
public class Program
{
  ...
  {
    string filter = @"(&(objectClass=trustedDomain)(trustDirection=3))";
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'TRST-004'
                adSuiteCheckName  = 'Bidirectional_Trusts'
    ...
    $bhFile = Join-Path $bhDir "TRST-004_$bhTs.json"
    @{
