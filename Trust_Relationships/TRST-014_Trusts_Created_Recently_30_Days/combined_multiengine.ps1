...
Write-Host "=== Multi-Engine Execution: Trusts Created Recently (30 Days) ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        $props = @('name', 'distinguishedName', 'cn', 'flatName', 'whenCreated', 'whenChanged')

        Get-ADObject -LDAPFilter $ldapFilter -Properties $props -ErrorAction Stop |
          Select-Object name, distinguishedName, cn, flatName, whenCreated, whenChanged |
          Sort-Object name |
          ForEach-Object { $_ }
...
public class Program
{
  public static void Run()
  {
    ...
    string[] props = new string[] { "name", "distinguishedName", "cn", "flatName", "whenCreated", "whenChanged" };
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'TRST-014'
                adSuiteCheckName  = 'Trusts_Created_Recently_30_Days'
    ...
    $bhFile = Join-Path $bhDir "TRST-014_$bhTs.json"
    @{
