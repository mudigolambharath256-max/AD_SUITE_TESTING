...
@('name', 'distinguishedName', 'cn', 'flatName', 'trustDirection', 'trustType', 'trustAttributes', 'securityIdentifier', 'trustPartner', 'objectSid', 'samAccountName') | ForEach-Object { [void]$searcher.PropertiesToLoad.Add($_) }
...
try {
    ...
    if ($results -and $results.Count -gt 0) {
        ...
        $bhFile = Join-Path $bhDir "TRST-001_nodes.json"
    ...
    Write-Warning "[BloodHound] Export failed: # Check: Trust Inventory (All Trusts)
# Category: Trust Relationships
# Severity: info
# ID: TRST-001
# Requirements: None
