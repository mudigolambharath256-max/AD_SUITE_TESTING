...
Write-Host "=== Multi-Engine Execution: Trust Inventory (All Trusts) ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        $props = @('name', 'distinguishedName', 'cn', 'flatName', 'trustDirection', 'trustType', 'trustAttributes', 'securityIdentifier', 'trustPartner')

        Get-ADObject -LDAPFilter $ldapFilter -Properties $props -ErrorAction Stop |
          Select-Object name, distinguishedName, cn, flatName, trustDirection, trustType, trustAttributes, securityIdentifier, trustPartner |
          Sort-Object name |
          ForEach-Object { $_ }
...
public class Program
{
  public static void Run()
  {
    ...
    string[] props = new string[] { "name", "distinguishedName", "cn", "flatName", "trustDirection", "trustType", "trustAttributes", "securityIdentifier", "trustPartner" };
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'TRST-001'
                adSuiteCheckName  = 'Trust_Inventory_All_Trusts'
    ...
    $bhFile = Join-Path $bhDir "TRST-001_$bhTs.json"
    @{
