...
Write-Host "=== Multi-Engine Execution: Trust Authentication Summary ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        $props = @('name', 'distinguishedName', 'cn', 'flatName', 'trustPartner', 'trustDirection', 'trustType', 'trustAttributes', 'securityIdentifier')

        Get-ADObject -LDAPFilter $ldapFilter -Properties $props -ErrorAction Stop |
          Select-Object name, distinguishedName, cn, flatName, trustPartner, trustDirection, trustType, trustAttributes, securityIdentifier |
          Sort-Object name |
          ForEach-Object { $_ }
...
public class Program
{
  public static void Run()
  {
    ...
    string[] props = new string[] { "name", "distinguishedName", "cn", "flatName", "trustPartner", "trustDirection", "trustType", "trustAttributes", "securityIdentifier" };
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'TRST-030'
                adSuiteCheckName  = 'Trust_Authentication_Summary'
    ...
    $bhFile = Join-Path $bhDir "TRST-030_$bhTs.json"
    @{
