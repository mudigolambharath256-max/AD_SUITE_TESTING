...
@('name', 'distinguishedName', 'cn', 'flatName', 'trustPartner', 'trustDirection', 'trustType', 'trustAttributes', 'securityIdentifier', 'objectSid', 'samAccountName') | ForEach-Object { [void]$searcher.PropertiesToLoad.Add($_) }
...
try {
    ...
    if ($results -and $results.Count -gt 0) {
        ...
        $bhFile = Join-Path $bhDir "TRST-030_nodes.json"
    ...
    Write-Warning "[BloodHound] Export failed: # Check: Trust Authentication Summary
# Category: Trust Relationships
# Severity: info
# ID: TRST-030
# Requirements: None
