...
# Check: Network Security Check 11
...
Write-Host "=== Network Security Check 11 ===" -ForegroundColor Cyan
...
try {
    ...
    Write-Warning "NET-011 PowerShell engine: $_"   # Fix R10: no silent catch
}
...
try {
    ...
    Write-Warning "NET-011 ADSI engine: $_"          # Fix R10
}
...
try {
    ...
    Write-Warning "NET-011 C# engine: $_"             # Fix R10
}
...
Write-Host ""
Write-Host "=== NET-011: $($uniqueResults.Count) unique findings ===" -ForegroundColor Cyan
$uniqueResults | Format-List

# Fix R14: Fixed 5-field schema enforced before CSV export
$csvPath = Join-Path $env:TEMP "NET-011_results.csv"
$uniqueResults |
