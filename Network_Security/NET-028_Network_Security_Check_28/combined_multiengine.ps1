...
# Check: Network Security Check 28
...
Write-Host "=== Network Security Check 28 ===" -ForegroundColor Cyan
...
try {
    ...
    Write-Warning "NET-028 PowerShell engine: $_"   # Fix R10: no silent catch
}
...
try {
    ...
    Write-Warning "NET-028 ADSI engine: $_"          # Fix R10
}
...
try {
    ...
    Write-Warning "NET-028 C# engine: $_"             # Fix R10
}
...
Write-Host ""
Write-Host "=== NET-028: $($uniqueResults.Count) unique findings ===" -ForegroundColor Cyan
$uniqueResults | Format-List

# Fix R14: Fixed 5-field schema enforced before CSV export
$csvPath = Join-Path $env:TEMP "NET-028_results.csv"
$uniqueResults |
