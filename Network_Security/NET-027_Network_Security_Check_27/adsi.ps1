...
Write-Host "NET-027: found $($results.Count) objects"
...
try {
    ...
    foreach ($r in $bhRaw) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                isdeleted         = $false
                adSuiteCheckId    = 'NET-027'
                adSuiteCheckName  = 'Network Security Check 27'
    ...
    @{ data = $bhNodes.ToArray()
       meta = @{ type = 'computers'; count = $bhNodes.Count; version = 5; methods = 0 }
        ...
        Out-File -FilePath (Join-Path $bhDir "NET-027_$bhTs.json") -Encoding UTF8 -Force

# Fix R10: No silent catch — warn on failure
} catch { Write-Warning "NET-027 BloodHound export error: $_" }
