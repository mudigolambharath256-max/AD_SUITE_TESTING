# ============================================================================
# NET-003: Network Segmentation Validation - Multi-Engine
# ============================================================================
...
#   It CANNOT determine network segmentation (requires routing/firewall configuration review) via LDAP alone.
...
Write-Host "[NET-003] Network Segmentation Validation" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    $results = Get-ADObject -LDAPFilter '(&(objectCategory=computer)(userAccountControl:1.2.840.113556.1.4.803:=8192))' -Properties name,distinguishedName,userAccountControl -SearchBase $SearchBase -SearchScope Subtree -ResultSetSize $null -ErrorAction SilentlyContinue
} else {
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'NET-003'
                adSuiteCheckName  = 'Network_Segmentation_Validation'
    ...
    $bhFile = Join-Path $bhDir "NET-003_$bhTs.json"
    @{
