...
# Check: Network Security Check 18
...
Write-Host "=== Network Security Check 18 ===" -ForegroundColor Cyan
...
try {
    ...
    Write-Warning "NET-018 PowerShell engine: $_"   # Fix R10: no silent catch
}
...
try {
    ...
    Write-Warning "NET-018 ADSI engine: $_"          # Fix R10
}
...
try {
    ...
    Write-Warning "NET-018 C# engine: $_"             # Fix R10
}
...
Write-Host ""
Write-Host "=== NET-018: $($uniqueResults.Count) unique findings ===" -ForegroundColor Cyan
$uniqueResults | Format-List

# Fix R14: Fixed 5-field schema enforced before CSV export
$csvPath = Join-Path $env:TEMP "NET-018_results.csv"
$uniqueResults |
