...
Write-Host "NET-017: found $($results.Count) objects"
...
try {
    ...
    foreach ($r in $bhRaw) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                isdeleted         = $false
                adSuiteCheckId    = 'NET-017'
                adSuiteCheckName  = 'Network Security Check 17'
                adSuiteSeverity   = 'medium'
                adSuiteCategory   = 'Network_Security'
                adSuiteFlag       = $true
            }
            Aces           = @()
            IsDeleted      = $false
    ...
    @{ data = $bhNodes.ToArray()
       meta = @{ type = 'computers'; count = $bhNodes.Count; version = 5; methods = 0 }
        ...
        Out-File -FilePath (Join-Path $bhDir "NET-017_$bhTs.json") -Encoding UTF8 -Force

# Fix R10: No silent catch — warn on failure
} catch { Write-Warning "NET-017 BloodHound export error: $_" }
