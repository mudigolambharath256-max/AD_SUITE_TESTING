...
(@('name', 'distinguishedName', 'objectClass', 'whenCreated', 'whenChanged', 'adminCount', 'objectSid', 'samAccountName') | ForEach-Object { [void]$searcher.PropertiesToLoad.Add($_)
}
...
Write-Host "Found $($results.Count) objects for check: Computers with AdminSDHolder Protection" -ForegroundColor Cyan
...
try {
    ...
    if ($results -and $results.Count -gt 0) {
        ...
        $bhFile = Join-Path $bhDir "CMGMT-014_nodes.json"
