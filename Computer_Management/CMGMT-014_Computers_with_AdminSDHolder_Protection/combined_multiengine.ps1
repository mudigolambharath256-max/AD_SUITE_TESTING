# ============================================================================
# CMGMT-014: Computers with AdminSDHolder Protection
# ============================================================================
...
Write-Host "=== CMGMT-014: Computers with AdminSDHolder Protection ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        # Execute query
        $adObjects = Get-ADObject -LDAPFilter '(&(objectCategory=computer)(adminCount=1))' `
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'CMGMT-014'}}, `
            @{N='CheckName'; E={'Computers with AdminSDHolder Protection'}}, `
...
if (-not $useADModule) {
    ...
    try {
        ...
        $searcher.Filter = '(&(objectCategory=computer)(adminCount=1))'
        ...
        $results = $adsiResults | ForEach-Object {
            ...
            [PSCustomObject]@{
                CheckID           = 'CMGMT-014'
                CheckName         = 'Computers with AdminSDHolder Protection'
                Method            = 'ADSI'
                Domain            = $domainNC
                Name              = if ($props['name'].Count -gt 0) { $props['name'][0]
        AdminCount = if ($props['admincount'].Count -gt 0) { $props['admincount'][0] } else { 'N/A' } } else { 'N/A' }
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'CMGMT-014'
                adSuiteCheckName  = 'Computers_with_AdminSDHolder_Protection'
    ...
    $bhFile = Join-Path $bhDir "CMGMT-014_$bhTs.json"
    @{
