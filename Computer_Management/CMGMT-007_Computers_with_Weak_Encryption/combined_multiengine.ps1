# ============================================================================
# CMGMT-007: Computers with Weak Encryption
# ============================================================================
...
Write-Host "=== CMGMT-007: Computers with Weak Encryption ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        # Execute query
        $adObjects = Get-ADObject -LDAPFilter '(&(objectCategory=computer)(userAccountControl:1.2.840.113556.1.4.803:=4194304))' `
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'CMGMT-007'}}, `
            @{N='CheckName'; E={'Computers with Weak Encryption'}}, `
...
if (-not $useADModule) {
    ...
    try {
        ...
        $searcher.Filter = '(&(objectCategory=computer)(userAccountControl:1.2.840.113556.1.4.803:=4194304))'
        ...
        $results = $adsiResults | ForEach-Object {
            ...
            [PSCustomObject]@{
                CheckID           = 'CMGMT-007'
                CheckName         = 'Computers with Weak Encryption'
                Method            = 'ADSI'
                Domain            = $domainNC
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'CMGMT-007'
                adSuiteCheckName  = 'Computers_with_Weak_Encryption'
    ...
    $bhFile = Join-Path $bhDir "CMGMT-007_$bhTs.json"
    @{
