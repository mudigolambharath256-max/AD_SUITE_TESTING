# ============================================================================
# CMGMT-008: Computers with Pre Windows 2000 Access
# ============================================================================
...
Write-Host "=== CMGMT-008: Computers with Pre Windows 2000 Access ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        # Execute query
        $adObjects = Get-ADObject -LDAPFilter '(&(objectCategory=computer)(operatingSystem=Windows*2000*))' `
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'CMGMT-008'}}, `
            @{N='CheckName'; E={'Computers with Pre Windows 2000 Access'}}, `
...
if (-not $useADModule) {
    ...
    try {
        ...
        $searcher.Filter = '(&(objectCategory=computer)(operatingSystem=Windows*2000*))'
        ...
        $results = $adsiResults | ForEach-Object {
            ...
            [PSCustomObject]@{
                CheckID           = 'CMGMT-008'
                CheckName         = 'Computers with Pre Windows 2000 Access'
                Method            = 'ADSI'
                Domain            = $domainNC
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'CMGMT-008'
                adSuiteCheckName  = 'Computers_with_Pre_Windows_2000_Access'
    ...
    $bhFile = Join-Path $bhDir "CMGMT-008_$bhTs.json"
    @{
