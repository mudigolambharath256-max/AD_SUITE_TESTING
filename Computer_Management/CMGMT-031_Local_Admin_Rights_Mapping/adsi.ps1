...
Write-Host "Found $($results.Count) objects for check: Local Admin Rights Mapping" -ForegroundColor Cyan

$results | ForEach-Object {
    $props = $_.Properties
    [PSCustomObject]@{
        CheckID = 'CMGMT-031'
        CheckName = 'Local Admin Rights Mapping'
...
try {
    ...
    if ($results -and $results.Count -gt 0) {
        ...
        $bhFile = Join-Path $bhDir "CMGMT-031_nodes.json"
