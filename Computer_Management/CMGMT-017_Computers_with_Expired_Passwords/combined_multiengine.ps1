...
Write-Host "=== CMGMT-017: Computers with Expired Passwords ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'CMGMT-017'}}, `
            @{N='CheckName'; E={'Computers with Expired Passwords'}}, `
...
if (-not $useADModule) {
    ...
    try {
        ...
        $results = $adsiResults | ForEach-Object {
            ...
            [PSCustomObject]@{
                CheckID           = 'CMGMT-017'
                CheckName         = 'Computers with Expired Passwords'
                Method            = 'ADSI'
                Domain            = $domainNC
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'CMGMT-017'
                adSuiteCheckName  = 'Computers_with_Expired_Passwords'
    ...
    $bhFile = Join-Path $bhDir "CMGMT-017_$bhTs.json"
    @{
