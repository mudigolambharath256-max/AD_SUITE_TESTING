...
Write-Host "=== CMGMT-030: Computers with Legacy Operating Systems ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        # Execute query
        $adObjects = Get-ADObject -LDAPFilter '(&(objectCategory=computer)(operatingSystem=*))' `
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'CMGMT-030'}}, `
            @{N='CheckName'; E={'Computers with Legacy Operating Systems'}}, `
...
if (-not $useADModule) {
    ...
    try {
        ...
        $results = $adsiResults | ForEach-Object {
            ...
            [PSCustomObject]@{
                CheckID           = 'CMGMT-030'
                CheckName         = 'Computers with Legacy Operating Systems'
                Method            = 'ADSI'
                Domain            = $domainNC
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'CMGMT-030'
                adSuiteCheckName  = 'Computers_with_Legacy_Operating_Systems'
    ...
    $bhFile = Join-Path $bhDir "CMGMT-030_$bhTs.json"
    @{
