...
Write-Host "=== CMGMT-029: Computers with Anonymous Access ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'CMGMT-029'}}, `
            @{N='CheckName'; E={'Computers with Anonymous Access'}}, `
...
if (-not $useADModule) {
    ...
    try {
        ...
        $results = $adsiResults | ForEach-Object {
            ...
            [PSCustomObject]@{
                CheckID           = 'CMGMT-029'
                CheckName         = 'Computers with Anonymous Access'
                Method            = 'ADSI'
                Domain            = $domainNC
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'CMGMT-029'
                adSuiteCheckName  = 'Computers_with_Anonymous_Access'
    ...
    $bhFile = Join-Path $bhDir "CMGMT-029_$bhTs.json"
    @{
