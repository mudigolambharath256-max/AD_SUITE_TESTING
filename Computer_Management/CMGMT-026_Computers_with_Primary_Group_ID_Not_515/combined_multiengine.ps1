# ============================================================================
# CMGMT-026: Computers with Primary Group ID Not 515
# ============================================================================
...
Write-Host "=== CMGMT-026: Computers with Primary Group ID Not 515 ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        # Execute query
        $adObjects = Get-ADObject -LDAPFilter '(&(objectCategory=computer)(!(primaryGroupID=515)))' `
                                   -Properties name,distinguishedName,objectClass,whenCreated,whenChanged,primaryGroupID `
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'CMGMT-026'}}, `
            @{N='CheckName'; E={'Computers with Primary Group ID Not 515'}}, `
...
if (-not $useADModule) {
    ...
    try {
        ...
        $searcher.Filter = '(&(objectCategory=computer)(!(primaryGroupID=515)))'
        $searcher.PageSize = 1000
        $searcher.PropertiesToLoad.Clear()
        (@('name', 'distinguishedName', 'objectClass', 'whenCreated', 'whenChanged', 'primaryGroupID') | ForEach-Object { [void]$searcher.PropertiesToLoad.Add($_) }
        ...
        $results = $adsiResults | ForEach-Object {
            ...
            [PSCustomObject]@{
                CheckID           = 'CMGMT-026'
                CheckName         = 'Computers with Primary Group ID Not 515'
                Method            = 'ADSI'
                Domain            = $domainNC
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'CMGMT-026'
                adSuiteCheckName  = 'Computers_with_Primary_Group_ID_Not_515'
    ...
    $bhFile = Join-Path $bhDir "CMGMT-026_$bhTs.json"
    @{
