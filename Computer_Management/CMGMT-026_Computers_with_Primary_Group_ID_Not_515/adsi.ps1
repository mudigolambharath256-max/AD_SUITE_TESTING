...
(@('name', 'distinguishedName', 'objectClass', 'whenCreated', 'whenChanged', 'primaryGroupID', 'objectSid', 'samAccountName') | ForEach-Object { [void]$searcher.PropertiesToLoad.Add($_)
}
...
Write-Host "Found $($results.Count) objects for check: Computers with Primary Group ID Not 515" -ForegroundColor Cyan
...
try {
    ...
    if ($results -and $results.Count -gt 0) {
        ...
        $bhFile = Join-Path $bhDir "CMGMT-026_nodes.json"
