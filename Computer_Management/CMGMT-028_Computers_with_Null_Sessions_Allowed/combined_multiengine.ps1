...
Write-Host "=== CMGMT-028: Computers with Null Sessions Allowed ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'CMGMT-028'}}, `
            @{N='CheckName'; E={'Computers with Null Sessions Allowed'}}, `
...
if (-not $useADModule) {
    ...
    try {
        ...
        $results = $adsiResults | ForEach-Object {
            ...
            [PSCustomObject]@{
                CheckID           = 'CMGMT-028'
                CheckName         = 'Computers with Null Sessions Allowed'
                Method            = 'ADSI'
                Domain            = $domainNC
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'CMGMT-028'
                adSuiteCheckName  = 'Computers_with_Null_Sessions_Allowed'
    ...
    $bhFile = Join-Path $bhDir "CMGMT-028_$bhTs.json"
    @{
