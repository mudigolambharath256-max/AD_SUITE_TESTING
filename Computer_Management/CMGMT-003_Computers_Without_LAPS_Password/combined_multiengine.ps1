# ============================================================================
# CMGMT-003: Computers Without LAPS Password
# ============================================================================
...
Write-Host "=== CMGMT-003: Computers Without LAPS Password ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        # Execute query
        $adObjects = Get-ADObject -LDAPFilter '(&(objectCategory=computer)(!(ms-Mcs-AdmPwdExpirationTime=*))(!(userAccountControl:1.2.840.113556.1.4.803:=2)))' `
                                   -Properties name,distinguishedName,objectClass,whenCreated,whenChanged,ms-Mcs-AdmPwdExpirationTime,userAccountControl `
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'CMGMT-003'}}, `
            @{N='CheckName'; E={'Computers Without LAPS Password'}}, `
...
if (-not $useADModule) {
    ...
    try {
        ...
        $searcher.Filter = '(&(objectCategory=computer)(!(ms-Mcs-AdmPwdExpirationTime=*))(!(userAccountControl:1.2.840.113556.1.4.803:=2)))'
        $searcher.PageSize = 1000
        $searcher.PropertiesToLoad.Clear()
        (@('name', 'distinguishedName', 'objectClass', 'whenCreated', 'whenChanged', 'ms-Mcs-AdmPwdExpirationTime', 'userAccountControl') | ForEach-Object { [void]$searcher.PropertiesToLoad.Add($_) }
        ...
        $results = $adsiResults | ForEach-Object {
            ...
            [PSCustomObject]@{
                CheckID           = 'CMGMT-003'
                CheckName         = 'Computers Without LAPS Password'
                Method            = 'ADSI'
                Domain            = $domainNC
                Name              = if ($props['name'].Count -gt 0) { $props['name'][0]
        MsMcsAdmPwdExpirationTime = if ($props['ms-mcs-admpwdexpirationtime'].Count -gt 0) { $props['ms-mcs-admpwdexpirationtime'][0]
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'CMGMT-003'
                adSuiteCheckName  = 'Computers_Without_LAPS_Password'
    ...
    $bhFile = Join-Path $bhDir "CMGMT-003_$bhTs.json"
    @{
