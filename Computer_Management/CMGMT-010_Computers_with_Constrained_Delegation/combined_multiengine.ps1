# ============================================================================
# CMGMT-010: Computers with Constrained Delegation
# ============================================================================
...
Write-Host "=== CMGMT-010: Computers with Constrained Delegation ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        # Execute query
        $adObjects = Get-ADObject -LDAPFilter '(&(objectCategory=computer)(msDS-AllowedToDelegateTo=*))' `
                                   -Properties name,distinguishedName,objectClass,whenCreated,whenChanged,msDS-AllowedToDelegateTo `
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'CMGMT-010'}}, `
            @{N='CheckName'; E={'Computers with Constrained Delegation'}}, `
...
if (-not $useADModule) {
    ...
    try {
        ...
        (@('name', 'distinguishedName', 'objectClass', 'whenCreated', 'whenChanged', 'msDS-AllowedToDelegateTo') | ForEach-Object { [void]$searcher.PropertiesToLoad.Add($_) }
        ...
        $results = $adsiResults | ForEach-Object {
            ...
            [PSCustomObject]@{
                CheckID           = 'CMGMT-010'
                CheckName         = 'Computers with Constrained Delegation'
                Method            = 'ADSI'
                Domain            = $domainNC
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'CMGMT-010'
                adSuiteCheckName  = 'Computers_with_Constrained_Delegation'
    ...
    $bhFile = Join-Path $bhDir "CMGMT-010_$bhTs.json"
    @{
