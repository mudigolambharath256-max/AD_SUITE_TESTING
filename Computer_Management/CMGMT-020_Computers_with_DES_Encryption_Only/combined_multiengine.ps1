# ============================================================================
# CMGMT-020: Computers with DES Encryption Only
# ============================================================================
...
Write-Host "=== CMGMT-020: Computers with DES Encryption Only ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        # Execute query
        $adObjects = Get-ADObject -LDAPFilter '(&(objectCategory=computer)(userAccountControl:1.2.840.113556.1.4.803:=2097152))' `
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'CMGMT-020'}}, `
            @{N='CheckName'; E={'Computers with DES Encryption Only'}}, `
...
if (-not $useADModule) {
    ...
    try {
        ...
        $searcher.Filter = '(&(objectCategory=computer)(userAccountControl:1.2.840.113556.1.4.803:=2097152))'
        ...
        $results = $adsiResults | ForEach-Object {
            ...
            [PSCustomObject]@{
                CheckID           = 'CMGMT-020'
                CheckName         = 'Computers with DES Encryption Only'
                Method            = 'ADSI'
                Domain            = $domainNC
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'CMGMT-020'
                adSuiteCheckName  = 'Computers_with_DES_Encryption_Only'
    ...
    $bhFile = Join-Path $bhDir "CMGMT-020_$bhTs.json"
    @{
