...
Write-Host "=== CMGMT-005: Computers with BitLocker Keys Stored ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        # Execute query
        $adObjects = Get-ADObject -LDAPFilter '(objectClass=msFVE-RecoveryInformation)' `
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'CMGMT-005'}}, `
            @{N='CheckName'; E={'Computers with BitLocker Keys Stored'}}, `
...
if (-not $useADModule) {
    ...
    try {
        ...
        $results = $adsiResults | ForEach-Object {
            ...
            [PSCustomObject]@{
                CheckID           = 'CMGMT-005'
                CheckName         = 'Computers with BitLocker Keys Stored'
                Method            = 'ADSI'
                Domain            = $domainNC
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'CMGMT-005'
                adSuiteCheckName  = 'Computers_with_BitLocker_Keys_Stored'
    ...
    $bhFile = Join-Path $bhDir "CMGMT-005_$bhTs.json"
    @{
