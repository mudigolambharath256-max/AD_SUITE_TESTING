# ============================================================================
# CMGMT-023: Computers with Smartcard Required
# ============================================================================
...
Write-Host "=== CMGMT-023: Computers with Smartcard Required ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        # Execute query
        $adObjects = Get-ADObject -LDAPFilter '(&(objectCategory=computer)(userAccountControl:1.2.840.113556.1.4.803:=262144))' `
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'CMGMT-023'}}, `
            @{N='CheckName'; E={'Computers with Smartcard Required'}}, `
...
if (-not $useADModule) {
    ...
    try {
        ...
        $searcher.Filter = '(&(objectCategory=computer)(userAccountControl:1.2.840.113556.1.4.803:=262144))'
        ...
        $results = $adsiResults | ForEach-Object {
            ...
            [PSCustomObject]@{
                CheckID           = 'CMGMT-023'
                CheckName         = 'Computers with Smartcard Required'
                Method            = 'ADSI'
                Domain            = $domainNC
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'CMGMT-023'
                adSuiteCheckName  = 'Computers_with_Smartcard_Required'
    ...
    $bhFile = Join-Path $bhDir "CMGMT-023_$bhTs.json"
    @{
