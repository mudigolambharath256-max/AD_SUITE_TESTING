...
Write-Host "=== CMGMT-002: Windows LAPS Schema Extended ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        # Execute query
        $adObjects = Get-ADObject -LDAPFilter '(objectClass=attributeSchema)' `
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'CMGMT-002'}}, `
            @{N='CheckName'; E={'Windows LAPS Schema Extended'}}, `
...
if (-not $useADModule) {
    ...
    try {
        ...
        $schemaNC = $root.schemaNamingContext.ToString()
        ...
        $results = $adsiResults | ForEach-Object {
            ...
            [PSCustomObject]@{
                CheckID           = 'CMGMT-002'
                CheckName         = 'Windows LAPS Schema Extended'
                Method            = 'ADSI'
                Domain            = $domainNC
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'CMGMT-002'
                adSuiteCheckName  = 'Windows_LAPS_Schema_Extended'
    ...
    $bhFile = Join-Path $bhDir "CMGMT-002_$bhTs.json"
    @{
