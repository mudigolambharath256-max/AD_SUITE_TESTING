# ============================================================================
# CMGMT-018: Computers with Password Never Expires
# ============================================================================
...
Write-Host "=== CMGMT-018: Computers with Password Never Expires ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        # Execute query
        $adObjects = Get-ADObject -LDAPFilter '(&(objectCategory=computer)(userAccountControl:1.2.840.113556.1.4.803:=65536))' `
                                   -Properties name,distinguishedName,objectClass,whenCreated,whenChanged,userAccountControl `
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'CMGMT-018'}}, `
            @{N='CheckName'; E={'Computers with Password Never Expires'}}, `
...
if (-not $useADModule) {
    ...
    try {
        ...
        $searcher.Filter = '(&(objectCategory=computer)(userAccountControl:1.2.840.113556.1.4.803:=65536))'
        $searcher.PageSize = 1000
        $searcher.PropertiesToLoad.Clear()
        (@('name', 'distinguishedName', 'objectClass', 'whenCreated', 'whenChanged', 'userAccountControl') | ForEach-Object { [void]$searcher.PropertiesToLoad.Add($_) }
        ...
        $results = $adsiResults | ForEach-Object {
            ...
            [PSCustomObject]@{
                CheckID           = 'CMGMT-018'
                CheckName         = 'Computers with Password Never Expires'
                Method            = 'ADSI'
                Domain            = $domainNC
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'CMGMT-018'
                adSuiteCheckName  = 'Computers_with_Password_Never_Expires'
    ...
    $bhFile = Join-Path $bhDir "CMGMT-018_$bhTs.json"
    @{
