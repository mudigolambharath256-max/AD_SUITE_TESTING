# ============================================================================
# CMGMT-024: Computers with Password Not Required
# ============================================================================
...
Write-Host "=== CMGMT-024: Computers with Password Not Required ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        # Execute query
        $adObjects = Get-ADObject -LDAPFilter '(&(objectCategory=computer)(userAccountControl:1.2.840.113556.1.4.803:=32))' `
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'CMGMT-024'}}, `
            @{N='CheckName'; E={'Computers with Password Not Required'}}, `
...
if (-not $useADModule) {
    ...
    try {
        ...
        $results = $adsiResults | ForEach-Object {
            ...
            [PSCustomObject]@{
                CheckID           = 'CMGMT-024'
                CheckName         = 'Computers with Password Not Required'
                Method            = 'ADSI'
                Domain            = $domainNC
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'CMGMT-024'
                adSuiteCheckName  = 'Computers_with_Password_Not_Required'
    ...
    $bhFile = Join-Path $bhDir "CMGMT-024_$bhTs.json"
    @{
