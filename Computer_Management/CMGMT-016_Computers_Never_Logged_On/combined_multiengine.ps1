...
Write-Host "=== CMGMT-016: Computers Never Logged On ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        # Execute query
        $adObjects = Get-ADObject -LDAPFilter '(&(objectCategory=computer)(!(lastLogonTimestamp=*)))' `
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'CMGMT-016'}}, `
            @{N='CheckName'; E={'Computers Never Logged On'}}, `
...
if (-not $useADModule) {
    ...
    try {
        ...
        $searcher.Filter = '(&(objectCategory=computer)(!(lastLogonTimestamp=*)))'
        ...
        $results = $adsiResults | ForEach-Object {
            ...
            [PSCustomObject]@{
                CheckID           = 'CMGMT-016'
                CheckName         = 'Computers Never Logged On'
                Method            = 'ADSI'
                Domain            = $domainNC
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'CMGMT-016'
                adSuiteCheckName  = 'Computers_Never_Logged_On'
    ...
    $bhFile = Join-Path $bhDir "CMGMT-016_$bhTs.json"
    @{
