...
$ldapFilter = '(&(objectCategory=computer)(!(lastLogonTimestamp=*)))'

# Define properties to retrieve
$properties = @('name', 'distinguishedName', 'objectClass', 'whenCreated', 'whenChanged', 'objectSid')

Write-Host "Executing security check: Computers Never Logged On" -ForegroundColor Yellow
Write-Host "Check ID: CMGMT-016" -ForegroundColor Gray
