...
Write-Host "Found $($results.Count) objects for check: LAPS Passwords Expiring Soon" -ForegroundColor Cyan

$results | ForEach-Object {
    $props = $_.Properties
    [PSCustomObject]@{
        CheckID = 'CMGMT-036'
        CheckName = 'LAPS Passwords Expiring Soon'
...
try {
    ...
    if ($results -and $results.Count -gt 0) {
        ...
        $bhFile = Join-Path $bhDir "CMGMT-036_nodes.json"
