...
Write-Host "=== CMGMT-006: Computers with Stale Passwords ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'CMGMT-006'}}, `
            @{N='CheckName'; E={'Computers with Stale Passwords'}}, `
...
if (-not $useADModule) {
    ...
    try {
        ...
        $results = $adsiResults | ForEach-Object {
            ...
            [PSCustomObject]@{
                CheckID           = 'CMGMT-006'
                CheckName         = 'Computers with Stale Passwords'
                Method            = 'ADSI'
                Domain            = $domainNC
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'CMGMT-006'
                adSuiteCheckName  = 'Computers_with_Stale_Passwords'
    ...
    $bhFile = Join-Path $bhDir "CMGMT-006_$bhTs.json"
    @{
