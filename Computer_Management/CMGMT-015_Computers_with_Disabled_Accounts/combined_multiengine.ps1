# ============================================================================
# CMGMT-015: Computers with Disabled Accounts
# ============================================================================
...
Write-Host "=== CMGMT-015: Computers with Disabled Accounts ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        # Execute query
        $adObjects = Get-ADObject -LDAPFilter '(&(objectCategory=computer)(userAccountControl:1.2.840.113556.1.4.803:=2))' `
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'CMGMT-015'}}, `
            @{N='CheckName'; E={'Computers with Disabled Accounts'}}, `
...
if (-not $useADModule) {
    ...
    try {
        ...
        $results = $adsiResults | ForEach-Object {
            ...
            [PSCustomObject]@{
                CheckID           = 'CMGMT-015'
                CheckName         = 'Computers with Disabled Accounts'
                Method            = 'ADSI'
                Domain            = $domainNC
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'CMGMT-015'
                adSuiteCheckName  = 'Computers_with_Disabled_Accounts'
    ...
    $bhFile = Join-Path $bhDir "CMGMT-015_$bhTs.json"
    @{
