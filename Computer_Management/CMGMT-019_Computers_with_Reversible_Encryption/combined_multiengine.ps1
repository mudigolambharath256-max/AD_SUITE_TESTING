# ============================================================================
# CMGMT-019: Computers with Reversible Encryption
# ============================================================================
...
Write-Host "=== CMGMT-019: Computers with Reversible Encryption ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        # Execute query
        $adObjects = Get-ADObject -LDAPFilter '(&(objectCategory=computer)(userAccountControl:1.2.840.113556.1.4.803:=128))' `
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'CMGMT-019'}}, `
            @{N='CheckName'; E={'Computers with Reversible Encryption'}}, `
...
if (-not $useADModule) {
    ...
    try {
        ...
        $searcher.Filter = '(&(objectCategory=computer)(userAccountControl:1.2.840.113556.1.4.803:=128))'
        ...
        $results = $adsiResults | ForEach-Object {
            ...
            [PSCustomObject]@{
                CheckID           = 'CMGMT-019'
                CheckName         = 'Computers with Reversible Encryption'
                Method            = 'ADSI'
                Domain            = $domainNC
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'CMGMT-019'
                adSuiteCheckName  = 'Computers_with_Reversible_Encryption'
    ...
    $bhFile = Join-Path $bhDir "CMGMT-019_$bhTs.json"
    @{
