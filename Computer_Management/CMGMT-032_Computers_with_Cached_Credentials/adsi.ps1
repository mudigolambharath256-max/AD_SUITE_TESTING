...
Write-Host "Found $($results.Count) objects for check: Computers with Cached Credentials" -ForegroundColor Cyan

$results | ForEach-Object {
    $props = $_.Properties
    [PSCustomObject]@{
        CheckID = 'CMGMT-032'
        CheckName = 'Computers with Cached Credentials'
...
try {
    ...
    if ($results -and $results.Count -gt 0) {
        ...
        $bhFile = Join-Path $bhDir "CMGMT-032_nodes.json"
