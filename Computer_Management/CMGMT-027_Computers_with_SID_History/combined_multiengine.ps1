# ============================================================================
# CMGMT-027: Computers with SID History
# ============================================================================
...
Write-Host "=== CMGMT-027: Computers with SID History ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        # Execute query
        $adObjects = Get-ADObject -LDAPFilter '(&(objectCategory=computer)(sIDHistory=*))' `
                                   -Properties name,distinguishedName,objectClass,whenCreated,whenChanged,sidHistory `
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'CMGMT-027'}}, `
            @{N='CheckName'; E={'Computers with SID History'}}, `
...
if (-not $useADModule) {
    ...
    try {
        ...
        $searcher.Filter = '(&(objectCategory=computer)(sIDHistory=*))'
        $searcher.PageSize = 1000
        $searcher.PropertiesToLoad.Clear()
        (@('name', 'distinguishedName', 'objectClass', 'whenCreated', 'whenChanged', 'sidHistory') | ForEach-Object { [void]$searcher.PropertiesToLoad.Add($_) }
        ...
        $results = $adsiResults | ForEach-Object {
            ...
            [PSCustomObject]@{
                CheckID           = 'CMGMT-027'
                CheckName         = 'Computers with SID History'
                Method            = 'ADSI'
                Domain            = $domainNC
                Name              = if ($props['name'].Count -gt 0) { $props['name'][0]
        SIDHistory = if ($props['sidhistory'].Count -gt 0) { $props['sidhistory'] } else { 'N/A' } } else { 'N/A' }
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'CMGMT-027'
                adSuiteCheckName  = 'Computers_with_SID_History'
    ...
    $bhFile = Join-Path $bhDir "CMGMT-027_$bhTs.json"
    @{
