...
Write-Host "CMP-026: found $($results.Count) objects"
...
try {
    ...
    $bhS.Filter   = '(&(objectCategory=computer))'
    $bhS.PageSize = 1000
    ...
    foreach ($r in $bhRaw) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                isdeleted         = $false
                adSuiteCheckId    = 'CMP-026'
                adSuiteCheckName  = 'Computer Management Check 26'
                adSuiteSeverity   = 'medium'
                adSuiteCategory   = 'Computer_Management'
                adSuiteFlag       = $true
            }
            Aces           = @()
            IsDeleted      = $false
    ...
    @{ data = $bhNodes.ToArray()
       meta = @{ type = 'computers'; count = $bhNodes.Count; version = 5; methods = 0 }
        ...
        Out-File -FilePath (Join-Path $bhDir "CMP-026_$bhTs.json") -Encoding UTF8 -Force

# Fix R10: No silent catch — warn on failure
} catch { Write-Warning "CMP-026 BloodHound export error: $_" }
