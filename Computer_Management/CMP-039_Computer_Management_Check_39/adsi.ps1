...
Write-Host "CMP-039: found $($results.Count) objects"
...
try {
    ...
    foreach ($r in $bhRaw) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                isdeleted         = $false
                adSuiteCheckId    = 'CMP-039'
                adSuiteCheckName  = 'Computer Management Check 39'
    ...
    @{ data = $bhNodes.ToArray()
       meta = @{ type = 'computers'; count = $bhNodes.Count; version = 5; methods = 0 }
        ...
        Out-File -FilePath (Join-Path $bhDir "CMP-039_$bhTs.json") -Encoding UTF8 -Force

# Fix R10: No silent catch — warn on failure
} catch { Write-Warning "CMP-039 BloodHound export error: $_" }
