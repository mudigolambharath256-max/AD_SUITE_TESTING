...
Write-Host "=== CMGMT-013: Computers in Default Computers Container ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        # Execute query
        $adObjects = Get-ADObject -LDAPFilter '(&(objectCategory=computer))' `
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'CMGMT-013'}}, `
            @{N='CheckName'; E={'Computers in Default Computers Container'}}, `
...
if (-not $useADModule) {
    ...
    try {
        ...
        $results = $adsiResults | ForEach-Object {
            ...
            [PSCustomObject]@{
                CheckID           = 'CMGMT-013'
                CheckName         = 'Computers in Default Computers Container'
                Method            = 'ADSI'
                Domain            = $domainNC
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'CMGMT-013'
                adSuiteCheckName  = 'Computers_in_Default_Computers_Container'
    ...
    $bhFile = Join-Path $bhDir "CMGMT-013_$bhTs.json"
    @{
