...
Write-Host "Found $($results.Count) objects for check: Computers Without LAPS" -ForegroundColor Cyan

$results | ForEach-Object {
    $props = $_.Properties
    [PSCustomObject]@{
        CheckID = 'CMGMT-035'
        CheckName = 'Computers Without LAPS'
...
try {
    ...
    if ($results -and $results.Count -gt 0) {
        ...
        $bhFile = Join-Path $bhDir "CMGMT-035_nodes.json"
