...
Write-Host "=== CMGMT-035: Computers Without LAPS ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    $results = Get-ADObject -LDAPFilter '(&(objectCategory=computer)(!(userAccountControl:1.2.840.113556.1.4.803:=2))(!(ms-Mcs-AdmPwd=*)))' -Properties name,distinguishedName,userAccountControl -SearchBase $SearchBase -SearchScope Subtree -ResultSetSize $null
} else {
    $root = [ADSI]'LDAP://RootDSE'
    $searcher = New-Object System.DirectoryServices.DirectorySearcher
    $searcher.SearchRoot = [ADSI]"LDAP://$($root.defaultNamingContext)"
    $searcher.Filter = '(&(objectCategory=computer)(!(userAccountControl:1.2.840.113556.1.4.803:=2))(!(ms-Mcs-AdmPwd=*)))'
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'CMGMT-035'
                adSuiteCheckName  = 'Computers_Without_LAPS'
    ...
    $bhFile = Join-Path $bhDir "CMGMT-035_$bhTs.json"
    @{
