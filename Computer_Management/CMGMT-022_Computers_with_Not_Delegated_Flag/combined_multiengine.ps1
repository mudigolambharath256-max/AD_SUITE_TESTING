# ============================================================================
# CMGMT-022: Computers with Not Delegated Flag
# ============================================================================
...
Write-Host "=== CMGMT-022: Computers with Not Delegated Flag ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        # Execute query
        $adObjects = Get-ADObject -LDAPFilter '(&(objectCategory=computer)(userAccountControl:1.2.840.113556.1.4.803:=1048576))' `
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'CMGMT-022'}}, `
            @{N='CheckName'; E={'Computers with Not Delegated Flag'}}, `
...
if (-not $useADModule) {
    ...
    try {
        ...
        $searcher.Filter = '(&(objectCategory=computer)(userAccountControl:1.2.840.113556.1.4.803:=1048576))'
        ...
        $results = $adsiResults | ForEach-Object {
            ...
            [PSCustomObject]@{
                CheckID           = 'CMGMT-022'
                CheckName         = 'Computers with Not Delegated Flag'
                Method            = 'ADSI'
                Domain            = $domainNC
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'CMGMT-022'
                adSuiteCheckName  = 'Computers_with_Not_Delegated_Flag'
    ...
    $bhFile = Join-Path $bhDir "CMGMT-022_$bhTs.json"
    @{
