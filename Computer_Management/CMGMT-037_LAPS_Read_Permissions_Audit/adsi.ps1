...
Write-Host "Found $($results.Count) objects for check: LAPS Read Permissions Audit" -ForegroundColor Cyan

$results | ForEach-Object {
    $props = $_.Properties
    [PSCustomObject]@{
        CheckID = 'CMGMT-037'
        CheckName = 'LAPS Read Permissions Audit'
...
try {
    ...
    if ($results -and $results.Count -gt 0) {
        ...
        $bhFile = Join-Path $bhDir "CMGMT-037_nodes.json"
