# ============================================================================
# CMGMT-021: Computers with Trusted for Delegation
# ============================================================================
...
Write-Host "=== CMGMT-021: Computers with Trusted for Delegation ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        # Execute query
        $adObjects = Get-ADObject -LDAPFilter '(&(objectCategory=computer)(userAccountControl:1.2.840.113556.1.4.803:=524288))' `
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'CMGMT-021'}}, `
            @{N='CheckName'; E={'Computers with Trusted for Delegation'}}, `
...
if (-not $useADModule) {
    ...
    try {
        ...
        $results = $adsiResults | ForEach-Object {
            ...
            [PSCustomObject]@{
                CheckID           = 'CMGMT-021'
                CheckName         = 'Computers with Trusted for Delegation'
                Method            = 'ADSI'
                Domain            = $domainNC
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'CMGMT-021'
                adSuiteCheckName  = 'Computers_with_Trusted_for_Delegation'
    ...
    $bhFile = Join-Path $bhDir "CMGMT-021_$bhTs.json"
    @{
