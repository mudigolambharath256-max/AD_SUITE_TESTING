...
Write-Host "Found $($results.Count) objects for check: RDP Access Mapping" -ForegroundColor Cyan

$results | ForEach-Object {
    $props = $_.Properties
    [PSCustomObject]@{
        CheckID = 'CMGMT-033'
        CheckName = 'RDP Access Mapping'
...
try {
    ...
    if ($results -and $results.Count -gt 0) {
        ...
        $bhFile = Join-Path $bhDir "CMGMT-033_nodes.json"
