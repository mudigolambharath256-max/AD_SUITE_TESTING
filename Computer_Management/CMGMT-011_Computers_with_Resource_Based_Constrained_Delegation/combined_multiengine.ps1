# ============================================================================
# CMGMT-011: Computers with Resource Based Constrained Delegation
# ============================================================================
...
Write-Host "=== CMGMT-011: Computers with Resource Based Constrained Delegation ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        # Execute query
        $adObjects = Get-ADObject -LDAPFilter '(&(objectCategory=computer)(msDS-AllowedToActOnBehalfOfOtherIdentity=*))' `
                                   -Properties name,distinguishedName,objectClass,whenCreated,whenChanged,msDS-AllowedToActOnBehalfOfOtherIdentity `
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'CMGMT-011'}}, `
            @{N='CheckName'; E={'Computers with Resource Based Constrained Delegation'}}, `
...
if (-not $useADModule) {
    ...
    try {
        ...
        (@('name', 'distinguishedName', 'objectClass', 'whenCreated', 'whenChanged', 'msDS-AllowedToActOnBehalfOfOtherIdentity') | ForEach-Object { [void]$searcher.PropertiesToLoad.Add($_) }
        ...
        $results = $adsiResults | ForEach-Object {
            ...
            [PSCustomObject]@{
                CheckID           = 'CMGMT-011'
                CheckName         = 'Computers with Resource Based Constrained Delegation'
                Method            = 'ADSI'
                Domain            = $domainNC
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'CMGMT-011'
                adSuiteCheckName  = 'Computers_with_Resource_Based_Constrained_Delegation'
    ...
    $bhFile = Join-Path $bhDir "CMGMT-011_$bhTs.json"
    @{
