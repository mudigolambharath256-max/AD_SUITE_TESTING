...
# Check: Computer Management Check 45
...
Write-Host "=== Computer Management Check 45 ===" -ForegroundColor Cyan
...
try {
    ...
    Write-Warning "CMP-045 PowerShell engine: $_"   # Fix R10: no silent catch
}
...
try {
    ...
    $adsiS.Filter   = '(&(objectCategory=computer))'
    $adsiS.PageSize = 1000
    ...
    Write-Warning "CMP-045 ADSI engine: $_"          # Fix R10
}
...
try {
    $csFilter  = '(&(objectCategory=computer))'
    ...
    Write-Warning "CMP-045 C# engine: $_"             # Fix R10
}
...
Write-Host ""
Write-Host "=== CMP-045: $($uniqueResults.Count) unique findings ===" -ForegroundColor Cyan
$uniqueResults | Format-List

# Fix R14: Fixed 5-field schema enforced before CSV export
$csvPath = Join-Path $env:TEMP "CMP-045_results.csv"
$uniqueResults |
