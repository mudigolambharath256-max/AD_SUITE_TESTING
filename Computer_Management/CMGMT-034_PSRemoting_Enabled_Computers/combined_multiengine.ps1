...
Write-Host "=== CMGMT-034: PSRemoting Enabled Computers ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    $results = Get-ADObject -LDAPFilter '(&(objectCategory=computer)(!(userAccountControl:1.2.840.113556.1.4.803:=2)))' -Properties name,distinguishedName,userAccountControl -SearchBase $SearchBase -SearchScope Subtree -ResultSetSize $null
} else {
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'CMGMT-034'
                adSuiteCheckName  = 'PSRemoting_Enabled_Computers'
    ...
    $bhFile = Join-Path $bhDir "CMGMT-034_$bhTs.json"
    @{
