...
Write-Host "Found $($results.Count) objects for check: PSRemoting Enabled Computers" -ForegroundColor Cyan

$results | ForEach-Object {
    $props = $_.Properties
    [PSCustomObject]@{
        CheckID = 'CMGMT-034'
        CheckName = 'PSRemoting Enabled Computers'
...
try {
    ...
    if ($results -and $results.Count -gt 0) {
        ...
        $bhFile = Join-Path $bhDir "CMGMT-034_nodes.json"
