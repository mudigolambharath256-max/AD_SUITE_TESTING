...
Write-Host "=== CMGMT-001: LAPS Schema Extended ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'CMGMT-001'}}, `
            @{N='CheckName'; E={'LAPS Schema Extended'}}, `
...
if (-not $useADModule) {
    ...
    try {
        ...
        $results = $adsiResults | ForEach-Object {
            ...
            [PSCustomObject]@{
                CheckID           = 'CMGMT-001'
                CheckName         = 'LAPS Schema Extended'
                Method            = 'ADSI'
                Domain            = $domainNC
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'CMGMT-001'
                adSuiteCheckName  = 'LAPS_Schema_Extended'
    ...
    $bhFile = Join-Path $bhDir "CMGMT-001_$bhTs.json"
    @{
