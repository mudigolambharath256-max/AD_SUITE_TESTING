# ============================================================================
# CMGMT-012: Computers with Duplicate SPNs
# ============================================================================
...
Write-Host "=== CMGMT-012: Computers with Duplicate SPNs ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        # Execute query
        $adObjects = Get-ADObject -LDAPFilter '(&(objectCategory=computer)(servicePrincipalName=*))' `
                                   -Properties name,distinguishedName,objectClass,whenCreated,whenChanged,servicePrincipalName `
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'CMGMT-012'}}, `
            @{N='CheckName'; E={'Computers with Duplicate SPNs'}}, `
...
if (-not $useADModule) {
    ...
    try {
        ...
        (@('name', 'distinguishedName', 'objectClass', 'whenCreated', 'whenChanged', 'servicePrincipalName') | ForEach-Object { [void]$searcher.PropertiesToLoad.Add($_) }
        ...
        $results = $adsiResults | ForEach-Object {
            ...
            [PSCustomObject]@{
                CheckID           = 'CMGMT-012'
                CheckName         = 'Computers with Duplicate SPNs'
                Method            = 'ADSI'
                Domain            = $domainNC
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'CMGMT-012'
                adSuiteCheckName  = 'Computers_with_Duplicate_SPNs'
    ...
    $bhFile = Join-Path $bhDir "CMGMT-012_$bhTs.json"
    @{
