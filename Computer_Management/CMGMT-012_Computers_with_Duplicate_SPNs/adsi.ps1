...
(@('name', 'distinguishedName', 'objectClass', 'whenCreated', 'whenChanged', 'servicePrincipalName', 'objectSid', 'samAccountName') | ForEach-Object { [void]$searcher.PropertiesToLoad.Add($_)
}
...
Write-Host "Found $($results.Count) objects for check: Computers with Duplicate SPNs" -ForegroundColor Cyan
...
try {
    ...
    if ($results -and $results.Count -gt 0) {
        ...
        $bhFile = Join-Path $bhDir "CMGMT-012_nodes.json"
