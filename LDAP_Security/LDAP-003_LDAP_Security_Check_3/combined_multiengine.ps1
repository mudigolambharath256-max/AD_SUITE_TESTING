...
Write-Host "=== LDAP Security Check 3 ===" -ForegroundColor Cyan
...
try {
    ...
    Write-Warning "LDAP-003 PowerShell engine: $_"   # Fix R10: no silent catch
}
...
try {
    ...
    Write-Warning "LDAP-003 ADSI engine: $_"          # Fix R10
}
...
try {
    ...
    Write-Warning "LDAP-003 C# engine: $_"             # Fix R10
}
...
Write-Host ""
Write-Host "=== LDAP-003: $($uniqueResults.Count) unique findings ===" -ForegroundColor Cyan
$uniqueResults | Format-List

# Fix R14: Fixed 5-field schema enforced before CSV export
$csvPath = Join-Path $env:TEMP "LDAP-003_results.csv"
$uniqueResults |
