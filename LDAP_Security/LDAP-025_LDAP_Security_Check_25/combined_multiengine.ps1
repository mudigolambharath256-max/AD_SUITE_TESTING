...
Write-Host "=== LDAP Security Check 25 ===" -ForegroundColor Cyan
...
try {
    ...
    Write-Warning "LDAP-025 PowerShell engine: $_"   # Fix R10: no silent catch
}
...
try {
    ...
    Write-Warning "LDAP-025 ADSI engine: $_"          # Fix R10
}
...
try {
    ...
    Write-Warning "LDAP-025 C# engine: $_"             # Fix R10
}
...
Write-Host ""
Write-Host "=== LDAP-025: $($uniqueResults.Count) unique findings ===" -ForegroundColor Cyan
$uniqueResults | Format-List

# Fix R14: Fixed 5-field schema enforced before CSV export
$csvPath = Join-Path $env:TEMP "LDAP-025_results.csv"
$uniqueResults |
