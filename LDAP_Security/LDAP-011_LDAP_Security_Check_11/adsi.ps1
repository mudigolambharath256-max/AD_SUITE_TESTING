...
Write-Host "LDAP-011: found $($results.Count) objects"
...
try {
    ...
    foreach ($r in $bhRaw) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                adSuiteCheckName  = 'LDAP Security Check 11'
    ...
    @{ data = $bhNodes.ToArray()
       meta = @{ type = 'domains'; count = $bhNodes.Count; version = 5; methods = 0 }
        ...
        Out-File -FilePath (Join-Path $bhDir "LDAP-011_$bhTs.json") -Encoding UTF8 -Force

# Fix R10: No silent catch — warn on failure
} catch { Write-Warning "LDAP-011 BloodHound export error: $_" }
