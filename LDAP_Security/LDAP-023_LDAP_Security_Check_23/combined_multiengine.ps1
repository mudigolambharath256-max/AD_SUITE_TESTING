...
Write-Host "=== LDAP Security Check 23 ===" -ForegroundColor Cyan
...
try {
    ...
    Write-Warning "LDAP-023 PowerShell engine: $_"   # Fix R10: no silent catch
}
...
try {
    ...
    Write-Warning "LDAP-023 ADSI engine: $_"          # Fix R10
}
...
try {
    ...
    Write-Warning "LDAP-023 C# engine: $_"             # Fix R10
}
...
Write-Host ""
Write-Host "=== LDAP-023: $($uniqueResults.Count) unique findings ===" -ForegroundColor Cyan
$uniqueResults | Format-List

# Fix R14: Fixed 5-field schema enforced before CSV export
$csvPath = Join-Path $env:TEMP "LDAP-023_results.csv"
$uniqueResults |
