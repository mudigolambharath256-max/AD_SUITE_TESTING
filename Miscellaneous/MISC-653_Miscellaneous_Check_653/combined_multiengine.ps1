...
Write-Host "=== Miscellaneous Check 653 ===" -ForegroundColor Cyan
...
try {
    ...
    Write-Warning "MISC-653 PowerShell engine: $_"   # Fix R10: no silent catch
}
...
try {
    ...
    Write-Warning "MISC-653 ADSI engine: $_"          # Fix R10
}
...
try {
    ...
    Write-Warning "MISC-653 C# engine: $_"             # Fix R10
}
...
Write-Host ""
Write-Host "=== MISC-653: $($uniqueResults.Count) unique findings ===" -ForegroundColor Cyan
$uniqueResults | Format-List

# Fix R14: Fixed 5-field schema enforced before CSV export
$csvPath = Join-Path $env:TEMP "MISC-653_results.csv"
$uniqueResults |
