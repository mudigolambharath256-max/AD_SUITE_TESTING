...
Write-Host "=== Miscellaneous Check 685 ===" -ForegroundColor Cyan
...
try {
    ...
    Write-Warning "MISC-685 PowerShell engine: $_"   # Fix R10: no silent catch
}
...
try {
    ...
    Write-Warning "MISC-685 ADSI engine: $_"          # Fix R10
}
...
try {
    ...
    Write-Warning "MISC-685 C# engine: $_"             # Fix R10
}
...
Write-Host ""
Write-Host "=== MISC-685: $($uniqueResults.Count) unique findings ===" -ForegroundColor Cyan
$uniqueResults | Format-List

# Fix R14: Fixed 5-field schema enforced before CSV export
$csvPath = Join-Path $env:TEMP "MISC-685_results.csv"
$uniqueResults |
