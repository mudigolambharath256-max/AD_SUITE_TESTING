...
Write-Host "=== Miscellaneous Check 724 ===" -ForegroundColor Cyan
...
try {
    ...
    Write-Warning "MISC-724 PowerShell engine: $_"   # Fix R10: no silent catch
}
...
try {
    ...
    Write-Warning "MISC-724 ADSI engine: $_"          # Fix R10
}
...
try {
    ...
    Write-Warning "MISC-724 C# engine: $_"             # Fix R10
}
...
Write-Host ""
Write-Host "=== MISC-724: $($uniqueResults.Count) unique findings ===" -ForegroundColor Cyan
$uniqueResults | Format-List

# Fix R14: Fixed 5-field schema enforced before CSV export
$csvPath = Join-Path $env:TEMP "MISC-724_results.csv"
$uniqueResults |
