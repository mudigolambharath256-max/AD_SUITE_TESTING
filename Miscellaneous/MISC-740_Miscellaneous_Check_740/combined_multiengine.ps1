...
Write-Host "=== Miscellaneous Check 740 ===" -ForegroundColor Cyan
...
try {
    ...
    Write-Warning "MISC-740 PowerShell engine: $_"   # Fix R10: no silent catch
}
...
try {
    ...
    Write-Warning "MISC-740 ADSI engine: $_"          # Fix R10
}
...
try {
    ...
    Write-Warning "MISC-740 C# engine: $_"             # Fix R10
}
...
Write-Host ""
Write-Host "=== MISC-740: $($uniqueResults.Count) unique findings ===" -ForegroundColor Cyan
$uniqueResults | Format-List

# Fix R14: Fixed 5-field schema enforced before CSV export
$csvPath = Join-Path $env:TEMP "MISC-740_results.csv"
$uniqueResults |
