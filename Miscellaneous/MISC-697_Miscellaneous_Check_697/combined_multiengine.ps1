...
Write-Host "=== Miscellaneous Check 697 ===" -ForegroundColor Cyan
...
try {
    ...
    Write-Warning "MISC-697 PowerShell engine: $_"   # Fix R10: no silent catch
}
...
try {
    ...
    Write-Warning "MISC-697 ADSI engine: $_"          # Fix R10
}
...
try {
    ...
    Write-Warning "MISC-697 C# engine: $_"             # Fix R10
}
...
Write-Host ""
Write-Host "=== MISC-697: $($uniqueResults.Count) unique findings ===" -ForegroundColor Cyan
$uniqueResults | Format-List

# Fix R14: Fixed 5-field schema enforced before CSV export
$csvPath = Join-Path $env:TEMP "MISC-697_results.csv"
$uniqueResults |
