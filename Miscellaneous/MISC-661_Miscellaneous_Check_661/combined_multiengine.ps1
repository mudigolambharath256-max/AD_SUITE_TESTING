...
Write-Host "=== Miscellaneous Check 661 ===" -ForegroundColor Cyan
...
try {
    ...
    Write-Warning "MISC-661 PowerShell engine: $_"   # Fix R10: no silent catch
}
...
try {
    ...
    Write-Warning "MISC-661 ADSI engine: $_"          # Fix R10
}
...
try {
    ...
    Write-Warning "MISC-661 C# engine: $_"             # Fix R10
}
...
Write-Host ""
Write-Host "=== MISC-661: $($uniqueResults.Count) unique findings ===" -ForegroundColor Cyan
$uniqueResults | Format-List

# Fix R14: Fixed 5-field schema enforced before CSV export
$csvPath = Join-Path $env:TEMP "MISC-661_results.csv"
$uniqueResults |
