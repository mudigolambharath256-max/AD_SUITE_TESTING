...
Write-Host "=== Miscellaneous Check 767 ===" -ForegroundColor Cyan
...
try {
    ...
    Write-Warning "MISC-767 PowerShell engine: $_"   # Fix R10: no silent catch
}
...
try {
    ...
    Write-Warning "MISC-767 ADSI engine: $_"          # Fix R10
}
...
try {
    ...
    Write-Warning "MISC-767 C# engine: $_"             # Fix R10
}
...
Write-Host ""
Write-Host "=== MISC-767: $($uniqueResults.Count) unique findings ===" -ForegroundColor Cyan
$uniqueResults | Format-List

# Fix R14: Fixed 5-field schema enforced before CSV export
$csvPath = Join-Path $env:TEMP "MISC-767_results.csv"
$uniqueResults |
