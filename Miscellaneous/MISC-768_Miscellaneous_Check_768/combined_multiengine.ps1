...
Write-Host "=== Miscellaneous Check 768 ===" -ForegroundColor Cyan
...
try {
    ...
    Write-Warning "MISC-768 PowerShell engine: $_"   # Fix R10: no silent catch
}
...
try {
    ...
    Write-Warning "MISC-768 ADSI engine: $_"          # Fix R10
}
...
try {
    ...
    Write-Warning "MISC-768 C# engine: $_"             # Fix R10
}
...
Write-Host ""
Write-Host "=== MISC-768: $($uniqueResults.Count) unique findings ===" -ForegroundColor Cyan
$uniqueResults | Format-List

# Fix R14: Fixed 5-field schema enforced before CSV export
$csvPath = Join-Path $env:TEMP "MISC-768_results.csv"
$uniqueResults |
