...
Write-Host "=== Miscellaneous Check 646 ===" -ForegroundColor Cyan
...
try {
    ...
    Write-Warning "MISC-646 PowerShell engine: $_"   # Fix R10: no silent catch
}
...
try {
    ...
    Write-Warning "MISC-646 ADSI engine: $_"          # Fix R10
}
...
try {
    ...
    Write-Warning "MISC-646 C# engine: $_"             # Fix R10
}
...
Write-Host ""
Write-Host "=== MISC-646: $($uniqueResults.Count) unique findings ===" -ForegroundColor Cyan
$uniqueResults | Format-List

# Fix R14: Fixed 5-field schema enforced before CSV export
$csvPath = Join-Path $env:TEMP "MISC-646_results.csv"
$uniqueResults |
