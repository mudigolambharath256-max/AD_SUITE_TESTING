...
Write-Host "=== Miscellaneous Check 757 ===" -ForegroundColor Cyan
...
try {
    ...
    Write-Warning "MISC-757 PowerShell engine: $_"   # Fix R10: no silent catch
}
...
try {
    ...
    Write-Warning "MISC-757 ADSI engine: $_"          # Fix R10
}
...
try {
    ...
    Write-Warning "MISC-757 C# engine: $_"             # Fix R10
}
...
Write-Host ""
Write-Host "=== MISC-757: $($uniqueResults.Count) unique findings ===" -ForegroundColor Cyan
$uniqueResults | Format-List

# Fix R14: Fixed 5-field schema enforced before CSV export
$csvPath = Join-Path $env:TEMP "MISC-757_results.csv"
$uniqueResults |
