...
Write-Host "=== Miscellaneous Check 660 ===" -ForegroundColor Cyan
...
try {
    ...
    Write-Warning "MISC-660 PowerShell engine: $_"   # Fix R10: no silent catch
}
...
try {
    ...
    Write-Warning "MISC-660 ADSI engine: $_"          # Fix R10
}
...
try {
    ...
    Write-Warning "MISC-660 C# engine: $_"             # Fix R10
}
...
Write-Host ""
Write-Host "=== MISC-660: $($uniqueResults.Count) unique findings ===" -ForegroundColor Cyan
$uniqueResults | Format-List

# Fix R14: Fixed 5-field schema enforced before CSV export
$csvPath = Join-Path $env:TEMP "MISC-660_results.csv"
$uniqueResults |
