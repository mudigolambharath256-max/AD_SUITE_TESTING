...
Write-Host "=== Miscellaneous Check 736 ===" -ForegroundColor Cyan
...
try {
    ...
    Write-Warning "MISC-736 PowerShell engine: $_"   # Fix R10: no silent catch
}
...
try {
    ...
    Write-Warning "MISC-736 ADSI engine: $_"          # Fix R10
}
...
try {
    ...
    Write-Warning "MISC-736 C# engine: $_"             # Fix R10
}
...
Write-Host ""
Write-Host "=== MISC-736: $($uniqueResults.Count) unique findings ===" -ForegroundColor Cyan
$uniqueResults | Format-List

# Fix R14: Fixed 5-field schema enforced before CSV export
$csvPath = Join-Path $env:TEMP "MISC-736_results.csv"
$uniqueResults |
