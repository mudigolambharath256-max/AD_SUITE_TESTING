...
Write-Host "=== Miscellaneous Check 722 ===" -ForegroundColor Cyan
...
try {
    ...
    Write-Warning "MISC-722 PowerShell engine: $_"   # Fix R10: no silent catch
}
...
try {
    ...
    Write-Warning "MISC-722 ADSI engine: $_"          # Fix R10
}
...
try {
    ...
    Write-Warning "MISC-722 C# engine: $_"             # Fix R10
}
...
Write-Host ""
Write-Host "=== MISC-722: $($uniqueResults.Count) unique findings ===" -ForegroundColor Cyan
$uniqueResults | Format-List

# Fix R14: Fixed 5-field schema enforced before CSV export
$csvPath = Join-Path $env:TEMP "MISC-722_results.csv"
$uniqueResults |
