...
Write-Host "MISC-638: found $($results.Count) objects"
...
try {
    ...
    foreach ($r in $bhRaw) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                isdeleted         = $false
                adSuiteCheckId    = 'MISC-638'
                adSuiteCheckName  = 'Miscellaneous Check 638'
                adSuiteSeverity   = 'low'
    ...
    @{ data = $bhNodes.ToArray()
       meta = @{ type = 'containers'; count = $bhNodes.Count; version = 5; methods = 0 }
        ...
        Out-File -FilePath (Join-Path $bhDir "MISC-638_$bhTs.json") -Encoding UTF8 -Force

# Fix R10: No silent catch — warn on failure
} catch { Write-Warning "MISC-638 BloodHound export error: $_" }
