...
Write-Host "=== Miscellaneous Check 729 ===" -ForegroundColor Cyan
...
try {
    ...
    Write-Warning "MISC-729 PowerShell engine: $_"   # Fix R10: no silent catch
}
...
try {
    ...
    Write-Warning "MISC-729 ADSI engine: $_"          # Fix R10
}
...
try {
    ...
    Write-Warning "MISC-729 C# engine: $_"             # Fix R10
}
...
Write-Host ""
Write-Host "=== MISC-729: $($uniqueResults.Count) unique findings ===" -ForegroundColor Cyan
$uniqueResults | Format-List

# Fix R14: Fixed 5-field schema enforced before CSV export
$csvPath = Join-Path $env:TEMP "MISC-729_results.csv"
$uniqueResults |
