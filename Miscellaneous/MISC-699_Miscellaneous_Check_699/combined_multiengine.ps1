...
Write-Host "=== Miscellaneous Check 699 ===" -ForegroundColor Cyan
...
try {
    ...
    Write-Warning "MISC-699 PowerShell engine: $_"   # Fix R10: no silent catch
}
...
try {
    ...
    Write-Warning "MISC-699 ADSI engine: $_"          # Fix R10
}
...
try {
    ...
    Write-Warning "MISC-699 C# engine: $_"             # Fix R10
}
...
Write-Host ""
Write-Host "=== MISC-699: $($uniqueResults.Count) unique findings ===" -ForegroundColor Cyan
$uniqueResults | Format-List

# Fix R14: Fixed 5-field schema enforced before CSV export
$csvPath = Join-Path $env:TEMP "MISC-699_results.csv"
$uniqueResults |
