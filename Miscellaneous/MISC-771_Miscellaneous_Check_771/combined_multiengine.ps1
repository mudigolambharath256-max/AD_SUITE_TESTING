...
Write-Host "=== Miscellaneous Check 771 ===" -ForegroundColor Cyan
...
try {
    ...
    Write-Warning "MISC-771 PowerShell engine: $_"   # Fix R10: no silent catch
}
...
try {
    ...
    Write-Warning "MISC-771 ADSI engine: $_"          # Fix R10
}
...
try {
    ...
    Write-Warning "MISC-771 C# engine: $_"             # Fix R10
}
...
Write-Host ""
Write-Host "=== MISC-771: $($uniqueResults.Count) unique findings ===" -ForegroundColor Cyan
$uniqueResults | Format-List

# Fix R14: Fixed 5-field schema enforced before CSV export
$csvPath = Join-Path $env:TEMP "MISC-771_results.csv"
$uniqueResults |
