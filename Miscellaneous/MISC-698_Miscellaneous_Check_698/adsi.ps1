...
Write-Host "MISC-698: found $($results.Count) objects"
...
try {
    ...
    foreach ($r in $bhRaw) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                isdeleted         = $false
                adSuiteCheckId    = 'MISC-698'
                adSuiteCheckName  = 'Miscellaneous Check 698'
                adSuiteSeverity   = 'low'
                adSuiteCategory   = 'Miscellaneous'
                adSuiteFlag       = $true
            }
            Aces           = @()
            IsDeleted      = $false
    ...
    @{ data = $bhNodes.ToArray()
       meta = @{ type = 'containers'; count = $bhNodes.Count; version = 5; methods = 0 }
        ...
        Out-File -FilePath (Join-Path $bhDir "MISC-698_$bhTs.json") -Encoding UTF8 -Force

# Fix R10: No silent catch — warn on failure
} catch { Write-Warning "MISC-698 BloodHound export error: $_" }
