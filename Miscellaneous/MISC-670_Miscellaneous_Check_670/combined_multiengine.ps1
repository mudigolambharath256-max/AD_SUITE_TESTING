...
Write-Host "=== Miscellaneous Check 670 ===" -ForegroundColor Cyan
...
try {
    ...
    Write-Warning "MISC-670 PowerShell engine: $_"   # Fix R10: no silent catch
}
...
try {
    ...
    Write-Warning "MISC-670 ADSI engine: $_"          # Fix R10
}
...
try {
    ...
    Write-Warning "MISC-670 C# engine: $_"             # Fix R10
}
...
Write-Host ""
Write-Host "=== MISC-670: $($uniqueResults.Count) unique findings ===" -ForegroundColor Cyan
$uniqueResults | Format-List

# Fix R14: Fixed 5-field schema enforced before CSV export
$csvPath = Join-Path $env:TEMP "MISC-670_results.csv"
$uniqueResults |
