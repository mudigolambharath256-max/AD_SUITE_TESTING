...
Write-Host "=== Miscellaneous Check 758 ===" -ForegroundColor Cyan
...
try {
    ...
    Write-Warning "MISC-758 PowerShell engine: $_"   # Fix R10: no silent catch
}
...
try {
    ...
    Write-Warning "MISC-758 ADSI engine: $_"          # Fix R10
}
...
try {
    ...
    Write-Warning "MISC-758 C# engine: $_"             # Fix R10
}
...
Write-Host ""
Write-Host "=== MISC-758: $($uniqueResults.Count) unique findings ===" -ForegroundColor Cyan
$uniqueResults | Format-List

# Fix R14: Fixed 5-field schema enforced before CSV export
$csvPath = Join-Path $env:TEMP "MISC-758_results.csv"
$uniqueResults |
