...
Write-Host "=== ACC-044: Admin SDHolder Template ===" -ForegroundColor Cyan
...
if (-not $useADModule) {
    ...
    try {
        ...
        $results = $adsiResults | ForEach-Object {
            ...
            [PSCustomObject]@{
                CheckID           = 'ACC-044'
                CheckName         = 'Admin SDHolder Template'
                Method            = 'ADSI'
                Domain            = $domainNC
...
try {
    ...
    $bhFile = Join-Path $bhDir "ACC-044_$bhTs.json"
    @{
