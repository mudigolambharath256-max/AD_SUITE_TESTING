...
Write-Host "Found $($results.Count) objects for check: Shortest Path to Domain Admin" -ForegroundColor Cyan

$results | ForEach-Object {
    $props = $_.Properties
    [PSCustomObject]@{
        CheckID = 'ACC-031'
        CheckName = 'Shortest Path to Domain Admin'
...
try {
    ...
    if ($results -and $results.Count -gt 0) {
        ...
        $bhFile = Join-Path $bhDir "ACC-031_nodes.json"
