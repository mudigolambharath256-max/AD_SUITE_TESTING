...
class Program
{
    ...
    static void Main()
    {
        string filter = @"(&(objectCategory=computer)(msDS-AllowedToActOnBehalfOfOtherIdentity=*))";
        ...
        try
        {
            using (DirectoryEntry rootEntry = new DirectoryEntry("LDAP://RootDSE"))
            {
                ...
                using (DirectorySearcher searcher = new DirectorySearcher(searchEntry))
                {
                    ...
                    {
                        Console.WriteLine("ACC-007: found " + results.Count + " objects");
        ...
        {
            Console.Error.WriteLine("ACC-007 error: " + ex.Message);
            Environment.Exit(1);
        }
    }
}
