...
Write-Host "=== Computers with AllowedToActOnBehalfOfOtherIdentity ===" -ForegroundColor Cyan
...
try {
    ...
    Get-ADObject -LDAPFilter '(&(objectCategory=computer)(msDS-AllowedToActOnBehalfOfOtherIdentity=*))' `
    ...
    Write-Warning "ACC-007 PowerShell engine: $_"   # Fix R10: no silent catch
}
...
try {
    ...
    $adsiS.Filter   = '(&(objectCategory=computer)(msDS-AllowedToActOnBehalfOfOtherIdentity=*))'
    $adsiS.PageSize = 1000
    ...
    Write-Warning "ACC-007 ADSI engine: $_"          # Fix R10
}
...
try {
    $csFilter  = '(&(objectCategory=computer)(msDS-AllowedToActOnBehalfOfOtherIdentity=*))'
    ...
    Write-Warning "ACC-007 C# engine: $_"             # Fix R10
}
...
Write-Host ""
Write-Host "=== ACC-007: $($uniqueResults.Count) unique findings ===" -ForegroundColor Cyan
$uniqueResults | Format-List

# Fix R14: Fixed 5-field schema enforced before CSV export
$csvPath = Join-Path $env:TEMP "ACC-007_results.csv"
$uniqueResults |
