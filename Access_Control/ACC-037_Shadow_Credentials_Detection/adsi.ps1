...
(@('name', 'distinguishedName', 'whenCreated', 'whenChanged', 'msDS-KeyCredentialLink', 'objectSid', 'samAccountName') | ForEach-Object { [void]$searcher.PropertiesToLoad.Add($_)
}
...
Write-Host "Found $($results.Count) objects for check: Shadow Credentials Detection" -ForegroundColor Cyan

$results | ForEach-Object {
    $props = $_.Properties
    [PSCustomObject]@{
        CheckID = 'ACC-037'
        CheckName = 'Shadow Credentials Detection'
...
try {
    ...
    if ($results -and $results.Count -gt 0) {
        ...
        $bhFile = Join-Path $bhDir "ACC-037_nodes.json"
