...
Write-Host "=== ACC-037: Shadow Credentials Detection ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    $results = Get-ADObject -LDAPFilter '(msDS-KeyCredentialLink=*)' -Properties name,distinguishedName,msDS-KeyCredentialLink -SearchBase $SearchBase -SearchScope Subtree -ResultSetSize $null
} else {
...
try {
    ...
    $bhFile = Join-Path $bhDir "ACC-037_$bhTs.json"
    @{
