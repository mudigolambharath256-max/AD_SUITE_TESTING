...
Write-Host "Executing: Shadow Credentials Detection" -ForegroundColor Cyan
Write-Host "Check ID: ACC-037" -ForegroundColor Gray

$results = Get-ADObject -LDAPFilter '(msDS-KeyCredentialLink=*)' `
...
$output = $results | Select-Object `
    @{N='CheckID'; E={'ACC-037'}}, `
    @{N='CheckName'; E={'Shadow Credentials Detection'}}, `
    @{N='RiskScore'; E={9}}, `
...
try {
    ...
    foreach ($r in $output) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'C:\Users\acer\Downloads\AD_suiteXXX\Access_Control\ACC-037_Shadow_Credentials_Detection\powershell.ps1'
                adSuiteCheckName  = 'Shadow_Credentials_Detection'
                adSuiteHIGH   = 'HIGH'
    ...
    $bhFile = Join-Path $bhDir "C:\Users\acer\Downloads\AD_suiteXXX\Access_Control\ACC-037_Shadow_Credentials_Detection\powershell.ps1_$bhTs.json"
    @{
