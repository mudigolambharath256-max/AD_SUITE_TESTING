...
Write-Host "=== Privileged Groups (adminCount=1) ===" -ForegroundColor Cyan
...
try {
    ...
    Get-ADObject -LDAPFilter '(&(objectClass=group)(adminCount=1))' `
                 -Properties @('name','distinguishedName','cn','member', 'objectSid') `
                 -SearchBase $targetNC `
                 -ErrorAction Stop |
    ForEach-Object {
    ...
    Write-Warning "ACC-002 PowerShell engine: $_"   # Fix R10: no silent catch
}
...
try {
    ...
    $adsiS.Filter   = '(&(objectClass=group)(adminCount=1))'
    $adsiS.PageSize = 1000
    $adsiS.PropertiesToLoad.Clear()
    @('name','distinguishedName','cn','member', 'objectSid') | ForEach-Object { [void]$adsiS.PropertiesToLoad.Add($_) }
    ...
    Write-Warning "ACC-002 ADSI engine: $_"          # Fix R10
}
...
try {
    $csFilter  = '(&(objectClass=group)(adminCount=1))'
    $csProps   = @("name","distinguishedName","cn","member")   # PS array of double-quoted strings
    $csCheckId = 'ACC-002'
    ...
    Write-Warning "ACC-002 C# engine: $_"             # Fix R10
}
...
Write-Host ""
Write-Host "=== ACC-002: $($uniqueResults.Count) unique findings ===" -ForegroundColor Cyan
$uniqueResults | Format-List

# Fix R14: Fixed 5-field schema enforced before CSV export
$csvPath = Join-Path $env:TEMP "ACC-002_results.csv"
$uniqueResults |
