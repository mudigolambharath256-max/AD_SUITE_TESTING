...
Write-Host "ACC-002: found $($results.Count) objects"
...
$results | ForEach-Object {
    ...
    # Fix R13: Add relevant detection data to output
        MemberCount = $p['member'].Count
        Members = ($p['member'] | Select-Object -First 20) -join '; '
...
try {
    ...
    $bhS.Filter   = '(&(objectClass=group)(adminCount=1))'
    $bhS.PageSize = 1000
    ...
    foreach ($r in $bhRaw) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                adSuiteCheckName  = 'Privileged Groups (adminCount=1)'
    ...
    @{ data = $bhNodes.ToArray()
       meta = @{ type = 'groups'; count = $bhNodes.Count; version = 5; methods = 0 }
        ...
        Out-File -FilePath (Join-Path $bhDir "ACC-002_$bhTs.json") -Encoding UTF8 -Force

# Fix R10: No silent catch — warn on failure
} catch { Write-Warning "ACC-002 BloodHound export error: $_" }
