# ============================================================================
# ACC-043: Constrained Delegation Principals
# ============================================================================
...
Write-Host "=== ACC-043: Constrained Delegation Principals ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        # Execute query
        $adObjects = Get-ADObject -LDAPFilter '(&(objectCategory=person)(objectClass=user)(userAccountControl:1.2.840.113556.1.4.803:=262144)(!(userAccountControl:1.2.840.113556.1.4.803:=2)))' `
                                   -Properties name,distinguishedName,objectClass,whenCreated,whenChanged,userAccountControl,msDS-AllowedToDelegateTo `
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'ACC-043'}}, `
            @{N='CheckName'; E={'Constrained Delegation Principals'}}, `
...
if (-not $useADModule) {
    ...
    try {
        ...
        $searcher.Filter = '(&(|(objectCategory=person)(objectCategory=computer))(msDS-AllowedToDelegateTo=*)(!(userAccountControl:1.2.840.113556.1.4.803:=2)))'
        $searcher.PageSize = 1000
        $searcher.PropertiesToLoad.Clear()
        (@('name', 'distinguishedName', 'objectClass', 'whenCreated', 'whenChanged', 'userAccountControl', 'msDS-AllowedToDelegateTo') | ForEach-Object { [void]$searcher.PropertiesToLoad.Add($_) }
        ...
        $results = $adsiResults | ForEach-Object {
            ...
            [PSCustomObject]@{
                CheckID           = 'ACC-043'
                CheckName         = 'Constrained Delegation Principals'
                Method            = 'ADSI'
                Domain            = $domainNC
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'ACC-043'
                adSuiteCheckName  = 'Constrained_Delegation_Principals'
    ...
    $bhFile = Join-Path $bhDir "ACC-043_$bhTs.json"
    @{
