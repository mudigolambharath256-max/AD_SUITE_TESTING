...
(@('name', 'distinguishedName', 'objectClass', 'whenCreated', 'whenChanged', 'userAccountControl', 'msDS-AllowedToDelegateTo', 'objectSid', 'samAccountName') | ForEach-Object { [void]$searcher.PropertiesToLoad.Add($_)
}
...
Write-Host "Found $($results.Count) objects for check: Constrained Delegation Principals" -ForegroundColor Cyan
...
try {
    ...
    if ($results -and $results.Count -gt 0) {
        ...
        $bhFile = Join-Path $bhDir "ACC-043_nodes.json"
