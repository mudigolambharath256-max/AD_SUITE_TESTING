...
$ldapFilter = '(&(|(objectCategory=person)(objectCategory=computer))(msDS-AllowedToDelegateTo=*)(!(userAccountControl:1.2.840.113556.1.4.803:=2)))'

# Define properties to retrieve
$properties = @('name', 'distinguishedName', 'objectClass', 'whenCreated', 'whenChanged', 'objectSid')

Write-Host "Executing security check: Constrained Delegation Principals" -ForegroundColor Yellow
Write-Host "Check ID: ACC-028" -ForegroundColor Gray
...
try {
    ...
    foreach ($r in $output) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'C:\Users\acer\Downloads\AD_suiteXXX\Access_Control\ACC-043_Constrained_Delegation_Principals\powershell.ps1'
    ...
    $bhFile = Join-Path $bhDir "C:\Users\acer\Downloads\AD_suiteXXX\Access_Control\ACC-043_Constrained_Delegation_Principals\powershell.ps1_$bhTs.json"
    @{
