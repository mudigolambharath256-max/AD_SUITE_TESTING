...
Write-Host "=== ACC-045: Privileged Access Workstation PAW Computers ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        # Execute query
        $adObjects = Get-ADObject -LDAPFilter '(&(objectCategory=person)(objectClass=user)(userAccountControl:1.2.840.113556.1.4.803:=1048576)(!(userAccountControl:1.2.840.113556.1.4.803:=2)))' `
...
if (-not $useADModule) {
    ...
    try {
        ...
        $searcher.Filter = '(&(objectCategory=person)(objectClass=user)(userAccountControl:1.2.840.113556.1.4.803:=1048576)(!(userAccountControl:1.2.840.113556.1.4.803:=2)))'
        ...
        $results = $adsiResults | ForEach-Object {
            ...
            [PSCustomObject]@{
                CheckID           = 'ACC-045'
                CheckName         = 'Privileged Access Workstation PAW Computers'
                Method            = 'ADSI'
                Domain            = $domainNC
...
try {
    ...
    $bhFile = Join-Path $bhDir "ACC-045_$bhTs.json"
    @{
