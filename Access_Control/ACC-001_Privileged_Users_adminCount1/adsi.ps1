...
@('name', 'distinguishedName', 'samAccountName', 'adminCount', 'userAccountControl', 'objectClass', 'whenCreated', 'whenChanged', 'objectSid') | ForEach-Object {
...
Write-Host "Found $($results.Count) objects for check: Privileged Users adminCount1" -ForegroundColor Cyan
...
$results | ForEach-Object {
    ...
    SamAccountName = if ($props['samaccountname'] -and $props['samaccountname'].Count -gt 0) { $props['samaccountname'][0] } else { 'N/A' } } else { 'N/A' } } else { 'N/A'
        ...
        AdminCount         = if ($props['admincount'].Count -gt 0) { $props['admincount'][0] } else { 'N/A' }
...
try {
    ...
    if ($results -and $results.Count -gt 0) {
        ...
        $bhFile = Join-Path $bhDir "ACC-001_nodes.json"
