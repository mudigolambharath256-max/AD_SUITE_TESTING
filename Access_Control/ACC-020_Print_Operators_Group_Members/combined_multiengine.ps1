# ============================================================================
# ACC-020: Print Operators Group Members
# ============================================================================
...
Write-Host "=== ACC-020: Print Operators Group Members ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        # Execute query
        $adObjects = Get-ADObject -LDAPFilter '(&(objectClass=group)(samAccountName=Print Operators))' `
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'ACC-020'}}, `
            @{N='CheckName'; E={'Print Operators Group Members'}}, `
...
if (-not $useADModule) {
    ...
    try {
        ...
        $searcher.Filter = '(&(objectClass=group)(samAccountName=Print Operators))'
        ...
        $results = $adsiResults | ForEach-Object {
            ...
            [PSCustomObject]@{
                CheckID           = 'ACC-020'
                CheckName         = 'Print Operators Group Members'
                Method            = 'ADSI'
                Domain            = $domainNC
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'ACC-020'
                adSuiteCheckName  = 'Print_Operators_Group_Members'
    ...
    $bhFile = Join-Path $bhDir "ACC-020_$bhTs.json"
    @{
