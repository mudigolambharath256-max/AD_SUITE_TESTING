...
(@('name', 'distinguishedName', 'objectClass', 'whenCreated', 'whenChanged', 'userAccountControl', 'msDS-KeyCredentialLink', 'objectSid', 'samAccountName') | ForEach-Object { [void]$searcher.PropertiesToLoad.Add($_)
}
...
Write-Host "Found $($results.Count) objects for check: KeyCredentialLink on Computers" -ForegroundColor Cyan
...
try {
    ...
    if ($results -and $results.Count -gt 0) {
        ...
        $bhFile = Join-Path $bhDir "ACC-009_nodes.json"
