...
Write-Host "=== ACC-009: KeyCredentialLink on Computers ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        # Execute query
        $adObjects = Get-ADObject -LDAPFilter '(&(objectCategory=computer)(msDS-KeyCredentialLink=*)(!(userAccountControl:1.2.840.113556.1.4.803:=2)))' `
                                   -Properties name,distinguishedName,objectClass,whenCreated,whenChanged,userAccountControl,msDS-KeyCredentialLink `
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'ACC-009'}}, `
            @{N='CheckName'; E={'KeyCredentialLink on Computers'}}, `
...
if (-not $useADModule) {
    ...
    try {
        ...
        $searcher.Filter = '(&(objectCategory=computer)(msDS-KeyCredentialLink=*)(!(userAccountControl:1.2.840.113556.1.4.803:=2)))'
        $searcher.PageSize = 1000
        $searcher.PropertiesToLoad.Clear()
        (@('name', 'distinguishedName', 'objectClass', 'whenCreated', 'whenChanged', 'userAccountControl', 'msDS-KeyCredentialLink') | ForEach-Object { [void]$searcher.PropertiesToLoad.Add($_) }
        ...
        $results = $adsiResults | ForEach-Object {
            ...
            [PSCustomObject]@{
                CheckID           = 'ACC-009'
                CheckName         = 'KeyCredentialLink on Computers'
                Method            = 'ADSI'
                Domain            = $domainNC
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'ACC-009'
                adSuiteCheckName  = 'KeyCredentialLink_on_Computers'
    ...
    $bhFile = Join-Path $bhDir "ACC-009_$bhTs.json"
    @{
