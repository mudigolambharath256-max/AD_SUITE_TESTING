...
$ldapFilter = '(&(objectCategory=computer)(msDS-KeyCredentialLink=*)(!(userAccountControl:1.2.840.113556.1.4.803:=2)))'

# Define properties to retrieve
$properties = @('name', 'distinguishedName', 'objectClass', 'whenCreated', 'whenChanged', 'objectSid')

Write-Host "Executing security check: KeyCredentialLink on Computers" -ForegroundColor Yellow
Write-Host "Check ID: ACC-009" -ForegroundColor Gray
...
try {
    ...
    foreach ($r in $output) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'C:\Users\acer\Downloads\AD_suiteXXX\Access_Control\ACC-009_KeyCredentialLink_on_Computers\powershell.ps1'
    ...
    $bhFile = Join-Path $bhDir "C:\Users\acer\Downloads\AD_suiteXXX\Access_Control\ACC-009_KeyCredentialLink_on_Computers\powershell.ps1_$bhTs.json"
    @{
