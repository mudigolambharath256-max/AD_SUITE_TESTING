# ============================================================================
# ACC-003: Privileged Computers adminCount1
# ============================================================================
...
Write-Host "=== ACC-003: Privileged Computers adminCount1 ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        # Execute query
        $adObjects = Get-ADObject -LDAPFilter '(&(objectCategory=computer)(adminCount=1)(!(userAccountControl:1.2.840.113556.1.4.803:=2)))' `
                                   -Properties name,distinguishedName,objectClass,whenCreated,whenChanged,adminCount,userAccountControl `
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'ACC-003'}}, `
            @{N='CheckName'; E={'Privileged Computers adminCount1'}}, `
...
if (-not $useADModule) {
    ...
    try {
        ...
        $searcher.Filter = '(&(objectCategory=computer)(adminCount=1)(!(userAccountControl:1.2.840.113556.1.4.803:=2)))'
        $searcher.PageSize = 1000
        $searcher.PropertiesToLoad.Clear()
        (@('name', 'distinguishedName', 'objectClass', 'whenCreated', 'whenChanged', 'adminCount', 'userAccountControl') | ForEach-Object { [void]$searcher.PropertiesToLoad.Add($_) }
        ...
        $results = $adsiResults | ForEach-Object {
            ...
            [PSCustomObject]@{
                CheckID           = 'ACC-003'
                CheckName         = 'Privileged Computers adminCount1'
                Method            = 'ADSI'
                Domain            = $domainNC
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'ACC-003'
                adSuiteCheckName  = 'Privileged_Computers_adminCount1'
    ...
    $bhFile = Join-Path $bhDir "ACC-003_$bhTs.json"
    @{
