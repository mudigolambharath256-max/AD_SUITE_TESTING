...
$ldapFilter = '(&(objectClass=group)(samAccountName=Backup Operators))'

# Define properties to retrieve
$properties = @('name', 'distinguishedName', 'objectClass', 'whenCreated', 'whenChanged', 'objectSid')

Write-Host "Executing security check: Backup Operators Group Members" -ForegroundColor Yellow
Write-Host "Check ID: ACC-017" -ForegroundColor Gray
...
try {
    ...
    foreach ($r in $output) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'C:\Users\acer\Downloads\AD_suiteXXX\Access_Control\ACC-017_Backup_Operators_Group_Members\powershell.ps1'
    ...
    $bhFile = Join-Path $bhDir "C:\Users\acer\Downloads\AD_suiteXXX\Access_Control\ACC-017_Backup_Operators_Group_Members\powershell.ps1_$bhTs.json"
    @{
