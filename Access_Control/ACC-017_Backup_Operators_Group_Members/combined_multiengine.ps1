# ============================================================================
# ACC-017: Backup Operators Group Members
# ============================================================================
...
Write-Host "=== ACC-017: Backup Operators Group Members ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        # Execute query
        $adObjects = Get-ADObject -LDAPFilter '(&(objectClass=group)(samAccountName=Backup Operators))' `
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'ACC-017'}}, `
            @{N='CheckName'; E={'Backup Operators Group Members'}}, `
...
if (-not $useADModule) {
    ...
    try {
        ...
        $searcher.Filter = '(&(objectClass=group)(samAccountName=Backup Operators))'
        ...
        $results = $adsiResults | ForEach-Object {
            ...
            [PSCustomObject]@{
                CheckID           = 'ACC-017'
                CheckName         = 'Backup Operators Group Members'
                Method            = 'ADSI'
                Domain            = $domainNC
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'ACC-017'
                adSuiteCheckName  = 'Backup_Operators_Group_Members'
    ...
    $bhFile = Join-Path $bhDir "ACC-017_$bhTs.json"
    @{
