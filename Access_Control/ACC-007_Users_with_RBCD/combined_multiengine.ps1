...
Write-Host "=== ACC-007: Users with RBCD ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        # Execute query
        $adObjects = Get-ADObject -LDAPFilter '(&(objectCategory=person)(objectClass=user)(msDS-AllowedToActOnBehalfOfOtherIdentity=*)(!(userAccountControl:1.2.840.113556.1.4.803:=2)))' `
                                   -Properties name,distinguishedName,objectClass,whenCreated,whenChanged,msDS-AllowedToActOnBehalfOfOtherIdentity,userAccountControl,samAccountName `
...
if (-not $useADModule) {
    ...
    try {
        ...
        $searcher.Filter = '(&(objectCategory=person)(objectClass=user)(msDS-AllowedToActOnBehalfOfOtherIdentity=*)(!(userAccountControl:1.2.840.113556.1.4.803:=2)))'
        $searcher.PageSize = 1000
        $searcher.PropertiesToLoad.Clear()
        (@('name', 'distinguishedName', 'objectClass', 'whenCreated', 'whenChanged', 'msDS-AllowedToActOnBehalfOfOtherIdentity', 'userAccountControl', 'samAccountName') | ForEach-Object { [void]$searcher.PropertiesToLoad.Add($_) }
        ...
        $results = $adsiResults | ForEach-Object {
            ...
            [PSCustomObject]@{
                CheckID           = 'ACC-007'
                CheckName         = 'Users with RBCD'
                Method            = 'ADSI'
                Domain            = $domainNC
                Name              = if ($props['name'].Count -gt 0) { $props['name'][0]
        MsDsAllowedToActOnBehalfOfOtherIdentity = if ($props['msds-allowedtoactonbehalfofotheridentity'].Count -gt 0) { $props['msds-allowedtoactonbehalfofotheridentity'][0]
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'ACC-007'
                adSuiteCheckName  = 'Users_with_RBCD'
    ...
    $bhFile = Join-Path $bhDir "ACC-007_$bhTs.json"
    @{
