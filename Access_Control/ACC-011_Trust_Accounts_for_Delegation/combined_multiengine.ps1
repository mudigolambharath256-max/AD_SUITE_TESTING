# ============================================================================
# ACC-011: Trust Accounts for Delegation
# ============================================================================
...
Write-Host "=== ACC-011: Trust Accounts for Delegation ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        # Execute query
        $adObjects = Get-ADObject -LDAPFilter '(&(objectCategory=person)(objectClass=user)(userAccountControl:1.2.840.113556.1.4.803:=524288)(!(userAccountControl:1.2.840.113556.1.4.803:=2)))' `
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'ACC-011'}}, `
            @{N='CheckName'; E={'Trust Accounts for Delegation'}}, `
...
if (-not $useADModule) {
    ...
    try {
        ...
        $searcher.Filter = '(&(objectCategory=person)(objectClass=user)(userAccountControl:1.2.840.113556.1.4.803:=524288)(!(userAccountControl:1.2.840.113556.1.4.803:=2)))'
        ...
        $results = $adsiResults | ForEach-Object {
            ...
            [PSCustomObject]@{
                CheckID           = 'ACC-011'
                CheckName         = 'Trust Accounts for Delegation'
                Method            = 'ADSI'
                Domain            = $domainNC
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'ACC-011'
                adSuiteCheckName  = 'Trust_Accounts_for_Delegation'
    ...
    $bhFile = Join-Path $bhDir "ACC-011_$bhTs.json"
    @{
