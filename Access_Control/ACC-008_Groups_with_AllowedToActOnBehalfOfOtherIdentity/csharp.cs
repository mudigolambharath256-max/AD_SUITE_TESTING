...
class Program
{
    ...
    static void Main()
    {
        string filter = @"(&(objectClass=group)(msDS-AllowedToActOnBehalfOfOtherIdentity=*))";
        string[] props = new string[] { "name","distinguishedName","cn","msDS-AllowedToActOnBehalfOfOtherIdentity" };
        ...
        try
        {
            using (DirectoryEntry rootEntry = new DirectoryEntry("LDAP://RootDSE"))
            {
                ...
                using (DirectorySearcher searcher = new DirectorySearcher(searchEntry))
                {
                    ...
                    {
                        Console.WriteLine("ACC-008: found " + results.Count + " objects");
        ...
        {
            Console.Error.WriteLine("ACC-008 error: " + ex.Message);
            Environment.Exit(1);
        }
    }
}
