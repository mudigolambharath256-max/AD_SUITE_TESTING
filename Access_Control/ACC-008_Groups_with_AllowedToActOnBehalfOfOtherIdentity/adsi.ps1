...
$searcher.Filter   = '(&(objectClass=group)(msDS-AllowedToActOnBehalfOfOtherIdentity=*))'
$searcher.PageSize = 1000
$searcher.PropertiesToLoad.Clear()
# Fix R12: objectSid only for identity objects
@('name','distinguishedName','cn','msDS-AllowedToActOnBehalfOfOtherIdentity', 'objectSid') | ForEach-Object { [void]$searcher.PropertiesToLoad.Add($_) }

$results = $searcher.FindAll()
Write-Host "ACC-008: found $($results.Count) objects"
...
$results | ForEach-Object {
    ...
    # Fix R13: Add relevant detection data to output
        msDS-AllowedToActOnBehalfOfOtherIdentity = ($p['msds-allowedtoactonbehalfofotheridentity'] | Select-Object -First 1)
...
try {
    ...
    $bhS.Filter   = '(&(objectClass=group)(msDS-AllowedToActOnBehalfOfOtherIdentity=*))'
    $bhS.PageSize = 1000
    $bhS.PropertiesToLoad.Clear()
    @('name','distinguishedname','samaccountname','cn','displayname','objectsid') |
    ...
    foreach ($r in $bhRaw) {
        ...
        $bhName = if ($cn) { "$($cn.ToUpper())@$dom" } else { "$($nm.ToUpper())@$dom" }
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                isdeleted         = $false
                adSuiteCheckId    = 'ACC-008'
                adSuiteCheckName  = 'Groups with AllowedToActOnBehalfOfOtherIdentity'
    ...
    @{ data = $bhNodes.ToArray()
       meta = @{ type = 'groups'; count = $bhNodes.Count; version = 5; methods = 0 }
        ...
        Out-File -FilePath (Join-Path $bhDir "ACC-008_$bhTs.json") -Encoding UTF8 -Force

# Fix R10: No silent catch — warn on failure
} catch { Write-Warning "ACC-008 BloodHound export error: $_" }
