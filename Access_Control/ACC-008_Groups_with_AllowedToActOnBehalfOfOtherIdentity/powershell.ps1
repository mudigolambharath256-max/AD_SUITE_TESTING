...
$ldapFilter = '(&(objectClass=group)(msDS-AllowedToActOnBehalfOfOtherIdentity=*))'
$props      = @('name','distinguishedName','cn','msDS-AllowedToActOnBehalfOfOtherIdentity', 'objectSid')
...
try {
    ...
    Write-Host "ACC-008: found $($found.Count) objects"
    ...
    Write-Warning "ACC-008: Object not found — $_"
} catch [Microsoft.ActiveDirectory.Management.ADServerDownException] {
    Write-Warning "ACC-008: AD server unreachable — $_"
} catch {
    # Fix R10: no silent catch
    Write-Warning "ACC-008: Query failed — $_"
}
