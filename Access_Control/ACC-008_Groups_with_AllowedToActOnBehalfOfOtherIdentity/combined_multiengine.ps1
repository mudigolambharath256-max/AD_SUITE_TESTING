...
# Check: Groups with AllowedToActOnBehalfOfOtherIdentity
...
Write-Host "=== Groups with AllowedToActOnBehalfOfOtherIdentity ===" -ForegroundColor Cyan
...
try {
    ...
    Get-ADObject -LDAPFilter '(&(objectClass=group)(msDS-AllowedToActOnBehalfOfOtherIdentity=*))' `
                 -Properties @('name','distinguishedName','cn','msDS-AllowedToActOnBehalfOfOtherIdentity', 'objectSid') `
                 -SearchBase $targetNC `
                 -ErrorAction Stop |
    ForEach-Object {
    ...
    Write-Warning "ACC-008 PowerShell engine: $_"   # Fix R10: no silent catch
}
...
try {
    ...
    $adsiS.Filter   = '(&(objectClass=group)(msDS-AllowedToActOnBehalfOfOtherIdentity=*))'
    $adsiS.PageSize = 1000
    $adsiS.PropertiesToLoad.Clear()
    @('name','distinguishedName','cn','msDS-AllowedToActOnBehalfOfOtherIdentity', 'objectSid') | ForEach-Object { [void]$adsiS.PropertiesToLoad.Add($_) }
    ...
    Write-Warning "ACC-008 ADSI engine: $_"          # Fix R10
}
...
try {
    $csFilter  = '(&(objectClass=group)(msDS-AllowedToActOnBehalfOfOtherIdentity=*))'
    $csProps   = @("name","distinguishedName","cn","msDS-AllowedToActOnBehalfOfOtherIdentity")   # PS array of double-quoted strings
    $csCheckId = 'ACC-008'
    ...
    Write-Warning "ACC-008 C# engine: $_"             # Fix R10
}
...
Write-Host ""
Write-Host "=== ACC-008: $($uniqueResults.Count) unique findings ===" -ForegroundColor Cyan
$uniqueResults | Format-List

# Fix R14: Fixed 5-field schema enforced before CSV export
$csvPath = Join-Path $env:TEMP "ACC-008_results.csv"
$uniqueResults |
