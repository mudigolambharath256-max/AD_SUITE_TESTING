...
$ldapFilter = '(&(objectClass=group)(samAccountName=Enterprise Admins))'

# Define properties to retrieve
$properties = @('name', 'distinguishedName', 'objectClass', 'whenCreated', 'whenChanged', 'objectSid')

Write-Host "Executing security check: Enterprise Admins Group Members" -ForegroundColor Yellow
Write-Host "Check ID: ACC-015" -ForegroundColor Gray
...
try {
    ...
    foreach ($r in $output) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'C:\Users\acer\Downloads\AD_suiteXXX\Access_Control\ACC-015_Enterprise_Admins_Group_Members\powershell.ps1'
    ...
    $bhFile = Join-Path $bhDir "C:\Users\acer\Downloads\AD_suiteXXX\Access_Control\ACC-015_Enterprise_Admins_Group_Members\powershell.ps1_$bhTs.json"
    @{
