...
(@('name', 'distinguishedName', 'objectClass', 'whenCreated', 'whenChanged', 'samAccountName', 'objectSid') | ForEach-Object { [void]$searcher.PropertiesToLoad.Add($_)
}
...
Write-Host "Found $($results.Count) objects for check: Enterprise Admins Group Members" -ForegroundColor Cyan
...
try {
    ...
    if ($results -and $results.Count -gt 0) {
        ...
        $bhFile = Join-Path $bhDir "ACC-015_nodes.json"
