# ============================================================================
# ACC-015: Enterprise Admins Group Members
# ============================================================================
...
Write-Host "=== ACC-015: Enterprise Admins Group Members ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        # Execute query
        $adObjects = Get-ADObject -LDAPFilter '(&(objectClass=group)(samAccountName=Enterprise Admins))' `
                                   -Properties name,distinguishedName,objectClass,whenCreated,whenChanged,samAccountName `
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'ACC-015'}}, `
            @{N='CheckName'; E={'Enterprise Admins Group Members'}}, `
...
if (-not $useADModule) {
    ...
    try {
        ...
        $searcher.Filter = '(&(objectClass=group)(samAccountName=Enterprise Admins))'
        $searcher.PageSize = 1000
        $searcher.PropertiesToLoad.Clear()
        (@('name', 'distinguishedName', 'objectClass', 'whenCreated', 'whenChanged', 'samAccountName') | ForEach-Object { [void]$searcher.PropertiesToLoad.Add($_) }
        ...
        $results = $adsiResults | ForEach-Object {
            ...
            [PSCustomObject]@{
                CheckID           = 'ACC-015'
                CheckName         = 'Enterprise Admins Group Members'
                Method            = 'ADSI'
                Domain            = $domainNC
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'ACC-015'
                adSuiteCheckName  = 'Enterprise_Admins_Group_Members'
    ...
    $bhFile = Join-Path $bhDir "ACC-015_$bhTs.json"
    @{
