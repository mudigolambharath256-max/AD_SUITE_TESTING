...
Write-Host "=== ACC-038: Constrained Delegation Targets ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    $results = Get-ADObject -LDAPFilter '(msDS-AllowedToDelegateTo=*)' -Properties name,distinguishedName,msDS-AllowedToDelegateTo -SearchBase $SearchBase -SearchScope Subtree -ResultSetSize $null
} else {
...
try {
    ...
    $bhFile = Join-Path $bhDir "ACC-038_$bhTs.json"
    @{
