...
(@('name', 'distinguishedName', 'whenCreated', 'whenChanged', 'msDS-AllowedToDelegateTo', 'objectSid', 'samAccountName') | ForEach-Object { [void]$searcher.PropertiesToLoad.Add($_)
}
...
Write-Host "Found $($results.Count) objects for check: Constrained Delegation Targets" -ForegroundColor Cyan

$results | ForEach-Object {
    $props = $_.Properties
    [PSCustomObject]@{
        CheckID = 'ACC-038'
        CheckName = 'Constrained Delegation Targets'
...
try {
    ...
    if ($results -and $results.Count -gt 0) {
        ...
        $bhFile = Join-Path $bhDir "ACC-038_nodes.json"
