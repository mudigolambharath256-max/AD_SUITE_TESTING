...
$searcher.Filter   = '(&(objectClass=user)(msDS-KeyCredentialLink=*))'
$searcher.PageSize = 1000
$searcher.PropertiesToLoad.Clear()
# Fix R12: objectSid only for identity objects
@('name','distinguishedName','samAccountName','msDS-KeyCredentialLink', 'objectSid') | ForEach-Object { [void]$searcher.PropertiesToLoad.Add($_) }

$results = $searcher.FindAll()
Write-Host "ACC-009: found $($results.Count) objects"
...
$results | ForEach-Object {
    ...
    # Fix R13: Add relevant detection data to output
        KeyCredentialCount = $p['msds-keycredentiallink'].Count
...
try {
    ...
    $bhS.Filter   = '(&(objectClass=user)(msDS-KeyCredentialLink=*))'
    $bhS.PageSize = 1000
    ...
    foreach ($r in $bhRaw) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                adSuiteCheckName  = 'Users with Key Credentials'
    ...
    @{ data = $bhNodes.ToArray()
       meta = @{ type = 'users'; count = $bhNodes.Count; version = 5; methods = 0 }
        ...
        Out-File -FilePath (Join-Path $bhDir "ACC-009_$bhTs.json") -Encoding UTF8 -Force

# Fix R10: No silent catch — warn on failure
} catch { Write-Warning "ACC-009 BloodHound export error: $_" }
