...
# Check: Users with Key Credentials
...
Write-Host "=== Users with Key Credentials ===" -ForegroundColor Cyan
...
try {
    ...
    Write-Warning "ACC-009 PowerShell engine: $_"   # Fix R10: no silent catch
}
...
try {
    ...
    $adsiS.Filter   = '(&(objectClass=user)(msDS-KeyCredentialLink=*))'
    $adsiS.PageSize = 1000
    ...
    Write-Warning "ACC-009 ADSI engine: $_"          # Fix R10
}
...
try {
    $csFilter  = '(&(objectClass=user)(msDS-KeyCredentialLink=*))'
    ...
    Write-Warning "ACC-009 C# engine: $_"             # Fix R10
}
...
Write-Host ""
Write-Host "=== ACC-009: $($uniqueResults.Count) unique findings ===" -ForegroundColor Cyan
$uniqueResults | Format-List

# Fix R14: Fixed 5-field schema enforced before CSV export
$csvPath = Join-Path $env:TEMP "ACC-009_results.csv"
$uniqueResults |
