...
class Program
{
    ...
    static void Main()
    {
        string filter = @"(&(objectClass=user)(msDS-KeyCredentialLink=*))";
        ...
        try
        {
            using (DirectoryEntry rootEntry = new DirectoryEntry("LDAP://RootDSE"))
            {
                ...
                using (DirectorySearcher searcher = new DirectorySearcher(searchEntry))
                {
                    ...
                    {
                        Console.WriteLine("ACC-009: found " + results.Count + " objects");
        ...
        {
            Console.Error.WriteLine("ACC-009 error: " + ex.Message);
            Environment.Exit(1);
        }
    }
}
