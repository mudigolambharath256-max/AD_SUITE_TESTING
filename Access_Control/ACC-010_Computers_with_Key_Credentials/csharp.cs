...
class Program
{
    ...
    static void Main()
    {
        string filter = @"(&(objectCategory=computer)(msDS-KeyCredentialLink=*))";
        ...
        try
        {
            using (DirectoryEntry rootEntry = new DirectoryEntry("LDAP://RootDSE"))
            {
                ...
                using (DirectorySearcher searcher = new DirectorySearcher(searchEntry))
                {
                    ...
                    {
                        Console.WriteLine("ACC-010: found " + results.Count + " objects");
        ...
        {
            Console.Error.WriteLine("ACC-010 error: " + ex.Message);
            Environment.Exit(1);
        }
    }
}
