...
Write-Host "=== Computers with Key Credentials ===" -ForegroundColor Cyan
...
try {
    ...
    Get-ADObject -LDAPFilter '(&(objectCategory=computer)(msDS-KeyCredentialLink=*))' `
                 -Properties @('name','distinguishedName','samAccountName','msDS-KeyCredentialLink', 'objectSid') `
                 -SearchBase $targetNC `
                 -ErrorAction Stop |
    ForEach-Object {
    ...
    Write-Warning "ACC-010 PowerShell engine: $_"   # Fix R10: no silent catch
}
...
try {
    ...
    $adsiS.Filter   = '(&(objectCategory=computer)(msDS-KeyCredentialLink=*))'
    $adsiS.PageSize = 1000
    $adsiS.PropertiesToLoad.Clear()
    @('name','distinguishedName','samAccountName','msDS-KeyCredentialLink', 'objectSid') | ForEach-Object { [void]$adsiS.PropertiesToLoad.Add($_) }
    ...
    Write-Warning "ACC-010 ADSI engine: $_"          # Fix R10
}
...
try {
    $csFilter  = '(&(objectCategory=computer)(msDS-KeyCredentialLink=*))'
    $csProps   = @("name","distinguishedName","samAccountName","msDS-KeyCredentialLink")   # PS array of double-quoted strings
    $csCheckId = 'ACC-010'
    ...
    Write-Warning "ACC-010 C# engine: $_"             # Fix R10
}
...
Write-Host ""
Write-Host "=== ACC-010: $($uniqueResults.Count) unique findings ===" -ForegroundColor Cyan
$uniqueResults | Format-List

# Fix R14: Fixed 5-field schema enforced before CSV export
$csvPath = Join-Path $env:TEMP "ACC-010_results.csv"
$uniqueResults |
