...
$searcher.Filter   = '(&(objectCategory=computer)(msDS-KeyCredentialLink=*))'
...
Write-Host "ACC-010: found $($results.Count) objects"
...
try {
    ...
    $bhS.Filter   = '(&(objectCategory=computer)(msDS-KeyCredentialLink=*))'
    $bhS.PageSize = 1000
    ...
    foreach ($r in $bhRaw) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                isdeleted         = $false
                adSuiteCheckId    = 'ACC-010'
                adSuiteCheckName  = 'Computers with Key Credentials'
    ...
    @{ data = $bhNodes.ToArray()
       meta = @{ type = 'computers'; count = $bhNodes.Count; version = 5; methods = 0 }
        ...
        Out-File -FilePath (Join-Path $bhDir "ACC-010_$bhTs.json") -Encoding UTF8 -Force

# Fix R10: No silent catch — warn on failure
} catch { Write-Warning "ACC-010 BloodHound export error: $_" }
