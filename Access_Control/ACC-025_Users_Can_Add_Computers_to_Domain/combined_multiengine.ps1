...
Write-Host "=== ACC-025: Users Can Add Computers to Domain ===" -ForegroundColor Cyan
...
if (-not $useADModule) {
    ...
    try {
        ...
        $results = $adsiResults | ForEach-Object {
            ...
            [PSCustomObject]@{
                CheckID           = 'ACC-025'
                CheckName         = 'Users Can Add Computers to Domain'
                Method            = 'ADSI'
                Domain            = $domainNC
...
try {
    ...
    $bhFile = Join-Path $bhDir "ACC-025_$bhTs.json"
    @{
