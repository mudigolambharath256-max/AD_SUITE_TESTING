...
Write-Host "=== Groups with SIDHistory ===" -ForegroundColor Cyan
...
try {
    ...
    Get-ADObject -LDAPFilter '(&(objectClass=group)(sIDHistory=*))' `
                 -Properties @('name','distinguishedName','cn','sIDHistory', 'objectSid') `
                 -SearchBase $targetNC `
                 -ErrorAction Stop |
    ForEach-Object {
    ...
    Write-Warning "ACC-005 PowerShell engine: $_"   # Fix R10: no silent catch
}
...
try {
    ...
    $adsiS.Filter   = '(&(objectClass=group)(sIDHistory=*))'
    $adsiS.PageSize = 1000
    $adsiS.PropertiesToLoad.Clear()
    @('name','distinguishedName','cn','sIDHistory', 'objectSid') | ForEach-Object { [void]$adsiS.PropertiesToLoad.Add($_) }
    ...
    Write-Warning "ACC-005 ADSI engine: $_"          # Fix R10
}
...
try {
    $csFilter  = '(&(objectClass=group)(sIDHistory=*))'
    $csProps   = @("name","distinguishedName","cn","sIDHistory")   # PS array of double-quoted strings
    $csCheckId = 'ACC-005'
    ...
    Write-Warning "ACC-005 C# engine: $_"             # Fix R10
}
...
Write-Host ""
Write-Host "=== ACC-005: $($uniqueResults.Count) unique findings ===" -ForegroundColor Cyan
$uniqueResults | Format-List

# Fix R14: Fixed 5-field schema enforced before CSV export
$csvPath = Join-Path $env:TEMP "ACC-005_results.csv"
$uniqueResults |
