...
$searcher.Filter   = '(&(objectClass=group)(sIDHistory=*))'
$searcher.PageSize = 1000
$searcher.PropertiesToLoad.Clear()
# Fix R12: objectSid only for identity objects
@('name','distinguishedName','cn','sIDHistory', 'objectSid') | ForEach-Object { [void]$searcher.PropertiesToLoad.Add($_) }

$results = $searcher.FindAll()
Write-Host "ACC-005: found $($results.Count) objects"
...
try {
    ...
    $bhS.Filter   = '(&(objectClass=group)(sIDHistory=*))'
    $bhS.PageSize = 1000
    ...
    foreach ($r in $bhRaw) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                isdeleted         = $false
                adSuiteCheckId    = 'ACC-005'
                adSuiteCheckName  = 'Groups with SIDHistory'
    ...
    @{ data = $bhNodes.ToArray()
       meta = @{ type = 'groups'; count = $bhNodes.Count; version = 5; methods = 0 }
        ...
        Out-File -FilePath (Join-Path $bhDir "ACC-005_$bhTs.json") -Encoding UTF8 -Force

# Fix R10: No silent catch — warn on failure
} catch { Write-Warning "ACC-005 BloodHound export error: $_" }
