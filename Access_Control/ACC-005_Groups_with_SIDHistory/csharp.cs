...
class Program
{
    ...
    static void Main()
    {
        string filter = @"(&(objectClass=group)(sIDHistory=*))";
        string[] props = new string[] { "name","distinguishedName","cn","sIDHistory" };
        ...
        try
        {
            using (DirectoryEntry rootEntry = new DirectoryEntry("LDAP://RootDSE"))
            {
                ...
                using (DirectorySearcher searcher = new DirectorySearcher(searchEntry))
                {
                    ...
                    {
                        Console.WriteLine("ACC-005: found " + results.Count + " objects");
        ...
        {
            Console.Error.WriteLine("ACC-005 error: " + ex.Message);
            Environment.Exit(1);
        }
    }
}
