...
$ldapFilter = '(&(objectClass=group)(sIDHistory=*))'
$props      = @('name','distinguishedName','cn','sIDHistory', 'objectSid')
...
try {
    ...
    Write-Host "ACC-005: found $($found.Count) objects"
    ...
    $output = $found | ForEach-Object {
        ...
        # Fix R13: Add relevant detection data to output
                if ($_.'sIDHistory') { $obj | Add-Member -NotePropertyName 'sIDHistory' -NotePropertyValue ([string]$_.'sIDHistory') -Force }
    ...
    Write-Warning "ACC-005: Object not found — $_"
} catch [Microsoft.ActiveDirectory.Management.ADServerDownException] {
    Write-Warning "ACC-005: AD server unreachable — $_"
} catch {
    # Fix R10: no silent catch
    Write-Warning "ACC-005: Query failed — $_"
}
