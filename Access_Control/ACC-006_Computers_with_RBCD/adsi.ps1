...
(@('name', 'distinguishedName', 'objectClass', 'whenCreated', 'whenChanged', 'msDS-AllowedToActOnBehalfOfOtherIdentity', 'userAccountControl', 'objectSid', 'samAccountName') | ForEach-Object { [void]$searcher.PropertiesToLoad.Add($_)
}
...
Write-Host "Found $($results.Count) objects for check: Computers with RBCD" -ForegroundColor Cyan
...
try {
    ...
    if ($results -and $results.Count -gt 0) {
        ...
        $bhFile = Join-Path $bhDir "ACC-006_nodes.json"
