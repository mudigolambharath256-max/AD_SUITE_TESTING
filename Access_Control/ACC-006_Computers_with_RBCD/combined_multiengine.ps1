...
Write-Host "=== ACC-006: Computers with RBCD ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        # Execute query
        $adObjects = Get-ADObject -LDAPFilter '(&(objectCategory=computer)(msDS-AllowedToActOnBehalfOfOtherIdentity=*)(!(userAccountControl:1.2.840.113556.1.4.803:=2)))' `
                                   -Properties name,distinguishedName,objectClass,whenCreated,whenChanged,msDS-AllowedToActOnBehalfOfOtherIdentity,userAccountControl `
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'ACC-006'}}, `
            @{N='CheckName'; E={'Computers with RBCD'}}, `
...
if (-not $useADModule) {
    ...
    try {
        ...
        $searcher.Filter = '(&(objectCategory=computer)(msDS-AllowedToActOnBehalfOfOtherIdentity=*)(!(userAccountControl:1.2.840.113556.1.4.803:=2)))'
        $searcher.PageSize = 1000
        $searcher.PropertiesToLoad.Clear()
        (@('name', 'distinguishedName', 'objectClass', 'whenCreated', 'whenChanged', 'msDS-AllowedToActOnBehalfOfOtherIdentity', 'userAccountControl') | ForEach-Object { [void]$searcher.PropertiesToLoad.Add($_) }
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'ACC-006'
                adSuiteCheckName  = 'Computers_with_RBCD'
    ...
    $bhFile = Join-Path $bhDir "ACC-006_$bhTs.json"
    @{
