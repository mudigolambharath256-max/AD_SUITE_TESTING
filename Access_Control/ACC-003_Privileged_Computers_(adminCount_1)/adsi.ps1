...
Write-Host "ACC-003: found $($results.Count) objects"
...
try {
    ...
    $bhS.Filter   = '(&(objectCategory=computer)(adminCount=1))'
    $bhS.PageSize = 1000
    ...
    foreach ($r in $bhRaw) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                adSuiteCheckName  = 'Privileged Computers (adminCount=1)'
    ...
    @{ data = $bhNodes.ToArray()
       meta = @{ type = 'computers'; count = $bhNodes.Count; version = 5; methods = 0 }
        ...
        Out-File -FilePath (Join-Path $bhDir "ACC-003_$bhTs.json") -Encoding UTF8 -Force

# Fix R10: No silent catch — warn on failure
} catch { Write-Warning "ACC-003 BloodHound export error: $_" }
