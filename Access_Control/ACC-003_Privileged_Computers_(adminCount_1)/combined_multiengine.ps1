...
Write-Host "=== Privileged Computers (adminCount=1) ===" -ForegroundColor Cyan
...
try {
    ...
    Get-ADObject -LDAPFilter '(&(objectCategory=computer)(adminCount=1))' `
    ...
    Write-Warning "ACC-003 PowerShell engine: $_"   # Fix R10: no silent catch
}
...
try {
    ...
    $adsiS.Filter   = '(&(objectCategory=computer)(adminCount=1))'
    $adsiS.PageSize = 1000
    ...
    Write-Warning "ACC-003 ADSI engine: $_"          # Fix R10
}
...
try {
    $csFilter  = '(&(objectCategory=computer)(adminCount=1))'
    ...
    Write-Warning "ACC-003 C# engine: $_"             # Fix R10
}
...
Write-Host ""
Write-Host "=== ACC-003: $($uniqueResults.Count) unique findings ===" -ForegroundColor Cyan
$uniqueResults | Format-List

# Fix R14: Fixed 5-field schema enforced before CSV export
$csvPath = Join-Path $env:TEMP "ACC-003_results.csv"
$uniqueResults |
