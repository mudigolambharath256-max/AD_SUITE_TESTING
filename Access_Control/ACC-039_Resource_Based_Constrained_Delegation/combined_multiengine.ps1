...
Write-Host "=== ACC-039: Resource Based Constrained Delegation ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    $results = Get-ADObject -LDAPFilter '(msDS-AllowedToActOnBehalfOfOtherIdentity=*)' -Properties name,distinguishedName,msDS-AllowedToActOnBehalfOfOtherIdentity -SearchBase $SearchBase -SearchScope Subtree -ResultSetSize $null
} else {
...
try {
    ...
    $bhFile = Join-Path $bhDir "ACC-039_$bhTs.json"
    @{
