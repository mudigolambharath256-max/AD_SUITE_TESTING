...
(@('name', 'distinguishedName', 'whenCreated', 'whenChanged', 'msDS-AllowedToActOnBehalfOfOtherIdentity', 'objectSid', 'samAccountName') | ForEach-Object { [void]$searcher.PropertiesToLoad.Add($_)
}
...
Write-Host "Found $($results.Count) objects for check: Resource Based Constrained Delegation" -ForegroundColor Cyan
...
try {
    ...
    if ($results -and $results.Count -gt 0) {
        ...
        $bhFile = Join-Path $bhDir "ACC-039_nodes.json"
