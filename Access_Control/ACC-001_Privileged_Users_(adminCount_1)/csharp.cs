...
class Program
{
    ...
    static void Main()
    {
        string filter = @"(&(objectClass=user)(adminCount=1)(!(userAccountControl:1.2.840.113556.1.4.803:=2)))";
        ...
        try
        {
            using (DirectoryEntry rootEntry = new DirectoryEntry("LDAP://RootDSE"))
            {
                ...
                using (DirectorySearcher searcher = new DirectorySearcher(searchEntry))
                {
                    ...
                    {
                        Console.WriteLine("ACC-001: found " + results.Count + " objects");
        ...
        {
            Console.Error.WriteLine("ACC-001 error: " + ex.Message);
            Environment.Exit(1);
        }
    }
}
