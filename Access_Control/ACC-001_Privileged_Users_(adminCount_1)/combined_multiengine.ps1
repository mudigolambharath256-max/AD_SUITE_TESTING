...
Write-Host "=== Privileged Users (adminCount=1) ===" -ForegroundColor Cyan
...
try {
    ...
    Get-ADObject -LDAPFilter '(&(objectClass=user)(adminCount=1)(!(userAccountControl:1.2.840.113556.1.4.803:=2)))' `
                 -Properties @('name','distinguishedName','samAccountName','adminCount','memberOf', 'objectSid') `
                 -SearchBase $targetNC `
                 -ErrorAction Stop |
    ForEach-Object {
    ...
    Write-Warning "ACC-001 PowerShell engine: $_"   # Fix R10: no silent catch
}
...
try {
    ...
    $adsiS.Filter   = '(&(objectClass=user)(adminCount=1)(!(userAccountControl:1.2.840.113556.1.4.803:=2)))'
    $adsiS.PageSize = 1000
    $adsiS.PropertiesToLoad.Clear()
    @('name','distinguishedName','samAccountName','adminCount','memberOf', 'objectSid') | ForEach-Object { [void]$adsiS.PropertiesToLoad.Add($_) }
    ...
    Write-Warning "ACC-001 ADSI engine: $_"          # Fix R10
}
...
try {
    $csFilter  = '(&(objectClass=user)(adminCount=1)(!(userAccountControl:1.2.840.113556.1.4.803:=2)))'
    $csProps   = @("name","distinguishedName","samAccountName","adminCount","memberOf")   # PS array of double-quoted strings
    $csCheckId = 'ACC-001'
    ...
    Write-Warning "ACC-001 C# engine: $_"             # Fix R10
}
...
Write-Host ""
Write-Host "=== ACC-001: $($uniqueResults.Count) unique findings ===" -ForegroundColor Cyan
$uniqueResults | Format-List

# Fix R14: Fixed 5-field schema enforced before CSV export
$csvPath = Join-Path $env:TEMP "ACC-001_results.csv"
$uniqueResults |
