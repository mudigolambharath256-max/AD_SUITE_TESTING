...
$searcher.Filter   = '(&(objectClass=user)(adminCount=1)(!(userAccountControl:1.2.840.113556.1.4.803:=2)))'
$searcher.PageSize = 1000
$searcher.PropertiesToLoad.Clear()
# Fix R12: objectSid only for identity objects
@('name','distinguishedName','samAccountName','adminCount','memberOf', 'objectSid') | ForEach-Object { [void]$searcher.PropertiesToLoad.Add($_) }

$results = $searcher.FindAll()
Write-Host "ACC-001: found $($results.Count) objects"
...
$results | ForEach-Object {
    ...
    # Fix R13: Add relevant detection data to output
        ...
        memberOf = ($p['memberof'] | Select-Object -First 1)
...
try {
    ...
    $bhS.Filter   = '(&(objectClass=user)(adminCount=1)(!(userAccountControl:1.2.840.113556.1.4.803:=2)))'
    $bhS.PageSize = 1000
    ...
    @{ data = $bhNodes.ToArray()
       meta = @{ type = 'users'; count = $bhNodes.Count; version = 5; methods = 0 }
        ...
        Out-File -FilePath (Join-Path $bhDir "ACC-001_$bhTs.json") -Encoding UTF8 -Force

# Fix R10: No silent catch — warn on failure
} catch { Write-Warning "ACC-001 BloodHound export error: $_" }
