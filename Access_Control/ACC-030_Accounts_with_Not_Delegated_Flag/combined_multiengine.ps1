...
Write-Host "=== ACC-030: Accounts with Not Delegated Flag ===" -ForegroundColor Cyan
...
if (-not $useADModule) {
    ...
    try {
        ...
        $results = $adsiResults | ForEach-Object {
            ...
            [PSCustomObject]@{
                CheckID           = 'ACC-030'
                CheckName         = 'Accounts with Not Delegated Flag'
                Method            = 'ADSI'
                Domain            = $domainNC
...
try {
    ...
    $bhFile = Join-Path $bhDir "ACC-030_$bhTs.json"
    @{
