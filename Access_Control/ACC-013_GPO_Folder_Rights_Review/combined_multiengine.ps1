...
Write-Host "=== ACC-013: GPO Folder Rights Review ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        # Execute query
        $adObjects = Get-ADObject -LDAPFilter '(&(objectClass=groupPolicyContainer)(!(flags:1.2.840.113556.1.4.803:=1)))' `
...
if (-not $useADModule) {
    ...
    try {
        ...
        $searcher.Filter = '(&(objectClass=groupPolicyContainer)(!(flags:1.2.840.113556.1.4.803:=1)))'
        ...
        $results = $adsiResults | ForEach-Object {
            ...
            [PSCustomObject]@{
                CheckID           = 'ACC-013'
                CheckName         = 'GPO Folder Rights Review'
                Method            = 'ADSI'
                Domain            = $domainNC
...
try {
    ...
    $bhFile = Join-Path $bhDir "ACC-013_$bhTs.json"
    @{
