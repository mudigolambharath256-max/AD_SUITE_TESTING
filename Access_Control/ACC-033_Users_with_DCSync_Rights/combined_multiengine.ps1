...
Write-Host "=== ACC-033: Users with DCSync Rights ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    $results = Get-ADObject -LDAPFilter '(&(objectCategory=person)(objectClass=user)(!(userAccountControl:1.2.840.113556.1.4.803:=2)))' -Properties name,distinguishedName -SearchBase $SearchBase -SearchScope Subtree -ResultSetSize $null
} else {
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'ACC-033'
                adSuiteCheckName  = 'Users_with_DCSync_Rights'
    ...
    $bhFile = Join-Path $bhDir "ACC-033_$bhTs.json"
    @{
