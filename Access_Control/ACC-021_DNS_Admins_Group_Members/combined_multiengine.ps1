# ============================================================================
# ACC-021: DNS Admins Group Members
# ============================================================================
...
Write-Host "=== ACC-021: DNS Admins Group Members ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        # Execute query
        $adObjects = Get-ADObject -LDAPFilter '(&(objectClass=group)(samAccountName=DnsAdmins))' `
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'ACC-021'}}, `
            @{N='CheckName'; E={'DNS Admins Group Members'}}, `
...
if (-not $useADModule) {
    ...
    try {
        ...
        $searcher.Filter = '(&(objectClass=group)(samAccountName=DnsAdmins))'
        ...
        $results = $adsiResults | ForEach-Object {
            ...
            [PSCustomObject]@{
                CheckID           = 'ACC-021'
                CheckName         = 'DNS Admins Group Members'
                Method            = 'ADSI'
                Domain            = $domainNC
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'ACC-021'
                adSuiteCheckName  = 'DNS_Admins_Group_Members'
    ...
    $bhFile = Join-Path $bhDir "ACC-021_$bhTs.json"
    @{
