...
$searcher.Filter   = '(&(objectClass=user)(msDS-AllowedToActOnBehalfOfOtherIdentity=*))'
$searcher.PageSize = 1000
$searcher.PropertiesToLoad.Clear()
# Fix R12: objectSid only for identity objects
@('name','distinguishedName','samAccountName','msDS-AllowedToActOnBehalfOfOtherIdentity', 'objectSid') | ForEach-Object { [void]$searcher.PropertiesToLoad.Add($_) }

$results = $searcher.FindAll()
Write-Host "ACC-006: found $($results.Count) objects"
...
try {
    ...
    $bhS.Filter   = '(&(objectClass=user)(msDS-AllowedToActOnBehalfOfOtherIdentity=*))'
    $bhS.PageSize = 1000
    ...
    foreach ($r in $bhRaw) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                adSuiteCheckName  = 'Users with AllowedToActOnBehalfOfOtherIdentity'
    ...
    @{ data = $bhNodes.ToArray()
       meta = @{ type = 'users'; count = $bhNodes.Count; version = 5; methods = 0 }
        ...
        Out-File -FilePath (Join-Path $bhDir "ACC-006_$bhTs.json") -Encoding UTF8 -Force

# Fix R10: No silent catch — warn on failure
} catch { Write-Warning "ACC-006 BloodHound export error: $_" }
