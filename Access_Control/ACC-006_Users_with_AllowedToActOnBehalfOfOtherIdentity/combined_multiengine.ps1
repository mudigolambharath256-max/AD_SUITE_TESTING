...
# Check: Users with AllowedToActOnBehalfOfOtherIdentity
...
Write-Host "=== Users with AllowedToActOnBehalfOfOtherIdentity ===" -ForegroundColor Cyan
...
try {
    ...
    Get-ADObject -LDAPFilter '(&(objectClass=user)(msDS-AllowedToActOnBehalfOfOtherIdentity=*))' `
                 -Properties @('name','distinguishedName','samAccountName','msDS-AllowedToActOnBehalfOfOtherIdentity', 'objectSid') `
                 -SearchBase $targetNC `
                 -ErrorAction Stop |
    ForEach-Object {
    ...
    Write-Warning "ACC-006 PowerShell engine: $_"   # Fix R10: no silent catch
}
...
try {
    ...
    $adsiS.Filter   = '(&(objectClass=user)(msDS-AllowedToActOnBehalfOfOtherIdentity=*))'
    $adsiS.PageSize = 1000
    $adsiS.PropertiesToLoad.Clear()
    @('name','distinguishedName','samAccountName','msDS-AllowedToActOnBehalfOfOtherIdentity', 'objectSid') | ForEach-Object { [void]$adsiS.PropertiesToLoad.Add($_) }
    ...
    Write-Warning "ACC-006 ADSI engine: $_"          # Fix R10
}
...
try {
    $csFilter  = '(&(objectClass=user)(msDS-AllowedToActOnBehalfOfOtherIdentity=*))'
    $csProps   = @("name","distinguishedName","samAccountName","msDS-AllowedToActOnBehalfOfOtherIdentity")   # PS array of double-quoted strings
    $csCheckId = 'ACC-006'
    ...
    Write-Warning "ACC-006 C# engine: $_"             # Fix R10
}
...
Write-Host ""
Write-Host "=== ACC-006: $($uniqueResults.Count) unique findings ===" -ForegroundColor Cyan
$uniqueResults | Format-List

# Fix R14: Fixed 5-field schema enforced before CSV export
$csvPath = Join-Path $env:TEMP "ACC-006_results.csv"
$uniqueResults |
