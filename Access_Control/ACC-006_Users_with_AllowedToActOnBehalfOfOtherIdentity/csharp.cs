...
class Program
{
    ...
    static void Main()
    {
        string filter = @"(&(objectClass=user)(msDS-AllowedToActOnBehalfOfOtherIdentity=*))";
        ...
        try
        {
            using (DirectoryEntry rootEntry = new DirectoryEntry("LDAP://RootDSE"))
            {
                ...
                using (DirectorySearcher searcher = new DirectorySearcher(searchEntry))
                {
                    ...
                    {
                        Console.WriteLine("ACC-006: found " + results.Count + " objects");
        ...
        {
            Console.Error.WriteLine("ACC-006 error: " + ex.Message);
            Environment.Exit(1);
        }
    }
}
