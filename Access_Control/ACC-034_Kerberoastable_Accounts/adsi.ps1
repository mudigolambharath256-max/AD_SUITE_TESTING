...
(@('name', 'distinguishedName', 'whenCreated', 'whenChanged', 'servicePrincipalName', 'samAccountName', 'userAccountControl', 'objectSid') | ForEach-Object { [void]$searcher.PropertiesToLoad.Add($_)
}
...
Write-Host "Found $($results.Count) objects for check: Kerberoastable Accounts" -ForegroundColor Cyan

$results | ForEach-Object {
    $props = $_.Properties
    [PSCustomObject]@{
        CheckID = 'ACC-034'
        CheckName = 'Kerberoastable Accounts'
...
try {
    ...
    if ($results -and $results.Count -gt 0) {
        ...
        $bhFile = Join-Path $bhDir "ACC-034_nodes.json"
