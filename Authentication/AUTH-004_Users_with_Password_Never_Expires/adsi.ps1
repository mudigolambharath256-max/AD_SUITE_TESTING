...
$searcher.Filter   = '(&(objectClass=user)(!(userAccountControl:1.2.840.113556.1.4.803:=2))(userAccountControl:1.2.840.113556.1.4.803:=65536))'
...
Write-Host "AUTH-004: found $($results.Count) objects"
...
$results | ForEach-Object {
    ...
    # Fix R13: Add relevant detection data to output
        UACFlags = [int]($p['useraccountcontrol'] | Select-Object -First 1)
...
try {
    ...
    $bhS.Filter   = '(&(objectClass=user)(!(userAccountControl:1.2.840.113556.1.4.803:=2))(userAccountControl:1.2.840.113556.1.4.803:=65536))'
    $bhS.PageSize = 1000
    ...
    foreach ($r in $bhRaw) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                adSuiteCheckName  = 'Users with Password Never Expires'
    ...
    @{ data = $bhNodes.ToArray()
       meta = @{ type = 'users'; count = $bhNodes.Count; version = 5; methods = 0 }
        ...
        Out-File -FilePath (Join-Path $bhDir "AUTH-004_$bhTs.json") -Encoding UTF8 -Force

# Fix R10: No silent catch — warn on failure
} catch { Write-Warning "AUTH-004 BloodHound export error: $_" }
