...
(@('name', 'distinguishedName', 'whenCreated', 'whenChanged', 'userAccountControl', 'samAccountName', 'objectSid') | ForEach-Object { [void]$searcher.PropertiesToLoad.Add($_)
}
...
Write-Host "Found $($results.Count) objects for check: Accounts with Never Expiring Passwords" -ForegroundColor Cyan

$results | ForEach-Object {
    $props = $_.Properties
    [PSCustomObject]@{
        CheckID = 'AUTH-031'
        CheckName = 'Accounts with Never Expiring Passwords'
...
try {
    ...
    if ($results -and $results.Count -gt 0) {
        ...
        $bhFile = Join-Path $bhDir "AUTH-031_nodes.json"
