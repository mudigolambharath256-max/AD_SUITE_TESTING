...
try {
    $adsiResults = @(
        ...
        (@('name', 'distinguishedName', 'samAccountName', 'userAccountControl', 'adminCount') | ForEach-Object { [void]$searcher.PropertiesToLoad.Add($_) }
...
try {
    ...
    $bhFile = Join-Path $bhDir "AUTH-018_$bhTs.json"
    @{
