...
# Check: Accounts with Never Expiring Password
...
Write-Host "=== Multi-Engine Execution: Accounts with Never Expiring Password ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'AUTH-004'
                adSuiteCheckName  = 'Accounts_with_Never_Expiring_Password'
    ...
    $bhFile = Join-Path $bhDir "AUTH-004_$bhTs.json"
    @{
