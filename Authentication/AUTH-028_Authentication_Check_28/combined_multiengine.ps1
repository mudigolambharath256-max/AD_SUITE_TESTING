...
Write-Host "=== Authentication Check 28 ===" -ForegroundColor Cyan
...
try {
    ...
    Write-Warning "AUTH-028 PowerShell engine: $_"   # Fix R10: no silent catch
}
...
try {
    ...
    Write-Warning "AUTH-028 ADSI engine: $_"          # Fix R10
}
...
try {
    ...
    Write-Warning "AUTH-028 C# engine: $_"             # Fix R10
}
...
Write-Host ""
Write-Host "=== AUTH-028: $($uniqueResults.Count) unique findings ===" -ForegroundColor Cyan
$uniqueResults | Format-List

# Fix R14: Fixed 5-field schema enforced before CSV export
$csvPath = Join-Path $env:TEMP "AUTH-028_results.csv"
$uniqueResults |
