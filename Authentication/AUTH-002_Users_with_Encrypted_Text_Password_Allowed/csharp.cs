...
class Program
{
    ...
    static void Main()
    {
        string filter = @"(&(objectClass=user)(userAccountControl:1.2.840.113556.1.4.803:=128))";
        ...
        try
        {
            using (DirectoryEntry rootEntry = new DirectoryEntry("LDAP://RootDSE"))
            {
                ...
                using (DirectorySearcher searcher = new DirectorySearcher(searchEntry))
                {
                    ...
                    {
                        Console.WriteLine("AUTH-002: found " + results.Count + " objects");
        ...
        {
            Console.Error.WriteLine("AUTH-002 error: " + ex.Message);
            Environment.Exit(1);
        }
    }
}
