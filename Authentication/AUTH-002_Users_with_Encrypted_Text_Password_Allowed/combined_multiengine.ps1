...
# Check: Users with Encrypted Text Password Allowed
...
Write-Host "=== Users with Encrypted Text Password Allowed ===" -ForegroundColor Cyan
...
try {
    ...
    Get-ADObject -LDAPFilter '(&(objectClass=user)(userAccountControl:1.2.840.113556.1.4.803:=128))' `
    ...
    Write-Warning "AUTH-002 PowerShell engine: $_"   # Fix R10: no silent catch
}
...
try {
    ...
    $adsiS.Filter   = '(&(objectClass=user)(userAccountControl:1.2.840.113556.1.4.803:=128))'
    $adsiS.PageSize = 1000
    ...
    Write-Warning "AUTH-002 ADSI engine: $_"          # Fix R10
}
...
try {
    $csFilter  = '(&(objectClass=user)(userAccountControl:1.2.840.113556.1.4.803:=128))'
    ...
    Write-Warning "AUTH-002 C# engine: $_"             # Fix R10
}
...
Write-Host ""
Write-Host "=== AUTH-002: $($uniqueResults.Count) unique findings ===" -ForegroundColor Cyan
$uniqueResults | Format-List

# Fix R14: Fixed 5-field schema enforced before CSV export
$csvPath = Join-Path $env:TEMP "AUTH-002_results.csv"
$uniqueResults |
