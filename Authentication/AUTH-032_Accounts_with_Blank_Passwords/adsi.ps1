...
Write-Host "Found $($results.Count) objects for check: Accounts with Blank Passwords" -ForegroundColor Cyan

$results | ForEach-Object {
    $props = $_.Properties
    [PSCustomObject]@{
        CheckID = 'AUTH-032'
        CheckName = 'Accounts with Blank Passwords'
...
try {
    ...
    if ($results -and $results.Count -gt 0) {
        ...
        $bhFile = Join-Path $bhDir "AUTH-032_nodes.json"
