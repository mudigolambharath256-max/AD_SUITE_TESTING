...
Write-Host "=== Authentication Check 16 ===" -ForegroundColor Cyan
...
try {
    ...
    Write-Warning "AUTH-016 PowerShell engine: $_"   # Fix R10: no silent catch
}
...
try {
    ...
    $adsiS.Filter   = '(&(objectClass=user)(!(userAccountControl:1.2.840.113556.1.4.803:=2)))'
    $adsiS.PageSize = 1000
    ...
    Write-Warning "AUTH-016 ADSI engine: $_"          # Fix R10
}
...
try {
    $csFilter  = '(&(objectClass=user)(!(userAccountControl:1.2.840.113556.1.4.803:=2)))'
    ...
    Write-Warning "AUTH-016 C# engine: $_"             # Fix R10
}
...
Write-Host ""
Write-Host "=== AUTH-016: $($uniqueResults.Count) unique findings ===" -ForegroundColor Cyan
$uniqueResults | Format-List

# Fix R14: Fixed 5-field schema enforced before CSV export
$csvPath = Join-Path $env:TEMP "AUTH-016_results.csv"
$uniqueResults |
