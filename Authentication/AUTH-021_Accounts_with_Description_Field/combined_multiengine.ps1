...
# Check: Accounts with Description Field
...
Write-Host "=== Multi-Engine Execution: Accounts with Description Field ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    $adsiResults = @(
        ...
        $searcher.FindAll() | ForEach-Object {
          $p = $_.Properties
          [PSCustomObject]@{
            Label = 'Accounts with Description Field'
            Name = $p['name'][0]
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'AUTH-021'
                adSuiteCheckName  = 'Accounts_with_Description_Field'
    ...
    $bhFile = Join-Path $bhDir "AUTH-021_$bhTs.json"
    @{
