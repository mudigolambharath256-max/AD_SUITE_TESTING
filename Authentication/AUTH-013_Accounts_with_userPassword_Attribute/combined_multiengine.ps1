...
Write-Host "=== Multi-Engine Execution: Accounts with userPassword Attribute ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        $ldapFilter = '(&(objectCategory=person)(objectClass=user)(userPassword=*))'
...
public class Program
{
  ...
  {
    string filter = @"(&(objectCategory=person)(objectClass=user)(userPassword=*))";
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'AUTH-013'
                adSuiteCheckName  = 'Accounts_with_userPassword_Attribute'
    ...
    $bhFile = Join-Path $bhDir "AUTH-013_$bhTs.json"
    @{
