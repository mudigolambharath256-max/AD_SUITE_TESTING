...
Write-Host "=== Authentication Check 22 ===" -ForegroundColor Cyan
...
try {
    ...
    Write-Warning "AUTH-022 PowerShell engine: $_"   # Fix R10: no silent catch
}
...
try {
    ...
    Write-Warning "AUTH-022 ADSI engine: $_"          # Fix R10
}
...
try {
    ...
    Write-Warning "AUTH-022 C# engine: $_"             # Fix R10
}
...
Write-Host ""
Write-Host "=== AUTH-022: $($uniqueResults.Count) unique findings ===" -ForegroundColor Cyan
$uniqueResults | Format-List

# Fix R14: Fixed 5-field schema enforced before CSV export
$csvPath = Join-Path $env:TEMP "AUTH-022_results.csv"
$uniqueResults |
