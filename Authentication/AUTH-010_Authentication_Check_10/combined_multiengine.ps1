...
Write-Host "=== Authentication Check 10 ===" -ForegroundColor Cyan
...
try {
    ...
    Write-Warning "AUTH-010 PowerShell engine: $_"   # Fix R10: no silent catch
}
...
try {
    ...
    Write-Warning "AUTH-010 ADSI engine: $_"          # Fix R10
}
...
try {
    ...
    Write-Warning "AUTH-010 C# engine: $_"             # Fix R10
}
...
Write-Host ""
Write-Host "=== AUTH-010: $($uniqueResults.Count) unique findings ===" -ForegroundColor Cyan
$uniqueResults | Format-List

# Fix R14: Fixed 5-field schema enforced before CSV export
$csvPath = Join-Path $env:TEMP "AUTH-010_results.csv"
$uniqueResults |
