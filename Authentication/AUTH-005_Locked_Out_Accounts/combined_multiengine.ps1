...
Write-Host "=== Multi-Engine Execution: Locked Out Accounts ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    $adsiResults = @(
        ...
        (@('name', 'distinguishedName', 'samAccountName', 'lockoutTime', 'badPwdCount', 'userAccountControl') | ForEach-Object { [void]$searcher.PropertiesToLoad.Add($_) }
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'AUTH-005'
                adSuiteCheckName  = 'Locked_Out_Accounts'
    ...
    $bhFile = Join-Path $bhDir "AUTH-005_$bhTs.json"
    @{
