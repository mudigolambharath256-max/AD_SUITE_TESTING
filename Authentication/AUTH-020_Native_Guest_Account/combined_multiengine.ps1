...
Write-Host "=== Multi-Engine Execution: Native Guest Account ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        $ldapFilter = '(&(objectCategory=person)(objectClass=user)(objectSid=*-501))'
...
public class Program
{
  ...
  {
    string filter = @"(&(objectCategory=person)(objectClass=user)(objectSid=*-501))";
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'AUTH-020'
                adSuiteCheckName  = 'Native_Guest_Account'
    ...
    $bhFile = Join-Path $bhDir "AUTH-020_$bhTs.json"
    @{
