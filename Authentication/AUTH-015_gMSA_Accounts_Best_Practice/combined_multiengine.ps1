...
Write-Host "=== Multi-Engine Execution: gMSA Accounts (Best Practice) ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        $props = @('name', 'distinguishedName', 'samAccountName', 'msDS-ManagedPasswordInterval', 'msDS-HostServiceAccount')

        Get-ADObject -LDAPFilter $ldapFilter -Properties $props -ErrorAction Stop |
          Select-Object name, distinguishedName, samAccountName, msDS-ManagedPasswordInterval, msDS-HostServiceAccount |
          Sort-Object name |
          ForEach-Object { $_ }
...
public class Program
{
  public static void Run()
  {
    ...
    string[] props = new string[] { "name", "distinguishedName", "samAccountName", "msDS-ManagedPasswordInterval", "msDS-HostServiceAccount" };
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'AUTH-015'
                adSuiteCheckName  = 'gMSA_Accounts_Best_Practice'
    ...
    $bhFile = Join-Path $bhDir "AUTH-015_$bhTs.json"
    @{
