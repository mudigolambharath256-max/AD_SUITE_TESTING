...
# Check: Accounts with Reversible Encryption
...
Write-Host "=== Multi-Engine Execution: Accounts with Reversible Encryption ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    $adsiResults = @(
        ...
        $searcher.FindAll() | ForEach-Object {
          $p = $_.Properties
          [PSCustomObject]@{
            Label = 'Accounts with Reversible Encryption'
            Name = $p['name'][0]
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'AUTH-002'
                adSuiteCheckName  = 'Accounts_with_Reversible_Encryption'
    ...
    $bhFile = Join-Path $bhDir "AUTH-002_$bhTs.json"
    @{
