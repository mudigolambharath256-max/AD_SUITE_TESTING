...
(@('name', 'distinguishedName', 'whenCreated', 'whenChanged', 'adminCount', 'userAccountControl', 'samAccountName', 'objectSid') | ForEach-Object { [void]$searcher.PropertiesToLoad.Add($_)
}
...
Write-Host "Found $($results.Count) objects for check: Privileged Accounts with Stale Passwords" -ForegroundColor Cyan

$results | ForEach-Object {
    $props = $_.Properties
    [PSCustomObject]@{
        CheckID = 'AUTH-033'
        CheckName = 'Privileged Accounts with Stale Passwords'
...
try {
    ...
    if ($results -and $results.Count -gt 0) {
        ...
        $bhFile = Join-Path $bhDir "AUTH-033_nodes.json"
