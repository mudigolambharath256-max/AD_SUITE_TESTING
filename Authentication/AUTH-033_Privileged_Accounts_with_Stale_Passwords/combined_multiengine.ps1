...
Write-Host "=== AUTH-033: Privileged Accounts with Stale Passwords ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    $results = Get-ADObject -LDAPFilter '(&(objectCategory=person)(objectClass=user)(adminCount=1)(!(userAccountControl:1.2.840.113556.1.4.803:=2)))' -Properties name,distinguishedName,adminCount,userAccountControl,samAccountName -SearchBase $SearchBase -SearchScope Subtree -ResultSetSize $null
} else {
