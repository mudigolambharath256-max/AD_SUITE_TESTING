...
Write-Host "=== Authentication Check 33 ===" -ForegroundColor Cyan
...
try {
    ...
    Write-Warning "AUTH-033 PowerShell engine: $_"   # Fix R10: no silent catch
}
...
try {
    ...
    Write-Warning "AUTH-033 ADSI engine: $_"          # Fix R10
}
...
try {
    ...
    Write-Warning "AUTH-033 C# engine: $_"             # Fix R10
}
...
Write-Host ""
Write-Host "=== AUTH-033: $($uniqueResults.Count) unique findings ===" -ForegroundColor Cyan
$uniqueResults | Format-List

# Fix R14: Fixed 5-field schema enforced before CSV export
$csvPath = Join-Path $env:TEMP "AUTH-033_results.csv"
$uniqueResults |
