...
class Program
{
    ...
    static void Main()
    {
        string filter = @"(&(objectClass=user)(cn=krbtgt))";
        string[] props = new string[] { "name","distinguishedName","samAccountName","pwdLastSet","whenChanged" };
        ...
        try
        {
            using (DirectoryEntry rootEntry = new DirectoryEntry("LDAP://RootDSE"))
            {
                ...
                using (DirectorySearcher searcher = new DirectorySearcher(searchEntry))
                {
                    ...
                    {
                        Console.WriteLine("AUTH-005: found " + results.Count + " objects");
        ...
        {
            Console.Error.WriteLine("AUTH-005 error: " + ex.Message);
            Environment.Exit(1);
        }
    }
}
