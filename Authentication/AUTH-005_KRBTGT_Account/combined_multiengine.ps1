...
Write-Host "=== KRBTGT Account ===" -ForegroundColor Cyan
...
try {
    ...
    Get-ADObject -LDAPFilter '(&(objectClass=user)(cn=krbtgt))' `
                 -Properties @('name','distinguishedName','samAccountName','pwdLastSet','whenChanged', 'objectSid') `
                 -SearchBase $targetNC `
                 -ErrorAction Stop |
    ForEach-Object {
    ...
    Write-Warning "AUTH-005 PowerShell engine: $_"   # Fix R10: no silent catch
}
...
try {
    ...
    $adsiS.Filter   = '(&(objectClass=user)(cn=krbtgt))'
    $adsiS.PageSize = 1000
    $adsiS.PropertiesToLoad.Clear()
    @('name','distinguishedName','samAccountName','pwdLastSet','whenChanged', 'objectSid') | ForEach-Object { [void]$adsiS.PropertiesToLoad.Add($_) }
    ...
    Write-Warning "AUTH-005 ADSI engine: $_"          # Fix R10
}
...
try {
    $csFilter  = '(&(objectClass=user)(cn=krbtgt))'
    $csProps   = @("name","distinguishedName","samAccountName","pwdLastSet","whenChanged")   # PS array of double-quoted strings
    $csCheckId = 'AUTH-005'
    ...
    Write-Warning "AUTH-005 C# engine: $_"             # Fix R10
}
...
Write-Host ""
Write-Host "=== AUTH-005: $($uniqueResults.Count) unique findings ===" -ForegroundColor Cyan
$uniqueResults | Format-List

# Fix R14: Fixed 5-field schema enforced before CSV export
$csvPath = Join-Path $env:TEMP "AUTH-005_results.csv"
$uniqueResults |
