...
Write-Host "=== Authentication Check 26 ===" -ForegroundColor Cyan
...
try {
    ...
    Write-Warning "AUTH-026 PowerShell engine: $_"   # Fix R10: no silent catch
}
...
try {
    ...
    Write-Warning "AUTH-026 ADSI engine: $_"          # Fix R10
}
...
try {
    ...
    Write-Warning "AUTH-026 C# engine: $_"             # Fix R10
}
...
Write-Host ""
Write-Host "=== AUTH-026: $($uniqueResults.Count) unique findings ===" -ForegroundColor Cyan
$uniqueResults | Format-List

# Fix R14: Fixed 5-field schema enforced before CSV export
$csvPath = Join-Path $env:TEMP "AUTH-026_results.csv"
$uniqueResults |
