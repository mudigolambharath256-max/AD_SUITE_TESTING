...
# Check: Accounts with Password Not Required
...
Write-Host "=== Multi-Engine Execution: Accounts with Password Not Required ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    $adsiResults = @(
        ...
        $searcher.FindAll() | ForEach-Object {
          $p = $_.Properties
          [PSCustomObject]@{
            Label = 'Accounts with Password Not Required'
            Name = $p['name'][0]
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'AUTH-003'
                adSuiteCheckName  = 'Accounts_with_Password_Not_Required'
    ...
    $bhFile = Join-Path $bhDir "AUTH-003_$bhTs.json"
    @{
