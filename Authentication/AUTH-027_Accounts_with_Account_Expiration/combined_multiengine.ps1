...
# Check: Accounts with Account Expiration
...
Write-Host "=== Multi-Engine Execution: Accounts with Account Expiration ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        $ldapFilter = '(&(objectCategory=person)(objectClass=user)(!(userAccountControl:1.2.840.113556.1.4.803:=2))(accountExpires>=1)(!(accountExpires=9223372036854775807)))'
        $props = @('name', 'distinguishedName', 'samAccountName', 'accountExpires')

        Get-ADObject -LDAPFilter $ldapFilter -Properties $props,userAccountControl,samAccountName -ErrorAction Stop |
          Select-Object name, distinguishedName, samAccountName, accountExpires |
          Sort-Object name |
          ForEach-Object { $_ }
...
try {
    $adsiResults = @(
        ...
        $searcher = [ADSISearcher]'(&(objectCategory=person)(objectClass=user)(!(userAccountControl:1.2.840.113556.1.4.803:=2))(accountExpires>=1)(!(accountExpires=9223372036854775807)))'
        $searcher.PageSize = 1000
        $searcher.PropertiesToLoad.Clear()
        (@('name', 'distinguishedName', 'samAccountName', 'accountExpires', 'userAccountControl') | ForEach-Object { [void]$searcher.PropertiesToLoad.Add($_) }

        $searcher.FindAll() | ForEach-Object {
          $p = $_.Properties
          [PSCustomObject]@{
            Label = 'Accounts with Account Expiration'
            Name = $p['name'][0]
...
public class Program
{
  ...
  {
    string filter = @"(&(objectCategory=person)(objectClass=user)(!(userAccountControl:1.2.840.113556.1.4.803:=2))(accountExpires>=1)(!(accountExpires=9223372036854775807)))";
    string[] props = new string[] { "name", "distinguishedName", "samAccountName", "accountExpires" };
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'AUTH-027'
                adSuiteCheckName  = 'Accounts_with_Account_Expiration'
    ...
    $bhFile = Join-Path $bhDir "AUTH-027_$bhTs.json"
    @{
