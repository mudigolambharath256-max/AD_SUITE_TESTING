...
(@('name', 'distinguishedName', 'samAccountName', 'accountExpires', 'userAccountControl', 'objectSid') | ForEach-Object { [void]$searcher.PropertiesToLoad.Add($_) }
...
$output = $results | ForEach-Object {
  ...
  [PSCustomObject]@{
    ...
    AccountExpires = Convert-FileTime ($p['accountexpires'] | Select-Object -First 1)
  }
}
...
try {
    ...
    if ($results -and $results.Count -gt 0) {
        ...
        $bhFile = Join-Path $bhDir "AUTH-027_nodes.json"
    ...
    Write-Warning "[BloodHound] Export failed: # Check: Accounts with Account Expiration
# Category: Authentication
# Severity: info
# ID: AUTH-027
# Requirements: None
