...
Write-Host "AUTH-003: found $($results.Count) objects"
...
try {
    ...
    $bhS.Filter   = '(&(objectClass=user)(userAccountControl:1.2.840.113556.1.4.803:=32))'
    $bhS.PageSize = 1000
    ...
    foreach ($r in $bhRaw) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                adSuiteCheckName  = 'Users with Password Not Required'
    ...
    @{ data = $bhNodes.ToArray()
       meta = @{ type = 'users'; count = $bhNodes.Count; version = 5; methods = 0 }
        ...
        Out-File -FilePath (Join-Path $bhDir "AUTH-003_$bhTs.json") -Encoding UTF8 -Force

# Fix R10: No silent catch — warn on failure
} catch { Write-Warning "AUTH-003 BloodHound export error: $_" }
