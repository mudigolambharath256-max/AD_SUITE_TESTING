...
Write-Host "=== Multi-Engine Execution: Anonymous LDAP Access Check ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        $ldapFilter = '(&(objectClass=domainDNS)(dSHeuristics=0000002*))'
        $props = @('name', 'distinguishedName', 'dSHeuristics')

        Get-ADObject -LDAPFilter $ldapFilter -Properties $props -ErrorAction Stop |
          Select-Object name, distinguishedName, dSHeuristics |
          Sort-Object name |
          ForEach-Object { $_ }
...
public class Program
{
  ...
  {
    string filter = @"(&(objectClass=domainDNS)(dSHeuristics=0000002*))";
    string[] props = new string[] { "name", "distinguishedName", "dSHeuristics" };
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'AUTH-029'
                adSuiteCheckName  = 'Anonymous_LDAP_Access_Check'
    ...
    $bhFile = Join-Path $bhDir "AUTH-029_$bhTs.json"
    @{
