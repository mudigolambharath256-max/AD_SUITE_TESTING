...
Write-Host "=== Multi-Engine Execution: Protected Users Group Members ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        $ldapFilter = '(&(objectCategory=person)(objectClass=user)(memberOf:1.2.840.113556.1.4.1941:=CN=Protected Users,CN=Users,*))'
...
public class Program
{
  ...
  {
    string filter = @"(&(objectCategory=person)(objectClass=user)(memberOf:1.2.840.113556.1.4.1941:=CN=Protected Users,CN=Users,*))";
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'AUTH-009'
                adSuiteCheckName  = 'Protected_Users_Group_Members'
    ...
    $bhFile = Join-Path $bhDir "AUTH-009_$bhTs.json"
    @{
