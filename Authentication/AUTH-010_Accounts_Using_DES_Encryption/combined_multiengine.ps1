...
# Check: Accounts Using DES Encryption
...
Write-Host "=== Multi-Engine Execution: Accounts Using DES Encryption ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    $adsiResults = @(
        ...
        $searcher.FindAll() | ForEach-Object {
          $p = $_.Properties
          [PSCustomObject]@{
            Label = 'Accounts Using DES Encryption'
            Name = $p['name'][0]
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'AUTH-010'
                adSuiteCheckName  = 'Accounts_Using_DES_Encryption'
    ...
    $bhFile = Join-Path $bhDir "AUTH-010_$bhTs.json"
    @{
