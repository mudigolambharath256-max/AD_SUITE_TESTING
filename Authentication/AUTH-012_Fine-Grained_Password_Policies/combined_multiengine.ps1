...
try {
    ...
    $psResults = @(
        ...
        $props = @('name', 'distinguishedName', 'cn', 'msDS-MinimumPasswordLength', 'msDS-PasswordComplexityEnabled', 'msDS-LockoutThreshold', 'msDS-PasswordSettingsPrecedence')

        Get-ADObject -LDAPFilter $ldapFilter -Properties $props -ErrorAction Stop |
          Select-Object name, distinguishedName, cn, msDS-MinimumPasswordLength, msDS-PasswordComplexityEnabled, msDS-LockoutThreshold, msDS-PasswordSettingsPrecedence |
          Sort-Object name |
          ForEach-Object { $_ }
...
public class Program
{
  public static void Run()
  {
    ...
    string[] props = new string[] { "name", "distinguishedName", "cn", "msDS-MinimumPasswordLength", "msDS-PasswordComplexityEnabled", "msDS-LockoutThreshold", "msDS-PasswordSettingsPrecedence" };
...
try {
    ...
    $bhFile = Join-Path $bhDir "AUTH-012_$bhTs.json"
    @{
