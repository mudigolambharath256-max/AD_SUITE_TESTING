...
Write-Host "=== Multi-Engine Execution: Authentication Policy Silos ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        $ldapFilter = '(objectClass=msDS-AuthNPolicySilo)'
        $props = @('name', 'distinguishedName', 'cn', 'msDS-AuthNPolicySiloEnforced')

        Get-ADObject -LDAPFilter $ldapFilter -Properties $props -ErrorAction Stop |
          Select-Object name, distinguishedName, cn, msDS-AuthNPolicySiloEnforced |
          Sort-Object name |
          ForEach-Object { $_ }
...
public class Program
{
  ...
  {
    string filter = @"(objectClass=msDS-AuthNPolicySilo)";
    string[] props = new string[] { "name", "distinguishedName", "cn", "msDS-AuthNPolicySiloEnforced" };
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'AUTH-028'
                adSuiteCheckName  = 'Authentication_Policy_Silos'
    ...
    $bhFile = Join-Path $bhDir "AUTH-028_$bhTs.json"
    @{
