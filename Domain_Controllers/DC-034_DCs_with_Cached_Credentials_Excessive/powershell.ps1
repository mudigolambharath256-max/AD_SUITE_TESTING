...
try {
    ...
    foreach ($dc in $dcs) {
        ...
        try {
            # TODO: Implement specific check logic for DC-058
...
try {
    ...
    foreach ($r in $output) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'DC-034'
                adSuiteCheckName  = 'DCs_with_Cached_Credentials_Excessive'
    ...
    $bhFile = Join-Path $bhDir "DC-034_$bhTs.json"
    @{
