...
try {
    ...
    @('name', 'distinguishedName', 'fSMORoleOwner', 'objectSid', 'samAccountName') | ForEach-Object { [void]$searcher.PropertiesToLoad.Add($_) }
    ...
    $output = $results | ForEach-Object {
      ...
      [PSCustomObject]@{
        Label         = 'Schema Master'
        name          = if ($p['name'] -and $p['name'].Count -gt 0) { if ($p['name'] -and $p['name'].Count -gt 0) { $p['name'][0] } else { 'N/A' } } else { $null }
        distinguishedName = if ($p['distinguishedname'] -and $p['distinguishedname'].Count -gt 0) { if ($p['distinguishedname'] -and $p['distinguishedname'].Count -gt 0) { $p['distinguishedname'][0] } else { 'N/A' } } else { $null }
        fSMORoleOwner = if ($p['fsmoroleowner'] -and $p['fsmoroleowner'].Count -gt 0) { if ($p['fsmoroleowner'] -and $p['fsmoroleowner'].Count -gt 0) { $p['fsmoroleowner'][0] } else { 'N/A' } } else { $null }
      }
    }
...
try {
    ...
    if ($results -and $results.Count -gt 0) {
        ...
        $bhFile = Join-Path $bhDir "DC-008_nodes.json"
    ...
    Write-Warning "[BloodHound] Export failed: # Check: Schema Master
# Category: Domain Controllers
# Severity: info
# ID: DC-008
# Requirements: None
