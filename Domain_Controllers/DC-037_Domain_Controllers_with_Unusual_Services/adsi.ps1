...
Write-Host "Found $($results.Count) objects for check: Domain Controllers with Unusual Services" -ForegroundColor Cyan

$results | ForEach-Object {
    $props = $_.Properties
    [PSCustomObject]@{
        CheckID = 'DC-037'
        CheckName = 'Domain Controllers with Unusual Services'
...
try {
    ...
    if ($results -and $results.Count -gt 0) {
        ...
        $bhFile = Join-Path $bhDir "DC-037_nodes.json"
