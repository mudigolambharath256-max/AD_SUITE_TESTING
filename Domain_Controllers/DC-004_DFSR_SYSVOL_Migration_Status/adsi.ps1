...
try {
    ...
    @('name', 'distinguishedName', 'cn', 'msDFSR-ReplicationGroupType', 'objectSid', 'samAccountName') | ForEach-Object { [void]$searcher.PropertiesToLoad.Add($_) }
    ...
    $output = $results | ForEach-Object {
      $p = $_.Properties
        ...
        ReplicationGroupType      = if ($p['msdfsr-replicationgrouptype'] -and $p['msdfsr-replicationgrouptype'].Count -gt 0) { $p['msdfsr-replicationgrouptype'][0] } else { 'N/A' }
        }
    }
...
try {
    ...
    if ($results -and $results.Count -gt 0) {
        ...
        $bhFile = Join-Path $bhDir "DC-004_nodes.json"
    ...
    Write-Warning "[BloodHound] Export failed: # Check: DFSR SYSVOL Migration Status
# Category: Domain Controllers
# Severity: info
# ID: DC-004
# Requirements: None
