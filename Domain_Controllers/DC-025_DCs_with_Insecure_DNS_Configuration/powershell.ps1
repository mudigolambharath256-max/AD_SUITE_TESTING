...
try {
    ...
    foreach ($dc in $dcs) {
        ...
        try {
            # TODO: Implement specific check logic for DC-049
...
try {
    ...
    foreach ($r in $output) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'DC-025'
                adSuiteCheckName  = 'DCs_with_Insecure_DNS_Configuration'
    ...
    $bhFile = Join-Path $bhDir "DC-025_$bhTs.json"
    @{
