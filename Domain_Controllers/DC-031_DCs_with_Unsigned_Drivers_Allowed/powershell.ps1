...
try {
    ...
    foreach ($dc in $dcs) {
        ...
        try {
            # TODO: Implement specific check logic for DC-055
...
try {
    ...
    foreach ($r in $output) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'DC-031'
                adSuiteCheckName  = 'DCs_with_Unsigned_Drivers_Allowed'
    ...
    $bhFile = Join-Path $bhDir "DC-031_$bhTs.json"
    @{
