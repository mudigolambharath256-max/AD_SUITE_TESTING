...
Write-Host "Found $($results.Count) objects for check: Event Log Size and Retention" -ForegroundColor Cyan

$results | ForEach-Object {
    $props = $_.Properties
    [PSCustomObject]@{
        CheckID = 'DC-039'
        CheckName = 'Event Log Size and Retention'
...
try {
    ...
    if ($results -and $results.Count -gt 0) {
        ...
        $bhFile = Join-Path $bhDir "DC-039_nodes.json"
