...
Write-Host "Found $($results.Count) objects for check: Domain Controllers with Audit Policy Gaps" -ForegroundColor Cyan

$results | ForEach-Object {
    $props = $_.Properties
    [PSCustomObject]@{
        CheckID = 'DC-038'
        CheckName = 'Domain Controllers with Audit Policy Gaps'
...
try {
    ...
    if ($results -and $results.Count -gt 0) {
        ...
        $bhFile = Join-Path $bhDir "DC-038_nodes.json"
