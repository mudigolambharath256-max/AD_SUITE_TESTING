...
Write-Host "=== DC-038: Domain Controllers with Audit Policy Gaps ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    $results = Get-ADObject -LDAPFilter '(&(objectCategory=computer)(userAccountControl:1.2.840.113556.1.4.803:=8192))' -Properties name,distinguishedName,userAccountControl -SearchBase $SearchBase -SearchScope Subtree -ResultSetSize $null
} else {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                adSuiteCheckName  = 'Domain_Controllers_with_Audit_Policy_Gaps'
