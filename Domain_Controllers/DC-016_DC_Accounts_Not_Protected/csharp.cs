...
class Program
{
  static void Main()
  {
    ...
    string[] props = new string[] { "name", "distinguishedName", "dNSHostName", "adminCount" };
    ...
    using (var searcher = new DirectorySearcher(domain))
    {
      ...
      foreach (SearchResult r in searcher.FindAll())
      {
        ...
        var adminCount = r.Properties.Contains("adminCount") && r.Properties["adminCount"].Count > 0 ? r.Properties["adminCount"][0].ToString() : "0";

        Console.WriteLine($"name              : {name}");
        Console.WriteLine($"distinguishedName : {dn}");
        Console.WriteLine($"dNSHostName       : {dns}");
        Console.WriteLine($"adminCount        : {adminCount}");
