...
try {
    ...
    (@('name', 'distinguishedName', 'dNSHostName', 'adminCount', 'userAccountControl', 'objectSid', 'samAccountName') | ForEach-Object { [void]$searcher.PropertiesToLoad.Add($_) }
    ...
    $output = $results | ForEach-Object {
      $p = $_.Properties
      $dn = if ($p['distinguishedname'] -and $p['distinguishedname'].Count -gt 0) { $p['distinguishedname'][0] } else { 'N/A' }

      # Check if DC is in Domain Controllers OU
      $isInDCOU = $dn -match "OU=Domain Controllers,"

      # Check if adminCount is set
      $adminCount = if ($p['admincount'].Count -gt 0) { if ($p['admincount'] -and $p['admincount'].Count -gt 0) { $p['admincount'][0] } else { 'N/A' } } else { 0 }
      $hasAdminCount = $adminCount -eq 1

      # Flag if not properly protected
      if (-not $isInDCOU -or -not $hasAdminCount) {
        [PSCustomObject]@{
          Label = 'DC Accounts Not Protected'
...
try {
    ...
    if ($results -and $results.Count -gt 0) {
        ...
        $bhFile = Join-Path $bhDir "DC-016_nodes.json"
    ...
    Write-Warning "[BloodHound] Export failed: # Check: DC Accounts Not Protected
# Category: Domain Controllers
# Severity: high
# ID: DC-040
# Requirements: None
