...
class Program
{
  static void Main()
  {
    ...
    string[] props = new string[] { "name", "distinguishedName", "dNSHostName", "operatingSystem", "whenChanged" };
    ...
    using (var searcher = new DirectorySearcher(domain))
    {
      ...
      foreach (SearchResult r in searcher.FindAll())
      {
        ...
        var changed = r.Properties.Contains("whenChanged") ? r.Properties["whenChanged"][0].ToString() : "(none)";

        Console.WriteLine($"name              : {name}");
        Console.WriteLine($"distinguishedName : {dn}");
        Console.WriteLine($"dNSHostName       : {dns}");
        Console.WriteLine($"operatingSystem   : {os}");
        Console.WriteLine($"whenChanged       : {changed}");
