# Check: DCs Running Outdated OS (2012 or Earlier)
...
try {
    ...
    $searcher.Filter     = '(&(objectCategory=computer)(userAccountControl:1.2.840.113556.1.4.803:=8192)(|(operatingSystem=*2012*)(operatingSystem=*2008*)(operatingSystem=*2003*)(operatingSystem=*2000*)(operatingSystem=*NT*)))'
    $searcher.PageSize   = 1000
    $searcher.PropertiesToLoad.Clear()
    (@('name', 'distinguishedName', 'samAccountName', 'dNSHostName', 'operatingSystem', 'operatingSystemVersion', 'userAccountControl', 'objectSid') | ForEach-Object { [void]$searcher.PropertiesToLoad.Add($_) })

    $results = $searcher.FindAll()
    Write-Host "Found $($results.Count) Domain Controllers with outdated OS" -ForegroundColor Cyan

    $output = $results | ForEach-Object {
        $p = $_.Properties
        $os = if ($p['operatingsystem'] -and $p['operatingsystem'].Count -gt 0) { $p['operatingsystem'][0] } else { 'N/A' }

        # Determine EOL status and risk level
        $eolStatus = "Unknown"
        $riskLevel = "HIGH"

        if ($os -match "2000|NT") {
            $eolStatus = "End of Life (2000/NT)"
            $riskLevel = "CRITICAL"
        } elseif ($os -match "2003") {
            $eolStatus = "End of Life (2003)"
            $riskLevel = "CRITICAL"
        } elseif ($os -match "2008") {
            $eolStatus = "End of Life (2008)"
            $riskLevel = "CRITICAL"
        } elseif ($os -match "2012") {
            $eolStatus = "End of Life (2012)"
            $riskLevel = "CRITICAL"
        }

        [PSCustomObject]@{
            Label                   = 'DCs Running Outdated OS'
            ...
            EOLStatus               = $eolStatus
            RiskLevel               = $riskLevel
            SecurityRisk            = "No security patches available"
...
try {
    ...
    if ($results -and $results.Count -gt 0) {
        ...
        $bhFile = Join-Path $bhDir "DC-002_nodes.json"
    ...
    Write-Warning "[BloodHound] Export failed: # Check: DCs Running Outdated OS (2012 or Earlier)
# Category: Domain Controllers
# Severity: critical
# ID: DC-002
# Requirements: None
...
try {
    ...
    if ($output) {
        $output | Format-Table -AutoSize
        Write-Host "`nSummary: Found $($output.Count) Domain Controllers running end-of-life operating systems" -ForegroundColor Red
        Write-Host "These systems are at CRITICAL risk due to lack of security updates" -ForegroundColor Red
    } else {
        Write-Host 'No findings - All Domain Controllers are running supported operating systems' -ForegroundColor Green
    }
} catch {
