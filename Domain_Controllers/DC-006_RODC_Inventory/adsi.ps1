...
try {
    ...
    (@('name', 'distinguishedName', 'samAccountName', 'dNSHostName', 'operatingSystem', 'userAccountControl', 'primaryGroupID', 'objectSid') | ForEach-Object { [void]$searcher.PropertiesToLoad.Add($_) }
...
try {
    ...
    if ($results -and $results.Count -gt 0) {
        ...
        $bhFile = Join-Path $bhDir "DC-006_nodes.json"
    ...
    Write-Warning "[BloodHound] Export failed: # Check: RODC Inventory
# Category: Domain Controllers
# Severity: info
# ID: DC-006
# Requirements: None
