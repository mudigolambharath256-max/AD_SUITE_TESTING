...
class Program
{
  static void Main()
  {
    ...
    string[] props = new string[] { "name", "distinguishedName", "dNSHostName", "operatingSystem", "msDS-SupportedEncryptionTypes" };
    ...
    using (var searcher = new DirectorySearcher(domain))
    {
      ...
      {
        var name = r.Properties.Contains("name") ? r.Properties["name"][0].ToString() : "(no name)";
        var dn = r.Properties.Contains("distinguishedName") ? r.Properties["distinguishedName"][0].ToString() : "(none)";
        var dns = r.Properties.Contains("dNSHostName") ? r.Properties["dNSHostName"][0].ToString() : "(none)";
        var os = r.Properties.Contains("operatingSystem") ? r.Properties["operatingSystem"][0].ToString() : "(none)";
        
        int encTypes = r.Properties.Contains("msDS-SupportedEncryptionTypes") && r.Properties["msDS-SupportedEncryptionTypes"].Count > 0 
          ? Convert.ToInt32(r.Properties["msDS-SupportedEncryptionTypes"][0]) : 0;
        
        // Encryption type flags: DES=1+2, RC4=4, AES128=8, AES256=16
        bool hasDES = (encTypes & 3) != 0;
        bool hasRC4Only = (encTypes == 4);
        bool hasAES = (encTypes & 24) != 0;
        
        if (hasDES || (hasRC4Only && !hasAES))
        {
          Console.WriteLine($"name                      : {name}");
          Console.WriteLine($"distinguishedName         : {dn}");
          Console.WriteLine($"dNSHostName               : {dns}");
          Console.WriteLine($"operatingSystem           : {os}");
          Console.WriteLine($"supportedEncryptionTypes  : {encTypes}");
          Console.WriteLine($"hasDES                    : {hasDES}");
          Console.WriteLine($"hasRC4                    : {((encTypes & 4) != 0)}");
          Console.WriteLine($"hasAES128                 : {((encTypes & 8) != 0)}");
          Console.WriteLine($"hasAES256                 : {((encTypes & 16) != 0)}");
          Console.WriteLine($"status                    : {(hasDES ? "DES Enabled (Critical)" : hasRC4Only ? "RC4 Only (Weak)" : "Weak Encryption")}");
          Console.WriteLine(); // blank line between DCs
        }
      }   // end foreach
    }       // end using
  }         // end Main
