...
try {
    ...
    SamAccountName = if ($p['samaccountname'] -and $p['samaccountname'].Count -gt 0) { $p['samaccountname'][0] } else { 'N/A' } } else { 'N/A' } } else { 'N/A'
        ...
        AllowedToDelegateTo       = if ($p['msds-allowedtodelegateto'] -and $p['msds-allowedtodelegateto'].Count -gt 0) { $p['msds-allowedtodelegateto'][0] } else { 'N/A' }
        }
    }
...
try {
    ...
    if ($results -and $results.Count -gt 0) {
        ...
        $bhFile = Join-Path $bhDir "DC-003_nodes.json"
    ...
    Write-Warning "[BloodHound] Export failed: # Check: DCs with Constrained Delegation
# Category: Domain Controllers
# Severity: medium
# ID: DC-003
# Requirements: None
