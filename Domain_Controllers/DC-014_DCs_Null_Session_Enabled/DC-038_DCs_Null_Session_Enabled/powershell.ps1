...
foreach ($dc in $dcs) {
  try {
    # Check null session registry keys
    ...
    $restrictAnonymous = $regSubKey.GetValue("RestrictAnonymous")
    $restrictAnonymousSAM = $regSubKey.GetValue("RestrictAnonymousSAM")
    $everyoneIncludesAnonymous = $regSubKey.GetValue("EveryoneIncludesAnonymous")

    # Vulnerable if RestrictAnonymous < 2 or EveryoneIncludesAnonymous = 1
    if ($restrictAnonymous -lt 2 -or $everyoneIncludesAnonymous -eq 1) {
      [PSCustomObject]@{
        Name = $dc.name
    ...
    Write-Warning "Unable to check null session settings on $($dc.dNSHostName): $_"
  }
}
...
try {
    ...
    foreach ($r in $output) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'DC-038'
                adSuiteCheckName  = 'DCs_Null_Session_Enabled'
    ...
    $bhFile = Join-Path $bhDir "DC-038_$bhTs.json"
    @{
