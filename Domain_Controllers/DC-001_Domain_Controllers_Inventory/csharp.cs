...
class Program
{
  static void Main()
  {
    ...
    string[] props = new string[] { "name", "distinguishedName", "samAccountName", "dNSHostName", "operatingSystem", "operatingSystemVersion", "serverReferenceBL" };
      ...
      foreach (SearchResult r in searcher.FindAll())
{
    ...
    var sam         = r.Properties.Contains("samAccountName")         ? r.Properties["samAccountName"][0].ToString()         : "(none)";
    ...
    var osVer       = r.Properties.Contains("operatingSystemVersion") ? r.Properties["operatingSystemVersion"][0].ToString() : "(none)";
    var serverRef   = r.Properties.Contains("serverReferenceBL")      ? r.Properties["serverReferenceBL"][0].ToString()      : "(none)";

    Console.WriteLine($"name                   : {name}");
    Console.WriteLine($"distinguishedName       : {dn}");
    Console.WriteLine($"samAccountName          : {sam}");
    Console.WriteLine($"dNSHostName             : {dns}");
    Console.WriteLine($"operatingSystem         : {os}");
    Console.WriteLine($"operatingSystemVersion  : {osVer}");
    Console.WriteLine($"serverReferenceBL       : {serverRef}");
