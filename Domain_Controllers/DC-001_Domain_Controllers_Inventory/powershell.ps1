...
$props = @('name', 'distinguishedName', 'samAccountName', 'dNSHostName', 'operatingSystem', 'operatingSystemVersion', 'serverReferenceBL', 'objectSid')

Get-ADObject -LDAPFilter $ldapFilter -SearchBase $searchBase -Properties $props -ErrorAction Stop |
  Select-Object name, distinguishedName, samAccountName, dNSHostName, operatingSystem, operatingSystemVersion, serverReferenceBL |
  Sort-Object name |
  ForEach-Object { $_ }
