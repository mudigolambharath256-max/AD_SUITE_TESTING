...
// Severity: critical
// ID: DC-039
// Requirements: Compile with System.DirectoryServices, System.ServiceProcess
// ============================================

// LDAP search (C# DirectorySearcher) with Service Check
using System;
  ...
  static void Main()
  {
    ...
    using (var searcher = new DirectorySearcher(domain))
    {
      ...
      foreach (SearchResult r in searcher.FindAll())
      {
        ...
        try
        {
          ServiceController sc = new ServiceController("Spooler", dns);
          if (sc.Status == ServiceControllerStatus.Running)
          {
            ...
            Console.WriteLine($"spoolerStatus     : {sc.Status}");
            Console.WriteLine($"spoolerStartType  : {sc.StartType}");
            Console.WriteLine(); // blank line between DCs
          }
        }
        catch (Exception ex)
        {
          Console.WriteLine($"Warning: Unable to check Print Spooler on {dns}: {ex.Message}");
        }
      }   // end foreach
    }       // end using
  }         // end Main
