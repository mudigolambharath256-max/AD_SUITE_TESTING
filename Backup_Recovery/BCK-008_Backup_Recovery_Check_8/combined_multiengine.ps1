...
Write-Host "=== Backup Recovery Check 8 ===" -ForegroundColor Cyan
...
try {
    ...
    Write-Warning "BCK-008 PowerShell engine: $_"   # Fix R10: no silent catch
}
...
try {
    ...
    Write-Warning "BCK-008 ADSI engine: $_"          # Fix R10
}
...
try {
    ...
    Write-Warning "BCK-008 C# engine: $_"             # Fix R10
}
...
Write-Host ""
Write-Host "=== BCK-008: $($uniqueResults.Count) unique findings ===" -ForegroundColor Cyan
$uniqueResults | Format-List

# Fix R14: Fixed 5-field schema enforced before CSV export
$csvPath = Join-Path $env:TEMP "BCK-008_results.csv"
$uniqueResults |
