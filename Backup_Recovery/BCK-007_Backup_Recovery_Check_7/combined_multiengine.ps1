...
Write-Host "=== Backup Recovery Check 7 ===" -ForegroundColor Cyan
...
try {
    ...
    Write-Warning "BCK-007 PowerShell engine: $_"   # Fix R10: no silent catch
}
...
try {
    ...
    Write-Warning "BCK-007 ADSI engine: $_"          # Fix R10
}
...
try {
    ...
    Write-Warning "BCK-007 C# engine: $_"             # Fix R10
}
...
Write-Host ""
Write-Host "=== BCK-007: $($uniqueResults.Count) unique findings ===" -ForegroundColor Cyan
$uniqueResults | Format-List

# Fix R14: Fixed 5-field schema enforced before CSV export
$csvPath = Join-Path $env:TEMP "BCK-007_results.csv"
$uniqueResults |
