# ============================================================================
# BCK-004: Deleted Object Recovery Capability - Multi-Engine
# ============================================================================

[CmdletBinding()]
param([Parameter()][string]$SearchBase, [Parameter()][string]$ExportPath)

Write-Host "[BCK-004] Deleted Object Recovery Capability" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    $results = Get-ADObject -LDAPFilter '(objectClass=domain)' -Properties name,distinguishedName -SearchBase $SearchBase -SearchScope Subtree -ResultSetSize $null -ErrorAction SilentlyContinue
} else {
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'BCK-004'
                adSuiteCheckName  = 'Deleted_Object_Recovery_Capability'
    ...
    $bhFile = Join-Path $bhDir "BCK-004_$bhTs.json"
    @{
