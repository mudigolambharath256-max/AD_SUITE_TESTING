# ============================================================================
# BCK-002: System State Backup Validation - Multi-Engine
# ============================================================================

[CmdletBinding()]
param([Parameter()][string]$SearchBase, [Parameter()][string]$ExportPath)

Write-Host "[BCK-002] System State Backup Validation" -ForegroundColor Cyan
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'BCK-002'
                adSuiteCheckName  = 'System_State_Backup_Validation'
                adSuiteMEDIUM   = 'MEDIUM'
                adSuiteBackup_Recovery   = 'Backup_Recovery'
                adSuiteFlag       = $true
            }
            Aces      = @()
            IsDeleted = $false
    ...
    $bhFile = Join-Path $bhDir "BCK-002_$bhTs.json"
    @{
