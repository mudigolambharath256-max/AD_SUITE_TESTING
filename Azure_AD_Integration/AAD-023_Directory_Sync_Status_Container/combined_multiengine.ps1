...
Write-Host "=== Multi-Engine Execution: Directory Sync Status Container ===" -ForegroundColor Cyan
Write-Host ""
...
public class Program
{
  ...
  {
    string filter = @"(objectClass=msDS-DeviceRegistrationService)";
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                adSuiteCheckName  = 'Directory_Sync_Status_Container'
    ...
    $bhFile = Join-Path $bhDir "AAD-023_$bhTs.json"
    @{
