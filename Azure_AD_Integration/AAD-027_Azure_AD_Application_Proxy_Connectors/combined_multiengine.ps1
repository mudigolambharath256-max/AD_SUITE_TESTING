...
# Check: Azure AD Application Proxy Connectors
...
Write-Host "=== Multi-Engine Execution: Azure AD Application Proxy Connectors ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        $ldapFilter = '(&(&(objectCategory=person)(objectClass=user)(!(userAccountControl:1.2.840.113556.1.4.803:=2)))(|(description=*app proxy*)(description=*application proxy*)))'
...
try {
    $adsiResults = @(
        ...
        $searcher = [ADSISearcher]'(&(&(objectCategory=person)(objectClass=user)(!(userAccountControl:1.2.840.113556.1.4.803:=2)))(|(description=*app proxy*)(description=*application proxy*)))'
        ...
        $searcher.FindAll() | ForEach-Object {
          $p = $_.Properties
          [PSCustomObject]@{
            Label = 'Azure AD Application Proxy Connectors'
            Name = $p['name'][0]
...
public class Program
{
  ...
  {
    string filter = @"(&(&(objectCategory=person)(objectClass=user)(!(userAccountControl:1.2.840.113556.1.4.803:=2)))(|(description=*app proxy*)(description=*application proxy*)))";
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'AAD-027'
                adSuiteCheckName  = 'Azure_AD_Application_Proxy_Connectors'
    ...
    $bhFile = Join-Path $bhDir "AAD-027_$bhTs.json"
    @{
