...
# Check: Entra ID Connect Cloud Sync Accounts
...
Write-Host "=== Multi-Engine Execution: Entra ID Connect Cloud Sync Accounts ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        $ldapFilter = '(&(&(objectCategory=person)(objectClass=user)(!(userAccountControl:1.2.840.113556.1.4.803:=2)))(|(samAccountName=*Provisioning*)(description=*cloud sync*)))'
...
try {
    $adsiResults = @(
        ...
        $searcher = [ADSISearcher]'(&(&(objectCategory=person)(objectClass=user)(!(userAccountControl:1.2.840.113556.1.4.803:=2)))(|(samAccountName=*Provisioning*)(description=*cloud sync*)))'
        ...
        $searcher.FindAll() | ForEach-Object {
          $p = $_.Properties
          [PSCustomObject]@{
            Label = 'Entra ID Connect Cloud Sync Accounts'
            Name = $p['name'][0]
...
public class Program
{
  ...
  {
    string filter = @"(&(&(objectCategory=person)(objectClass=user)(!(userAccountControl:1.2.840.113556.1.4.803:=2)))(|(samAccountName=*Provisioning*)(description=*cloud sync*)))";
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'AAD-018'
                adSuiteCheckName  = 'Entra_ID_Connect_Cloud_Sync_Accounts'
    ...
    $bhFile = Join-Path $bhDir "AAD-018_$bhTs.json"
    @{
