# ============================================================================
# AAD-035: Seamless SSO Configuration - Multi-Engine
# ============================================================================

[CmdletBinding()]
param([Parameter()][string]$SearchBase, [Parameter()][string]$ExportPath)

Write-Host "[AAD-035] Seamless SSO Configuration" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    $results = Get-ADObject -LDAPFilter '(samAccountName=AZUREADSSOACC$)' -Properties name,distinguishedName,samAccountName -SearchBase $SearchBase -SearchScope Subtree -ResultSetSize $null -ErrorAction SilentlyContinue
} else {
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                adSuiteCheckName  = 'Seamless_SSO_Configuration'
    ...
    $bhFile = Join-Path $bhDir "AAD-035_$bhTs.json"
    @{
