...
# Check: Federation Service Accounts
...
Write-Host "=== Multi-Engine Execution: Federation Service Accounts ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        $ldapFilter = '(&(&(objectCategory=person)(objectClass=user)(!(userAccountControl:1.2.840.113556.1.4.803:=2)))(|(samAccountName=*ADFS*)(samAccountName=*adfssvc*)(description=*federation*)))'
...
try {
    $adsiResults = @(
        ...
        $searcher = [ADSISearcher]'(&(&(objectCategory=person)(objectClass=user)(!(userAccountControl:1.2.840.113556.1.4.803:=2)))(|(samAccountName=*ADFS*)(samAccountName=*adfssvc*)(description=*federation*)))'
        $searcher.PageSize = 1000
        $searcher.PropertiesToLoad.Clear()
        (@('name', 'distinguishedName', 'samAccountName', 'description', 'servicePrincipalName', 'userAccountControl') | ForEach-Object { [void]$searcher.PropertiesToLoad.Add($_) }

        $searcher.FindAll() | ForEach-Object {
          $p = $_.Properties
          [PSCustomObject]@{
            Label = 'Federation Service Accounts'
            Name = $p['name'][0]
...
public class Program
{
  ...
  {
    string filter = @"(&(&(objectCategory=person)(objectClass=user)(!(userAccountControl:1.2.840.113556.1.4.803:=2)))(|(samAccountName=*ADFS*)(samAccountName=*adfssvc*)(description=*federation*)))";
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'AAD-016'
                adSuiteCheckName  = 'Federation_Service_Accounts'
    ...
    $bhFile = Join-Path $bhDir "AAD-016_$bhTs.json"
    @{
