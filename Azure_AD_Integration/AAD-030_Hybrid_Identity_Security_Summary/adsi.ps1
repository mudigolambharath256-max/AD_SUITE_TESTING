...
(@('name', 'distinguishedName', 'samAccountName', 'userPrincipalName', 'description', 'memberOf', 'pwdLastSet', 'userAccountControl', 'objectSid') | ForEach-Object { [void]$searcher.PropertiesToLoad.Add($_) }
...
try {
    ...
    if ($results -and $results.Count -gt 0) {
        ...
        $bhFile = Join-Path $bhDir "AAD-030_nodes.json"
    ...
    Write-Warning "[BloodHound] Export failed: # Check: Hybrid Identity Security Summary
# Category: Azure AD Integration
# Severity: info
# ID: AAD-030
# Requirements: None
