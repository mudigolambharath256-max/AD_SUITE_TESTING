...
Write-Host "=== Multi-Engine Execution: Hybrid Identity Security Summary ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        $ldapFilter = '(&(&(objectCategory=person)(objectClass=user)(!(userAccountControl:1.2.840.113556.1.4.803:=2)))(|(samAccountName=MSOL_*)(samAccountName=AAD_*)(samAccountName=*sync*)(description=*Azure*)))'
        $props = @('name', 'distinguishedName', 'samAccountName', 'userPrincipalName', 'description', 'memberOf', 'pwdLastSet')

        Get-ADObject -LDAPFilter $ldapFilter -Properties $props,userAccountControl,samAccountName -ErrorAction Stop |
          Select-Object name, distinguishedName, samAccountName, userPrincipalName, description, memberOf, pwdLastSet |
          Sort-Object name |
          ForEach-Object { $_ }
...
public class Program
{
  ...
  {
    string filter = @"(&(&(objectCategory=person)(objectClass=user)(!(userAccountControl:1.2.840.113556.1.4.803:=2)))(|(samAccountName=MSOL_*)(samAccountName=AAD_*)(samAccountName=*sync*)(description=*Azure*)))";
    string[] props = new string[] { "name", "distinguishedName", "samAccountName", "userPrincipalName", "description", "memberOf", "pwdLastSet" };
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'AAD-030'
                adSuiteCheckName  = 'Hybrid_Identity_Security_Summary'
    ...
    $bhFile = Join-Path $bhDir "AAD-030_$bhTs.json"
    @{
