...
# Check: Azure AD Connect Accounts (MSOL_*)
...
Write-Host "=== Multi-Engine Execution: Azure AD Connect Accounts (MSOL_*) ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        $ldapFilter = '(&(&(objectCategory=person)(objectClass=user)(!(userAccountControl:1.2.840.113556.1.4.803:=2)))(samAccountName=MSOL_*))'
        $props = @('name', 'distinguishedName', 'samAccountName', 'userPrincipalName', 'memberOf', 'description', 'pwdLastSet')

        Get-ADObject -LDAPFilter $ldapFilter -Properties $props,userAccountControl,samAccountName -ErrorAction Stop |
          Select-Object name, distinguishedName, samAccountName, userPrincipalName, memberOf, description, pwdLastSet |
          Sort-Object name |
          ForEach-Object { $_ }
...
try {
    $adsiResults = @(
        ...
        $searcher = [ADSISearcher]'(&(&(objectCategory=person)(objectClass=user)(!(userAccountControl:1.2.840.113556.1.4.803:=2)))(samAccountName=MSOL_*))'
        $searcher.PageSize = 1000
        $searcher.PropertiesToLoad.Clear()
        (@('name', 'distinguishedName', 'samAccountName', 'userPrincipalName', 'memberOf', 'description', 'pwdLastSet', 'userAccountControl') | ForEach-Object { [void]$searcher.PropertiesToLoad.Add($_) }

        $searcher.FindAll() | ForEach-Object {
          $p = $_.Properties
          [PSCustomObject]@{
            Label = 'Azure AD Connect Accounts (MSOL_*)'
            Name = $p['name'][0]
...
public class Program
{
  ...
  {
    string filter = @"(&(&(objectCategory=person)(objectClass=user)(!(userAccountControl:1.2.840.113556.1.4.803:=2)))(samAccountName=MSOL_*))";
    string[] props = new string[] { "name", "distinguishedName", "samAccountName", "userPrincipalName", "memberOf", "description", "pwdLastSet" };
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'AAD-001'
                adSuiteCheckName  = 'Azure_AD_Connect_Accounts_MSOL_'
    ...
    $bhFile = Join-Path $bhDir "AAD-001_$bhTs.json"
    @{
