...
Write-Host "Found $($results.Count) objects for check: Password Hash Sync Status" -ForegroundColor Cyan

$results | ForEach-Object {
    $props = $_.Properties
    [PSCustomObject]@{
        CheckID = 'AAD-032'
        CheckName = 'Password Hash Sync Status'
...
try {
    ...
    if ($results -and $results.Count -gt 0) {
        ...
        $bhFile = Join-Path $bhDir "AAD-032_nodes.json"
