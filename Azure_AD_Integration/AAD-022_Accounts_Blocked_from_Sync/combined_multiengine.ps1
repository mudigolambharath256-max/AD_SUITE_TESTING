...
Write-Host "=== Multi-Engine Execution: Accounts Blocked from Sync ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        $ldapFilter = '(&(objectCategory=person)(objectClass=user)(isCriticalSystemObject=TRUE))'
        $props = @('name', 'distinguishedName', 'samAccountName', 'isCriticalSystemObject')

        Get-ADObject -LDAPFilter $ldapFilter -Properties $props,samAccountName -ErrorAction Stop |
          Select-Object name, distinguishedName, samAccountName, isCriticalSystemObject |
          Sort-Object name |
          ForEach-Object { $_ }
...
public class Program
{
  ...
  {
    string filter = @"(&(objectCategory=person)(objectClass=user)(isCriticalSystemObject=TRUE))";
    string[] props = new string[] { "name", "distinguishedName", "samAccountName", "isCriticalSystemObject" };
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'AAD-022'
                adSuiteCheckName  = 'Accounts_Blocked_from_Sync'
    ...
    $bhFile = Join-Path $bhDir "AAD-022_$bhTs.json"
    @{
