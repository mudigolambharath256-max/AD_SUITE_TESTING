...
# Check: Password Hash Sync Enabled Indicators
...
Write-Host "=== Multi-Engine Execution: Password Hash Sync Enabled Indicators ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    $adsiResults = @(
        ...
        $searcher.FindAll() | ForEach-Object {
          $p = $_.Properties
          [PSCustomObject]@{
            Label = 'Password Hash Sync Enabled Indicators'
            Name = $p['name'][0]
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'AAD-014'
                adSuiteCheckName  = 'Password_Hash_Sync_Enabled_Indicators'
    ...
    $bhFile = Join-Path $bhDir "AAD-014_$bhTs.json"
    @{
