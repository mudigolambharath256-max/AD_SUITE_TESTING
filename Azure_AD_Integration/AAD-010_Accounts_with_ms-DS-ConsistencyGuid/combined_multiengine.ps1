...
Write-Host "=== Multi-Engine Execution: Accounts with ms-DS-ConsistencyGuid ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        $ldapFilter = '(&(&(objectCategory=person)(objectClass=user)(!(userAccountControl:1.2.840.113556.1.4.803:=2)))(ms-DS-ConsistencyGuid=*))'
        $props = @('name', 'distinguishedName', 'samAccountName', 'ms-DS-ConsistencyGuid', 'userPrincipalName')

        Get-ADObject -LDAPFilter $ldapFilter -Properties $props,userAccountControl,samAccountName -ErrorAction Stop |
          Select-Object name, distinguishedName, samAccountName, ms-DS-ConsistencyGuid, userPrincipalName |
          Sort-Object name |
          ForEach-Object { $_ }
...
public class Program
{
  ...
  {
    string filter = @"(&(&(objectCategory=person)(objectClass=user)(!(userAccountControl:1.2.840.113556.1.4.803:=2)))(ms-DS-ConsistencyGuid=*))";
    string[] props = new string[] { "name", "distinguishedName", "samAccountName", "ms-DS-ConsistencyGuid", "userPrincipalName" };
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'AAD-010'
                adSuiteCheckName  = 'Accounts_with_ms-DS-ConsistencyGuid'
    ...
    $bhFile = Join-Path $bhDir "AAD-010_$bhTs.json"
    @{
