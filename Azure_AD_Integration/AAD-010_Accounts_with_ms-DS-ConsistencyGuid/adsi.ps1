...
(@('name', 'distinguishedName', 'samAccountName', 'ms-DS-ConsistencyGuid', 'userPrincipalName', 'userAccountControl', 'objectSid') | ForEach-Object { [void]$searcher.PropertiesToLoad.Add($_) }
...
try {
    ...
    if ($results -and $results.Count -gt 0) {
        ...
        $bhFile = Join-Path $bhDir "AAD-010_nodes.json"
    ...
    Write-Warning "[BloodHound] Export failed: # Check: Accounts with ms-DS-ConsistencyGuid
# Category: Azure AD Integration
# Severity: info
# ID: AAD-010
# Requirements: None
