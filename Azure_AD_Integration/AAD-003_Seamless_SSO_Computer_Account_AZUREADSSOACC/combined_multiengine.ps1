...
Write-Host "=== Multi-Engine Execution: Seamless SSO Computer Account (AZUREADSSOACC) ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        $ldapFilter = '(&(objectCategory=computer)(samAccountName=AZUREADSSOACC$))'
        $props = @('name', 'distinguishedName', 'samAccountName', 'pwdLastSet', 'userAccountControl')

        Get-ADObject -LDAPFilter $ldapFilter -Properties $props,samAccountName -ErrorAction Stop |
          Select-Object name, distinguishedName, samAccountName, pwdLastSet, userAccountControl |
          Sort-Object name |
          ForEach-Object { $_ }
...
public class Program
{
  ...
  {
    string filter = @"(&(objectCategory=computer)(samAccountName=AZUREADSSOACC$))";
    string[] props = new string[] { "name", "distinguishedName", "samAccountName", "pwdLastSet", "userAccountControl" };
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'AAD-003'
                adSuiteCheckName  = 'Seamless_SSO_Computer_Account_AZUREADSSOACC'
    ...
    $bhFile = Join-Path $bhDir "AAD-003_$bhTs.json"
    @{
