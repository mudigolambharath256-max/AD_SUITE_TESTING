...
Write-Host "=== Multi-Engine Execution: Azure AD Connect Accounts (AAD_*) ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        $props = @('name', 'distinguishedName', 'samAccountName', 'userPrincipalName', 'memberOf', 'description')
...
public class Program
{
  ...
  {
    string filter = @"(&(&(objectCategory=person)(objectClass=user)(!(userAccountControl:1.2.840.113556.1.4.803:=2)))(samAccountName=AAD_*))";
    string[] props = new string[] { "name", "distinguishedName", "samAccountName", "userPrincipalName", "memberOf", "description" };
