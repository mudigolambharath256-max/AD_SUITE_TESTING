...
Write-Host "=== Multi-Engine Execution: Sync Account Last Logon ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        $props = @('name', 'distinguishedName', 'samAccountName', 'lastLogonTimestamp', 'pwdLastSet')

        Get-ADObject -LDAPFilter $ldapFilter -Properties $props,userAccountControl,samAccountName -ErrorAction Stop |
          Select-Object name, distinguishedName, samAccountName, lastLogonTimestamp, pwdLastSet |
          Sort-Object name |
          ForEach-Object { $_ }
...
try {
    $adsiResults = @(
        ...
        (@('name', 'distinguishedName', 'samAccountName', 'lastLogonTimestamp', 'pwdLastSet', 'userAccountControl') | ForEach-Object { [void]$searcher.PropertiesToLoad.Add($_) }

        $searcher.FindAll() | ForEach-Object {
          $p = $_.Properties
          [PSCustomObject]@{
            Label = 'Sync Account Last Logon'
            Name = $p['name'][0]
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'AAD-026'
                adSuiteCheckName  = 'Sync_Account_Last_Logon'
    ...
    $bhFile = Join-Path $bhDir "AAD-026_$bhTs.json"
    @{
