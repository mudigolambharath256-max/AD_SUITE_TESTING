...
Write-Host "=== Multi-Engine Execution: Sync Accounts with Password Never Expires ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        $ldapFilter = '(&(&(objectCategory=person)(objectClass=user)(!(userAccountControl:1.2.840.113556.1.4.803:=2)))(|(samAccountName=MSOL_*)(samAccountName=AAD_*))(userAccountControl:1.2.840.113556.1.4.803:=65536))'
        $props = @('name', 'distinguishedName', 'samAccountName', 'userAccountControl', 'pwdLastSet')

        Get-ADObject -LDAPFilter $ldapFilter -Properties $props,samAccountName,userAccountControl -ErrorAction Stop |
...
public class Program
{
  ...
  {
    string filter = @"(&(&(objectCategory=person)(objectClass=user)(!(userAccountControl:1.2.840.113556.1.4.803:=2)))(|(samAccountName=MSOL_*)(samAccountName=AAD_*))(userAccountControl:1.2.840.113556.1.4.803:=65536))";
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'AAD-005'
                adSuiteCheckName  = 'Sync_Accounts_with_Password_Never_Expires'
    ...
    $bhFile = Join-Path $bhDir "AAD-005_$bhTs.json"
    @{
