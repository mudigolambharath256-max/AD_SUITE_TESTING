...
$searcher.Filter   = '(&(objectClass=user)(|(samAccountName=MSOL_*)(samAccountName=AAD_*)))'
...
Write-Host "AAD-026: found $($results.Count) objects"
...
try {
    ...
    $bhS.Filter   = '(&(objectClass=user)(|(samAccountName=MSOL_*)(samAccountName=AAD_*)))'
    $bhS.PageSize = 1000
    ...
    foreach ($r in $bhRaw) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                adSuiteCheckName  = 'Azure AD Integration Check 26'
                adSuiteSeverity   = 'medium'
                adSuiteCategory   = 'Azure_AD_Integration'
                adSuiteFlag       = $true
            }
            Aces           = @()
            IsDeleted      = $false
    ...
    @{ data = $bhNodes.ToArray()
       meta = @{ type = 'users'; count = $bhNodes.Count; version = 5; methods = 0 }
        ...
        Out-File -FilePath (Join-Path $bhDir "AAD-026_$bhTs.json") -Encoding UTF8 -Force

# Fix R10: No silent catch — warn on failure
} catch { Write-Warning "AAD-026 BloodHound export error: $_" }
