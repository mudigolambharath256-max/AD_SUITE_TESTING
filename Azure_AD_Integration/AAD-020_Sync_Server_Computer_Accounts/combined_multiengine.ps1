...
Write-Host "=== Multi-Engine Execution: Sync Server Computer Accounts ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        $ldapFilter = '(&(objectCategory=computer)(|(cn=*AADConnect*)(cn=*AADC*)(description=*Azure AD Connect*)))'
        $props = @('name', 'distinguishedName', 'samAccountName', 'dNSHostName', 'operatingSystem', 'description')

        Get-ADObject -LDAPFilter $ldapFilter -Properties $props -ErrorAction Stop |
          Select-Object name, distinguishedName, samAccountName, dNSHostName, operatingSystem, description |
          Sort-Object name |
          ForEach-Object { $_ }
...
public class Program
{
  ...
  {
    string filter = @"(&(objectCategory=computer)(|(cn=*AADConnect*)(cn=*AADC*)(description=*Azure AD Connect*)))";
    string[] props = new string[] { "name", "distinguishedName", "samAccountName", "dNSHostName", "operatingSystem", "description" };
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'AAD-020'
                adSuiteCheckName  = 'Sync_Server_Computer_Accounts'
    ...
    $bhFile = Join-Path $bhDir "AAD-020_$bhTs.json"
    @{
