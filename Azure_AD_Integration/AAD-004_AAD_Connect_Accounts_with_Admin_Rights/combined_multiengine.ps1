...
# Check: AAD Connect Accounts with Admin Rights
...
Write-Host "=== Multi-Engine Execution: AAD Connect Accounts with Admin Rights ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        $ldapFilter = '(&(&(objectCategory=person)(objectClass=user)(!(userAccountControl:1.2.840.113556.1.4.803:=2)))(|(samAccountName=MSOL_*)(samAccountName=AAD_*))(adminCount=1))'
        $props = @('name', 'distinguishedName', 'samAccountName', 'adminCount', 'memberOf')

        Get-ADObject -LDAPFilter $ldapFilter -Properties $props,userAccountControl,adminCount,samAccountName -ErrorAction Stop |
          Select-Object name, distinguishedName, samAccountName, adminCount, memberOf |
          Sort-Object name |
          ForEach-Object { $_ }
...
try {
    $adsiResults = @(
        ...
        $searcher = [ADSISearcher]'(&(&(objectCategory=person)(objectClass=user)(!(userAccountControl:1.2.840.113556.1.4.803:=2)))(|(samAccountName=MSOL_*)(samAccountName=AAD_*))(adminCount=1))'
        $searcher.PageSize = 1000
        $searcher.PropertiesToLoad.Clear()
        (@('name', 'distinguishedName', 'samAccountName', 'adminCount', 'memberOf', 'userAccountControl') | ForEach-Object { [void]$searcher.PropertiesToLoad.Add($_) }

        $searcher.FindAll() | ForEach-Object {
          $p = $_.Properties
          [PSCustomObject]@{
            Label = 'AAD Connect Accounts with Admin Rights'
            Name = $p['name'][0]
...
public class Program
{
  ...
  {
    string filter = @"(&(&(objectCategory=person)(objectClass=user)(!(userAccountControl:1.2.840.113556.1.4.803:=2)))(|(samAccountName=MSOL_*)(samAccountName=AAD_*))(adminCount=1))";
    string[] props = new string[] { "name", "distinguishedName", "samAccountName", "adminCount", "memberOf" };
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'AAD-004'
                adSuiteCheckName  = 'AAD_Connect_Accounts_with_Admin_Rights'
    ...
    $bhFile = Join-Path $bhDir "AAD-004_$bhTs.json"
    @{
