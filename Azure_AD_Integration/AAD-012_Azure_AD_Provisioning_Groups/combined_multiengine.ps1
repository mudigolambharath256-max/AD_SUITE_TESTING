...
Write-Host "=== Multi-Engine Execution: Azure AD Provisioning Groups ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        $ldapFilter = '(&(objectCategory=group)(|(cn=*Azure*)(cn=*AAD*)(cn=*O365*)(cn=*M365*)))'
        $props = @('name', 'distinguishedName', 'cn', 'member', 'description')

        Get-ADObject -LDAPFilter $ldapFilter -Properties $props -ErrorAction Stop |
          Select-Object name, distinguishedName, cn, member, description |
          Sort-Object name |
          ForEach-Object { $_ }
...
public class Program
{
  ...
  {
    string filter = @"(&(objectCategory=group)(|(cn=*Azure*)(cn=*AAD*)(cn=*O365*)(cn=*M365*)))";
    string[] props = new string[] { "name", "distinguishedName", "cn", "member", "description" };
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'AAD-012'
                adSuiteCheckName  = 'Azure_AD_Provisioning_Groups'
    ...
    $bhFile = Join-Path $bhDir "AAD-012_$bhTs.json"
    @{
