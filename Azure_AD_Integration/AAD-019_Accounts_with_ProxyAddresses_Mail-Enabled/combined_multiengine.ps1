...
# Check: Accounts with ProxyAddresses (Mail-Enabled)
...
Write-Host "=== Multi-Engine Execution: Accounts with ProxyAddresses (Mail-Enabled) ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        $ldapFilter = '(&(&(objectCategory=person)(objectClass=user)(!(userAccountControl:1.2.840.113556.1.4.803:=2)))(proxyAddresses=*))'
        $props = @('name', 'distinguishedName', 'samAccountName', 'proxyAddresses', 'mail')

        Get-ADObject -LDAPFilter $ldapFilter -Properties $props,userAccountControl,samAccountName -ErrorAction Stop |
          Select-Object name, distinguishedName, samAccountName, proxyAddresses, mail |
          Sort-Object name |
          ForEach-Object { $_ }
...
try {
    $adsiResults = @(
        ...
        $searcher = [ADSISearcher]'(&(&(objectCategory=person)(objectClass=user)(!(userAccountControl:1.2.840.113556.1.4.803:=2)))(proxyAddresses=*))'
        $searcher.PageSize = 1000
        $searcher.PropertiesToLoad.Clear()
        (@('name', 'distinguishedName', 'samAccountName', 'proxyAddresses', 'mail', 'userAccountControl') | ForEach-Object { [void]$searcher.PropertiesToLoad.Add($_) }

        $searcher.FindAll() | ForEach-Object {
          $p = $_.Properties
          [PSCustomObject]@{
            Label = 'Accounts with ProxyAddresses (Mail-Enabled)'
            Name = $p['name'][0]
...
public class Program
{
  ...
  {
    string filter = @"(&(&(objectCategory=person)(objectClass=user)(!(userAccountControl:1.2.840.113556.1.4.803:=2)))(proxyAddresses=*))";
    string[] props = new string[] { "name", "distinguishedName", "samAccountName", "proxyAddresses", "mail" };
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'AAD-019'
                adSuiteCheckName  = 'Accounts_with_ProxyAddresses_Mail-Enabled'
    ...
    $bhFile = Join-Path $bhDir "AAD-019_$bhTs.json"
    @{
