...
# Check: Accounts with ExtensionAttributes (Exchange)
...
Write-Host "=== Multi-Engine Execution: Accounts with ExtensionAttributes (Exchange) ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        $ldapFilter = '(&(&(objectCategory=person)(objectClass=user)(!(userAccountControl:1.2.840.113556.1.4.803:=2)))(|(extensionAttribute1=*)(extensionAttribute2=*)(extensionAttribute15=*)))'
        $props = @('name', 'distinguishedName', 'samAccountName', 'extensionAttribute1', 'extensionAttribute15')

        Get-ADObject -LDAPFilter $ldapFilter -Properties $props,userAccountControl,samAccountName -ErrorAction Stop |
          Select-Object name, distinguishedName, samAccountName, extensionAttribute1, extensionAttribute15 |
          Sort-Object name |
          ForEach-Object { $_ }
...
try {
    $adsiResults = @(
        ...
        $searcher = [ADSISearcher]'(&(&(objectCategory=person)(objectClass=user)(!(userAccountControl:1.2.840.113556.1.4.803:=2)))(|(extensionAttribute1=*)(extensionAttribute2=*)(extensionAttribute15=*)))'
        $searcher.PageSize = 1000
        $searcher.PropertiesToLoad.Clear()
        (@('name', 'distinguishedName', 'samAccountName', 'extensionAttribute1', 'extensionAttribute15', 'userAccountControl') | ForEach-Object { [void]$searcher.PropertiesToLoad.Add($_) }

        $searcher.FindAll() | ForEach-Object {
          $p = $_.Properties
          [PSCustomObject]@{
            Label = 'Accounts with ExtensionAttributes (Exchange)'
            Name = $p['name'][0]
...
public class Program
{
  ...
  {
    string filter = @"(&(&(objectCategory=person)(objectClass=user)(!(userAccountControl:1.2.840.113556.1.4.803:=2)))(|(extensionAttribute1=*)(extensionAttribute2=*)(extensionAttribute15=*)))";
    string[] props = new string[] { "name", "distinguishedName", "samAccountName", "extensionAttribute1", "extensionAttribute15" };
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'AAD-029'
                adSuiteCheckName  = 'Accounts_with_ExtensionAttributes_Exchange'
    ...
    $bhFile = Join-Path $bhDir "AAD-029_$bhTs.json"
    @{
