...
(@('name', 'distinguishedName', 'samAccountName', 'extensionAttribute1', 'extensionAttribute15', 'userAccountControl', 'objectSid') | ForEach-Object { [void]$searcher.PropertiesToLoad.Add($_) }
...
try {
    ...
    if ($results -and $results.Count -gt 0) {
        ...
        $bhFile = Join-Path $bhDir "AAD-029_nodes.json"
    ...
    Write-Warning "[BloodHound] Export failed: # Check: Accounts with ExtensionAttributes (Exchange)
# Category: Azure AD Integration
# Severity: info
# ID: AAD-029
# Requirements: None
