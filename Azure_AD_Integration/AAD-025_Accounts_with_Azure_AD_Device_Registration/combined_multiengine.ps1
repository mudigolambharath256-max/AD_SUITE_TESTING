...
Write-Host "=== Multi-Engine Execution: Accounts with Azure AD Device Registration ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        $ldapFilter = '(&(&(objectCategory=person)(objectClass=user)(!(userAccountControl:1.2.840.113556.1.4.803:=2)))(msDS-RegisteredOwner=*))'
        $props = @('name', 'distinguishedName', 'samAccountName', 'msDS-RegisteredOwner')

        Get-ADObject -LDAPFilter $ldapFilter -Properties $props,userAccountControl,samAccountName -ErrorAction Stop |
          Select-Object name, distinguishedName, samAccountName, msDS-RegisteredOwner |
          Sort-Object name |
          ForEach-Object { $_ }
...
public class Program
{
  ...
  {
    string filter = @"(&(&(objectCategory=person)(objectClass=user)(!(userAccountControl:1.2.840.113556.1.4.803:=2)))(msDS-RegisteredOwner=*))";
    string[] props = new string[] { "name", "distinguishedName", "samAccountName", "msDS-RegisteredOwner" };
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'AAD-025'
                adSuiteCheckName  = 'Accounts_with_Azure_AD_Device_Registration'
    ...
    $bhFile = Join-Path $bhDir "AAD-025_$bhTs.json"
    @{
