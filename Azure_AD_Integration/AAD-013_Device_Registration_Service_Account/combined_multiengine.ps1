...
# Check: Device Registration Service Account
...
Write-Host "=== Multi-Engine Execution: Device Registration Service Account ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        $ldapFilter = '(&(&(objectCategory=person)(objectClass=user)(!(userAccountControl:1.2.840.113556.1.4.803:=2)))(|(samAccountName=*DeviceReg*)(description=*device registration*)))'
        $props = @('name', 'distinguishedName', 'samAccountName', 'description', 'memberOf')

        Get-ADObject -LDAPFilter $ldapFilter -Properties $props,userAccountControl,samAccountName -ErrorAction Stop |
          Select-Object name, distinguishedName, samAccountName, description, memberOf |
          Sort-Object name |
          ForEach-Object { $_ }
...
try {
    $adsiResults = @(
        ...
        $searcher = [ADSISearcher]'(&(&(objectCategory=person)(objectClass=user)(!(userAccountControl:1.2.840.113556.1.4.803:=2)))(|(samAccountName=*DeviceReg*)(description=*device registration*)))'
        $searcher.PageSize = 1000
        $searcher.PropertiesToLoad.Clear()
        (@('name', 'distinguishedName', 'samAccountName', 'description', 'memberOf', 'userAccountControl') | ForEach-Object { [void]$searcher.PropertiesToLoad.Add($_) }

        $searcher.FindAll() | ForEach-Object {
          $p = $_.Properties
          [PSCustomObject]@{
            Label = 'Device Registration Service Account'
            Name = $p['name'][0]
...
public class Program
{
  ...
  {
    string filter = @"(&(&(objectCategory=person)(objectClass=user)(!(userAccountControl:1.2.840.113556.1.4.803:=2)))(|(samAccountName=*DeviceReg*)(description=*device registration*)))";
    string[] props = new string[] { "name", "distinguishedName", "samAccountName", "description", "memberOf" };
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'AAD-013'
                adSuiteCheckName  = 'Device_Registration_Service_Account'
    ...
    $bhFile = Join-Path $bhDir "AAD-013_$bhTs.json"
    @{
