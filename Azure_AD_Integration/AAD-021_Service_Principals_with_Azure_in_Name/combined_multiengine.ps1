...
# Check: Service Principals with Azure in Name
...
Write-Host "=== Multi-Engine Execution: Service Principals with Azure in Name ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        $ldapFilter = '(&(&(objectCategory=person)(objectClass=user)(!(userAccountControl:1.2.840.113556.1.4.803:=2)))(servicePrincipalName=*azure*))'
...
try {
    $adsiResults = @(
        ...
        $searcher = [ADSISearcher]'(&(&(objectCategory=person)(objectClass=user)(!(userAccountControl:1.2.840.113556.1.4.803:=2)))(servicePrincipalName=*azure*))'
        ...
        $searcher.FindAll() | ForEach-Object {
          $p = $_.Properties
          [PSCustomObject]@{
            Label = 'Service Principals with Azure in Name'
            Name = $p['name'][0]
...
public class Program
{
  ...
  {
    string filter = @"(&(&(objectCategory=person)(objectClass=user)(!(userAccountControl:1.2.840.113556.1.4.803:=2)))(servicePrincipalName=*azure*))";
    string[] props = new string[] { "name", "distinguishedName", "samAccountName", "servicePrincipalName" };
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'AAD-021'
                adSuiteCheckName  = 'Service_Principals_with_Azure_in_Name'
    ...
    $bhFile = Join-Path $bhDir "AAD-021_$bhTs.json"
    @{
