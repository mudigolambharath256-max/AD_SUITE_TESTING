...
# Check: Azure AD Integration Check 38
...
Write-Host "=== Azure AD Integration Check 38 ===" -ForegroundColor Cyan
...
try {
    ...
    Get-ADObject -LDAPFilter '(&(objectClass=user)(|(samAccountName=MSOL_*)(samAccountName=AAD_*)))' `
                 -Properties @('name','distinguishedName','samAccountName', 'objectSid') `
                 -SearchBase $targetNC `
                 -ErrorAction Stop |
    ForEach-Object {
    ...
    Write-Warning "AAD-038 PowerShell engine: $_"   # Fix R10: no silent catch
}
...
try {
    ...
    $adsiS.Filter   = '(&(objectClass=user)(|(samAccountName=MSOL_*)(samAccountName=AAD_*)))'
    $adsiS.PageSize = 1000
    $adsiS.PropertiesToLoad.Clear()
    @('name','distinguishedName','samAccountName', 'objectSid') | ForEach-Object { [void]$adsiS.PropertiesToLoad.Add($_) }
    ...
    Write-Warning "AAD-038 ADSI engine: $_"          # Fix R10
}
...
try {
    $csFilter  = '(&(objectClass=user)(|(samAccountName=MSOL_*)(samAccountName=AAD_*)))'
    $csProps   = @("name","distinguishedName","samAccountName")   # PS array of double-quoted strings
    $csCheckId = 'AAD-038'
    ...
    Write-Warning "AAD-038 C# engine: $_"             # Fix R10
}
...
Write-Host ""
Write-Host "=== AAD-038: $($uniqueResults.Count) unique findings ===" -ForegroundColor Cyan
$uniqueResults | Format-List

# Fix R14: Fixed 5-field schema enforced before CSV export
$csvPath = Join-Path $env:TEMP "AAD-038_results.csv"
$uniqueResults |
