# ============================================================================
# AAD-042: Azure AD Connect Health Status - Multi-Engine
# ============================================================================

[CmdletBinding()]
param([Parameter()][string]$SearchBase, [Parameter()][string]$ExportPath)

Write-Host "[AAD-042] Azure AD Connect Health Status" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    $results = Get-ADObject -LDAPFilter '(samAccountName=MSOL_*)' -Properties name,distinguishedName,samAccountName -SearchBase $SearchBase -SearchScope Subtree -ResultSetSize $null -ErrorAction SilentlyContinue
} else {
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'AAD-042'
                adSuiteCheckName  = 'Azure_AD_Connect_Health_Status'
    ...
    $bhFile = Join-Path $bhDir "AAD-042_$bhTs.json"
    @{
