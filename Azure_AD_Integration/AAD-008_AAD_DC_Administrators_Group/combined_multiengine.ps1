...
Write-Host "=== Multi-Engine Execution: AAD DC Administrators Group ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        $ldapFilter = '(&(objectCategory=group)(cn=AAD DC Administrators))'
...
public class Program
{
  ...
  {
    string filter = @"(&(objectCategory=group)(cn=AAD DC Administrators))";
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                adSuiteCheckName  = 'AAD_DC_Administrators_Group'
    ...
    $bhFile = Join-Path $bhDir "AAD-008_$bhTs.json"
    @{
