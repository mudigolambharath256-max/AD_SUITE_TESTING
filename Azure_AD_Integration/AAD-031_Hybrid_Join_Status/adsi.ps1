...
Write-Host "Found $($results.Count) objects for check: Hybrid Join Status" -ForegroundColor Cyan

$results | ForEach-Object {
    $props = $_.Properties
    [PSCustomObject]@{
        CheckID = 'AAD-031'
        CheckName = 'Hybrid Join Status'
...
try {
    ...
    if ($results -and $results.Count -gt 0) {
        ...
        $bhFile = Join-Path $bhDir "AAD-031_nodes.json"
