...
Write-Host "=== Multi-Engine Execution: Hybrid Azure AD Joined Devices ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        $ldapFilter = '(&(objectCategory=computer)(msDS-DeviceID=*))'
        $props = @('name', 'distinguishedName', 'samAccountName', 'msDS-DeviceID', 'operatingSystem')

        Get-ADObject -LDAPFilter $ldapFilter -Properties $props -ErrorAction Stop |
          Select-Object name, distinguishedName, samAccountName, msDS-DeviceID, operatingSystem |
          Sort-Object name |
          ForEach-Object { $_ }
...
public class Program
{
  ...
  {
    string filter = @"(&(objectCategory=computer)(msDS-DeviceID=*))";
    string[] props = new string[] { "name", "distinguishedName", "samAccountName", "msDS-DeviceID", "operatingSystem" };
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'AAD-024'
                adSuiteCheckName  = 'Hybrid_Azure_AD_Joined_Devices'
    ...
    $bhFile = Join-Path $bhDir "AAD-024_$bhTs.json"
    @{
