...
Write-Host "Found $($results.Count) objects for check: Accounts with Suspicious Naming Patterns" -ForegroundColor Cyan

$results | ForEach-Object {
    $props = $_.Properties
    [PSCustomObject]@{
        CheckID = 'USR-035'
        CheckName = 'Accounts with Suspicious Naming Patterns'
...
try {
    ...
    if ($results -and $results.Count -gt 0) {
        ...
        $bhFile = Join-Path $bhDir "USR-035_nodes.json"
