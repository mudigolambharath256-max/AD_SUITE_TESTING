...
Write-Host "=== Multi-Engine Execution: Accounts with unixUserPassword Attribute ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        $ldapFilter = '(&(objectCategory=person)(objectClass=user)(unixUserPassword=*))'
        $props = @('name', 'distinguishedName', 'samAccountName', 'unixUserPassword')

        Get-ADObject -LDAPFilter $ldapFilter -Properties $props,samAccountName -ErrorAction Stop |
          Select-Object name, distinguishedName, samAccountName, unixUserPassword |
          Sort-Object name |
          ForEach-Object { $_ }
...
public class Program
{
  ...
  {
    string filter = @"(&(objectCategory=person)(objectClass=user)(unixUserPassword=*))";
    string[] props = new string[] { "name", "distinguishedName", "samAccountName", "unixUserPassword" };
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'USR-009'
                adSuiteCheckName  = 'Accounts_with_unixUserPassword_Attribute'
    ...
    $bhFile = Join-Path $bhDir "USR-009_$bhTs.json"
    @{
