...
Write-Host "=== Users Accounts Check 18 ===" -ForegroundColor Cyan
...
try {
    ...
    Write-Warning "USER-018 PowerShell engine: $_"   # Fix R10: no silent catch
}
...
try {
    ...
    Write-Warning "USER-018 ADSI engine: $_"          # Fix R10
}
...
try {
    ...
    Write-Warning "USER-018 C# engine: $_"             # Fix R10
}
...
Write-Host ""
Write-Host "=== USER-018: $($uniqueResults.Count) unique findings ===" -ForegroundColor Cyan
$uniqueResults | Format-List

# Fix R14: Fixed 5-field schema enforced before CSV export
$csvPath = Join-Path $env:TEMP "USER-018_results.csv"
$uniqueResults |
