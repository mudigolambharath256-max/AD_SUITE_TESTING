...
Write-Host "=== Users Accounts Check 63 ===" -ForegroundColor Cyan
...
try {
    ...
    Write-Warning "USER-063 PowerShell engine: $_"   # Fix R10: no silent catch
}
...
try {
    ...
    Write-Warning "USER-063 ADSI engine: $_"          # Fix R10
}
...
try {
    ...
    Write-Warning "USER-063 C# engine: $_"             # Fix R10
}
...
Write-Host ""
Write-Host "=== USER-063: $($uniqueResults.Count) unique findings ===" -ForegroundColor Cyan
$uniqueResults | Format-List

# Fix R14: Fixed 5-field schema enforced before CSV export
$csvPath = Join-Path $env:TEMP "USER-063_results.csv"
$uniqueResults |
