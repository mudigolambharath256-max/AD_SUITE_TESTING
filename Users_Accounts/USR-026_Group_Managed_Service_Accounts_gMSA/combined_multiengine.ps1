...
Write-Host "=== Multi-Engine Execution: Group Managed Service Accounts (gMSA) ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        $props = @('name', 'distinguishedName', 'samAccountName', 'msDS-ManagedPasswordInterval', 'msDS-HostServiceAccount', 'servicePrincipalName')

        Get-ADObject -LDAPFilter $ldapFilter -Properties $props -ErrorAction Stop |
          Select-Object name, distinguishedName, samAccountName, msDS-ManagedPasswordInterval, msDS-HostServiceAccount, servicePrincipalName |
          Sort-Object name |
          ForEach-Object { $_ }
...
public class Program
{
  ...
  {
    string filter = @"(objectClass=msDS-GroupManagedServiceAccount)";
    string[] props = new string[] { "name", "distinguishedName", "samAccountName", "msDS-ManagedPasswordInterval", "msDS-HostServiceAccount", "servicePrincipalName" };
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'USR-026'
                adSuiteCheckName  = 'Group_Managed_Service_Accounts_gMSA'
    ...
    $bhFile = Join-Path $bhDir "USR-026_$bhTs.json"
    @{
