...
# Check: Accounts with KeyCredentialLink (Shadow Credentials)
...
Write-Host "=== Multi-Engine Execution: Accounts with KeyCredentialLink (Shadow Credentials) ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        $ldapFilter = '(&(objectCategory=person)(objectClass=user)(!(userAccountControl:1.2.840.113556.1.4.803:=2))(msDS-KeyCredentialLink=*))'
        $props = @('name', 'distinguishedName', 'samAccountName', 'msDS-KeyCredentialLink')

        Get-ADObject -LDAPFilter $ldapFilter -Properties $props,userAccountControl,msDS-KeyCredentialLink,samAccountName -ErrorAction Stop |
...
try {
    $adsiResults = @(
        ...
        $searcher = [ADSISearcher]'(&(objectCategory=person)(objectClass=user)(!(userAccountControl:1.2.840.113556.1.4.803:=2))(msDS-KeyCredentialLink=*))'
        $searcher.PageSize = 1000
        $searcher.PropertiesToLoad.Clear()
        (@('name', 'distinguishedName', 'samAccountName', 'msDS-KeyCredentialLink', 'userAccountControl', 'objectSid') | ForEach-Object { [void]$searcher.PropertiesToLoad.Add($_) }

        $searcher.FindAll() | ForEach-Object {
          $p = $_.Properties
          [PSCustomObject]@{
            Label = 'Accounts with KeyCredentialLink (Shadow Credentials)'
            Name = $p['name'][0]
...
public class Program
{
  ...
  {
    string filter = @"(&(objectCategory=person)(objectClass=user)(!(userAccountControl:1.2.840.113556.1.4.803:=2))(msDS-KeyCredentialLink=*))";
    string[] props = new string[] { "name", "distinguishedName", "samAccountName", "msDS-KeyCredentialLink" };
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'USR-030'
                adSuiteCheckName  = 'Accounts_with_KeyCredentialLink_Shadow_Credentials'
    ...
    $bhFile = Join-Path $bhDir "USR-030_$bhTs.json"
    @{
