...
Write-Host "=== Multi-Engine Execution: Standalone Managed Service Accounts (sMSA) ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        $ldapFilter = '(objectClass=msDS-ManagedServiceAccount)'
        $props = @('name', 'distinguishedName', 'samAccountName', 'msDS-ManagedPasswordInterval')

        Get-ADObject -LDAPFilter $ldapFilter -Properties $props -ErrorAction Stop |
          Select-Object name, distinguishedName, samAccountName, msDS-ManagedPasswordInterval |
          Sort-Object name |
          ForEach-Object { $_ }
...
public class Program
{
  ...
  {
    string filter = @"(objectClass=msDS-ManagedServiceAccount)";
    string[] props = new string[] { "name", "distinguishedName", "samAccountName", "msDS-ManagedPasswordInterval" };
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'USR-027'
                adSuiteCheckName  = 'Standalone_Managed_Service_Accounts_sMSA'
    ...
    $bhFile = Join-Path $bhDir "USR-027_$bhTs.json"
    @{
