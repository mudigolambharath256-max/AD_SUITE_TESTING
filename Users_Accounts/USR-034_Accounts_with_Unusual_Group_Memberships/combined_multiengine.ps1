...
Write-Host "=== USR-034: Accounts with Unusual Group Memberships ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    $results = Get-ADObject -LDAPFilter '(&(objectCategory=person)(objectClass=user)(!(userAccountControl:1.2.840.113556.1.4.803:=2)))' -Properties name,distinguishedName,userAccountControl,samAccountName -SearchBase $SearchBase -SearchScope Subtree -ResultSetSize $null
} else {
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'USR-034'
                adSuiteCheckName  = 'Accounts_with_Unusual_Group_Memberships'
    ...
    $bhFile = Join-Path $bhDir "USR-034_$bhTs.json"
    @{
