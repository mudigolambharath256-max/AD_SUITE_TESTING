...
Write-Host "Found $($results.Count) objects for check: Accounts with Unusual Group Memberships" -ForegroundColor Cyan

$results | ForEach-Object {
    $props = $_.Properties
    [PSCustomObject]@{
        CheckID = 'USR-034'
        CheckName = 'Accounts with Unusual Group Memberships'
...
try {
    ...
    if ($results -and $results.Count -gt 0) {
        ...
        $bhFile = Join-Path $bhDir "USR-034_nodes.json"
