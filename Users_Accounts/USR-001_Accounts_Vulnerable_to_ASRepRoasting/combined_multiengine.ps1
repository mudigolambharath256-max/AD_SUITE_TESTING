...
# Check: Accounts Vulnerable to ASRepRoasting
...
Write-Host "=== Multi-Engine Execution: Accounts Vulnerable to ASRepRoasting ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    $adsiResults = @(
        ...
        $searcher.FindAll() | ForEach-Object {
          $p = $_.Properties
          [PSCustomObject]@{
            Label = 'Accounts Vulnerable to ASRepRoasting'
            Name = $p['name'][0]
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'USR-001'
                adSuiteCheckName  = 'Accounts_Vulnerable_to_ASRepRoasting'
    ...
    $bhFile = Join-Path $bhDir "USR-001_$bhTs.json"
    @{
