...
Write-Host "=== Multi-Engine Execution: Native Administrator Account Recent Use ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        $ldapFilter = '(&(objectCategory=person)(objectClass=user)(objectSid=*-500))'
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'USR-025'
                adSuiteCheckName  = 'Native_Administrator_Account_Recent_Use'
    ...
    $bhFile = Join-Path $bhDir "USR-025_$bhTs.json"
    @{
