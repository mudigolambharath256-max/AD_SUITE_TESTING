...
Write-Host "=== Multi-Engine Execution: Domain Admins Members ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        $ldapFilter = '(&(objectCategory=person)(objectClass=user)(memberOf:1.2.840.113556.1.4.1941:=CN=Domain Admins,CN=Users,*))'
        $props = @('name', 'distinguishedName', 'samAccountName', 'memberOf', 'lastLogonTimestamp')

        Get-ADObject -LDAPFilter $ldapFilter -Properties $props,samAccountName -ErrorAction Stop |
          Select-Object name, distinguishedName, samAccountName, memberOf, lastLogonTimestamp |
          Sort-Object name |
          ForEach-Object { $_ }
...
public class Program
{
  ...
  {
    string filter = @"(&(objectCategory=person)(objectClass=user)(memberOf:1.2.840.113556.1.4.1941:=CN=Domain Admins,CN=Users,*))";
    string[] props = new string[] { "name", "distinguishedName", "samAccountName", "memberOf", "lastLogonTimestamp" };
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'USR-019'
                adSuiteCheckName  = 'Domain_Admins_Members'
    ...
    $bhFile = Join-Path $bhDir "USR-019_$bhTs.json"
    @{
