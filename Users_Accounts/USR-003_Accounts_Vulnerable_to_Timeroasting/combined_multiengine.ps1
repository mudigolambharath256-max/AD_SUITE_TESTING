...
Write-Host "=== Multi-Engine Execution: Accounts Vulnerable to Timeroasting ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'USR-003'
                adSuiteCheckName  = 'Accounts_Vulnerable_to_Timeroasting'
    ...
    $bhFile = Join-Path $bhDir "USR-003_$bhTs.json"
    @{
