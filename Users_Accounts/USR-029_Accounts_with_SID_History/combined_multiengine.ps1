...
# Check: Accounts with SID History
...
Write-Host "=== Multi-Engine Execution: Accounts with SID History ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        $ldapFilter = '(&(objectCategory=person)(objectClass=user)(!(userAccountControl:1.2.840.113556.1.4.803:=2))(sIDHistory=*))'
        $props = @('name', 'distinguishedName', 'samAccountName', 'sIDHistory')

        Get-ADObject -LDAPFilter $ldapFilter -Properties $props,userAccountControl,sidHistory,samAccountName -ErrorAction Stop |
...
try {
    $adsiResults = @(
        ...
        $searcher = [ADSISearcher]'(&(objectCategory=person)(objectClass=user)(!(userAccountControl:1.2.840.113556.1.4.803:=2))(sIDHistory=*))'
        $searcher.PageSize = 1000
        $searcher.PropertiesToLoad.Clear()
        (@('name', 'distinguishedName', 'samAccountName', 'sIDHistory', 'userAccountControl', 'objectSid') | ForEach-Object { [void]$searcher.PropertiesToLoad.Add($_) }

        $searcher.FindAll() | ForEach-Object {
          $p = $_.Properties
          [PSCustomObject]@{
            Label = 'Accounts with SID History'
            Name = $p['name'][0]
            DistinguishedName = $p['distinguishedname'][0]
        UserAccountControl = if ($props['useraccountcontrol'].Count -gt 0) { $props['useraccountcontrol'][0]
        SIDHistory = if ($props['sidhistory'].Count -gt 0) { $props['sidhistory']
...
public class Program
{
  ...
  {
    string filter = @"(&(objectCategory=person)(objectClass=user)(!(userAccountControl:1.2.840.113556.1.4.803:=2))(sIDHistory=*))";
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'USR-029'
                adSuiteCheckName  = 'Accounts_with_SID_History'
    ...
    $bhFile = Join-Path $bhDir "USR-029_$bhTs.json"
    @{
