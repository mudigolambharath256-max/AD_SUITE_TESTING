...
# Check: Admin Accounts That Can Be Delegated
...
Write-Host "=== Multi-Engine Execution: Admin Accounts That Can Be Delegated ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    $adsiResults = @(
        ...
        (@('name', 'distinguishedName', 'samAccountName', 'userAccountControl', 'adminCount', 'objectSid') | ForEach-Object { [void]$searcher.PropertiesToLoad.Add($_) }

        $searcher.FindAll() | ForEach-Object {
          $p = $_.Properties
          [PSCustomObject]@{
            Label = 'Admin Accounts That Can Be Delegated'
            Name = $p['name'][0]
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'USR-005'
                adSuiteCheckName  = 'Admin_Accounts_That_Can_Be_Delegated'
    ...
    $bhFile = Join-Path $bhDir "USR-005_$bhTs.json"
    @{
