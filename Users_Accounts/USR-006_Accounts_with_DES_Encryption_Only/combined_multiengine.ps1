...
# Check: Accounts with DES Encryption Only
...
Write-Host "=== Multi-Engine Execution: Accounts with DES Encryption Only ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    $adsiResults = @(
        ...
        $searcher = [ADSISearcher]'(&(objectCategory=person)(objectClass=user)(!(userAccountControl:1.2.840.113556.1.4.803:=2))(userAccountControl:1.2.840.113556.1.4.803:=2097152))'
        ...
        $searcher.FindAll() | ForEach-Object {
          $p = $_.Properties
          [PSCustomObject]@{
            Label = 'Accounts with DES Encryption Only'
            Name = $p['name'][0]
...
public class Program
{
  ...
  {
    string filter = @"(&(objectCategory=person)(objectClass=user)(!(userAccountControl:1.2.840.113556.1.4.803:=2))(userAccountControl:1.2.840.113556.1.4.803:=2097152))";
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'USR-006'
                adSuiteCheckName  = 'Accounts_with_DES_Encryption_Only'
    ...
    $bhFile = Join-Path $bhDir "USR-006_$bhTs.json"
    @{
