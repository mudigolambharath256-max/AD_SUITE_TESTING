...
Write-Host "=== Users Accounts Check 16 ===" -ForegroundColor Cyan
...
try {
    ...
    Write-Warning "USER-016 PowerShell engine: $_"   # Fix R10: no silent catch
}
...
try {
    ...
    Write-Warning "USER-016 ADSI engine: $_"          # Fix R10
}
...
try {
    ...
    Write-Warning "USER-016 C# engine: $_"             # Fix R10
}
...
Write-Host ""
Write-Host "=== USER-016: $($uniqueResults.Count) unique findings ===" -ForegroundColor Cyan
$uniqueResults | Format-List

# Fix R14: Fixed 5-field schema enforced before CSV export
$csvPath = Join-Path $env:TEMP "USER-016_results.csv"
$uniqueResults |
