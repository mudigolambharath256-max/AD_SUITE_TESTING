...
Write-Host "=== Multi-Engine Execution: Admin Accounts Not in Protected Users Group ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        $ldapFilter = '(&(objectCategory=person)(objectClass=user)(!(userAccountControl:1.2.840.113556.1.4.803:=2))(adminCount=1)(!(memberOf:1.2.840.113556.1.4.1941:=CN=Protected Users,CN=Users,*)))'
        $props = @('name', 'distinguishedName', 'samAccountName', 'memberOf', 'adminCount')

        Get-ADObject -LDAPFilter $ldapFilter -Properties $props,userAccountControl,adminCount,samAccountName -ErrorAction Stop |
          Select-Object name, distinguishedName, samAccountName, memberOf, adminCount |
          Sort-Object name |
          ForEach-Object { $_ }
...
public class Program
{
  ...
  {
    string filter = @"(&(objectCategory=person)(objectClass=user)(!(userAccountControl:1.2.840.113556.1.4.803:=2))(adminCount=1)(!(memberOf:1.2.840.113556.1.4.1941:=CN=Protected Users,CN=Users,*)))";
    string[] props = new string[] { "name", "distinguishedName", "samAccountName", "memberOf", "adminCount" };
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'USR-004'
                adSuiteCheckName  = 'Admin_Accounts_Not_in_Protected_Users_Group'
    ...
    $bhFile = Join-Path $bhDir "USR-004_$bhTs.json"
    @{
