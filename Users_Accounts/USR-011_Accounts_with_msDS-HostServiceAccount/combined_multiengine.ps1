...
Write-Host "=== Multi-Engine Execution: Accounts with msDS-HostServiceAccount ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        $props = @('name', 'distinguishedName', 'samAccountName', 'msDS-HostServiceAccount', 'msDS-ManagedPasswordInterval')

        Get-ADObject -LDAPFilter $ldapFilter -Properties $props -ErrorAction Stop |
          Select-Object name, distinguishedName, samAccountName, msDS-HostServiceAccount, msDS-ManagedPasswordInterval |
          Sort-Object name |
          ForEach-Object { $_ }
...
public class Program
{
  ...
  {
    string filter = @"(&(objectClass=msDS-GroupManagedServiceAccount)(msDS-HostServiceAccount=*))";
    string[] props = new string[] { "name", "distinguishedName", "samAccountName", "msDS-HostServiceAccount", "msDS-ManagedPasswordInterval" };
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'USR-011'
                adSuiteCheckName  = 'Accounts_with_msDS-HostServiceAccount'
    ...
    $bhFile = Join-Path $bhDir "USR-011_$bhTs.json"
    @{
