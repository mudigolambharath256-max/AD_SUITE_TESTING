...
Write-Host "=== USR-033: Recently Created Privileged Accounts ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    $results = Get-ADObject -LDAPFilter '(&(objectCategory=person)(objectClass=user)(adminCount=1))' -Properties name,distinguishedName,adminCount,samAccountName -SearchBase $SearchBase -SearchScope Subtree -ResultSetSize $null
} else {
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'USR-033'
                adSuiteCheckName  = 'Recently_Created_Privileged_Accounts'
    ...
    $bhFile = Join-Path $bhDir "USR-033_$bhTs.json"
    @{
