...
Write-Host "Found $($results.Count) objects for check: Recently Created Privileged Accounts" -ForegroundColor Cyan

$results | ForEach-Object {
    $props = $_.Properties
    [PSCustomObject]@{
        CheckID = 'USR-033'
        CheckName = 'Recently Created Privileged Accounts'
...
try {
    ...
    if ($results -and $results.Count -gt 0) {
        ...
        $bhFile = Join-Path $bhDir "USR-033_nodes.json"
