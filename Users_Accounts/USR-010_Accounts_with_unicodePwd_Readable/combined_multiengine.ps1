...
# Check: Accounts with unicodePwd Readable
...
Write-Host "=== Multi-Engine Execution: Accounts with unicodePwd Readable ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    $adsiResults = @(
        ...
        $searcher.FindAll() | ForEach-Object {
          $p = $_.Properties
          [PSCustomObject]@{
            Label = 'Accounts with unicodePwd Readable'
            Name = $p['name'][0]
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'USR-010'
                adSuiteCheckName  = 'Accounts_with_unicodePwd_Readable'
    ...
    $bhFile = Join-Path $bhDir "USR-010_$bhTs.json"
    @{
