...
Write-Host "=== Users Accounts Check 48 ===" -ForegroundColor Cyan
...
try {
    ...
    Write-Warning "USER-048 PowerShell engine: $_"   # Fix R10: no silent catch
}
...
try {
    ...
    Write-Warning "USER-048 ADSI engine: $_"          # Fix R10
}
...
try {
    ...
    Write-Warning "USER-048 C# engine: $_"             # Fix R10
}
...
Write-Host ""
Write-Host "=== USER-048: $($uniqueResults.Count) unique findings ===" -ForegroundColor Cyan
$uniqueResults | Format-List

# Fix R14: Fixed 5-field schema enforced before CSV export
$csvPath = Join-Path $env:TEMP "USER-048_results.csv"
$uniqueResults |
