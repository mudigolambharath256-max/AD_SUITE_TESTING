...
Write-Host "=== Multi-Engine Execution: Accounts with Constrained Delegation ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'USR-031'
                adSuiteCheckName  = 'Accounts_with_Constrained_Delegation'
    ...
    $bhFile = Join-Path $bhDir "USR-031_$bhTs.json"
    @{
