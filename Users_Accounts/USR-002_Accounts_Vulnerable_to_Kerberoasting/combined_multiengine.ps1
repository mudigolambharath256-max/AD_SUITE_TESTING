...
# Check: Accounts Vulnerable to Kerberoasting
...
Write-Host "=== Multi-Engine Execution: Accounts Vulnerable to Kerberoasting ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    $adsiResults = @(
        ...
        (@('name', 'distinguishedName', 'samAccountName', 'servicePrincipalName', 'pwdLastSet', 'userAccountControl', 'objectSid') | ForEach-Object { [void]$searcher.PropertiesToLoad.Add($_) }

        $searcher.FindAll() | ForEach-Object {
          $p = $_.Properties
          [PSCustomObject]@{
            Label = 'Accounts Vulnerable to Kerberoasting'
            Name = $p['name'][0]
...
public class Program
{
  ...
  {
    string filter = @"(&(objectCategory=person)(objectClass=user)(!(userAccountControl:1.2.840.113556.1.4.803:=2))(servicePrincipalName=*))";
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'USR-002'
                adSuiteCheckName  = 'Accounts_Vulnerable_to_Kerberoasting'
    ...
    $bhFile = Join-Path $bhDir "USR-002_$bhTs.json"
    @{
