...
# Check: Certificate Services Check 38
...
Write-Host "=== Certificate Services Check 38 ===" -ForegroundColor Cyan
...
try {
    ...
    Write-Warning "CERT-038 PowerShell engine: $_"   # Fix R10: no silent catch
}
...
try {
    ...
    Write-Warning "CERT-038 ADSI engine: $_"          # Fix R10
}
...
try {
    ...
    Write-Warning "CERT-038 C# engine: $_"             # Fix R10
}
...
Write-Host ""
Write-Host "=== CERT-038: $($uniqueResults.Count) unique findings ===" -ForegroundColor Cyan
$uniqueResults | Format-List

# Fix R14: Fixed 5-field schema enforced before CSV export
$csvPath = Join-Path $env:TEMP "CERT-038_results.csv"
$uniqueResults |
