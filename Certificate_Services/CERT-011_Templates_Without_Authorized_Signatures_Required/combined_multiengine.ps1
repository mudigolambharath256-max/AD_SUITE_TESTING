...
Write-Host "=== Multi-Engine Execution: Templates Without Authorized Signatures Required ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        $props = @('name', 'distinguishedName', 'cn', 'displayName', 'msPKI-RA-Signature')
...
public class Program
{
  ...
  {
    string filter = @"(&(objectClass=pKICertificateTemplate)(|(msPKI-RA-Signature=0)(!(msPKI-RA-Signature=*))))";
    string[] props = new string[] { "name", "distinguishedName", "cn", "displayName", "msPKI-RA-Signature" };
