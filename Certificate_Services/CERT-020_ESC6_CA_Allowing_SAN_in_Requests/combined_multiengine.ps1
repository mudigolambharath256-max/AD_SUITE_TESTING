...
Write-Host "=== Multi-Engine Execution: ESC6: CA Allowing SAN in Requests ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        $props = @('name', 'distinguishedName', 'cn', 'dNSHostName', 'flags')

        Get-ADObject -LDAPFilter $ldapFilter -Properties $props -ErrorAction Stop |
          Select-Object name, distinguishedName, cn, dNSHostName, flags |
          Sort-Object name |
          ForEach-Object { $_ }
...
public class Program
{
  public static void Run()
  {
    ...
    string[] props = new string[] { "name", "distinguishedName", "cn", "dNSHostName", "flags" };
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                adSuiteCheckName  = 'ESC6_CA_Allowing_SAN_in_Requests'
    ...
    $bhFile = Join-Path $bhDir "CERT-020_$bhTs.json"
    @{
