...
Write-Host "=== Multi-Engine Execution: Templates Allowing Renewal with Different Key ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        $ldapFilter = '(&(objectClass=pKICertificateTemplate)(msPKI-Enrollment-Flag:1.2.840.113556.1.4.803:=32))'
        $props = @('name', 'distinguishedName', 'cn', 'displayName', 'msPKI-Enrollment-Flag')

        Get-ADObject -LDAPFilter $ldapFilter -Properties $props -ErrorAction Stop |
          Select-Object name, distinguishedName, cn, displayName, msPKI-Enrollment-Flag |
          Sort-Object name |
          ForEach-Object { $_ }
...
public class Program
{
  ...
  {
    string filter = @"(&(objectClass=pKICertificateTemplate)(msPKI-Enrollment-Flag:1.2.840.113556.1.4.803:=32))";
    string[] props = new string[] { "name", "distinguishedName", "cn", "displayName", "msPKI-Enrollment-Flag" };
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'CERT-019'
                adSuiteCheckName  = 'Templates_Allowing_Renewal_with_Different_Key'
    ...
    $bhFile = Join-Path $bhDir "CERT-019_$bhTs.json"
    @{
