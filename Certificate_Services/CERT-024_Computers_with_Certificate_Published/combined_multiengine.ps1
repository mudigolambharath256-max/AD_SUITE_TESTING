...
Write-Host "=== Multi-Engine Execution: Computers with Certificate Published ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        $ldapFilter = '(&(objectCategory=computer)(userCertificate=*))'
        $props = @('name', 'distinguishedName', 'samAccountName', 'userCertificate', 'operatingSystem')

        Get-ADObject -LDAPFilter $ldapFilter -Properties $props -ErrorAction Stop |
          Select-Object name, distinguishedName, samAccountName, userCertificate, operatingSystem |
          Sort-Object name |
          ForEach-Object { $_ }
...
public class Program
{
  ...
  {
    string filter = @"(&(objectCategory=computer)(userCertificate=*))";
    string[] props = new string[] { "name", "distinguishedName", "samAccountName", "userCertificate", "operatingSystem" };
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'CERT-024'
                adSuiteCheckName  = 'Computers_with_Certificate_Published'
    ...
    $bhFile = Join-Path $bhDir "CERT-024_$bhTs.json"
    @{
