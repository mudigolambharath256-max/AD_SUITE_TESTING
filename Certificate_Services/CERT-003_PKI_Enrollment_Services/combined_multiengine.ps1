...
Write-Host "=== PKI Enrollment Services ===" -ForegroundColor Cyan
...
try {
    ...
    Get-ADObject -LDAPFilter '(&(objectClass=pKIEnrollmentService))' `
                 -Properties @('name','distinguishedName','cn','dNSHostName') `
                 -SearchBase $targetNC `
                 -ErrorAction Stop |
    ForEach-Object {
    ...
    Write-Warning "CERT-003 PowerShell engine: $_"   # Fix R10: no silent catch
}
...
try {
    ...
    $adsiS.Filter   = '(&(objectClass=pKIEnrollmentService))'
    $adsiS.PageSize = 1000
    $adsiS.PropertiesToLoad.Clear()
    @('name','distinguishedName','cn','dNSHostName') | ForEach-Object { [void]$adsiS.PropertiesToLoad.Add($_) }
    ...
    Write-Warning "CERT-003 ADSI engine: $_"          # Fix R10
}
...
try {
    $csFilter  = '(&(objectClass=pKIEnrollmentService))'
    $csProps   = @("name","distinguishedName","cn","dNSHostName")   # PS array of double-quoted strings
    $csCheckId = 'CERT-003'
    ...
    Write-Warning "CERT-003 C# engine: $_"             # Fix R10
}
...
Write-Host ""
Write-Host "=== CERT-003: $($uniqueResults.Count) unique findings ===" -ForegroundColor Cyan
$uniqueResults | Format-List

# Fix R14: Fixed 5-field schema enforced before CSV export
$csvPath = Join-Path $env:TEMP "CERT-003_results.csv"
$uniqueResults |
