...
# Check: Certificate Services Check 30
...
Write-Host "=== Certificate Services Check 30 ===" -ForegroundColor Cyan
...
try {
    ...
    Write-Warning "CERT-030 PowerShell engine: $_"   # Fix R10: no silent catch
}
...
try {
    ...
    Write-Warning "CERT-030 ADSI engine: $_"          # Fix R10
}
...
try {
    ...
    Write-Warning "CERT-030 C# engine: $_"             # Fix R10
}
...
Write-Host ""
Write-Host "=== CERT-030: $($uniqueResults.Count) unique findings ===" -ForegroundColor Cyan
$uniqueResults | Format-List

# Fix R14: Fixed 5-field schema enforced before CSV export
$csvPath = Join-Path $env:TEMP "CERT-030_results.csv"
$uniqueResults |
