...
Write-Host "=== Multi-Engine Execution: Templates Allowing PKINIT Client Auth ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        $ldapFilter = '(&(objectClass=pKICertificateTemplate)(pKIExtendedKeyUsage=1.3.6.1.5.2.3.4))'
...
public class Program
{
  ...
  {
    string filter = @"(&(objectClass=pKICertificateTemplate)(pKIExtendedKeyUsage=1.3.6.1.5.2.3.4))";
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'CERT-008'
                adSuiteCheckName  = 'Templates_Allowing_PKINIT_Client_Auth'
    ...
    $bhFile = Join-Path $bhDir "CERT-008_$bhTs.json"
    @{
