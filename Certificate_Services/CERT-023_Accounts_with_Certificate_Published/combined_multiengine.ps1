...
Write-Host "=== Multi-Engine Execution: Accounts with Certificate Published ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        $ldapFilter = '(&(objectCategory=person)(objectClass=user)(userCertificate=*))'
        $props = @('name', 'distinguishedName', 'samAccountName', 'userCertificate')

        Get-ADObject -LDAPFilter $ldapFilter -Properties $props,samAccountName -ErrorAction Stop |
          Select-Object name, distinguishedName, samAccountName, userCertificate |
          Sort-Object name |
          ForEach-Object { $_ }
...
public class Program
{
  ...
  {
    string filter = @"(&(objectCategory=person)(objectClass=user)(userCertificate=*))";
    string[] props = new string[] { "name", "distinguishedName", "samAccountName", "userCertificate" };
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'CERT-023'
                adSuiteCheckName  = 'Accounts_with_Certificate_Published'
    ...
    $bhFile = Join-Path $bhDir "CERT-023_$bhTs.json"
    @{
