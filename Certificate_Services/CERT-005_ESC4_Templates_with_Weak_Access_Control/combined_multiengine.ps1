...
Write-Host "=== Multi-Engine Execution: ESC4: Templates with Weak Access Control ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        $props = @('name', 'distinguishedName', 'cn', 'displayName', 'nTSecurityDescriptor')

        Get-ADObject -LDAPFilter $ldapFilter -Properties $props -ErrorAction Stop |
          Select-Object name, distinguishedName, cn, displayName, nTSecurityDescriptor |
          Sort-Object name |
          ForEach-Object { $_ }
...
public class Program
{
  public static void Run()
  {
    ...
    string[] props = new string[] { "name", "distinguishedName", "cn", "displayName", "nTSecurityDescriptor" };
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                adSuiteCheckName  = 'ESC4_Templates_with_Weak_Access_Control'
    ...
    $bhFile = Join-Path $bhDir "CERT-005_$bhTs.json"
    @{
