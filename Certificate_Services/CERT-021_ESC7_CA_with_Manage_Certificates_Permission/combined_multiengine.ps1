...
Write-Host "=== Multi-Engine Execution: ESC7: CA with Manage Certificates Permission ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        $props = @('name', 'distinguishedName', 'cn', 'dNSHostName', 'nTSecurityDescriptor')

        Get-ADObject -LDAPFilter $ldapFilter -Properties $props -ErrorAction Stop |
          Select-Object name, distinguishedName, cn, dNSHostName, nTSecurityDescriptor |
          Sort-Object name |
          ForEach-Object { $_ }
...
public class Program
{
  public static void Run()
  {
    ...
    string[] props = new string[] { "name", "distinguishedName", "cn", "dNSHostName", "nTSecurityDescriptor" };
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'CERT-021'
                adSuiteCheckName  = 'ESC7_CA_with_Manage_Certificates_Permission'
    ...
    $bhFile = Join-Path $bhDir "CERT-021_$bhTs.json"
    @{
