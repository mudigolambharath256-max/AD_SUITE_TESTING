...
@('name', 'distinguishedName', 'cn', 'displayName', 'msPKI-Certificate-Name-Flag', 'pKIExtendedKeyUsage', 'objectSid', 'samAccountName') | ForEach-Object { [void]$searcher.PropertiesToLoad.Add($_) }
...
try {
    ...
    if ($results -and $results.Count -gt 0) {
        ...
        $bhFile = Join-Path $bhDir "CERT-002_nodes.json"
    ...
    Write-Warning "[BloodHound] Export failed: # Check: ESC1: Templates Allowing SAN Specification
# Category: Certificate Services
# Severity: critical
# ID: CERT-002
# Requirements: None
