...
Write-Host "=== Multi-Engine Execution: ESC1: Templates Allowing SAN Specification ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        $props = @('name', 'distinguishedName', 'cn', 'displayName', 'msPKI-Certificate-Name-Flag', 'pKIExtendedKeyUsage')

        Get-ADObject -LDAPFilter $ldapFilter -Properties $props -ErrorAction Stop |
          Select-Object name, distinguishedName, cn, displayName, msPKI-Certificate-Name-Flag, pKIExtendedKeyUsage |
          Sort-Object name |
          ForEach-Object { $_ }
...
public class Program
{
  ...
  {
    string filter = @"(&(objectClass=pKICertificateTemplate)(msPKI-Certificate-Name-Flag:1.2.840.113556.1.4.803:=1))";
    string[] props = new string[] { "name", "distinguishedName", "cn", "displayName", "msPKI-Certificate-Name-Flag", "pKIExtendedKeyUsage" };
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                adSuiteCheckName  = 'ESC1_Templates_Allowing_SAN_Specification'
    ...
    $bhFile = Join-Path $bhDir "CERT-002_$bhTs.json"
    @{
