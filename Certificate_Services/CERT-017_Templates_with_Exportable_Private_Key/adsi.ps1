...
@('name', 'distinguishedName', 'cn', 'displayName', 'msPKI-Private-Key-Flag', 'objectSid', 'samAccountName') | ForEach-Object { [void]$searcher.PropertiesToLoad.Add($_) }
...
try {
    ...
    if ($results -and $results.Count -gt 0) {
        ...
        $bhFile = Join-Path $bhDir "CERT-017_nodes.json"
    ...
    Write-Warning "[BloodHound] Export failed: # Check: Templates with Exportable Private Key
# Category: Certificate Services
# Severity: medium
# ID: CERT-017
# Requirements: None
