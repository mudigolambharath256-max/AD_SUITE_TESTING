...
Write-Host "=== Multi-Engine Execution: Templates with Exportable Private Key ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        $ldapFilter = '(&(objectClass=pKICertificateTemplate)(msPKI-Private-Key-Flag:1.2.840.113556.1.4.803:=16))'
        $props = @('name', 'distinguishedName', 'cn', 'displayName', 'msPKI-Private-Key-Flag')

        Get-ADObject -LDAPFilter $ldapFilter -Properties $props -ErrorAction Stop |
          Select-Object name, distinguishedName, cn, displayName, msPKI-Private-Key-Flag |
          Sort-Object name |
          ForEach-Object { $_ }
...
public class Program
{
  ...
  {
    string filter = @"(&(objectClass=pKICertificateTemplate)(msPKI-Private-Key-Flag:1.2.840.113556.1.4.803:=16))";
    string[] props = new string[] { "name", "distinguishedName", "cn", "displayName", "msPKI-Private-Key-Flag" };
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'CERT-017'
                adSuiteCheckName  = 'Templates_with_Exportable_Private_Key'
    ...
    $bhFile = Join-Path $bhDir "CERT-017_$bhTs.json"
    @{
