...
Write-Host "=== Multi-Engine Execution: ESC3: Templates with Certificate Request Agent ===" -ForegroundColor Cyan
Write-Host ""
...
public class Program
{
  ...
  {
    string filter = @"(&(objectClass=pKICertificateTemplate)(pKIExtendedKeyUsage=1.3.6.1.4.1.311.20.2.1))";
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                adSuiteCheckName  = 'ESC3_Templates_with_Certificate_Request_Agent'
    ...
    $bhFile = Join-Path $bhDir "CERT-004_$bhTs.json"
    @{
