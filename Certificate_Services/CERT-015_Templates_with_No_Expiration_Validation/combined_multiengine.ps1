...
Write-Host "=== Multi-Engine Execution: Templates with No Expiration Validation ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        $ldapFilter = '(&(objectClass=pKICertificateTemplate)(!(pKIExpirationPeriod=*)))'
        $props = @('name', 'distinguishedName', 'cn', 'displayName')

        Get-ADObject -LDAPFilter $ldapFilter -Properties $props -ErrorAction Stop |
          Select-Object name, distinguishedName, cn, displayName |
          Sort-Object name |
          ForEach-Object { $_ }
...
public class Program
{
  ...
  {
    string filter = @"(&(objectClass=pKICertificateTemplate)(!(pKIExpirationPeriod=*)))";
    string[] props = new string[] { "name", "distinguishedName", "cn", "displayName" };
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'CERT-015'
                adSuiteCheckName  = 'Templates_with_No_Expiration_Validation'
    ...
    $bhFile = Join-Path $bhDir "CERT-015_$bhTs.json"
    @{
