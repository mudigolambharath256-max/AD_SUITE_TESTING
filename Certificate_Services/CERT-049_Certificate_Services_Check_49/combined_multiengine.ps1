...
# Check: Certificate Services Check 49
...
Write-Host "=== Certificate Services Check 49 ===" -ForegroundColor Cyan
...
try {
    ...
    Write-Warning "CERT-049 PowerShell engine: $_"   # Fix R10: no silent catch
}
...
try {
    ...
    Write-Warning "CERT-049 ADSI engine: $_"          # Fix R10
}
...
try {
    ...
    Write-Warning "CERT-049 C# engine: $_"             # Fix R10
}
...
Write-Host ""
Write-Host "=== CERT-049: $($uniqueResults.Count) unique findings ===" -ForegroundColor Cyan
$uniqueResults | Format-List

# Fix R14: Fixed 5-field schema enforced before CSV export
$csvPath = Join-Path $env:TEMP "CERT-049_results.csv"
$uniqueResults |
