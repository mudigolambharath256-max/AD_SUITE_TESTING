...
Write-Host "=== Multi-Engine Execution: Offline Root CA References ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        $ldapFilter = '(&(objectClass=certificationAuthority)(cn=*))'
        $props = @('name', 'distinguishedName', 'cn', 'cACertificate', 'certificateRevocationList')

        Get-ADObject -LDAPFilter $ldapFilter -Properties $props -ErrorAction Stop |
          Select-Object name, distinguishedName, cn, cACertificate, certificateRevocationList |
          Sort-Object name |
          ForEach-Object { $_ }
...
public class Program
{
  ...
  {
    string filter = @"(&(objectClass=certificationAuthority)(cn=*))";
    string[] props = new string[] { "name", "distinguishedName", "cn", "cACertificate", "certificateRevocationList" };
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'CERT-027'
                adSuiteCheckName  = 'Offline_Root_CA_References'
    ...
    $bhFile = Join-Path $bhDir "CERT-027_$bhTs.json"
    @{
