# ============================================================================
# CERT-041: Certificate Template Superseded Not Disabled - Multi-Engine
# ============================================================================

[CmdletBinding()]
param([Parameter()][string]$SearchBase, [Parameter()][string]$ExportPath)

Write-Host "[CERT-041] Certificate Template Superseded Not Disabled" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    $results = Get-ADObject -LDAPFilter '(objectClass=pKICertificateTemplate)' -Properties name,distinguishedName -SearchBase $SearchBase -SearchScope Subtree -ResultSetSize $null -ErrorAction SilentlyContinue
} else {
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                adSuiteCheckName  = 'Certificate_Template_Superseded_Not_Disabled'
    ...
    $bhFile = Join-Path $bhDir "CERT-041_$bhTs.json"
    @{
