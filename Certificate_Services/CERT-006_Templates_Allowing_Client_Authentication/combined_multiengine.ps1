...
# Check: Templates Allowing Client Authentication
...
Write-Host "=== Multi-Engine Execution: Templates Allowing Client Authentication ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        $ldapFilter = '(&(objectClass=pKICertificateTemplate)(pKIExtendedKeyUsage=1.3.6.1.5.5.7.3.2))'
        $props = @('name', 'distinguishedName', 'cn', 'displayName', 'pKIExtendedKeyUsage', 'msPKI-Certificate-Name-Flag')

        Get-ADObject -LDAPFilter $ldapFilter -Properties $props -ErrorAction Stop |
          Select-Object name, distinguishedName, cn, displayName, pKIExtendedKeyUsage, msPKI-Certificate-Name-Flag |
          Sort-Object name |
          ForEach-Object { $_ }
...
try {
    $adsiResults = @(
        ...
        $searcher = [ADSISearcher]'(&(objectClass=pKICertificateTemplate)(pKIExtendedKeyUsage=1.3.6.1.5.5.7.3.2))'
        $searcher.PageSize = 1000
        $searcher.PropertiesToLoad.Clear()
        @('name', 'distinguishedName', 'cn', 'displayName', 'pKIExtendedKeyUsage', 'msPKI-Certificate-Name-Flag') | ForEach-Object { [void]$searcher.PropertiesToLoad.Add($_) }

        $searcher.FindAll() | ForEach-Object {
          $p = $_.Properties
          [PSCustomObject]@{
            Label = 'Templates Allowing Client Authentication'
            Name = $p['name'][0]
...
public class Program
{
  ...
  {
    string filter = @"(&(objectClass=pKICertificateTemplate)(pKIExtendedKeyUsage=1.3.6.1.5.5.7.3.2))";
    string[] props = new string[] { "name", "distinguishedName", "cn", "displayName", "pKIExtendedKeyUsage", "msPKI-Certificate-Name-Flag" };
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'CERT-006'
                adSuiteCheckName  = 'Templates_Allowing_Client_Authentication'
    ...
    $bhFile = Join-Path $bhDir "CERT-006_$bhTs.json"
    @{
