...
@('name', 'distinguishedName', 'cn', 'displayName', 'pKIExtendedKeyUsage', 'msPKI-Certificate-Name-Flag', 'objectSid', 'samAccountName') | ForEach-Object { [void]$searcher.PropertiesToLoad.Add($_) }
...
try {
    ...
    if ($results -and $results.Count -gt 0) {
        ...
        $bhFile = Join-Path $bhDir "CERT-006_nodes.json"
    ...
    Write-Warning "[BloodHound] Export failed: # Check: Templates Allowing Client Authentication
# Category: Certificate Services
# Severity: high
# ID: CERT-006
# Requirements: None
