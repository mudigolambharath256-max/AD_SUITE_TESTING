...
Write-Host "=== Multi-Engine Execution: Templates Allowing Smart Card Logon ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        $ldapFilter = '(&(objectClass=pKICertificateTemplate)(pKIExtendedKeyUsage=1.3.6.1.4.1.311.20.2.2))'
...
public class Program
{
  ...
  {
    string filter = @"(&(objectClass=pKICertificateTemplate)(pKIExtendedKeyUsage=1.3.6.1.4.1.311.20.2.2))";
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'CERT-007'
                adSuiteCheckName  = 'Templates_Allowing_Smart_Card_Logon'
    ...
    $bhFile = Join-Path $bhDir "CERT-007_$bhTs.json"
    @{
