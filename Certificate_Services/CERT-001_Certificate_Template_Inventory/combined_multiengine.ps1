...
Write-Host "=== Multi-Engine Execution: Certificate Template Inventory ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        $props = @('name', 'distinguishedName', 'cn', 'displayName', 'pKIExtendedKeyUsage', 'msPKI-Enrollment-Flag', 'msPKI-Certificate-Name-Flag', 'msPKI-RA-Signature')

        Get-ADObject -LDAPFilter $ldapFilter -Properties $props -ErrorAction Stop |
          Select-Object name, distinguishedName, cn, displayName, pKIExtendedKeyUsage, msPKI-Enrollment-Flag, msPKI-Certificate-Name-Flag, msPKI-RA-Signature |
          Sort-Object name |
          ForEach-Object { $_ }
...
public class Program
{
  ...
  {
    string filter = @"(objectClass=pKICertificateTemplate)";
    string[] props = new string[] { "name", "distinguishedName", "cn", "displayName", "pKIExtendedKeyUsage", "msPKI-Enrollment-Flag", "msPKI-Certificate-Name-Flag", "msPKI-RA-Signature" };
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'CERT-001'
                adSuiteCheckName  = 'Certificate_Template_Inventory'
    ...
    $bhFile = Join-Path $bhDir "CERT-001_$bhTs.json"
    @{
