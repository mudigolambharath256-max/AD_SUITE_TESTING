...
Write-Host "=== Multi-Engine Execution: AIA (Authority Information Access) Objects ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        $ldapFilter = '(&(objectClass=container)(cn=AIA))'
...
public class Program
{
  ...
  {
    string filter = @"(&(objectClass=container)(cn=AIA))";
    string[] props = new string[] { "name", "distinguishedName", "cn" };
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                adSuiteCheckName  = 'AIA_Authority_Information_Access_Objects'
    ...
    $bhFile = Join-Path $bhDir "CERT-028_$bhTs.json"
    @{
