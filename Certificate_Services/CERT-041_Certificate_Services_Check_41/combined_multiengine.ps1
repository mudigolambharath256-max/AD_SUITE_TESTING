...
# Check: Certificate Services Check 41
...
Write-Host "=== Certificate Services Check 41 ===" -ForegroundColor Cyan
...
try {
    ...
    Write-Warning "CERT-041 PowerShell engine: $_"   # Fix R10: no silent catch
}
...
try {
    ...
    Write-Warning "CERT-041 ADSI engine: $_"          # Fix R10
}
...
try {
    ...
    Write-Warning "CERT-041 C# engine: $_"             # Fix R10
}
...
Write-Host ""
Write-Host "=== CERT-041: $($uniqueResults.Count) unique findings ===" -ForegroundColor Cyan
$uniqueResults | Format-List

# Fix R14: Fixed 5-field schema enforced before CSV export
$csvPath = Join-Path $env:TEMP "CERT-041_results.csv"
$uniqueResults |
