...
Write-Host "=== Certificate Services Check 21 ===" -ForegroundColor Cyan
...
try {
    ...
    Write-Warning "CERT-021 PowerShell engine: $_"   # Fix R10: no silent catch
}
...
try {
    ...
    Write-Warning "CERT-021 ADSI engine: $_"          # Fix R10
}
...
try {
    ...
    Write-Warning "CERT-021 C# engine: $_"             # Fix R10
}
...
Write-Host ""
Write-Host "=== CERT-021: $($uniqueResults.Count) unique findings ===" -ForegroundColor Cyan
$uniqueResults | Format-List

# Fix R14: Fixed 5-field schema enforced before CSV export
$csvPath = Join-Path $env:TEMP "CERT-021_results.csv"
$uniqueResults |
