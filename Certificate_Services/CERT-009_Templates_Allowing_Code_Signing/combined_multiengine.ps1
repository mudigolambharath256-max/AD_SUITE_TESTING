...
Write-Host "=== Multi-Engine Execution: Templates Allowing Code Signing ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        $ldapFilter = '(&(objectClass=pKICertificateTemplate)(pKIExtendedKeyUsage=1.3.6.1.5.5.7.3.3))'
...
public class Program
{
  ...
  {
    string filter = @"(&(objectClass=pKICertificateTemplate)(pKIExtendedKeyUsage=1.3.6.1.5.5.7.3.3))";
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'CERT-009'
                adSuiteCheckName  = 'Templates_Allowing_Code_Signing'
    ...
    $bhFile = Join-Path $bhDir "CERT-009_$bhTs.json"
    @{
