...
Write-Host "=== Multi-Engine Execution: Templates with Subject Name from AD ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        $ldapFilter = '(&(objectClass=pKICertificateTemplate)(msPKI-Certificate-Name-Flag:1.2.840.113556.1.4.803:=134217728))'
        ...
        Get-ADObject -LDAPFilter $ldapFilter -Properties $props -ErrorAction Stop |
          Select-Object name, distinguishedName, cn, displayName, msPKI-Certificate-Name-Flag |
          Sort-Object name |
          ForEach-Object { $_ }
...
public class Program
{
  ...
  {
    string filter = @"(&(objectClass=pKICertificateTemplate)(msPKI-Certificate-Name-Flag:1.2.840.113556.1.4.803:=134217728))";
    string[] props = new string[] { "name", "distinguishedName", "cn", "displayName", "msPKI-Certificate-Name-Flag" };
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'CERT-029'
                adSuiteCheckName  = 'Templates_with_Subject_Name_from_AD'
    ...
    $bhFile = Join-Path $bhDir "CERT-029_$bhTs.json"
    @{
