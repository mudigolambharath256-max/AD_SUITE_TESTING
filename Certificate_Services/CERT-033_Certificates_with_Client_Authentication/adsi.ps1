...
Write-Host "Found $($results.Count) objects for check: Certificates with Client Authentication" -ForegroundColor Cyan

$results | ForEach-Object {
    $props = $_.Properties
    [PSCustomObject]@{
        CheckID = 'CERT-033'
        CheckName = 'Certificates with Client Authentication'
...
try {
    ...
    if ($results -and $results.Count -gt 0) {
        ...
        $bhFile = Join-Path $bhDir "CERT-033_nodes.json"
