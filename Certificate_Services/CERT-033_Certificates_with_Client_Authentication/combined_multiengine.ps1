...
Write-Host "=== CERT-033: Certificates with Client Authentication ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    $results = Get-ADObject -LDAPFilter '(objectClass=pKICertificateTemplate)' -Properties name,distinguishedName -SearchBase $SearchBase -SearchScope Subtree -ResultSetSize $null
} else {
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                adSuiteCheckName  = 'Certificates_with_Client_Authentication'
    ...
    $bhFile = Join-Path $bhDir "CERT-033_$bhTs.json"
    @{
