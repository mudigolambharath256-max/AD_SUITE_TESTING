...
Write-Host "=== Multi-Engine Execution: Templates Without Manager Approval ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        $ldapFilter = '(&(objectClass=pKICertificateTemplate)(!(msPKI-Enrollment-Flag:1.2.840.113556.1.4.803:=2)))'
...
public class Program
{
  ...
  {
    string filter = @"(&(objectClass=pKICertificateTemplate)(!(msPKI-Enrollment-Flag:1.2.840.113556.1.4.803:=2)))";
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'CERT-010'
                adSuiteCheckName  = 'Templates_Without_Manager_Approval'
    ...
    $bhFile = Join-Path $bhDir "CERT-010_$bhTs.json"
    @{
