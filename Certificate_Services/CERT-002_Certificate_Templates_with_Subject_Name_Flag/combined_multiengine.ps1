...
Write-Host "=== Certificate Templates with Subject Name Flag ===" -ForegroundColor Cyan
...
$configNC = $root.Properties['configurationNamingContext'].Value
$targetNC = "$configNC"
...
try {
    ...
    Get-ADObject -LDAPFilter '(&(objectClass=pKICertificateTemplate)(msPKI-Certificate-Name-Flag:1.2.840.113556.1.4.803:=1))' `
                 -Properties @('name','distinguishedName','cn','displayName','msPKI-Certificate-Name-Flag') `
                 -SearchBase $targetNC `
                 -ErrorAction Stop |
    ForEach-Object {
    ...
    Write-Warning "CERT-002 PowerShell engine: $_"   # Fix R10: no silent catch
}
...
try {
    ...
    $adsiS.Filter   = '(&(objectClass=pKICertificateTemplate)(msPKI-Certificate-Name-Flag:1.2.840.113556.1.4.803:=1))'
    $adsiS.PageSize = 1000
    $adsiS.PropertiesToLoad.Clear()
    @('name','distinguishedName','cn','displayName','msPKI-Certificate-Name-Flag') | ForEach-Object { [void]$adsiS.PropertiesToLoad.Add($_) }
    ...
    Write-Warning "CERT-002 ADSI engine: $_"          # Fix R10
}
...
try {
    $csFilter  = '(&(objectClass=pKICertificateTemplate)(msPKI-Certificate-Name-Flag:1.2.840.113556.1.4.803:=1))'
    $csProps   = @("name","distinguishedName","cn","displayName","msPKI-Certificate-Name-Flag")   # PS array of double-quoted strings
    $csCheckId = 'CERT-002'
    ...
    Write-Warning "CERT-002 C# engine: $_"             # Fix R10
}
...
Write-Host ""
Write-Host "=== CERT-002: $($uniqueResults.Count) unique findings ===" -ForegroundColor Cyan
$uniqueResults | Format-List

# Fix R14: Fixed 5-field schema enforced before CSV export
$csvPath = Join-Path $env:TEMP "CERT-002_results.csv"
$uniqueResults |
