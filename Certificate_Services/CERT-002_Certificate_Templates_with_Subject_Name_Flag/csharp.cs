...
class Program
{
    ...
    static void Main()
    {
        ...
        try
        {
            ...
            {
                string targetNC = rootEntry.Properties["configurationNamingContext"].Value.ToString();
                ...
                using (DirectorySearcher searcher = new DirectorySearcher(searchEntry))
                {
                    ...
                    {
                        Console.WriteLine("CERT-002: found " + results.Count + " objects");
        ...
        {
            Console.Error.WriteLine("CERT-002 error: " + ex.Message);
            Environment.Exit(1);
        }
    }
}
