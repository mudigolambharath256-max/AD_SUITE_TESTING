...
Write-Host "CERT-010: found $($results.Count) objects"
...
try {
    ...
    foreach ($r in $bhRaw) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                adSuiteCheckName  = 'Certificate Services Check 10'
    ...
    @{ data = $bhNodes.ToArray()
       meta = @{ type = 'containers'; count = $bhNodes.Count; version = 5; methods = 0 }
        ...
        Out-File -FilePath (Join-Path $bhDir "CERT-010_$bhTs.json") -Encoding UTF8 -Force

# Fix R10: No silent catch — warn on failure
} catch { Write-Warning "CERT-010 BloodHound export error: $_" }
