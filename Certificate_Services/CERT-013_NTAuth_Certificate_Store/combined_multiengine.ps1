...
Write-Host "=== Multi-Engine Execution: NTAuth Certificate Store ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        $ldapFilter = '(&(objectClass=certificationAuthority)(cn=NTAuthCertificates))'
        $props = @('name', 'distinguishedName', 'cn', 'cACertificate')

        Get-ADObject -LDAPFilter $ldapFilter -Properties $props -ErrorAction Stop |
          Select-Object name, distinguishedName, cn, cACertificate |
          Sort-Object name |
          ForEach-Object { $_ }
...
public class Program
{
  ...
  {
    string filter = @"(&(objectClass=certificationAuthority)(cn=NTAuthCertificates))";
    string[] props = new string[] { "name", "distinguishedName", "cn", "cACertificate" };
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                adSuiteCheckName  = 'NTAuth_Certificate_Store'
    ...
    $bhFile = Join-Path $bhDir "CERT-013_$bhTs.json"
    @{
