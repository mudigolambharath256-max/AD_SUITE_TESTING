...
try {
    ...
    Get-ADObject -LDAPFilter '(&(objectClass=pKICertificateTemplate))' `
                 -Properties @('name','distinguishedName','cn','displayName','pKIExtendedKeyUsage') `
                 -SearchBase $targetNC `
                 -ErrorAction Stop |
    ForEach-Object {
    ...
    Write-Warning "CERT-001 PowerShell engine: $_"   # Fix R10: no silent catch
}
...
try {
    ...
    @('name','distinguishedName','cn','displayName','pKIExtendedKeyUsage') | ForEach-Object { [void]$adsiS.PropertiesToLoad.Add($_) }
    ...
    Write-Warning "CERT-001 ADSI engine: $_"          # Fix R10
}
...
try {
    ...
    $csProps   = @("name","distinguishedName","cn","displayName","pKIExtendedKeyUsage")   # PS array of double-quoted strings
    $csCheckId = 'CERT-001'
    ...
    Write-Warning "CERT-001 C# engine: $_"             # Fix R10
}
...
Write-Host ""
Write-Host "=== CERT-001: $($uniqueResults.Count) unique findings ===" -ForegroundColor Cyan
$uniqueResults | Format-List

# Fix R14: Fixed 5-field schema enforced before CSV export
$csvPath = Join-Path $env:TEMP "CERT-001_results.csv"
$uniqueResults |
