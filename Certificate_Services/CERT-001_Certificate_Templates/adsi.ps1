...
Write-Host "CERT-001: found $($results.Count) objects"
...
$results | ForEach-Object {
    ...
    $obj = [PSCustomObject]@{
        ...
        pKIExtendedKeyUsage = ($p['pkiextendedkeyusage'] | Select-Object -First 1)
    }
...
try {
    ...
    $bhS.Filter   = '(&(objectClass=pKICertificateTemplate))'
    $bhS.PageSize = 1000
    ...
    foreach ($r in $bhRaw) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                adSuiteCheckName  = 'Certificate Templates'
                adSuiteSeverity   = 'info'
    ...
    @{ data = $bhNodes.ToArray()
       meta = @{ type = 'containers'; count = $bhNodes.Count; version = 5; methods = 0 }
        ...
        Out-File -FilePath (Join-Path $bhDir "CERT-001_$bhTs.json") -Encoding UTF8 -Force

# Fix R10: No silent catch — warn on failure
} catch { Write-Warning "CERT-001 BloodHound export error: $_" }
