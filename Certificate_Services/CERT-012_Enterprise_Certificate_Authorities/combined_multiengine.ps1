...
Write-Host "=== Multi-Engine Execution: Enterprise Certificate Authorities ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        $props = @('name', 'distinguishedName', 'cn', 'dNSHostName', 'cACertificate', 'certificateTemplates')

        Get-ADObject -LDAPFilter $ldapFilter -Properties $props -ErrorAction Stop |
          Select-Object name, distinguishedName, cn, dNSHostName, cACertificate, certificateTemplates |
          Sort-Object name |
          ForEach-Object { $_ }
...
public class Program
{
  ...
  {
    string filter = @"(objectClass=pKIEnrollmentService)";
    string[] props = new string[] { "name", "distinguishedName", "cn", "dNSHostName", "cACertificate", "certificateTemplates" };
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'CERT-012'
                adSuiteCheckName  = 'Enterprise_Certificate_Authorities'
    ...
    $bhFile = Join-Path $bhDir "CERT-012_$bhTs.json"
    @{
