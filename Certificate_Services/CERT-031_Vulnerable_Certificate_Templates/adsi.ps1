...
Write-Host "Found $($results.Count) objects for check: Vulnerable Certificate Templates" -ForegroundColor Cyan

$results | ForEach-Object {
    $props = $_.Properties
    [PSCustomObject]@{
        CheckID = 'CERT-031'
        CheckName = 'Vulnerable Certificate Templates'
...
try {
    ...
    if ($results -and $results.Count -gt 0) {
        ...
        $bhFile = Join-Path $bhDir "CERT-031_nodes.json"
