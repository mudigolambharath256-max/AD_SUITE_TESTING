...
# Check: Certificate Services Check 39
...
Write-Host "=== Certificate Services Check 39 ===" -ForegroundColor Cyan
...
try {
    ...
    Write-Warning "CERT-039 PowerShell engine: $_"   # Fix R10: no silent catch
}
...
try {
    ...
    Write-Warning "CERT-039 ADSI engine: $_"          # Fix R10
}
...
try {
    ...
    Write-Warning "CERT-039 C# engine: $_"             # Fix R10
}
...
Write-Host ""
Write-Host "=== CERT-039: $($uniqueResults.Count) unique findings ===" -ForegroundColor Cyan
$uniqueResults | Format-List

# Fix R14: Fixed 5-field schema enforced before CSV export
$csvPath = Join-Path $env:TEMP "CERT-039_results.csv"
$uniqueResults |
