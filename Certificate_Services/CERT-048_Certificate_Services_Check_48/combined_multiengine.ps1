...
# Check: Certificate Services Check 48
...
Write-Host "=== Certificate Services Check 48 ===" -ForegroundColor Cyan
...
try {
    ...
    Get-ADObject -LDAPFilter '(&(objectClass=pKICertificateTemplate))' `
                 -Properties @('name','distinguishedName','cn') `
                 -SearchBase $targetNC `
                 -ErrorAction Stop |
    ForEach-Object {
    ...
    Write-Warning "CERT-048 PowerShell engine: $_"   # Fix R10: no silent catch
}
...
try {
    ...
    $adsiS.Filter   = '(&(objectClass=pKICertificateTemplate))'
    $adsiS.PageSize = 1000
    $adsiS.PropertiesToLoad.Clear()
    @('name','distinguishedName','cn') | ForEach-Object { [void]$adsiS.PropertiesToLoad.Add($_) }
    ...
    Write-Warning "CERT-048 ADSI engine: $_"          # Fix R10
}
...
try {
    $csFilter  = '(&(objectClass=pKICertificateTemplate))'
    $csProps   = @("name","distinguishedName","cn")   # PS array of double-quoted strings
    $csCheckId = 'CERT-048'
    ...
    Write-Warning "CERT-048 C# engine: $_"             # Fix R10
}
...
Write-Host ""
Write-Host "=== CERT-048: $($uniqueResults.Count) unique findings ===" -ForegroundColor Cyan
$uniqueResults | Format-List

# Fix R14: Fixed 5-field schema enforced before CSV export
$csvPath = Join-Path $env:TEMP "CERT-048_results.csv"
$uniqueResults |
