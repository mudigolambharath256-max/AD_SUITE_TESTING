...
# Check: Certificate Services Check 50
...
Write-Host "=== Certificate Services Check 50 ===" -ForegroundColor Cyan
...
try {
    ...
    Write-Warning "CERT-050 PowerShell engine: $_"   # Fix R10: no silent catch
}
...
try {
    ...
    Write-Warning "CERT-050 ADSI engine: $_"          # Fix R10
}
...
try {
    ...
    Write-Warning "CERT-050 C# engine: $_"             # Fix R10
}
...
Write-Host ""
Write-Host "=== CERT-050: $($uniqueResults.Count) unique findings ===" -ForegroundColor Cyan
$uniqueResults | Format-List

# Fix R14: Fixed 5-field schema enforced before CSV export
$csvPath = Join-Path $env:TEMP "CERT-050_results.csv"
$uniqueResults |
