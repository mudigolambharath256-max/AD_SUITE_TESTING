...
@('name', 'distinguishedName', 'cn', 'displayName', 'msPKI-Template-Schema-Version', 'msPKI-Template-Minor-Revision', 'objectSid', 'samAccountName') | ForEach-Object { [void]$searcher.PropertiesToLoad.Add($_) }
...
try {
    ...
    if ($results -and $results.Count -gt 0) {
        ...
        $bhFile = Join-Path $bhDir "CERT-030_nodes.json"
    ...
    Write-Warning "[BloodHound] Export failed: # Check: Certificate Templates Version
# Category: Certificate Services
# Severity: info
# ID: CERT-030
# Requirements: None
