...
Write-Host "=== Multi-Engine Execution: Certificate Templates Version ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        $props = @('name', 'distinguishedName', 'cn', 'displayName', 'msPKI-Template-Schema-Version', 'msPKI-Template-Minor-Revision')

        Get-ADObject -LDAPFilter $ldapFilter -Properties $props -ErrorAction Stop |
          Select-Object name, distinguishedName, cn, displayName, msPKI-Template-Schema-Version, msPKI-Template-Minor-Revision |
          Sort-Object name |
          ForEach-Object { $_ }
...
public class Program
{
  public static void Run()
  {
    ...
    string[] props = new string[] { "name", "distinguishedName", "cn", "displayName", "msPKI-Template-Schema-Version", "msPKI-Template-Minor-Revision" };
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'CERT-030'
                adSuiteCheckName  = 'Certificate_Templates_Version'
    ...
    $bhFile = Join-Path $bhDir "CERT-030_$bhTs.json"
    @{
