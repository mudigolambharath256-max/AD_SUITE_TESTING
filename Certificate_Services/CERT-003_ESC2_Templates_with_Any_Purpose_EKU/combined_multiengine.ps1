...
Write-Host "=== Multi-Engine Execution: ESC2: Templates with Any Purpose EKU ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        $ldapFilter = '(&(objectClass=pKICertificateTemplate)(|(pKIExtendedKeyUsage=2.5.29.37.0)(!(pKIExtendedKeyUsage=*))))'
        ...
        Get-ADObject -LDAPFilter $ldapFilter -Properties $props -ErrorAction Stop |
          Select-Object name, distinguishedName, cn, displayName, pKIExtendedKeyUsage |
          Sort-Object name |
          ForEach-Object { $_ }
...
public class Program
{
  ...
  {
    string filter = @"(&(objectClass=pKICertificateTemplate)(|(pKIExtendedKeyUsage=2.5.29.37.0)(!(pKIExtendedKeyUsage=*))))";
    string[] props = new string[] { "name", "distinguishedName", "cn", "displayName", "pKIExtendedKeyUsage" };
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'CERT-003'
                adSuiteCheckName  = 'ESC2_Templates_with_Any_Purpose_EKU'
    ...
    $bhFile = Join-Path $bhDir "CERT-003_$bhTs.json"
    @{
