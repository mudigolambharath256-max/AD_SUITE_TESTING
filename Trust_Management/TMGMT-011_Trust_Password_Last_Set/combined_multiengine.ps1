...
Write-Host "=== TMGMT-011: Trust Password Last Set ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'TMGMT-011'}}, `
            @{N='CheckName'; E={'Trust Password Last Set'}}, `
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'TMGMT-011'
                adSuiteCheckName  = 'Trust_Password_Last_Set'
    ...
    $bhFile = Join-Path $bhDir "TMGMT-011_$bhTs.json"
    @{
