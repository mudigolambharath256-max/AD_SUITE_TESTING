...
Write-Host "=== TMGMT-026: Trust RC4 Encryption Usage ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'TMGMT-026'}}, `
            @{N='CheckName'; E={'Trust RC4 Encryption Usage'}}, `
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'TMGMT-026'
                adSuiteCheckName  = 'Trust_RC4_Encryption_Usage'
    ...
    $bhFile = Join-Path $bhDir "TMGMT-026_$bhTs.json"
    @{
