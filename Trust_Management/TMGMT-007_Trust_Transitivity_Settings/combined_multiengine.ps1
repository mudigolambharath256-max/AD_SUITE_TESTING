...
Write-Host "=== TMGMT-007: Trust Transitivity Settings ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'TMGMT-007'}}, `
            @{N='CheckName'; E={'Trust Transitivity Settings'}}, `
...
if (-not $useADModule) {
    ...
    try {
        ...
        $results = $adsiResults | ForEach-Object {
            ...
            [PSCustomObject]@{
                CheckID           = 'TMGMT-007'
                CheckName         = 'Trust Transitivity Settings'
                Method            = 'ADSI'
                Domain            = $domainNC
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'TMGMT-007'
                adSuiteCheckName  = 'Trust_Transitivity_Settings'
    ...
    $bhFile = Join-Path $bhDir "TMGMT-007_$bhTs.json"
    @{
