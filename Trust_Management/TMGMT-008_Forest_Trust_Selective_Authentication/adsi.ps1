...
@('name', 'distinguishedName', 'trustAttributes', 'trustDirection', 'trustType', 'trustPartner', 'flatName', 'whenCreated', 'whenChanged', 'objectClass', 'objectSid', 'samAccountName') | ForEach-Object {
...
Write-Host "Found $($results.Count) objects for check: Forest Trust Selective Authentication" -ForegroundColor Cyan
...
try {
    ...
    if ($results -and $results.Count -gt 0) {
        ...
        $bhFile = Join-Path $bhDir "TMGMT-008_nodes.json"
