...
Write-Host "=== TMGMT-008: Forest Trust Selective Authentication ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'TMGMT-008'}}, `
            @{N='CheckName'; E={'Forest Trust Selective Authentication'}}, `
...
if (-not $useADModule) {
    ...
    try {
        ...
        $results = $adsiResults | ForEach-Object {
            ...
            [PSCustomObject]@{
                CheckID           = 'TMGMT-008'
                CheckName         = 'Forest Trust Selective Authentication'
                Method            = 'ADSI'
                Domain            = $domainNC
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'TMGMT-008'
                adSuiteCheckName  = 'Forest_Trust_Selective_Authentication'
    ...
    $bhFile = Join-Path $bhDir "TMGMT-008_$bhTs.json"
    @{
