...
Write-Host "Found $($results.Count) objects for check: Trust Quarantine Status" -ForegroundColor Cyan
...
$results | ForEach-Object {
    ...
    [PSCustomObject]@{
        CheckID           = 'TMGMT-013'
        CheckName         = 'Trust Quarantine Status'
...
try {
    ...
    if ($results -and $results.Count -gt 0) {
        ...
        $bhFile = Join-Path $bhDir "TMGMT-013_nodes.json"
