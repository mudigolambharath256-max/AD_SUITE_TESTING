...
Write-Host "=== TMGMT-013: Trust Quarantine Status ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'TMGMT-013'}}, `
            @{N='CheckName'; E={'Trust Quarantine Status'}}, `
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'TMGMT-013'
                adSuiteCheckName  = 'Trust_Quarantine_Status'
    ...
    $bhFile = Join-Path $bhDir "TMGMT-013_$bhTs.json"
    @{
