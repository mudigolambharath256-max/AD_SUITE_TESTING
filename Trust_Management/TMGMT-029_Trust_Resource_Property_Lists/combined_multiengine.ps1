...
Write-Host "=== TMGMT-029: Trust Resource Property Lists ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'TMGMT-029'}}, `
            @{N='CheckName'; E={'Trust Resource Property Lists'}}, `
...
if (-not $useADModule) {
    ...
    try {
        ...
        $results = $adsiResults | ForEach-Object {
            ...
            [PSCustomObject]@{
                CheckID           = 'TMGMT-029'
                CheckName         = 'Trust Resource Property Lists'
                Method            = 'ADSI'
                Domain            = $domainNC
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'TMGMT-029'
                adSuiteCheckName  = 'Trust_Resource_Property_Lists'
    ...
    $bhFile = Join-Path $bhDir "TMGMT-029_$bhTs.json"
    @{
