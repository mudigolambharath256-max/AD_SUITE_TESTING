...
Write-Host "=== TMGMT-010: Realm Trust Relationships ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'TMGMT-010'}}, `
            @{N='CheckName'; E={'Realm Trust Relationships'}}, `
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'TMGMT-010'
                adSuiteCheckName  = 'Realm_Trust_Relationships'
    ...
    $bhFile = Join-Path $bhDir "TMGMT-010_$bhTs.json"
    @{
