...
Write-Host "Found $($results.Count) objects for check: Realm Trust Relationships" -ForegroundColor Cyan
...
$results | ForEach-Object {
    ...
    [PSCustomObject]@{
        CheckID           = 'TMGMT-010'
        CheckName         = 'Realm Trust Relationships'
...
try {
    ...
    if ($results -and $results.Count -gt 0) {
        ...
        $bhFile = Join-Path $bhDir "TMGMT-010_nodes.json"
