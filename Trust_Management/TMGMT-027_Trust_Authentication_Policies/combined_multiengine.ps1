...
Write-Host "=== TMGMT-027: Trust Authentication Policies ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'TMGMT-027'}}, `
            @{N='CheckName'; E={'Trust Authentication Policies'}}, `
...
if (-not $useADModule) {
    ...
    try {
        ...
        $results = $adsiResults | ForEach-Object {
            ...
            [PSCustomObject]@{
                CheckID           = 'TMGMT-027'
                CheckName         = 'Trust Authentication Policies'
                Method            = 'ADSI'
                Domain            = $domainNC
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'TMGMT-027'
                adSuiteCheckName  = 'Trust_Authentication_Policies'
    ...
    $bhFile = Join-Path $bhDir "TMGMT-027_$bhTs.json"
    @{
