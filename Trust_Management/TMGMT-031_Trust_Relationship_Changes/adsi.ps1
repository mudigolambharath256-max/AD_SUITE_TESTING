...
Write-Host "Found $($results.Count) objects for check: Trust Relationship Changes" -ForegroundColor Cyan

$results | ForEach-Object {
    $props = $_.Properties
    [PSCustomObject]@{
        CheckID = 'TMGMT-031'
        CheckName = 'Trust Relationship Changes'
...
try {
    ...
    if ($results -and $results.Count -gt 0) {
        ...
        $bhFile = Join-Path $bhDir "TMGMT-031_nodes.json"
