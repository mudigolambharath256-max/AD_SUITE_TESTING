...
Write-Host "=== TMGMT-016: Trust Partner Domain Controllers ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'TMGMT-016'}}, `
            @{N='CheckName'; E={'Trust Partner Domain Controllers'}}, `
...
if (-not $useADModule) {
    ...
    try {
        ...
        $results = $adsiResults | ForEach-Object {
            ...
            [PSCustomObject]@{
                CheckID           = 'TMGMT-016'
                CheckName         = 'Trust Partner Domain Controllers'
                Method            = 'ADSI'
                Domain            = $domainNC
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'TMGMT-016'
                adSuiteCheckName  = 'Trust_Partner_Domain_Controllers'
    ...
    $bhFile = Join-Path $bhDir "TMGMT-016_$bhTs.json"
    @{
