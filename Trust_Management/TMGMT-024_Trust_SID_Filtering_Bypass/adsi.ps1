...
Write-Host "Found $($results.Count) objects for check: Trust SID Filtering Bypass" -ForegroundColor Cyan
...
$results | ForEach-Object {
    ...
    [PSCustomObject]@{
        CheckID           = 'TMGMT-024'
        CheckName         = 'Trust SID Filtering Bypass'
...
try {
    ...
    if ($results -and $results.Count -gt 0) {
        ...
        $bhFile = Join-Path $bhDir "TMGMT-024_nodes.json"
