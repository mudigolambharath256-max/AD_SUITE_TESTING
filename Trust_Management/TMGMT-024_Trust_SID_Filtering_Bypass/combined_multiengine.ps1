...
Write-Host "=== TMGMT-024: Trust SID Filtering Bypass ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'TMGMT-024'}}, `
            @{N='CheckName'; E={'Trust SID Filtering Bypass'}}, `
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'TMGMT-024'
                adSuiteCheckName  = 'Trust_SID_Filtering_Bypass'
    ...
    $bhFile = Join-Path $bhDir "TMGMT-024_$bhTs.json"
    @{
