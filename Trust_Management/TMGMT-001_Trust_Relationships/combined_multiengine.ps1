...
Write-Host "=== TMGMT-001: Trust Relationships ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'TMGMT-001'}}, `
            @{N='CheckName'; E={'Trust Relationships'}}, `
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'TMGMT-001'
                adSuiteCheckName  = 'Trust_Relationships'
    ...
    $bhFile = Join-Path $bhDir "TMGMT-001_$bhTs.json"
    @{
