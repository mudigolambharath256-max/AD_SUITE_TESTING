...
Write-Host "=== TMGMT-015: Cross Forest Trust Validation ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'TMGMT-015'}}, `
            @{N='CheckName'; E={'Cross Forest Trust Validation'}}, `
...
if (-not $useADModule) {
    ...
    try {
        ...
        $results = $adsiResults | ForEach-Object {
            ...
            [PSCustomObject]@{
                CheckID           = 'TMGMT-015'
                CheckName         = 'Cross Forest Trust Validation'
                Method            = 'ADSI'
                Domain            = $domainNC
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'TMGMT-015'
                adSuiteCheckName  = 'Cross_Forest_Trust_Validation'
    ...
    $bhFile = Join-Path $bhDir "TMGMT-015_$bhTs.json"
    @{
