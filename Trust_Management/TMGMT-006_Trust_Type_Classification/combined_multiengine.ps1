...
Write-Host "=== TMGMT-006: Trust Type Classification ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'TMGMT-006'}}, `
            @{N='CheckName'; E={'Trust Type Classification'}}, `
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'TMGMT-006'
                adSuiteCheckName  = 'Trust_Type_Classification'
    ...
    $bhFile = Join-Path $bhDir "TMGMT-006_$bhTs.json"
    @{
