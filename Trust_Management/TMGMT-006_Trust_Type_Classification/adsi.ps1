...
Write-Host "Found $($results.Count) objects for check: Trust Type Classification" -ForegroundColor Cyan
...
$results | ForEach-Object {
    ...
    [PSCustomObject]@{
        CheckID           = 'TMGMT-006'
        CheckName         = 'Trust Type Classification'
...
try {
    ...
    if ($results -and $results.Count -gt 0) {
        ...
        $bhFile = Join-Path $bhDir "TMGMT-006_nodes.json"
