...
Write-Host "=== TMGMT-030: Trust Central Access Policies ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'TMGMT-030'}}, `
            @{N='CheckName'; E={'Trust Central Access Policies'}}, `
...
if (-not $useADModule) {
    ...
    try {
        ...
        $results = $adsiResults | ForEach-Object {
            ...
            [PSCustomObject]@{
                CheckID           = 'TMGMT-030'
                CheckName         = 'Trust Central Access Policies'
                Method            = 'ADSI'
                Domain            = $domainNC
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'TMGMT-030'
                adSuiteCheckName  = 'Trust_Central_Access_Policies'
    ...
    $bhFile = Join-Path $bhDir "TMGMT-030_$bhTs.json"
    @{
