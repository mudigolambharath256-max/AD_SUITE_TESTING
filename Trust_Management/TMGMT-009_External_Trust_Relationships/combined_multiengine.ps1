...
Write-Host "=== TMGMT-009: External Trust Relationships ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'TMGMT-009'}}, `
            @{N='CheckName'; E={'External Trust Relationships'}}, `
...
if (-not $useADModule) {
    ...
    try {
        ...
        $results = $adsiResults | ForEach-Object {
            ...
            [PSCustomObject]@{
                CheckID           = 'TMGMT-009'
                CheckName         = 'External Trust Relationships'
                Method            = 'ADSI'
                Domain            = $domainNC
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'TMGMT-009'
                adSuiteCheckName  = 'External_Trust_Relationships'
    ...
    $bhFile = Join-Path $bhDir "TMGMT-009_$bhTs.json"
    @{
