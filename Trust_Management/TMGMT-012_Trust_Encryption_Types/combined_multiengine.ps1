...
Write-Host "=== TMGMT-012: Trust Encryption Types ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'TMGMT-012'}}, `
            @{N='CheckName'; E={'Trust Encryption Types'}}, `
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'TMGMT-012'
                adSuiteCheckName  = 'Trust_Encryption_Types'
    ...
    $bhFile = Join-Path $bhDir "TMGMT-012_$bhTs.json"
    @{
