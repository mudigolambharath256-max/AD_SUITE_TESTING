...
Write-Host "=== TMGMT-022: Orphaned Trust Objects ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'TMGMT-022'}}, `
            @{N='CheckName'; E={'Orphaned Trust Objects'}}, `
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'TMGMT-022'
                adSuiteCheckName  = 'Orphaned_Trust_Objects'
    ...
    $bhFile = Join-Path $bhDir "TMGMT-022_$bhTs.json"
    @{
