...
Write-Host "=== TMGMT-020: Trust Posix Offset ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'TMGMT-020'}}, `
            @{N='CheckName'; E={'Trust Posix Offset'}}, `
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'TMGMT-020'
                adSuiteCheckName  = 'Trust_Posix_Offset'
    ...
    $bhFile = Join-Path $bhDir "TMGMT-020_$bhTs.json"
    @{
