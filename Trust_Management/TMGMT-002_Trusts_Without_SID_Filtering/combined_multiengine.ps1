...
Write-Host "=== TMGMT-002: Trusts Without SID Filtering ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        # Execute query
        $adObjects = Get-ADObject -LDAPFilter '(objectClass=trustedDomain)' `
                                   -Properties name,distinguishedName,trustAttributes,trustDirection,trustType,trustPartner,flatName,whenCreated,whenChanged,objectClass `
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'TMGMT-002'}}, `
            @{N='CheckName'; E={'Trusts Without SID Filtering'}}, `
...
if (-not $useADModule) {
    ...
    try {
        ...
        @('name', 'distinguishedName', 'trustAttributes', 'trustDirection', 'trustType', 'trustPartner', 'flatName', 'whenCreated', 'whenChanged', 'objectClass', 'objectSid') |
        ...
        $results = $adsiResults | ForEach-Object {
            ...
            [PSCustomObject]@{
                CheckID           = 'TMGMT-002'
                CheckName         = 'Trusts Without SID Filtering'
                Method            = 'ADSI'
                Domain            = $domainNC
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'TMGMT-002'
                adSuiteCheckName  = 'Trusts_Without_SID_Filtering'
    ...
    $bhFile = Join-Path $bhDir "TMGMT-002_$bhTs.json"
    @{
