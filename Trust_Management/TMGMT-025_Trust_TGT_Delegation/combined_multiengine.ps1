...
Write-Host "=== TMGMT-025: Trust TGT Delegation ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'TMGMT-025'}}, `
            @{N='CheckName'; E={'Trust TGT Delegation'}}, `
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'TMGMT-025'
                adSuiteCheckName  = 'Trust_TGT_Delegation'
    ...
    $bhFile = Join-Path $bhDir "TMGMT-025_$bhTs.json"
    @{
