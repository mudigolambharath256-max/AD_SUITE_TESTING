...
Write-Host "Found $($results.Count) objects for check: Trust TGT Delegation" -ForegroundColor Cyan
...
$results | ForEach-Object {
    ...
    [PSCustomObject]@{
        CheckID           = 'TMGMT-025'
        CheckName         = 'Trust TGT Delegation'
...
try {
    ...
    if ($results -and $results.Count -gt 0) {
        ...
        $bhFile = Join-Path $bhDir "TMGMT-025_nodes.json"
