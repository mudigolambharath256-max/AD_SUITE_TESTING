...
Write-Host "=== TMGMT-005: Trust Direction Configuration ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'TMGMT-005'}}, `
            @{N='CheckName'; E={'Trust Direction Configuration'}}, `
...
if (-not $useADModule) {
    ...
    try {
        ...
        $results = $adsiResults | ForEach-Object {
            ...
            [PSCustomObject]@{
                CheckID           = 'TMGMT-005'
                CheckName         = 'Trust Direction Configuration'
                Method            = 'ADSI'
                Domain            = $domainNC
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'TMGMT-005'
                adSuiteCheckName  = 'Trust_Direction_Configuration'
    ...
    $bhFile = Join-Path $bhDir "TMGMT-005_$bhTs.json"
    @{
