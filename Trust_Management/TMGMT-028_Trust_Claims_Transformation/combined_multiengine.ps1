...
Write-Host "=== TMGMT-028: Trust Claims Transformation ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'TMGMT-028'}}, `
            @{N='CheckName'; E={'Trust Claims Transformation'}}, `
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'TMGMT-028'
                adSuiteCheckName  = 'Trust_Claims_Transformation'
    ...
    $bhFile = Join-Path $bhDir "TMGMT-028_$bhTs.json"
    @{
