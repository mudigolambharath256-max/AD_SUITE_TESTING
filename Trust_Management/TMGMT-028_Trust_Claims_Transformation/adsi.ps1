...
Write-Host "Found $($results.Count) objects for check: Trust Claims Transformation" -ForegroundColor Cyan
...
$results | ForEach-Object {
    ...
    [PSCustomObject]@{
        CheckID           = 'TMGMT-028'
        CheckName         = 'Trust Claims Transformation'
...
try {
    ...
    if ($results -and $results.Count -gt 0) {
        ...
        $bhFile = Join-Path $bhDir "TMGMT-028_nodes.json"
