...
Write-Host "=== TMGMT-018: Trust Name Suffix Routing ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'TMGMT-018'}}, `
            @{N='CheckName'; E={'Trust Name Suffix Routing'}}, `
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'TMGMT-018'
                adSuiteCheckName  = 'Trust_Name_Suffix_Routing'
    ...
    $bhFile = Join-Path $bhDir "TMGMT-018_$bhTs.json"
    @{
