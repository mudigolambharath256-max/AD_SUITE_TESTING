...
Write-Host "Found $($results.Count) objects for check: Trust Routing Status" -ForegroundColor Cyan
...
$results | ForEach-Object {
    ...
    [PSCustomObject]@{
        CheckID           = 'TMGMT-017'
        CheckName         = 'Trust Routing Status'
...
try {
    ...
    if ($results -and $results.Count -gt 0) {
        ...
        $bhFile = Join-Path $bhDir "TMGMT-017_nodes.json"
