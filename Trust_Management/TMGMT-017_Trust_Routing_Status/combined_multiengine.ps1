...
Write-Host "=== TMGMT-017: Trust Routing Status ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'TMGMT-017'}}, `
            @{N='CheckName'; E={'Trust Routing Status'}}, `
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'TMGMT-017'
                adSuiteCheckName  = 'Trust_Routing_Status'
    ...
    $bhFile = Join-Path $bhDir "TMGMT-017_$bhTs.json"
    @{
