...
Write-Host "Found $($results.Count) objects for check: Trusts with Weak Encryption" -ForegroundColor Cyan

$results | ForEach-Object {
    $props = $_.Properties
    [PSCustomObject]@{
        CheckID = 'TMGMT-032'
        CheckName = 'Trusts with Weak Encryption'
...
try {
    ...
    if ($results -and $results.Count -gt 0) {
        ...
        $bhFile = Join-Path $bhDir "TMGMT-032_nodes.json"
