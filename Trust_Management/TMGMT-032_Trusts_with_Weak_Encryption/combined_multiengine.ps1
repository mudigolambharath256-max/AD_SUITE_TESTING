...
Write-Host "=== TMGMT-032: Trusts with Weak Encryption ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    $results = Get-ADObject -LDAPFilter '(objectClass=trustedDomain)' -Properties name,distinguishedName,objectClass -SearchBase $SearchBase -SearchScope Subtree -ResultSetSize $null
} else {
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'TMGMT-032'
                adSuiteCheckName  = 'Trusts_with_Weak_Encryption'
    ...
    $bhFile = Join-Path $bhDir "TMGMT-032_$bhTs.json"
    @{
