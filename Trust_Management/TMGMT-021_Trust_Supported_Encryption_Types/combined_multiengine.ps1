...
Write-Host "=== TMGMT-021: Trust Supported Encryption Types ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'TMGMT-021'}}, `
            @{N='CheckName'; E={'Trust Supported Encryption Types'}}, `
...
if (-not $useADModule) {
    ...
    try {
        ...
        $results = $adsiResults | ForEach-Object {
            ...
            [PSCustomObject]@{
                CheckID           = 'TMGMT-021'
                CheckName         = 'Trust Supported Encryption Types'
                Method            = 'ADSI'
                Domain            = $domainNC
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'TMGMT-021'
                adSuiteCheckName  = 'Trust_Supported_Encryption_Types'
    ...
    $bhFile = Join-Path $bhDir "TMGMT-021_$bhTs.json"
    @{
