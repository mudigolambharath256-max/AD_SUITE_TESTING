...
Write-Host "=== TMGMT-023: Disabled Trust Relationships ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'TMGMT-023'}}, `
            @{N='CheckName'; E={'Disabled Trust Relationships'}}, `
...
if (-not $useADModule) {
    ...
    try {
        ...
        $results = $adsiResults | ForEach-Object {
            ...
            [PSCustomObject]@{
                CheckID           = 'TMGMT-023'
                CheckName         = 'Disabled Trust Relationships'
                Method            = 'ADSI'
                Domain            = $domainNC
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'TMGMT-023'
                adSuiteCheckName  = 'Disabled_Trust_Relationships'
    ...
    $bhFile = Join-Path $bhDir "TMGMT-023_$bhTs.json"
    @{
