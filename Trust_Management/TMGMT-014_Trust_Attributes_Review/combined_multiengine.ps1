...
Write-Host "=== TMGMT-014: Trust Attributes Review ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'TMGMT-014'}}, `
            @{N='CheckName'; E={'Trust Attributes Review'}}, `
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'TMGMT-014'
                adSuiteCheckName  = 'Trust_Attributes_Review'
    ...
    $bhFile = Join-Path $bhDir "TMGMT-014_$bhTs.json"
    @{
