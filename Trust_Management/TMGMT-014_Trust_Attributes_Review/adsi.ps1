...
Write-Host "Found $($results.Count) objects for check: Trust Attributes Review" -ForegroundColor Cyan
...
$results | ForEach-Object {
    ...
    [PSCustomObject]@{
        CheckID           = 'TMGMT-014'
        CheckName         = 'Trust Attributes Review'
...
try {
    ...
    if ($results -and $results.Count -gt 0) {
        ...
        $bhFile = Join-Path $bhDir "TMGMT-014_nodes.json"
