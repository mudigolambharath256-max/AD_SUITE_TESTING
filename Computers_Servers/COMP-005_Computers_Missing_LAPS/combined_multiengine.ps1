...
Write-Host "=== Multi-Engine Execution: Computers Missing LAPS ===" -ForegroundColor Cyan
Write-Host ""
...
public class Program
{
  ...
  {
    string filter = @"(&(objectCategory=computer)(!(userAccountControl:1.2.840.113556.1.4.803:=2))(operatingSystem=*Windows*)(!(ms-Mcs-AdmPwd=*)))";
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'COMP-005'
                adSuiteCheckName  = 'Computers_Missing_LAPS'
    ...
    $bhFile = Join-Path $bhDir "COMP-005_$bhTs.json"
    @{
