...
(@('name', 'distinguishedName', 'samAccountName', 'operatingSystem', 'userAccountControl', 'msDS-SupportedEncryptionTypes', 'objectSid') | ForEach-Object { [void]$searcher.PropertiesToLoad.Add($_) }
...
try {
    ...
    if ($results -and $results.Count -gt 0) {
        ...
        $bhFile = Join-Path $bhDir "COMP-015_nodes.json"
    ...
    Write-Warning "[BloodHound] Export failed: # Check: Computers Missing Encryption Types
# Category: Computers & Servers
# Severity: medium
# ID: COMP-015
# Requirements: None
