...
Write-Host "=== Multi-Engine Execution: Computers Missing Encryption Types ===" -ForegroundColor Cyan
Write-Host ""
...
public class Program
{
  ...
  {
    string filter = @"(&(objectCategory=computer)(!(userAccountControl:1.2.840.113556.1.4.803:=2))(!(msDS-SupportedEncryptionTypes=*)))";
    string[] props = new string[] { "name", "distinguishedName", "samAccountName", "operatingSystem" };
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'COMP-015'
                adSuiteCheckName  = 'Computers_Missing_Encryption_Types'
    ...
    $bhFile = Join-Path $bhDir "COMP-015_$bhTs.json"
    @{
