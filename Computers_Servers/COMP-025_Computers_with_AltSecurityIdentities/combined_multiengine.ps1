...
Write-Host "=== Multi-Engine Execution: Computers with AltSecurityIdentities ===" -ForegroundColor Cyan
Write-Host ""
...
public class Program
{
  ...
  {
    string filter = @"(&(objectCategory=computer)(!(userAccountControl:1.2.840.113556.1.4.803:=2))(altSecurityIdentities=*))";
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'COMP-025'
                adSuiteCheckName  = 'Computers_with_AltSecurityIdentities'
    ...
    $bhFile = Join-Path $bhDir "COMP-025_$bhTs.json"
    @{
