...
try {
$searcher = [ADSISearcher]'(&(objectCategory=computer)(!(userAccountControl:1.2.840.113556.1.4.803:=2))(description=*))'
...
$output = $results | ForEach-Object {
  $p = $_.Properties
  [PSCustomObject]@{
    Label = 'Computers with Description Containing Sensitive Info'
...
try {
    ...
    if ($results -and $results.Count -gt 0) {
        ...
        $bhFile = Join-Path $bhDir "COMP-022_nodes.json"
    ...
    Write-Warning "[BloodHound] Export failed: # Check: Computers with Description Containing Sensitive Info
# Category: Computers & Servers
# Severity: low
# ID: COMP-022
# Requirements: None
