...
Write-Host "=== Multi-Engine Execution: Computers with Description Containing Sensitive Info ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'COMP-022'
                adSuiteCheckName  = 'Computers_with_Description_Containing_Sensitive_Info'
    ...
    $bhFile = Join-Path $bhDir "COMP-022_$bhTs.json"
    @{
