...
Write-Host "=== Multi-Engine Execution: Computers with Service Principal Names ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        Get-ADObject -LDAPFilter $ldapFilter -Properties $props,userAccountControl,servicePrincipalName -ErrorAction Stop |
...
public class Program
{
  ...
  {
    string filter = @"(&(objectCategory=computer)(!(userAccountControl:1.2.840.113556.1.4.803:=2))(servicePrincipalName=*))";
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'COMP-024'
                adSuiteCheckName  = 'Computers_with_Service_Principal_Names'
    ...
    $bhFile = Join-Path $bhDir "COMP-024_$bhTs.json"
    @{
