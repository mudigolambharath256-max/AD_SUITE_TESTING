...
Write-Host "=== Multi-Engine Execution: Computers with KeyCredentialLink ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        Get-ADObject -LDAPFilter $ldapFilter -Properties $props,userAccountControl,msDS-KeyCredentialLink -ErrorAction Stop |
...
public class Program
{
  ...
  {
    string filter = @"(&(objectCategory=computer)(!(userAccountControl:1.2.840.113556.1.4.803:=2))(msDS-KeyCredentialLink=*))";
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'COMP-010'
                adSuiteCheckName  = 'Computers_with_KeyCredentialLink'
    ...
    $bhFile = Join-Path $bhDir "COMP-010_$bhTs.json"
    @{
