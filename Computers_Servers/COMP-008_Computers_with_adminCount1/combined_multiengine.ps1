...
Write-Host "=== Multi-Engine Execution: Computers with adminCount1 ===" -ForegroundColor Cyan
Write-Host ""
...
public class Program
{
  ...
  {
    string filter = @"(&(objectCategory=computer)(!(userAccountControl:1.2.840.113556.1.4.803:=2))(adminCount=1))";
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'COMP-008'
                adSuiteCheckName  = 'Computers_with_adminCount1'
    ...
    $bhFile = Join-Path $bhDir "COMP-008_$bhTs.json"
    @{
