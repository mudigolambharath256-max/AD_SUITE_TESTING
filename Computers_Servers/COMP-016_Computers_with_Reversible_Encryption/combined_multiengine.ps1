...
Write-Host "=== Multi-Engine Execution: Computers with Reversible Encryption ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    $adsiResults = @(
        ...
        $searcher = [ADSISearcher]'(&(objectCategory=computer)(!(userAccountControl:1.2.840.113556.1.4.803:=2))(userAccountControl:1.2.840.113556.1.4.803:=128))'
        ...
        $searcher.FindAll() | ForEach-Object {
          $p = $_.Properties
          [PSCustomObject]@{
            Label = 'Computers with Reversible Encryption'
            Name = $p['name'][0]
...
try {
    ...
    $bhFile = Join-Path $bhDir "COMP-016_$bhTs.json"
    @{
