...
Write-Host "=== Multi-Engine Execution: Computers Running Unsupported OS ===" -ForegroundColor Cyan
Write-Host ""
...
public class Program
{
  ...
  {
    string filter = @"(&(objectCategory=computer)(!(userAccountControl:1.2.840.113556.1.4.803:=2))(|(operatingSystem=*XP*)(operatingSystem=*Vista*)(operatingSystem=*Windows 7*)(operatingSystem=*2003*)(operatingSystem=*2008*)(operatingSystem=*2012 *)))";
    string[] props = new string[] { "name", "distinguishedName", "samAccountName", "operatingSystem", "operatingSystemVersion", "lastLogonTimestamp" };
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'COMP-006'
                adSuiteCheckName  = 'Computers_Running_Unsupported_OS'
    ...
    $bhFile = Join-Path $bhDir "COMP-006_$bhTs.json"
    @{
