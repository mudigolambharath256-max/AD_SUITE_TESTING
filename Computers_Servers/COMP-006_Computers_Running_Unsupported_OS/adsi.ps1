...
(@('name', 'distinguishedName', 'samAccountName', 'operatingSystem', 'operatingSystemVersion', 'lastLogonTimestamp', 'userAccountControl', 'objectSid') | ForEach-Object { [void]$searcher.PropertiesToLoad.Add($_) }
...
try {
    ...
    if ($results -and $results.Count -gt 0) {
        ...
        $bhFile = Join-Path $bhDir "COMP-006_nodes.json"
    ...
    Write-Warning "[BloodHound] Export failed: # Check: Computers Running Unsupported OS
# Category: Computers & Servers
# Severity: critical
# ID: COMP-006
# Requirements: None
