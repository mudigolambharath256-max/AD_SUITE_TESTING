...
(@('name', 'distinguishedName', 'samAccountName', 'pwdLastSet', 'operatingSystem', 'userAccountControl', 'objectSid') | ForEach-Object { [void]$searcher.PropertiesToLoad.Add($_) }
...
try {
    ...
    if ($results -and $results.Count -gt 0) {
        ...
        $bhFile = Join-Path $bhDir "COMP-026_nodes.json"
    ...
    Write-Warning "[BloodHound] Export failed: # Check: Computer Accounts with Old Password (1+ Year)
# Category: Computers & Servers
# Severity: medium
# ID: COMP-026
# Requirements: None
