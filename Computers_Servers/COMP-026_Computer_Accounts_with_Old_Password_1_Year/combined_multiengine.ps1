...
Write-Host "=== Multi-Engine Execution: Computer Accounts with Old Password (1+ Year) ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        $props = @('name', 'distinguishedName', 'samAccountName', 'pwdLastSet', 'operatingSystem')

        Get-ADObject -LDAPFilter $ldapFilter -Properties $props,userAccountControl -ErrorAction Stop |
          Select-Object name, distinguishedName, samAccountName, pwdLastSet, operatingSystem |
          Sort-Object name |
          ForEach-Object { $_ }
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'COMP-026'
                adSuiteCheckName  = 'Computer_Accounts_with_Old_Password_1_Year'
    ...
    $bhFile = Join-Path $bhDir "COMP-026_$bhTs.json"
    @{
