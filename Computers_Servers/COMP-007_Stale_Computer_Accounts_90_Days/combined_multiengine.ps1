...
Write-Host "=== Multi-Engine Execution: Stale Computer Accounts (90+ Days) ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    $adsiResults = @(
        ...
        (@('name', 'distinguishedName', 'samAccountName', 'lastLogonTimestamp', 'pwdLastSet', 'operatingSystem', 'userAccountControl') | ForEach-Object { [void]$searcher.PropertiesToLoad.Add($_) })
...
public class Program
{
  public static void Run()
  {
    ...
    string[] props = new string[] { "name", "distinguishedName", "samAccountName", "lastLogonTimestamp", "pwdLastSet", "operatingSystem" };
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'COMP-007'
                adSuiteCheckName  = 'Stale_Computer_Accounts_90_Days'
    ...
    $bhFile = Join-Path $bhDir "COMP-007_$bhTs.json"
    @{
