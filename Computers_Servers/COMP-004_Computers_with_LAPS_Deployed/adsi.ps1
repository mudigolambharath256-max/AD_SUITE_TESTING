...
(@('name', 'distinguishedName', 'samAccountName', 'ms-Mcs-AdmPwdExpirationTime', 'operatingSystem', 'userAccountControl', 'objectSid') | ForEach-Object { [void]$searcher.PropertiesToLoad.Add($_) }
...
try {
    ...
    if ($results -and $results.Count -gt 0) {
        ...
        $bhFile = Join-Path $bhDir "COMP-004_nodes.json"
    ...
    Write-Warning "[BloodHound] Export failed: # Check: Computers with LAPS Deployed
# Category: Computers & Servers
# Severity: info
# ID: COMP-004
# Requirements: None
