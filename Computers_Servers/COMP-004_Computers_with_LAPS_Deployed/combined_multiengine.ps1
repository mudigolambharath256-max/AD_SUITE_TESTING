...
Write-Host "=== Multi-Engine Execution: Computers with LAPS Deployed ===" -ForegroundColor Cyan
Write-Host ""
...
public class Program
{
  ...
  {
    string filter = @"(&(objectCategory=computer)(!(userAccountControl:1.2.840.113556.1.4.803:=2))(ms-Mcs-AdmPwd=*))";
    string[] props = new string[] { "name", "distinguishedName", "samAccountName", "ms-Mcs-AdmPwdExpirationTime", "operatingSystem" };
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'COMP-004'
                adSuiteCheckName  = 'Computers_with_LAPS_Deployed'
    ...
    $bhFile = Join-Path $bhDir "COMP-004_$bhTs.json"
    @{
