...
Write-Host "=== Multi-Engine Execution: Computers Trusted as DCs (Not Actual DCs) ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        Get-ADObject -LDAPFilter $ldapFilter -Properties $props,userAccountControl,primaryGroupID -ErrorAction Stop |
...
public class Program
{
  ...
  {
    string filter = @"(&(objectCategory=computer)(!(userAccountControl:1.2.840.113556.1.4.803:=2))(userAccountControl:1.2.840.113556.1.4.803:=8192)(!(primaryGroupID=516)))";
    string[] props = new string[] { "name", "distinguishedName", "samAccountName", "userAccountControl", "primaryGroupID" };
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'COMP-017'
                adSuiteCheckName  = 'Computers_Trusted_as_DCs_Not_Actual_DCs'
    ...
    $bhFile = Join-Path $bhDir "COMP-017_$bhTs.json"
    @{
