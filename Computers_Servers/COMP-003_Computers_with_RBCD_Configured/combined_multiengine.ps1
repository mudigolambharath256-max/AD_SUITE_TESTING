...
Write-Host "=== Multi-Engine Execution: Computers with RBCD Configured ===" -ForegroundColor Cyan
Write-Host ""
...
public class Program
{
  ...
  {
    string filter = @"(&(objectCategory=computer)(!(userAccountControl:1.2.840.113556.1.4.803:=2))(msDS-AllowedToActOnBehalfOfOtherIdentity=*))";
    string[] props = new string[] { "name", "distinguishedName", "samAccountName", "msDS-AllowedToActOnBehalfOfOtherIdentity" };
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'COMP-003'
                adSuiteCheckName  = 'Computers_with_RBCD_Configured'
    ...
    $bhFile = Join-Path $bhDir "COMP-003_$bhTs.json"
    @{
