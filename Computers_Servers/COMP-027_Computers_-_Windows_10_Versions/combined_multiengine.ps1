...
Write-Host "=== Multi-Engine Execution: Computers - Windows 10 Versions ===" -ForegroundColor Cyan
Write-Host ""
...
public class Program
{
  ...
  {
    string filter = @"(&(objectCategory=computer)(!(userAccountControl:1.2.840.113556.1.4.803:=2))(operatingSystem=*Windows 10*))";
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'COMP-027'
                adSuiteCheckName  = 'Computers_-_Windows_10_Versions'
    ...
    $bhFile = Join-Path $bhDir "COMP-027_$bhTs.json"
    @{
