...
Write-Host "=== Multi-Engine Execution: LinuxUnix Computers ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    $adsiResults = @(
        ...
        $searcher = [ADSISearcher]'(&(objectCategory=computer)(!(userAccountControl:1.2.840.113556.1.4.803:=2))(!(operatingSystem=*Windows*)))'
        $searcher.PageSize = 1000
        $searcher.PropertiesToLoad.Clear()
        (@('name', 'distinguishedName', 'samAccountName', 'operatingSystem', 'dNSHostName', 'userAccountControl') | ForEach-Object { [void]$searcher.PropertiesToLoad.Add($_) })

        $searcher.FindAll() | ForEach-Object {
          $p = $_.Properties
          [PSCustomObject]@{
            Label = 'LinuxUnix Computers'
          }
        }
...
public class Program
{
  ...
  {
    string filter = @"(&(objectCategory=computer)(!(userAccountControl:1.2.840.113556.1.4.803:=2))(!(operatingSystem=*Windows*)))";
    string[] props = new string[] { "name", "distinguishedName", "samAccountName", "operatingSystem", "dNSHostName" };
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'COMP-029'
                adSuiteCheckName  = 'LinuxUnix_Computers'
    ...
    $bhFile = Join-Path $bhDir "COMP-029_$bhTs.json"
    @{
