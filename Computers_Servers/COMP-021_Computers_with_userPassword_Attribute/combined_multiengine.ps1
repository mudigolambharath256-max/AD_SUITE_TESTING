...
Write-Host "=== Multi-Engine Execution: Computers with userPassword Attribute ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'COMP-021'
                adSuiteCheckName  = 'Computers_with_userPassword_Attribute'
    ...
    $bhFile = Join-Path $bhDir "COMP-021_$bhTs.json"
    @{
