# ============================================================================
# COMPLY-007: Privileged Account Naming Convention - Multi-Engine
# ============================================================================

[CmdletBinding()]
param([Parameter()][string]$SearchBase, [Parameter()][string]$ExportPath)

Write-Host "[CMP-007] Privileged Account Naming Convention" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    $results = Get-ADObject -LDAPFilter '(&(objectCategory=person)(objectClass=user)(adminCount=1))' -Properties name,distinguishedName,adminCount,samAccountName -SearchBase $SearchBase -SearchScope Subtree -ResultSetSize $null -ErrorAction SilentlyContinue
} else {
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'COMPLY-007'
                adSuiteCheckName  = 'Privileged_Account_Naming_Convention'
    ...
    $bhFile = Join-Path $bhDir "COMPLY-007_$bhTs.json"
    @{
