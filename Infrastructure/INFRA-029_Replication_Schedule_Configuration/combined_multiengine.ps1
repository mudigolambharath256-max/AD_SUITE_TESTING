...
Write-Host "=== INFRA-029: Replication Schedule Configuration ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        # Execute query
        $adObjects = Get-ADObject -LDAPFilter '(objectClass=siteLink)' `
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'INFRA-029'}}, `
            @{N='CheckName'; E={'Replication Schedule Configuration'}}, `
...
if (-not $useADModule) {
    ...
    try {
        ...
        $results = $adsiResults | ForEach-Object {
            ...
            [PSCustomObject]@{
                CheckID           = 'INFRA-029'
                CheckName         = 'Replication Schedule Configuration'
                Method            = 'ADSI'
                Domain            = $domainNC
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                adSuiteCheckName  = 'Replication_Schedule_Configuration'
    ...
    $bhFile = Join-Path $bhDir "INFRA-029_$bhTs.json"
    @{
