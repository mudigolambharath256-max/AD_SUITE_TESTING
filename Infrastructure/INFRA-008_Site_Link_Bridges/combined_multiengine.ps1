...
Write-Host "=== INFRA-008: Site Link Bridges ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        # Execute query
        $adObjects = Get-ADObject -LDAPFilter '(objectClass=siteLinkBridge)' `
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'INFRA-008'}}, `
            @{N='CheckName'; E={'Site Link Bridges'}}, `
...
if (-not $useADModule) {
    ...
    try {
        ...
        $searcher.Filter = '(objectClass=siteLinkBridge)'
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                adSuiteCheckName  = 'Site_Link_Bridges'
    ...
    $bhFile = Join-Path $bhDir "INFRA-008_$bhTs.json"
    @{
