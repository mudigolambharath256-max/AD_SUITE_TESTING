...
Write-Host "=== INFRA-009: Connection Objects ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        # Execute query
        $adObjects = Get-ADObject -LDAPFilter '(objectClass=nTDSConnection)' `
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'INFRA-009'}}, `
            @{N='CheckName'; E={'Connection Objects'}}, `
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                adSuiteCheckName  = 'Connection_Objects'
    ...
    $bhFile = Join-Path $bhDir "INFRA-009_$bhTs.json"
    @{
