...
Write-Host "=== Infrastructure Check 21 ===" -ForegroundColor Cyan
...
try {
    ...
    Write-Warning "INFRA-021 PowerShell engine: $_"   # Fix R10: no silent catch
}
...
try {
    ...
    Write-Warning "INFRA-021 ADSI engine: $_"          # Fix R10
}
...
try {
    ...
    Write-Warning "INFRA-021 C# engine: $_"             # Fix R10
}
...
Write-Host ""
Write-Host "=== INFRA-021: $($uniqueResults.Count) unique findings ===" -ForegroundColor Cyan
$uniqueResults | Format-List

# Fix R14: Fixed 5-field schema enforced before CSV export
$csvPath = Join-Path $env:TEMP "INFRA-021_results.csv"
$uniqueResults |
