...
Write-Host "=== INFRA-027: Sites Without Subnets ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        # Execute query
        $adObjects = Get-ADObject -LDAPFilter '(objectClass=site)' `
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'INFRA-027'}}, `
            @{N='CheckName'; E={'Sites Without Subnets'}}, `
...
if (-not $useADModule) {
    ...
    try {
        ...
        $results = $adsiResults | ForEach-Object {
            ...
            [PSCustomObject]@{
                CheckID           = 'INFRA-027'
                CheckName         = 'Sites Without Subnets'
                Method            = 'ADSI'
                Domain            = $domainNC
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                adSuiteCheckName  = 'Sites_Without_Subnets'
    ...
    $bhFile = Join-Path $bhDir "INFRA-027_$bhTs.json"
    @{
