...
Write-Host "=== Infrastructure Check 17 ===" -ForegroundColor Cyan
...
try {
    ...
    Write-Warning "INFRA-017 PowerShell engine: $_"   # Fix R10: no silent catch
}
...
try {
    ...
    Write-Warning "INFRA-017 ADSI engine: $_"          # Fix R10
}
...
try {
    ...
    Write-Warning "INFRA-017 C# engine: $_"             # Fix R10
}
...
Write-Host ""
Write-Host "=== INFRA-017: $($uniqueResults.Count) unique findings ===" -ForegroundColor Cyan
$uniqueResults | Format-List

# Fix R14: Fixed 5-field schema enforced before CSV export
$csvPath = Join-Path $env:TEMP "INFRA-017_results.csv"
$uniqueResults |
