...
Write-Host "=== INFRA-012: DNS Zone Transfers ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'INFRA-012'}}, `
            @{N='CheckName'; E={'DNS Zone Transfers'}}, `
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                adSuiteCheckName  = 'DNS_Zone_Transfers'
    ...
    $bhFile = Join-Path $bhDir "INFRA-012_$bhTs.json"
    @{
