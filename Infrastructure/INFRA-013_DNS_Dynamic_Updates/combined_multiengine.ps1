...
Write-Host "=== INFRA-013: DNS Dynamic Updates ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'INFRA-013'}}, `
            @{N='CheckName'; E={'DNS Dynamic Updates'}}, `
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                adSuiteCheckName  = 'DNS_Dynamic_Updates'
    ...
    $bhFile = Join-Path $bhDir "INFRA-013_$bhTs.json"
    @{
