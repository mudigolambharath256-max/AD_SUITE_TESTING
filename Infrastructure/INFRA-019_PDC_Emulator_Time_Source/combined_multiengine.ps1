...
Write-Host "=== INFRA-019: PDC Emulator Time Source ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'INFRA-019'}}, `
            @{N='CheckName'; E={'PDC Emulator Time Source'}}, `
...
if (-not $useADModule) {
    ...
    try {
        ...
        $results = $adsiResults | ForEach-Object {
            ...
            [PSCustomObject]@{
                CheckID           = 'INFRA-019'
                CheckName         = 'PDC Emulator Time Source'
                Method            = 'ADSI'
                Domain            = $domainNC
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                adSuiteCheckName  = 'PDC_Emulator_Time_Source'
    ...
    $bhFile = Join-Path $bhDir "INFRA-019_$bhTs.json"
    @{
