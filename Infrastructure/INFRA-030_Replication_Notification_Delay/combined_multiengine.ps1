...
Write-Host "=== INFRA-030: Replication Notification Delay ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'INFRA-030'}}, `
            @{N='CheckName'; E={'Replication Notification Delay'}}, `
...
if (-not $useADModule) {
    ...
    try {
        ...
        $results = $adsiResults | ForEach-Object {
            ...
            [PSCustomObject]@{
                CheckID           = 'INFRA-030'
                CheckName         = 'Replication Notification Delay'
                Method            = 'ADSI'
                Domain            = $domainNC
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                adSuiteCheckName  = 'Replication_Notification_Delay'
    ...
    $bhFile = Join-Path $bhDir "INFRA-030_$bhTs.json"
    @{
