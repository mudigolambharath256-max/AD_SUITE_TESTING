...
Write-Host "Found $($results.Count) objects for check: Stale DNS Records" -ForegroundColor Cyan

$results | ForEach-Object {
    $props = $_.Properties
    [PSCustomObject]@{
        CheckID = 'INFRA-033'
        CheckName = 'Stale DNS Records'
...
try {
    ...
    if ($results -and $results.Count -gt 0) {
        ...
        $bhFile = Join-Path $bhDir "INFRA-033_nodes.json"
