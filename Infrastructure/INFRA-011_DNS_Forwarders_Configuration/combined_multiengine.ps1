...
Write-Host "=== INFRA-011: DNS Forwarders Configuration ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        # Execute query
        $adObjects = Get-ADObject -LDAPFilter '(objectClass=dnsZone)' `
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'INFRA-011'}}, `
            @{N='CheckName'; E={'DNS Forwarders Configuration'}}, `
...
if (-not $useADModule) {
    ...
    try {
        ...
        $results = $adsiResults | ForEach-Object {
            ...
            [PSCustomObject]@{
                CheckID           = 'INFRA-011'
                CheckName         = 'DNS Forwarders Configuration'
                Method            = 'ADSI'
                Domain            = $domainNC
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                adSuiteCheckName  = 'DNS_Forwarders_Configuration'
    ...
    $bhFile = Join-Path $bhDir "INFRA-011_$bhTs.json"
    @{
