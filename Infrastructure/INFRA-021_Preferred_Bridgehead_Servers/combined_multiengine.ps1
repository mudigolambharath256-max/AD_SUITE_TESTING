...
Write-Host "=== INFRA-021: Preferred Bridgehead Servers ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        # Execute query
        $adObjects = Get-ADObject -LDAPFilter '(objectClass=server)' `
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'INFRA-021'}}, `
            @{N='CheckName'; E={'Preferred Bridgehead Servers'}}, `
...
if (-not $useADModule) {
    ...
    try {
        ...
        $results = $adsiResults | ForEach-Object {
            ...
            [PSCustomObject]@{
                CheckID           = 'INFRA-021'
                CheckName         = 'Preferred Bridgehead Servers'
                Method            = 'ADSI'
                Domain            = $domainNC
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                adSuiteCheckName  = 'Preferred_Bridgehead_Servers'
    ...
    $bhFile = Join-Path $bhDir "INFRA-021_$bhTs.json"
    @{
