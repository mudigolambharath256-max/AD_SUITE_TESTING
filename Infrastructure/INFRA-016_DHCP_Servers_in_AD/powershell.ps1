...
$ldapFilter = '(objectClass=dHCPClass)'

# Define properties to retrieve
$properties = @('name', 'distinguishedName', 'objectClass', 'whenCreated', 'whenChanged', 'objectSid')

Write-Host "Executing security check: DHCP Servers in AD" -ForegroundColor Yellow
Write-Host "Check ID: INFRA-016" -ForegroundColor Gray
...
$output = $results | Select-Object `
    @{N='CheckID'; E={'INFRA-016'}}, `
    @{N='CheckName'; E={'DHCP Servers in AD'}}, `
...
try {
    ...
    foreach ($r in $output) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                adSuiteCheckName  = 'DHCP_Servers_in_AD'
    ...
    $bhFile = Join-Path $bhDir "INFRA-016_$bhTs.json"
    @{
