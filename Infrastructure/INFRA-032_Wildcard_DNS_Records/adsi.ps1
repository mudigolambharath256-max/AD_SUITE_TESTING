...
Write-Host "Found $($results.Count) objects for check: Wildcard DNS Records" -ForegroundColor Cyan

$results | ForEach-Object {
    $props = $_.Properties
    [PSCustomObject]@{
        CheckID = 'INFRA-032'
        CheckName = 'Wildcard DNS Records'
...
try {
    ...
    if ($results -and $results.Count -gt 0) {
        ...
        $bhFile = Join-Path $bhDir "INFRA-032_nodes.json"
