...
Write-Host "=== INFRA-028: Universal Group Membership Caching ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        # Execute query
        $adObjects = Get-ADObject -LDAPFilter '(objectClass=nTDSSiteSettings)' `
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'INFRA-028'}}, `
            @{N='CheckName'; E={'Universal Group Membership Caching'}}, `
...
if (-not $useADModule) {
    ...
    try {
        ...
        $results = $adsiResults | ForEach-Object {
            ...
            [PSCustomObject]@{
                CheckID           = 'INFRA-028'
                CheckName         = 'Universal Group Membership Caching'
                Method            = 'ADSI'
                Domain            = $domainNC
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                adSuiteCheckName  = 'Universal_Group_Membership_Caching'
    ...
    $bhFile = Join-Path $bhDir "INFRA-028_$bhTs.json"
    @{
