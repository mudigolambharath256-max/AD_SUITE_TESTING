...
Write-Host "=== INFRA-005: Replication Partners ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'INFRA-005'}}, `
            @{N='CheckName'; E={'Replication Partners'}}, `
...
if (-not $useADModule) {
    ...
    try {
        ...
        $results = $adsiResults | ForEach-Object {
            ...
            [PSCustomObject]@{
                CheckID           = 'INFRA-005'
                CheckName         = 'Replication Partners'
                Method            = 'ADSI'
                Domain            = $domainNC
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                adSuiteCheckName  = 'Replication_Partners'
    ...
    $bhFile = Join-Path $bhDir "INFRA-005_$bhTs.json"
    @{
