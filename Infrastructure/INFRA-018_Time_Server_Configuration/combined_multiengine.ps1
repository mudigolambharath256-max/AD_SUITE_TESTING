...
Write-Host "=== INFRA-018: Time Server Configuration ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'INFRA-018'}}, `
            @{N='CheckName'; E={'Time Server Configuration'}}, `
...
if (-not $useADModule) {
    ...
    try {
        ...
        $results = $adsiResults | ForEach-Object {
            ...
            [PSCustomObject]@{
                CheckID           = 'INFRA-018'
                CheckName         = 'Time Server Configuration'
                Method            = 'ADSI'
                Domain            = $domainNC
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                adSuiteCheckName  = 'Time_Server_Configuration'
    ...
    $bhFile = Join-Path $bhDir "INFRA-018_$bhTs.json"
    @{
