...
Write-Host "=== INFRA-026: Empty Sites ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'INFRA-026'}}, `
            @{N='CheckName'; E={'Empty Sites'}}, `
...
try {
    ...
    $bhFile = Join-Path $bhDir "INFRA-026_$bhTs.json"
    @{
