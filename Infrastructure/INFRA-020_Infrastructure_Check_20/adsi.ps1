...
Write-Host "INFRA-020: found $($results.Count) objects"
...
try {
    ...
    foreach ($r in $bhRaw) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                isdeleted         = $false
                adSuiteCheckId    = 'INFRA-020'
                adSuiteCheckName  = 'Infrastructure Check 20'
                adSuiteSeverity   = 'medium'
                adSuiteCategory   = 'Infrastructure'
                adSuiteFlag       = $true
            }
            Aces           = @()
            IsDeleted      = $false
    ...
    @{ data = $bhNodes.ToArray()
       meta = @{ type = 'containers'; count = $bhNodes.Count; version = 5; methods = 0 }
        ...
        Out-File -FilePath (Join-Path $bhDir "INFRA-020_$bhTs.json") -Encoding UTF8 -Force

# Fix R10: No silent catch — warn on failure
} catch { Write-Warning "INFRA-020 BloodHound export error: $_" }
