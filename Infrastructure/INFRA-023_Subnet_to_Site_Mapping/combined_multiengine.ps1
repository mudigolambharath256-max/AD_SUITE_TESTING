...
Write-Host "=== INFRA-023: Subnet to Site Mapping ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        # Execute query
        $adObjects = Get-ADObject -LDAPFilter '(objectClass=subnet)' `
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'INFRA-023'}}, `
            @{N='CheckName'; E={'Subnet to Site Mapping'}}, `
...
if (-not $useADModule) {
    ...
    try {
        ...
        $results = $adsiResults | ForEach-Object {
            ...
            [PSCustomObject]@{
                CheckID           = 'INFRA-023'
                CheckName         = 'Subnet to Site Mapping'
                Method            = 'ADSI'
                Domain            = $domainNC
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                adSuiteCheckName  = 'Subnet_to_Site_Mapping'
    ...
    $bhFile = Join-Path $bhDir "INFRA-023_$bhTs.json"
    @{
