...
Write-Host "=== INFRA-015: DNS Aging Configuration ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'INFRA-015'}}, `
            @{N='CheckName'; E={'DNS Aging Configuration'}}, `
...
if (-not $useADModule) {
    ...
    try {
        ...
        $results = $adsiResults | ForEach-Object {
            ...
            [PSCustomObject]@{
                CheckID           = 'INFRA-015'
                CheckName         = 'DNS Aging Configuration'
                Method            = 'ADSI'
                Domain            = $domainNC
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                adSuiteCheckName  = 'DNS_Aging_Configuration'
    ...
    $bhFile = Join-Path $bhDir "INFRA-015_$bhTs.json"
    @{
