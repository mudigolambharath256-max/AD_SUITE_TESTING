...
Write-Host "=== INFRA-014: DNS Scavenging Settings ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'INFRA-014'}}, `
            @{N='CheckName'; E={'DNS Scavenging Settings'}}, `
...
if (-not $useADModule) {
    ...
    try {
        ...
        $results = $adsiResults | ForEach-Object {
            ...
            [PSCustomObject]@{
                CheckID           = 'INFRA-014'
                CheckName         = 'DNS Scavenging Settings'
                Method            = 'ADSI'
                Domain            = $domainNC
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                adSuiteCheckName  = 'DNS_Scavenging_Settings'
    ...
    $bhFile = Join-Path $bhDir "INFRA-014_$bhTs.json"
    @{
