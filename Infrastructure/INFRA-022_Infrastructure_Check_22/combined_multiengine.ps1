...
Write-Host "=== Infrastructure Check 22 ===" -ForegroundColor Cyan
...
try {
    ...
    Write-Warning "INFRA-022 PowerShell engine: $_"   # Fix R10: no silent catch
}
...
try {
    ...
    Write-Warning "INFRA-022 ADSI engine: $_"          # Fix R10
}
...
try {
    ...
    Write-Warning "INFRA-022 C# engine: $_"             # Fix R10
}
...
Write-Host ""
Write-Host "=== INFRA-022: $($uniqueResults.Count) unique findings ===" -ForegroundColor Cyan
$uniqueResults | Format-List

# Fix R14: Fixed 5-field schema enforced before CSV export
$csvPath = Join-Path $env:TEMP "INFRA-022_results.csv"
$uniqueResults |
