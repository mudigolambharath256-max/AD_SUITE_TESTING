...
Write-Host "=== INFRA-004: Organizational Units OU Inventory ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        # Execute query
        $adObjects = Get-ADObject -LDAPFilter '(objectClass=organizationalUnit)' `
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'INFRA-004'}}, `
            @{N='CheckName'; E={'Organizational Units OU Inventory'}}, `
...
if (-not $useADModule) {
    ...
    try {
        ...
        $results = $adsiResults | ForEach-Object {
            ...
            [PSCustomObject]@{
                CheckID           = 'INFRA-004'
                CheckName         = 'Organizational Units OU Inventory'
                Method            = 'ADSI'
                Domain            = $domainNC
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                adSuiteCheckName  = 'Organizational_Units_OU_Inventory'
    ...
    $bhFile = Join-Path $bhDir "INFRA-004_$bhTs.json"
    @{
