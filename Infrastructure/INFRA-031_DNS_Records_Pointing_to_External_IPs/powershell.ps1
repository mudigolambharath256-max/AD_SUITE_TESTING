...
Write-Host "Executing: DNS Records Pointing to External IPs" -ForegroundColor Cyan
Write-Host "Check ID: INFRA-031" -ForegroundColor Gray

$results = Get-ADObject -LDAPFilter '(objectClass=dnsNode)' `
...
$output = $results | Select-Object `
    @{N='CheckID'; E={'INFRA-031'}}, `
    @{N='CheckName'; E={'DNS Records Pointing to External IPs'}}, `
    @{N='RiskScore'; E={6}}, `
    @{N='Severity'; E={'MEDIUM'}}, `
    @{N='MITRE'; E={'T1071.004'}}, `
...
try {
    ...
    foreach ($r in $output) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'INFRA-031'
                adSuiteCheckName  = 'DNS_Records_Pointing_to_External_IPs'
    ...
    $bhFile = Join-Path $bhDir "INFRA-031_$bhTs.json"
    @{
