...
Write-Host "Found $($results.Count) objects for check: DNS Records Pointing to External IPs" -ForegroundColor Cyan

$results | ForEach-Object {
    $props = $_.Properties
    [PSCustomObject]@{
        CheckID = 'INFRA-031'
        CheckName = 'DNS Records Pointing to External IPs'
...
try {
    ...
    if ($results -and $results.Count -gt 0) {
        ...
        $bhFile = Join-Path $bhDir "INFRA-031_nodes.json"
