...
Write-Host "ADV-004: found $($results.Count) objects"
...
try {
    ...
    foreach ($r in $bhRaw) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                isdeleted         = $false
                adSuiteCheckId    = 'ADV-004'
                adSuiteCheckName  = 'Advanced Security Check 4'
    ...
    @{ data = $bhNodes.ToArray()
       meta = @{ type = 'computers'; count = $bhNodes.Count; version = 5; methods = 0 }
        ...
        Out-File -FilePath (Join-Path $bhDir "ADV-004_$bhTs.json") -Encoding UTF8 -Force

# Fix R10: No silent catch — warn on failure
} catch { Write-Warning "ADV-004 BloodHound export error: $_" }
