# ============================================================================
# ADV-007: NTLM Relay Attack Surface - Multi-Engine
# ============================================================================
...
#   It CANNOT determine NTLM relay attack surface (requires SMB signing, LDAP signing, EPA checks via registry/GPO) via LDAP alone.
...
Write-Host "[ADV-007] NTLM Relay Attack Surface" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    $results = Get-ADObject -LDAPFilter '(&(objectCategory=computer)(!(userAccountControl:1.2.840.113556.1.4.803:=2)))' -Properties name,distinguishedName,userAccountControl -SearchBase $SearchBase -SearchScope Subtree -ResultSetSize $null -ErrorAction SilentlyContinue
} else {
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'ADV-007'
                adSuiteCheckName  = 'NTLM_Relay_Attack_Surface'
                adSuiteMEDIUM   = 'MEDIUM'
                adSuiteAdvanced_Security   = 'Advanced_Security'
                adSuiteFlag       = $true
            }
            Aces      = @()
            IsDeleted = $false
    ...
    $bhFile = Join-Path $bhDir "ADV-007_$bhTs.json"
    @{
