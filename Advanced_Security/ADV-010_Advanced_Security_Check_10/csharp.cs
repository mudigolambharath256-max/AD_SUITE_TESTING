...
class Program
{
    ...
    static void Main()
    {
        string filter = @"(&(objectCategory=computer)(primaryGroupID=516))";
        ...
        try
        {
            using (DirectoryEntry rootEntry = new DirectoryEntry("LDAP://RootDSE"))
            {
                ...
                using (DirectorySearcher searcher = new DirectorySearcher(searchEntry))
                {
                    ...
                    {
                        Console.WriteLine("ADV-010: found " + results.Count + " objects");
        ...
        {
            Console.Error.WriteLine("ADV-010 error: " + ex.Message);
            Environment.Exit(1);
        }
    }
}
