...
# Check: Advanced Security Check 10
...
Write-Host "=== Advanced Security Check 10 ===" -ForegroundColor Cyan
...
try {
    ...
    Get-ADObject -LDAPFilter '(&(objectCategory=computer)(primaryGroupID=516))' `
    ...
    Write-Warning "ADV-010 PowerShell engine: $_"   # Fix R10: no silent catch
}
...
try {
    ...
    $adsiS.Filter   = '(&(objectCategory=computer)(primaryGroupID=516))'
    $adsiS.PageSize = 1000
    ...
    Write-Warning "ADV-010 ADSI engine: $_"          # Fix R10
}
...
try {
    $csFilter  = '(&(objectCategory=computer)(primaryGroupID=516))'
    ...
    Write-Warning "ADV-010 C# engine: $_"             # Fix R10
}
...
Write-Host ""
Write-Host "=== ADV-010: $($uniqueResults.Count) unique findings ===" -ForegroundColor Cyan
$uniqueResults | Format-List

# Fix R14: Fixed 5-field schema enforced before CSV export
$csvPath = Join-Path $env:TEMP "ADV-010_results.csv"
$uniqueResults |
