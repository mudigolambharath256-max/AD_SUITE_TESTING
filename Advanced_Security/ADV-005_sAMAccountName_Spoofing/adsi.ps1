...
(@('name', 'distinguishedName', 'whenCreated', 'whenChanged', 'userAccountControl', 'objectSid') | ForEach-Object { [void]$searcher.PropertiesToLoad.Add($_)
}
...
$results | ForEach-Object {
    ...
    [PSCustomObject]@{
        CheckID = 'ADV-005'
        CheckName = 'sAMAccountName Spoofing'
...
try {
    ...
    if ($results -and $results.Count -gt 0) {
        ...
        $bhFile = Join-Path $bhDir "ADV-005_nodes.json"
