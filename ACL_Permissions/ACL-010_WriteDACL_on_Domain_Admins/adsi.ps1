# Check: WriteDACL on Domain Admins
...
Write-Host "ACL-010: found $($findings.Count) trustees with WriteDacl"
$findings
# ── BloodHound Export ─────────────────────────────────────────────────────────
try {
    ...
    foreach ($f in $findings) {
        ...
        $bhNodes.Add(@{
            ...
            Properties = @{
                ...
                enabled          = $true
                isdeleted        = $false
                adSuiteCheckId   = 'ACL-010'
                adSuiteCheckName = 'WriteDACL on Domain Admins'
    ...
    @{ data = $bhNodes.ToArray()
       meta = @{ type = 'users'; count = $bhNodes.Count; version = 5; methods = 0 }
        ...
        Out-File -FilePath (Join-Path $bhDir "ACL-010_$bhTs.json") -Encoding UTF8 -Force
} catch { Write-Warning "ACL-010 BloodHound export error: $_" }
