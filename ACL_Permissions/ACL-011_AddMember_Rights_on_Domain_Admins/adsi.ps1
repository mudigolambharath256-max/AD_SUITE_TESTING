# Check: AddMember Rights on Domain Admins
...
$acl.GetAccessRules($true, $true, [System.Security.Principal.SecurityIdentifier]) |
    Where-Object {
        ($_.AccessControlType -eq 'Allow') -and ($_.ActiveDirectoryRights -band [System.DirectoryServices.ActiveDirectoryRights]::Self) -and ($_.ObjectType -ne $null) -and ($_.ObjectType.ToString() -eq 'bf9679c0-0de6-11d0-a285-00aa003049e2')
    } | ForEach-Object {
        ...
        $findings.Add([PSCustomObject]@{
            ...
            Domain            = $dom
            Engine            = 'ADSI'
            Rights            = 'Self-Membership (AddMember)'
            TargetObject      = $targetDN
            TrusteeSID        = $trusteeSid
        })
    }

Write-Host "ACL-011: found $($findings.Count) trustees with Self-Membership (AddMember)"
$findings
# ── BloodHound Export ─────────────────────────────────────────────────────────
try {
    ...
    foreach ($f in $findings) {
        ...
        $bhNodes.Add(@{
            ...
            Properties = @{
                ...
                enabled          = $true
                isdeleted        = $false
                adSuiteCheckId   = 'ACL-011'
                adSuiteCheckName = 'AddMember Rights on Domain Admins'
    ...
    @{ data = $bhNodes.ToArray()
       meta = @{ type = 'users'; count = $bhNodes.Count; version = 5; methods = 0 }
        ...
        Out-File -FilePath (Join-Path $bhDir "ACL-011_$bhTs.json") -Encoding UTF8 -Force
} catch { Write-Warning "ACL-011 BloodHound export error: $_" }
