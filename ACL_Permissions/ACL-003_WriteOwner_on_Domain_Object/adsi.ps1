...
Write-Host "ACL-003: found $($findings.Count) trustees with WriteOwner on Domain Object"
$findings
...
try {
    ...
    foreach ($f in $findings) {
        ...
        $bhNodes.Add(@{
            ...
            Properties = @{
                ...
                enabled          = $true
                isdeleted        = $false
                adSuiteCheckId   = 'ACL-003'
                adSuiteCheckName = 'WriteOwner on Domain Object'
    ...
    @{ data = $bhNodes.ToArray()
       meta = @{ type = 'users'; count = $bhNodes.Count; version = 5; methods = 0 }
        ...
        Out-File -FilePath (Join-Path $bhDir "ACL-003_$bhTs.json") -Encoding UTF8 -Force
} catch { Write-Warning "ACL-003 BloodHound export error: $_" }
