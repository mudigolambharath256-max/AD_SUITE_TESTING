...
Write-Host "=== WriteOwner on Domain Object ===" -ForegroundColor Cyan
...
try {
    ...
    Write-Warning "ACL-003 PowerShell engine: $_"
}
...
try {
    ...
    Write-Warning "ACL-003 ADSI engine: $_"
}
...
try {
    ...
    [void]$csCode.AppendLine('                if ((rule.ActiveDirectoryRights & ActiveDirectoryRights.WriteOwner) == 0) continue;')
    ...
    [void]$csCode.AppendLine('                results.Add(new string[]{name, targetDN.ToUpper(), name, dom, "WriteOwner", targetDN, sid}); } }')
    ...
    Write-Warning "ACL-003 C# engine: $_"
}
...
Write-Host ""
Write-Host "=== ACL-003: $($uniqueResults.Count) unique findings ===" -ForegroundColor Cyan
$uniqueResults | Format-List

$csvPath = Join-Path $env:TEMP "ACL-003_results.csv"
$uniqueResults |
