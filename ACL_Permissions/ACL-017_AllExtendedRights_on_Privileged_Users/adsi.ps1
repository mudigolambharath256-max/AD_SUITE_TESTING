# Check: AllExtendedRights on Privileged Users
...
foreach ($sr in $scanResults) {
    ...
    $acl.GetAccessRules($true, $true, [System.Security.Principal.SecurityIdentifier]) |
        Where-Object {
            ($_.AccessControlType -eq 'Allow') -and ($_.ActiveDirectoryRights -band [System.DirectoryServices.ActiveDirectoryRights]::ExtendedRight) -and ($null -eq $_.ObjectType -or $_.ObjectType -eq [guid]::Empty)
        } |
        ForEach-Object {
...
Write-Host "ACL-017: found $($findings.Count) trustees with AllExtendedRights"
$findings
# ── BloodHound Export ─────────────────────────────────────────────────────────
try {
    ...
    foreach ($f in $findings) {
        ...
        $bhNodes.Add(@{
            ...
            Properties = @{
                ...
                enabled          = $true
                isdeleted        = $false
                adSuiteCheckId   = 'ACL-017'
                adSuiteCheckName = 'AllExtendedRights on Privileged Users'
    ...
    @{ data = $bhNodes.ToArray()
       meta = @{ type = 'users'; count = $bhNodes.Count; version = 5; methods = 0 }
        ...
        Out-File -FilePath (Join-Path $bhDir "ACL-017_$bhTs.json") -Encoding UTF8 -Force
} catch { Write-Warning "ACL-017 BloodHound export error: $_" }
