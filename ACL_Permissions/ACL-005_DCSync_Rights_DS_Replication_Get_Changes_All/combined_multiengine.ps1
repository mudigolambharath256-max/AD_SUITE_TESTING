...
Write-Host "=== DCSync Rights DS-Replication-Get-Changes-All ===" -ForegroundColor Cyan
...
try {
    ...
    # Check DS-Replication-Get-Changes-All
    $targetAcl |
        Where-Object {
    ...
    # Check DS-Replication-Get-Changes
    $targetAcl |
        Where-Object {
            ($_.AccessControlType -eq [System.Security.AccessControl.AccessControlType]::Allow) -and
            ($_.ActiveDirectoryRights -band [System.DirectoryServices.ActiveDirectoryRights]::ExtendedRight) -and
    ...
    Write-Warning "ACL-005 PowerShell engine: $_"
}
...
try {
    ...
    Write-Warning "ACL-005 ADSI engine: $_"
}
...
try {
    ...
    [void]$csCode.AppendLine('        Guid dcsyncAllGuid = new Guid("1131f6ad-9c07-11d1-f79f-00c04fc2dcd2");')
    [void]$csCode.AppendLine('        Guid dcsyncGuid = new Guid("1131f6aa-9c07-11d1-f79f-00c04fc2dcd2");')
    ...
    [void]$csCode.AppendLine('                string rightsDesc = "";')
    [void]$csCode.AppendLine('                if (rule.ObjectType == dcsyncAllGuid) rightsDesc = "DS-Replication-Get-Changes-All (DCSync)";')
    [void]$csCode.AppendLine('                else if (rule.ObjectType == dcsyncGuid) rightsDesc = "DS-Replication-Get-Changes (DCSync)";')
    [void]$csCode.AppendLine('                else continue;')
    ...
    [void]$csCode.AppendLine('                results.Add(new string[]{name, targetDN.ToUpper(), name, dom, rightsDesc, targetDN, sid}); } }')
    ...
    Write-Warning "ACL-005 C# engine: $_"
}

# ── DEDUPLICATION ─────────────────────────────────────────────────────────────
$uniqueResults = $allResults |
    Group-Object -Property TrusteeSID, TargetObject, Rights |
...
Write-Host ""
Write-Host "=== ACL-005: $($uniqueResults.Count) unique findings ===" -ForegroundColor Cyan
$uniqueResults | Format-List

$csvPath = Join-Path $env:TEMP "ACL-005_results.csv"
$uniqueResults |
