...
# Check for DS-Replication-Get-Changes-All (1131f6ad-9c07-11d1-f79f-00c04fc2dcd2)
...
# Also check for DS-Replication-Get-Changes (1131f6aa-9c07-11d1-f79f-00c04fc2dcd2)
$acl.GetAccessRules($true, $true, [System.Security.Principal.SecurityIdentifier]) |
    Where-Object {
        ($_.AccessControlType -eq 'Allow') -and
        ($_.ActiveDirectoryRights -band [System.DirectoryServices.ActiveDirectoryRights]::ExtendedRight) -and
...
Write-Host "ACL-005: found $($findings.Count) trustees with DCSync rights on Domain Object"
$findings
...
try {
    ...
    foreach ($f in $findings) {
        ...
        $bhNodes.Add(@{
            ...
            Properties = @{
                ...
                enabled          = $true
                isdeleted        = $false
                adSuiteCheckId   = 'ACL-005'
                adSuiteCheckName = 'DCSync Rights DS-Replication-Get-Changes-All'
    ...
    @{ data = $bhNodes.ToArray()
       meta = @{ type = 'users'; count = $bhNodes.Count; version = 5; methods = 0 }
        ...
        Out-File -FilePath (Join-Path $bhDir "ACL-005_$bhTs.json") -Encoding UTF8 -Force
} catch { Write-Warning "ACL-005 BloodHound export error: $_" }
