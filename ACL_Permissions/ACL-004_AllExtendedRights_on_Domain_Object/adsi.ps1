...
Write-Host "ACL-004: found $($findings.Count) trustees with AllExtendedRights on Domain Object"
$findings
...
try {
    ...
    foreach ($f in $findings) {
        ...
        $bhNodes.Add(@{
            ...
            Properties = @{
                ...
                enabled          = $true
                isdeleted        = $false
                adSuiteCheckId   = 'ACL-004'
                adSuiteCheckName = 'AllExtendedRights on Domain Object'
    ...
    @{ data = $bhNodes.ToArray()
       meta = @{ type = 'users'; count = $bhNodes.Count; version = 5; methods = 0 }
        ...
        Out-File -FilePath (Join-Path $bhDir "ACL-004_$bhTs.json") -Encoding UTF8 -Force
} catch { Write-Warning "ACL-004 BloodHound export error: $_" }
