...
Write-Host "ACL-012: found $($findings.Count) trustees with GenericAll"
$findings
# ── BloodHound Export ─────────────────────────────────────────────────────────
try {
    ...
    foreach ($f in $findings) {
        ...
        $bhNodes.Add(@{
            ...
            Properties = @{
                ...
                enabled          = $true
                isdeleted        = $false
                adSuiteCheckId   = 'ACL-012'
                adSuiteCheckName = 'GenericAll on Enterprise Admins'
    ...
    @{ data = $bhNodes.ToArray()
       meta = @{ type = 'users'; count = $bhNodes.Count; version = 5; methods = 0 }
        ...
        Out-File -FilePath (Join-Path $bhDir "ACL-012_$bhTs.json") -Encoding UTF8 -Force
} catch { Write-Warning "ACL-012 BloodHound export error: $_" }
