...
Write-Host "ACL-006: found $($findings.Count) trustees with GenericAll on AdminSDHolder"
$findings
...
try {
    ...
    foreach ($f in $findings) {
        ...
        $bhNodes.Add(@{
            ...
            Properties = @{
                ...
                enabled          = $true
                isdeleted        = $false
                adSuiteCheckId   = 'ACL-006'
                adSuiteCheckName = 'GenericAll on AdminSDHolder'
    ...
    @{ data = $bhNodes.ToArray()
       meta = @{ type = 'users'; count = $bhNodes.Count; version = 5; methods = 0 }
        ...
        Out-File -FilePath (Join-Path $bhDir "ACL-006_$bhTs.json") -Encoding UTF8 -Force
} catch { Write-Warning "ACL-006 BloodHound export error: $_" }
