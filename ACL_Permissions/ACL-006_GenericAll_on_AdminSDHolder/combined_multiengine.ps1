...
Write-Host "=== GenericAll on AdminSDHolder ===" -ForegroundColor Cyan
...
try {
    ...
    Write-Warning "ACL-006 PowerShell engine: $_"
}
...
try {
    ...
    Write-Warning "ACL-006 ADSI engine: $_"
}
...
try {
    ...
    [void]$csCode.AppendLine('                if ((rule.ActiveDirectoryRights & ActiveDirectoryRights.GenericAll) == 0) continue;')
    ...
    [void]$csCode.AppendLine('                results.Add(new string[]{name, targetDN.ToUpper(), name, dom, "GenericAll", targetDN, sid}); } }')
    ...
    Write-Warning "ACL-006 C# engine: $_"
}
...
Write-Host ""
Write-Host "=== ACL-006: $($uniqueResults.Count) unique findings ===" -ForegroundColor Cyan
$uniqueResults | Format-List

$csvPath = Join-Path $env:TEMP "ACL-006_results.csv"
$uniqueResults |
