    ...
    static void Main()
    {
        ...
        try
        {
            using (DirectoryEntry rootEntry = new DirectoryEntry("LDAP://RootDSE"))
            {
                ...
                string targetDN = "CN=AdminSDHolder,CN=System," + domainNC;
                ...
                using (DirectoryEntry targetEntry = new DirectoryEntry("LDAP://" + targetDN))
                {
                    ...
                    Console.WriteLine("ACL-006: scanning ACL on " + targetDN);
                    ...
                    foreach (ActiveDirectoryAccessRule rule in rules)
                    {
                        ...
                        if ((rule.ActiveDirectoryRights & ActiveDirectoryRights.GenericAll) == 0) continue;
                        ...
                        Console.WriteLine(name + "\t" + targetDN.ToUpper() + "\t" + name + "\t" + dom + "\tCSharp\tGenericAll\t" + targetDN + "\t" + sid);
                        count++;
                    }
                    Console.WriteLine("ACL-006: found " + count + " trustees");
                }
            }
        }
        catch (Exception ex)
        {
            Console.Error.WriteLine("ACL-006 error: " + ex.Message);
            Environment.Exit(1);
        }
    }
}
