...
Write-Host "ACL-002: found $($findings.Count) trustees with WriteDACL on Domain Object"
$findings
...
try {
    ...
    foreach ($f in $findings) {
        ...
        $bhNodes.Add(@{
            ...
            Properties = @{
                ...
                enabled          = $true
                isdeleted        = $false
                adSuiteCheckId   = 'ACL-002'
                adSuiteCheckName = 'WriteDACL on Domain Object'
    ...
    @{ data = $bhNodes.ToArray()
       meta = @{ type = 'users'; count = $bhNodes.Count; version = 5; methods = 0 }
        ...
        Out-File -FilePath (Join-Path $bhDir "ACL-002_$bhTs.json") -Encoding UTF8 -Force
} catch { Write-Warning "ACL-002 BloodHound export error: $_" }
