        ...
        try
        {
            using (DirectoryEntry rootEntry = new DirectoryEntry("LDAP://RootDSE"))
            {
                ...
                using (DirectoryEntry targetEntry = new DirectoryEntry("LDAP://" + targetDN))
                {
                    ...
                    Console.WriteLine("ACL-002: scanning ACL on " + targetDN);
                    ...
                    foreach (ActiveDirectoryAccessRule rule in rules)
                    {
                        ...
                        if ((rule.ActiveDirectoryRights & ActiveDirectoryRights.WriteDacl) == 0) continue;
                        ...
                        Console.WriteLine(name + "\t" + targetDN.ToUpper() + "\t" + name + "\t" + dom + "\tCSharp\tWriteDacl\t" + targetDN + "\t" + sid);
                        count++;
                    }
                    Console.WriteLine("ACL-002: found " + count + " trustees");
                }
            }
        }
        catch (Exception ex)
        {
            Console.Error.WriteLine("ACL-002 error: " + ex.Message);
            Environment.Exit(1);
        }
    }
}
