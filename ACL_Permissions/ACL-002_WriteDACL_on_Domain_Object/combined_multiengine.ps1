...
Write-Host "=== WriteDACL on Domain Object ===" -ForegroundColor Cyan
...
try {
    ...
    Write-Warning "ACL-002 PowerShell engine: $_"
}
...
try {
    ...
    Write-Warning "ACL-002 ADSI engine: $_"
}
...
try {
    ...
    [void]$csCode.AppendLine('                if ((rule.ActiveDirectoryRights & ActiveDirectoryRights.WriteDacl) == 0) continue;')
    ...
    [void]$csCode.AppendLine('                results.Add(new string[]{name, targetDN.ToUpper(), name, dom, "WriteDacl", targetDN, sid}); } }')
    ...
    Write-Warning "ACL-002 C# engine: $_"
}
...
Write-Host ""
Write-Host "=== ACL-002: $($uniqueResults.Count) unique findings ===" -ForegroundColor Cyan
$uniqueResults | Format-List

$csvPath = Join-Path $env:TEMP "ACL-002_results.csv"
$uniqueResults |
