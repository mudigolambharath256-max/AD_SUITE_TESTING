...
foreach ($sr in $scanResults) {
    ...
    $acl.GetAccessRules($true, $true, [System.Security.Principal.SecurityIdentifier]) |
        Where-Object {
            ($_.AccessControlType -eq 'Allow') -and ($_.ActiveDirectoryRights -band [System.DirectoryServices.ActiveDirectoryRights]::GenericWrite)
        } |
        ForEach-Object {
...
Write-Host "ACL-016: found $($findings.Count) trustees with GenericWrite"
$findings
# ── BloodHound Export ─────────────────────────────────────────────────────────
try {
    ...
    foreach ($f in $findings) {
        ...
        $bhNodes.Add(@{
            ...
            Properties = @{
                ...
                enabled          = $true
                isdeleted        = $false
                adSuiteCheckId   = 'ACL-016'
                adSuiteCheckName = 'GenericWrite on Privileged Users'
    ...
    @{ data = $bhNodes.ToArray()
       meta = @{ type = 'users'; count = $bhNodes.Count; version = 5; methods = 0 }
        ...
        Out-File -FilePath (Join-Path $bhDir "ACL-016_$bhTs.json") -Encoding UTF8 -Force
} catch { Write-Warning "ACL-016 BloodHound export error: $_" }
