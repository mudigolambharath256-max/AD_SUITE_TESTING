# Check: ForceChangePassword on Privileged Users
...
foreach ($sr in $scanResults) {
    ...
    $acl.GetAccessRules($true, $true, [System.Security.Principal.SecurityIdentifier]) |
        Where-Object {
            ($_.AccessControlType -eq 'Allow') -and ($_.ActiveDirectoryRights -band [System.DirectoryServices.ActiveDirectoryRights]::ExtendedRight) -and ($_.ObjectType -ne $null) -and ($_.ObjectType.ToString() -eq '00299570-246d-11d0-a768-00aa006e0529')
        } |
        ForEach-Object {
...
Write-Host "ACL-015: found $($findings.Count) trustees with ForceChangePassword"
$findings
# ── BloodHound Export ─────────────────────────────────────────────────────────
try {
    ...
    foreach ($f in $findings) {
        ...
        $bhNodes.Add(@{
            ...
            Properties = @{
                ...
                enabled          = $true
                isdeleted        = $false
                adSuiteCheckId   = 'ACL-015'
                adSuiteCheckName = 'ForceChangePassword on Privileged Users'
    ...
    @{ data = $bhNodes.ToArray()
       meta = @{ type = 'users'; count = $bhNodes.Count; version = 5; methods = 0 }
        ...
        Out-File -FilePath (Join-Path $bhDir "ACL-015_$bhTs.json") -Encoding UTF8 -Force
} catch { Write-Warning "ACL-015 BloodHound export error: $_" }
