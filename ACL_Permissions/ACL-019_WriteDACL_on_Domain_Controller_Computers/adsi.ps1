...
Write-Host "ACL-019: found $($findings.Count) trustees with WriteDacl"
$findings
# ── BloodHound Export ─────────────────────────────────────────────────────────
try {
    ...
    foreach ($f in $findings) {
        ...
        $bhNodes.Add(@{
            ...
            Properties = @{
                ...
                enabled          = $true
                isdeleted        = $false
                adSuiteCheckId   = 'ACL-019'
                adSuiteCheckName = 'WriteDACL on Domain Controller Computers'
    ...
    @{ data = $bhNodes.ToArray()
       meta = @{ type = 'users'; count = $bhNodes.Count; version = 5; methods = 0 }
        ...
        Out-File -FilePath (Join-Path $bhDir "ACL-019_$bhTs.json") -Encoding UTF8 -Force
} catch { Write-Warning "ACL-019 BloodHound export error: $_" }
