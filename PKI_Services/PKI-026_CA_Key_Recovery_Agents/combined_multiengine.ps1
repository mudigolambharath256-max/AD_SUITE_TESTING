...
Write-Host "=== PKI-026: CA Key Recovery Agents ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'PKI-026'}}, `
            @{N='CheckName'; E={'CA Key Recovery Agents'}}, `
...
if (-not $useADModule) {
    ...
    try {
        ...
        $results = $adsiResults | ForEach-Object {
            ...
            [PSCustomObject]@{
                CheckID           = 'PKI-026'
                CheckName         = 'CA Key Recovery Agents'
                Method            = 'ADSI'
                Domain            = $domainNC
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'PKI-026'
                adSuiteCheckName  = 'CA_Key_Recovery_Agents'
    ...
    $bhFile = Join-Path $bhDir "PKI-026_$bhTs.json"
    @{
