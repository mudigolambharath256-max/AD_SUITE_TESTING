...
Write-Host "=== PKI-018: Certificate Template Private Key Archival ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'PKI-018'}}, `
            @{N='CheckName'; E={'Certificate Template Private Key Archival'}}, `
...
if (-not $useADModule) {
    ...
    try {
        ...
        $results = $adsiResults | ForEach-Object {
            ...
            [PSCustomObject]@{
                CheckID           = 'PKI-018'
                CheckName         = 'Certificate Template Private Key Archival'
                Method            = 'ADSI'
                Domain            = $domainNC
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'PKI-018'
                adSuiteCheckName  = 'Certificate_Template_Private_Key_Archival'
    ...
    $bhFile = Join-Path $bhDir "PKI-018_$bhTs.json"
    @{
