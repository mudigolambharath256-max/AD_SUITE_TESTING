...
Write-Host "=== PKI-027: CA Certificate Managers ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'PKI-027'}}, `
            @{N='CheckName'; E={'CA Certificate Managers'}}, `
...
if (-not $useADModule) {
    ...
    try {
        ...
        $results = $adsiResults | ForEach-Object {
            ...
            [PSCustomObject]@{
                CheckID           = 'PKI-027'
                CheckName         = 'CA Certificate Managers'
                Method            = 'ADSI'
                Domain            = $domainNC
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'PKI-027'
                adSuiteCheckName  = 'CA_Certificate_Managers'
    ...
    $bhFile = Join-Path $bhDir "PKI-027_$bhTs.json"
    @{
