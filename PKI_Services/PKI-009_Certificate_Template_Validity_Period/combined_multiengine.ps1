...
Write-Host "=== PKI-009: Certificate Template Validity Period ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'PKI-009'}}, `
            @{N='CheckName'; E={'Certificate Template Validity Period'}}, `
...
if (-not $useADModule) {
    ...
    try {
        ...
        $results = $adsiResults | ForEach-Object {
            ...
            [PSCustomObject]@{
                CheckID           = 'PKI-009'
                CheckName         = 'Certificate Template Validity Period'
                Method            = 'ADSI'
                Domain            = $domainNC
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'PKI-009'
                adSuiteCheckName  = 'Certificate_Template_Validity_Period'
    ...
    $bhFile = Join-Path $bhDir "PKI-009_$bhTs.json"
    @{
