...
Write-Host "=== PKI-007: Certificate Template SAN Configuration ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'PKI-007'}}, `
            @{N='CheckName'; E={'Certificate Template SAN Configuration'}}, `
...
if (-not $useADModule) {
    ...
    try {
        ...
        $results = $adsiResults | ForEach-Object {
            ...
            [PSCustomObject]@{
                CheckID           = 'PKI-007'
                CheckName         = 'Certificate Template SAN Configuration'
                Method            = 'ADSI'
                Domain            = $domainNC
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'PKI-007'
                adSuiteCheckName  = 'Certificate_Template_SAN_Configuration'
    ...
    $bhFile = Join-Path $bhDir "PKI-007_$bhTs.json"
    @{
