...
Write-Host "=== PKI-002: Certificate Templates ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'PKI-002'}}, `
            @{N='CheckName'; E={'Certificate Templates'}}, `
...
if (-not $useADModule) {
    ...
    try {
        ...
        $results = $adsiResults | ForEach-Object {
            ...
            [PSCustomObject]@{
                CheckID           = 'PKI-002'
                CheckName         = 'Certificate Templates'
                Method            = 'ADSI'
                Domain            = $domainNC
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'PKI-002'
                adSuiteCheckName  = 'Certificate_Templates'
    ...
    $bhFile = Join-Path $bhDir "PKI-002_$bhTs.json"
    @{
