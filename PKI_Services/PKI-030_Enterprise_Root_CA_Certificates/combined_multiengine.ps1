...
Write-Host "=== PKI-030: Enterprise Root CA Certificates ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'PKI-030'}}, `
            @{N='CheckName'; E={'Enterprise Root CA Certificates'}}, `
...
if (-not $useADModule) {
    ...
    try {
        ...
        $results = $adsiResults | ForEach-Object {
            ...
            [PSCustomObject]@{
                CheckID           = 'PKI-030'
                CheckName         = 'Enterprise Root CA Certificates'
                Method            = 'ADSI'
                Domain            = $domainNC
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'PKI-030'
                adSuiteCheckName  = 'Enterprise_Root_CA_Certificates'
    ...
    $bhFile = Join-Path $bhDir "PKI-030_$bhTs.json"
    @{
