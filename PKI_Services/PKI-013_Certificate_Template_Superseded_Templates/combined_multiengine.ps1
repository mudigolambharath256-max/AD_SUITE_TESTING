...
Write-Host "=== PKI-013: Certificate Template Superseded Templates ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'PKI-013'}}, `
            @{N='CheckName'; E={'Certificate Template Superseded Templates'}}, `
...
if (-not $useADModule) {
    ...
    try {
        ...
        $results = $adsiResults | ForEach-Object {
            ...
            [PSCustomObject]@{
                CheckID           = 'PKI-013'
                CheckName         = 'Certificate Template Superseded Templates'
                Method            = 'ADSI'
                Domain            = $domainNC
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'PKI-013'
                adSuiteCheckName  = 'Certificate_Template_Superseded_Templates'
    ...
    $bhFile = Join-Path $bhDir "PKI-013_$bhTs.json"
    @{
