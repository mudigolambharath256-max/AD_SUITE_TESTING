...
Write-Host "=== PKI-022: CA Certificate Locations ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'PKI-022'}}, `
            @{N='CheckName'; E={'CA Certificate Locations'}}, `
...
if (-not $useADModule) {
    ...
    try {
        ...
        $results = $adsiResults | ForEach-Object {
            ...
            [PSCustomObject]@{
                CheckID           = 'PKI-022'
                CheckName         = 'CA Certificate Locations'
                Method            = 'ADSI'
                Domain            = $domainNC
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'PKI-022'
                adSuiteCheckName  = 'CA_Certificate_Locations'
    ...
    $bhFile = Join-Path $bhDir "PKI-022_$bhTs.json"
    @{
