...
Write-Host "=== PKI-003: Certificate Template Permissions ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'PKI-003'}}, `
            @{N='CheckName'; E={'Certificate Template Permissions'}}, `
...
if (-not $useADModule) {
    ...
    try {
        ...
        $results = $adsiResults | ForEach-Object {
            ...
            [PSCustomObject]@{
                CheckID           = 'PKI-003'
                CheckName         = 'Certificate Template Permissions'
                Method            = 'ADSI'
                Domain            = $domainNC
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'PKI-003'
                adSuiteCheckName  = 'Certificate_Template_Permissions'
    ...
    $bhFile = Join-Path $bhDir "PKI-003_$bhTs.json"
    @{
