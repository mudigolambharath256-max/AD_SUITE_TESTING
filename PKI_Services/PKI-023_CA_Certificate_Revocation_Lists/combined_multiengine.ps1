...
Write-Host "=== PKI-023: CA Certificate Revocation Lists ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        # Execute query
        $adObjects = Get-ADObject -LDAPFilter '(objectClass=cRLDistributionPoint)' `
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'PKI-023'}}, `
            @{N='CheckName'; E={'CA Certificate Revocation Lists'}}, `
...
if (-not $useADModule) {
    ...
    try {
        ...
        $results = $adsiResults | ForEach-Object {
            ...
            [PSCustomObject]@{
                CheckID           = 'PKI-023'
                CheckName         = 'CA Certificate Revocation Lists'
                Method            = 'ADSI'
                Domain            = $domainNC
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'PKI-023'
                adSuiteCheckName  = 'CA_Certificate_Revocation_Lists'
    ...
    $bhFile = Join-Path $bhDir "PKI-023_$bhTs.json"
    @{
