...
Write-Host "=== PKI-015: Certificate Template Application Policies ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'PKI-015'}}, `
            @{N='CheckName'; E={'Certificate Template Application Policies'}}, `
...
if (-not $useADModule) {
    ...
    try {
        ...
        $results = $adsiResults | ForEach-Object {
            ...
            [PSCustomObject]@{
                CheckID           = 'PKI-015'
                CheckName         = 'Certificate Template Application Policies'
                Method            = 'ADSI'
                Domain            = $domainNC
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'PKI-015'
                adSuiteCheckName  = 'Certificate_Template_Application_Policies'
    ...
    $bhFile = Join-Path $bhDir "PKI-015_$bhTs.json"
    @{
