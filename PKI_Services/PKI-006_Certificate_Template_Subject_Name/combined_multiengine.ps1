...
Write-Host "=== PKI-006: Certificate Template Subject Name ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'PKI-006'}}, `
            @{N='CheckName'; E={'Certificate Template Subject Name'}}, `
...
if (-not $useADModule) {
    ...
    try {
        ...
        $results = $adsiResults | ForEach-Object {
            ...
            [PSCustomObject]@{
                CheckID           = 'PKI-006'
                CheckName         = 'Certificate Template Subject Name'
                Method            = 'ADSI'
                Domain            = $domainNC
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'PKI-006'
                adSuiteCheckName  = 'Certificate_Template_Subject_Name'
    ...
    $bhFile = Join-Path $bhDir "PKI-006_$bhTs.json"
    @{
