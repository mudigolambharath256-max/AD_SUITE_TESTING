...
Write-Host "=== PKI-010: Certificate Template Renewal Period ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'PKI-010'}}, `
            @{N='CheckName'; E={'Certificate Template Renewal Period'}}, `
...
if (-not $useADModule) {
    ...
    try {
        ...
        $results = $adsiResults | ForEach-Object {
            ...
            [PSCustomObject]@{
                CheckID           = 'PKI-010'
                CheckName         = 'Certificate Template Renewal Period'
                Method            = 'ADSI'
                Domain            = $domainNC
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'PKI-010'
                adSuiteCheckName  = 'Certificate_Template_Renewal_Period'
    ...
    $bhFile = Join-Path $bhDir "PKI-010_$bhTs.json"
    @{
