...
Write-Host "=== PKI-012: Certificate Template Cryptographic Providers ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        # Execute query
        $adObjects = Get-ADObject -LDAPFilter '(objectClass=pKICertificateTemplate)' `
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'PKI-012'}}, `
            @{N='CheckName'; E={'Certificate Template Cryptographic Providers'}}, `
...
if (-not $useADModule) {
    ...
    try {
        ...
        $results = $adsiResults | ForEach-Object {
            ...
            [PSCustomObject]@{
                CheckID           = 'PKI-012'
                CheckName         = 'Certificate Template Cryptographic Providers'
                Method            = 'ADSI'
                Domain            = $domainNC
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'PKI-012'
                adSuiteCheckName  = 'Certificate_Template_Cryptographic_Providers'
                adSuiteMEDIUM   = 'MEDIUM'
                adSuitePKI_Services   = 'PKI_Services'
                adSuiteFlag       = $true
            }
            Aces      = @()
            IsDeleted = $false
    ...
    $bhFile = Join-Path $bhDir "PKI-012_$bhTs.json"
    @{
