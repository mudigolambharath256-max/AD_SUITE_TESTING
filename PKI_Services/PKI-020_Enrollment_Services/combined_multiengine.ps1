...
Write-Host "=== PKI-020: Enrollment Services ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        # Execute query
        $adObjects = Get-ADObject -LDAPFilter '(objectClass=pKIEnrollmentService)' `
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'PKI-020'}}, `
            @{N='CheckName'; E={'Enrollment Services'}}, `
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'PKI-020'
                adSuiteCheckName  = 'Enrollment_Services'
    ...
    $bhFile = Join-Path $bhDir "PKI-020_$bhTs.json"
    @{
