...
Write-Host "=== PKI-011: Certificate Template Key Size ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'PKI-011'}}, `
            @{N='CheckName'; E={'Certificate Template Key Size'}}, `
...
if (-not $useADModule) {
    ...
    try {
        ...
        $results = $adsiResults | ForEach-Object {
            ...
            [PSCustomObject]@{
                CheckID           = 'PKI-011'
                CheckName         = 'Certificate Template Key Size'
                Method            = 'ADSI'
                Domain            = $domainNC
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'PKI-011'
                adSuiteCheckName  = 'Certificate_Template_Key_Size'
    ...
    $bhFile = Join-Path $bhDir "PKI-011_$bhTs.json"
    @{
