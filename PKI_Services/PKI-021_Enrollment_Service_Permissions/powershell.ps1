...
Write-Host "Executing security check: Enrollment Service Permissions" -ForegroundColor Yellow
Write-Host "Check ID: PKI-021" -ForegroundColor Gray
...
$output = $results | Select-Object `
    @{N='CheckID'; E={'PKI-021'}}, `
    @{N='CheckName'; E={'Enrollment Service Permissions'}}, `
...
try {
    ...
    foreach ($r in $output) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'PKI-021'
                adSuiteCheckName  = 'Enrollment_Service_Permissions'
    ...
    $bhFile = Join-Path $bhDir "PKI-021_$bhTs.json"
    @{
