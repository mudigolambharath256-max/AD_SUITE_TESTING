...
Write-Host "=== PKI-017: Certificate Template Authorized Signatures ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'PKI-017'}}, `
            @{N='CheckName'; E={'Certificate Template Authorized Signatures'}}, `
...
if (-not $useADModule) {
    ...
    try {
        ...
        $results = $adsiResults | ForEach-Object {
            ...
            [PSCustomObject]@{
                CheckID           = 'PKI-017'
                CheckName         = 'Certificate Template Authorized Signatures'
                Method            = 'ADSI'
                Domain            = $domainNC
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'PKI-017'
                adSuiteCheckName  = 'Certificate_Template_Authorized_Signatures'
    ...
    $bhFile = Join-Path $bhDir "PKI-017_$bhTs.json"
    @{
