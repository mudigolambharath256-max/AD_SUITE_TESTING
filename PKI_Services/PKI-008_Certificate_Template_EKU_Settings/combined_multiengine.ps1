...
Write-Host "=== PKI-008: Certificate Template EKU Settings ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'PKI-008'}}, `
            @{N='CheckName'; E={'Certificate Template EKU Settings'}}, `
...
if (-not $useADModule) {
    ...
    try {
        ...
        $results = $adsiResults | ForEach-Object {
            ...
            [PSCustomObject]@{
                CheckID           = 'PKI-008'
                CheckName         = 'Certificate Template EKU Settings'
                Method            = 'ADSI'
                Domain            = $domainNC
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'PKI-008'
                adSuiteCheckName  = 'Certificate_Template_EKU_Settings'
    ...
    $bhFile = Join-Path $bhDir "PKI-008_$bhTs.json"
    @{
