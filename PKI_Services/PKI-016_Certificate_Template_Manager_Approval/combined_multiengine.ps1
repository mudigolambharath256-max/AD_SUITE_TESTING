...
Write-Host "=== PKI-016: Certificate Template Manager Approval ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'PKI-016'}}, `
            @{N='CheckName'; E={'Certificate Template Manager Approval'}}, `
...
if (-not $useADModule) {
    ...
    try {
        ...
        $results = $adsiResults | ForEach-Object {
            ...
            [PSCustomObject]@{
                CheckID           = 'PKI-016'
                CheckName         = 'Certificate Template Manager Approval'
                Method            = 'ADSI'
                Domain            = $domainNC
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'PKI-016'
                adSuiteCheckName  = 'Certificate_Template_Manager_Approval'
    ...
    $bhFile = Join-Path $bhDir "PKI-016_$bhTs.json"
    @{
