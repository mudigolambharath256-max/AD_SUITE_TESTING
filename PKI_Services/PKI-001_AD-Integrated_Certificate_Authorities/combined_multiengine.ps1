...
Write-Host "=== PKI-001: AD-Integrated Certificate Authorities ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        # Execute query
        $adObjects = Get-ADObject -LDAPFilter '(objectClass=certificationAuthority)' `
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'PKI-001'}}, `
            @{N='CheckName'; E={'AD-Integrated Certificate Authorities'}}, `
...
if (-not $useADModule) {
    ...
    try {
        ...
        $results = $adsiResults | ForEach-Object {
            ...
            [PSCustomObject]@{
                CheckID           = 'PKI-001'
                CheckName         = 'AD-Integrated Certificate Authorities'
                Method            = 'ADSI'
                Domain            = $domainNC
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'PKI-001'
                adSuiteCheckName  = 'AD-Integrated_Certificate_Authorities'
    ...
    $bhFile = Join-Path $bhDir "PKI-001_$bhTs.json"
    @{
