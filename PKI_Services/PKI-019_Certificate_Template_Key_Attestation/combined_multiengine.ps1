...
Write-Host "=== PKI-019: Certificate Template Key Attestation ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'PKI-019'}}, `
            @{N='CheckName'; E={'Certificate Template Key Attestation'}}, `
...
if (-not $useADModule) {
    ...
    try {
        ...
        $results = $adsiResults | ForEach-Object {
            ...
            [PSCustomObject]@{
                CheckID           = 'PKI-019'
                CheckName         = 'Certificate Template Key Attestation'
                Method            = 'ADSI'
                Domain            = $domainNC
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'PKI-019'
                adSuiteCheckName  = 'Certificate_Template_Key_Attestation'
    ...
    $bhFile = Join-Path $bhDir "PKI-019_$bhTs.json"
    @{
