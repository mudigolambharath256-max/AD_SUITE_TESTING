...
Write-Host "=== PKI-004: Certificate Template Enrollment Rights ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'PKI-004'}}, `
            @{N='CheckName'; E={'Certificate Template Enrollment Rights'}}, `
...
if (-not $useADModule) {
    ...
    try {
        ...
        $results = $adsiResults | ForEach-Object {
            ...
            [PSCustomObject]@{
                CheckID           = 'PKI-004'
                CheckName         = 'Certificate Template Enrollment Rights'
                Method            = 'ADSI'
                Domain            = $domainNC
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'PKI-004'
                adSuiteCheckName  = 'Certificate_Template_Enrollment_Rights'
    ...
    $bhFile = Join-Path $bhDir "PKI-004_$bhTs.json"
    @{
