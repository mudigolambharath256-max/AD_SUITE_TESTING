...
Write-Host "=== PKI-014: Certificate Template Issuance Requirements ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'PKI-014'}}, `
            @{N='CheckName'; E={'Certificate Template Issuance Requirements'}}, `
...
if (-not $useADModule) {
    ...
    try {
        ...
        $results = $adsiResults | ForEach-Object {
            ...
            [PSCustomObject]@{
                CheckID           = 'PKI-014'
                CheckName         = 'Certificate Template Issuance Requirements'
                Method            = 'ADSI'
                Domain            = $domainNC
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'PKI-014'
                adSuiteCheckName  = 'Certificate_Template_Issuance_Requirements'
    ...
    $bhFile = Join-Path $bhDir "PKI-014_$bhTs.json"
    @{
