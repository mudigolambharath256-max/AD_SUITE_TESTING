...
Write-Host "=== PKI-005: Certificate Template Auto Enrollment ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'PKI-005'}}, `
            @{N='CheckName'; E={'Certificate Template Auto Enrollment'}}, `
...
if (-not $useADModule) {
    ...
    try {
        ...
        $results = $adsiResults | ForEach-Object {
            ...
            [PSCustomObject]@{
                CheckID           = 'PKI-005'
                CheckName         = 'Certificate Template Auto Enrollment'
                Method            = 'ADSI'
                Domain            = $domainNC
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'PKI-005'
                adSuiteCheckName  = 'Certificate_Template_Auto_Enrollment'
    ...
    $bhFile = Join-Path $bhDir "PKI-005_$bhTs.json"
    @{
