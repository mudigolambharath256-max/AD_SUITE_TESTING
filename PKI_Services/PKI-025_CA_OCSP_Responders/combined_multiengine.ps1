...
Write-Host "=== PKI-025: CA OCSP Responders ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'PKI-025'}}, `
            @{N='CheckName'; E={'CA OCSP Responders'}}, `
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'PKI-025'
                adSuiteCheckName  = 'CA_OCSP_Responders'
    ...
    $bhFile = Join-Path $bhDir "PKI-025_$bhTs.json"
    @{
