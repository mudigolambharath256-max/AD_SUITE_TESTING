...
Write-Host "=== PKI-024: CA AIA Locations ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'PKI-024'}}, `
            @{N='CheckName'; E={'CA AIA Locations'}}, `
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'PKI-024'
                adSuiteCheckName  = 'CA_AIA_Locations'
    ...
    $bhFile = Join-Path $bhDir "PKI-024_$bhTs.json"
    @{
