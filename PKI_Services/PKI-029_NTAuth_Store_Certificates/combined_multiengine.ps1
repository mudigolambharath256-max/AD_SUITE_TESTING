...
Write-Host "=== PKI-029: NTAuth Store Certificates ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'PKI-029'}}, `
            @{N='CheckName'; E={'NTAuth Store Certificates'}}, `
...
if (-not $useADModule) {
    ...
    try {
        ...
        $results = $adsiResults | ForEach-Object {
            ...
            [PSCustomObject]@{
                CheckID           = 'PKI-029'
                CheckName         = 'NTAuth Store Certificates'
                Method            = 'ADSI'
                Domain            = $domainNC
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'PKI-029'
                adSuiteCheckName  = 'NTAuth_Store_Certificates'
    ...
    $bhFile = Join-Path $bhDir "PKI-029_$bhTs.json"
    @{
