...
Write-Host "=== DCONF-001: Kerberos Encryption Types Domain ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'DCONF-001'}}, `
            @{N='CheckName'; E={'Kerberos Encryption Types Domain'}}, `
...
if (-not $useADModule) {
    ...
    try {
        ...
        $results = $adsiResults | ForEach-Object {
            ...
            [PSCustomObject]@{
                CheckID           = 'DCONF-001'
                CheckName         = 'Kerberos Encryption Types Domain'
                Method            = 'ADSI'
                Domain            = $domainNC
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'DCONF-001'
                adSuiteCheckName  = 'Kerberos_Encryption_Types_Domain'
    ...
    $bhFile = Join-Path $bhDir "DCONF-001_$bhTs.json"
    @{
