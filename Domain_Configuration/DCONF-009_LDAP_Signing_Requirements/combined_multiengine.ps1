...
Write-Host "=== DCONF-009: LDAP Signing Requirements ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        # Execute query
        $adObjects = Get-ADObject -LDAPFilter '(objectClass=nTDSDSA)' `
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'DCONF-009'}}, `
            @{N='CheckName'; E={'LDAP Signing Requirements'}}, `
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'DCONF-009'
                adSuiteCheckName  = 'LDAP_Signing_Requirements'
    ...
    $bhFile = Join-Path $bhDir "DCONF-009_$bhTs.json"
    @{
