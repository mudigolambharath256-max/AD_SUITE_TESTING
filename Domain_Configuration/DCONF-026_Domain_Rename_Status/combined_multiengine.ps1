...
Write-Host "=== DCONF-026: Domain Rename Status ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'DCONF-026'}}, `
            @{N='CheckName'; E={'Domain Rename Status'}}, `
...
if (-not $useADModule) {
    ...
    try {
        ...
        $results = $adsiResults | ForEach-Object {
            ...
            [PSCustomObject]@{
                CheckID           = 'DCONF-026'
                CheckName         = 'Domain Rename Status'
                Method            = 'ADSI'
                Domain            = $domainNC
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'DCONF-026'
                adSuiteCheckName  = 'Domain_Rename_Status'
    ...
    $bhFile = Join-Path $bhDir "DCONF-026_$bhTs.json"
    @{
