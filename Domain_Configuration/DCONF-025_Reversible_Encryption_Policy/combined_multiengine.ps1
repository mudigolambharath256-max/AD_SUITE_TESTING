...
Write-Host "=== DCONF-025: Reversible Encryption Policy ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'DCONF-025'}}, `
            @{N='CheckName'; E={'Reversible Encryption Policy'}}, `
...
if (-not $useADModule) {
    ...
    try {
        ...
        $results = $adsiResults | ForEach-Object {
            ...
            [PSCustomObject]@{
                CheckID           = 'DCONF-025'
                CheckName         = 'Reversible Encryption Policy'
                Method            = 'ADSI'
                Domain            = $domainNC
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'DCONF-025'
                adSuiteCheckName  = 'Reversible_Encryption_Policy'
    ...
    $bhFile = Join-Path $bhDir "DCONF-025_$bhTs.json"
    @{
