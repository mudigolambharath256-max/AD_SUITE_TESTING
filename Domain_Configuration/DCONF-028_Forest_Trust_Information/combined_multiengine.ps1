...
Write-Host "=== DCONF-028: Forest Trust Information ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'DCONF-028'}}, `
            @{N='CheckName'; E={'Forest Trust Information'}}, `
...
if (-not $useADModule) {
    ...
    try {
        ...
        $results = $adsiResults | ForEach-Object {
            ...
            [PSCustomObject]@{
                CheckID           = 'DCONF-028'
                CheckName         = 'Forest Trust Information'
                Method            = 'ADSI'
                Domain            = $domainNC
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'DCONF-028'
                adSuiteCheckName  = 'Forest_Trust_Information'
    ...
    $bhFile = Join-Path $bhDir "DCONF-028_$bhTs.json"
    @{
