...
Write-Host "=== DCONF-010: LDAP Channel Binding ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'DCONF-010'}}, `
            @{N='CheckName'; E={'LDAP Channel Binding'}}, `
...
if (-not $useADModule) {
    ...
    try {
        ...
        $results = $adsiResults | ForEach-Object {
            ...
            [PSCustomObject]@{
                CheckID           = 'DCONF-010'
                CheckName         = 'LDAP Channel Binding'
                Method            = 'ADSI'
                Domain            = $domainNC
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'DCONF-010'
                adSuiteCheckName  = 'LDAP_Channel_Binding'
    ...
    $bhFile = Join-Path $bhDir "DCONF-010_$bhTs.json"
    @{
