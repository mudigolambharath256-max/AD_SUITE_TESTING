...
Write-Host "=== DCONF-017: Domain Controller Lockout Threshold ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        # Execute query
        $adObjects = Get-ADObject -LDAPFilter '(objectClass=domain)' `
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'DCONF-017'}}, `
            @{N='CheckName'; E={'Domain Controller Lockout Threshold'}}, `
...
if (-not $useADModule) {
    ...
    try {
        ...
        $results = $adsiResults | ForEach-Object {
            ...
            [PSCustomObject]@{
                CheckID           = 'DCONF-017'
                CheckName         = 'Domain Controller Lockout Threshold'
                Method            = 'ADSI'
                Domain            = $domainNC
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'DCONF-017'
                adSuiteCheckName  = 'Domain_Controller_Lockout_Threshold'
    ...
    $bhFile = Join-Path $bhDir "DCONF-017_$bhTs.json"
    @{
