...
# Check: Domain Configuration Check 5
...
Write-Host "=== Domain Configuration Check 5 ===" -ForegroundColor Cyan
...
try {
    ...
    Write-Warning "DOM-005 PowerShell engine: $_"   # Fix R10: no silent catch
}
...
try {
    ...
    Write-Warning "DOM-005 ADSI engine: $_"          # Fix R10
}
...
try {
    ...
    Write-Warning "DOM-005 C# engine: $_"             # Fix R10
}
...
Write-Host ""
Write-Host "=== DOM-005: $($uniqueResults.Count) unique findings ===" -ForegroundColor Cyan
$uniqueResults | Format-List

# Fix R14: Fixed 5-field schema enforced before CSV export
$csvPath = Join-Path $env:TEMP "DOM-005_results.csv"
$uniqueResults |
