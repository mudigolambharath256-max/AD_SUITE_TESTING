...
Write-Host "=== DCONF-016: Kerberos Renewal Lifetime ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'DCONF-016'}}, `
            @{N='CheckName'; E={'Kerberos Renewal Lifetime'}}, `
...
if (-not $useADModule) {
    ...
    try {
        ...
        $results = $adsiResults | ForEach-Object {
            ...
            [PSCustomObject]@{
                CheckID           = 'DCONF-016'
                CheckName         = 'Kerberos Renewal Lifetime'
                Method            = 'ADSI'
                Domain            = $domainNC
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'DCONF-016'
                adSuiteCheckName  = 'Kerberos_Renewal_Lifetime'
    ...
    $bhFile = Join-Path $bhDir "DCONF-016_$bhTs.json"
    @{
