...
Write-Host "=== DCONF-002: Recycle Bin Enabled Check ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        # Execute query
        $adObjects = Get-ADObject -LDAPFilter '(objectClass=msDS-EnabledFeature)' `
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'DCONF-002'}}, `
            @{N='CheckName'; E={'Recycle Bin Enabled Check'}}, `
...
if (-not $useADModule) {
    ...
    try {
        ...
        $searcher.Filter = '(objectClass=msDS-EnabledFeature)'
        ...
        $results = $adsiResults | ForEach-Object {
            ...
            [PSCustomObject]@{
                CheckID           = 'DCONF-002'
                CheckName         = 'Recycle Bin Enabled Check'
                Method            = 'ADSI'
                Domain            = $domainNC
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'DCONF-002'
                adSuiteCheckName  = 'Recycle_Bin_Enabled_Check'
    ...
    $bhFile = Join-Path $bhDir "DCONF-002_$bhTs.json"
    @{
