...
Write-Host "=== DCONF-006: Anonymous LDAP Access ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'DCONF-006'}}, `
            @{N='CheckName'; E={'Anonymous LDAP Access'}}, `
...
if (-not $useADModule) {
    ...
    try {
        ...
        $results = $adsiResults | ForEach-Object {
            ...
            [PSCustomObject]@{
                CheckID           = 'DCONF-006'
                CheckName         = 'Anonymous LDAP Access'
                Method            = 'ADSI'
                Domain            = $domainNC
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'DCONF-006'
                adSuiteCheckName  = 'Anonymous_LDAP_Access'
    ...
    $bhFile = Join-Path $bhDir "DCONF-006_$bhTs.json"
    @{
