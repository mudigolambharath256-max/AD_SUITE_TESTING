...
Write-Host "=== DCONF-019: Reset Account Lockout Counter ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'DCONF-019'}}, `
            @{N='CheckName'; E={'Reset Account Lockout Counter'}}, `
...
if (-not $useADModule) {
    ...
    try {
        ...
        $results = $adsiResults | ForEach-Object {
            ...
            [PSCustomObject]@{
                CheckID           = 'DCONF-019'
                CheckName         = 'Reset Account Lockout Counter'
                Method            = 'ADSI'
                Domain            = $domainNC
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'DCONF-019'
                adSuiteCheckName  = 'Reset_Account_Lockout_Counter'
    ...
    $bhFile = Join-Path $bhDir "DCONF-019_$bhTs.json"
    @{
