...
# Check: Domain Configuration Check 8
...
Write-Host "=== Domain Configuration Check 8 ===" -ForegroundColor Cyan
...
try {
    ...
    Write-Warning "DOM-008 PowerShell engine: $_"   # Fix R10: no silent catch
}
...
try {
    ...
    Write-Warning "DOM-008 ADSI engine: $_"          # Fix R10
}
...
try {
    ...
    Write-Warning "DOM-008 C# engine: $_"             # Fix R10
}
...
Write-Host ""
Write-Host "=== DOM-008: $($uniqueResults.Count) unique findings ===" -ForegroundColor Cyan
$uniqueResults | Format-List

# Fix R14: Fixed 5-field schema enforced before CSV export
$csvPath = Join-Path $env:TEMP "DOM-008_results.csv"
$uniqueResults |
