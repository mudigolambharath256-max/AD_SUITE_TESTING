...
# Check: Domain Configuration Check 41
...
Write-Host "=== Domain Configuration Check 41 ===" -ForegroundColor Cyan
...
try {
    ...
    Write-Warning "DOM-041 PowerShell engine: $_"   # Fix R10: no silent catch
}
...
try {
    ...
    Write-Warning "DOM-041 ADSI engine: $_"          # Fix R10
}
...
try {
    ...
    Write-Warning "DOM-041 C# engine: $_"             # Fix R10
}
...
Write-Host ""
Write-Host "=== DOM-041: $($uniqueResults.Count) unique findings ===" -ForegroundColor Cyan
$uniqueResults | Format-List

# Fix R14: Fixed 5-field schema enforced before CSV export
$csvPath = Join-Path $env:TEMP "DOM-041_results.csv"
$uniqueResults |
