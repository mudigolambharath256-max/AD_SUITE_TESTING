...
Write-Host "DOM-053: found $($results.Count) objects"
...
try {
    ...
    foreach ($r in $bhRaw) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                isdeleted         = $false
                adSuiteCheckId    = 'DOM-053'
                adSuiteCheckName  = 'Domain Configuration Check 53'
    ...
    @{ data = $bhNodes.ToArray()
       meta = @{ type = 'domains'; count = $bhNodes.Count; version = 5; methods = 0 }
        ...
        Out-File -FilePath (Join-Path $bhDir "DOM-053_$bhTs.json") -Encoding UTF8 -Force

# Fix R10: No silent catch — warn on failure
} catch { Write-Warning "DOM-053 BloodHound export error: $_" }
