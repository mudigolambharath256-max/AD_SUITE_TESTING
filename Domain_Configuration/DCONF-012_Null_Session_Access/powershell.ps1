...
Write-Host "Executing security check: Null Session Access" -ForegroundColor Yellow
Write-Host "Check ID: DCONF-012" -ForegroundColor Gray
...
$output = $results | Select-Object `
    @{N='CheckID'; E={'DCONF-012'}}, `
    @{N='CheckName'; E={'Null Session Access'}}, `
...
try {
    ...
    foreach ($r in $output) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'DCONF-012'
                adSuiteCheckName  = 'Null_Session_Access'
    ...
    $bhFile = Join-Path $bhDir "DCONF-012_$bhTs.json"
    @{
