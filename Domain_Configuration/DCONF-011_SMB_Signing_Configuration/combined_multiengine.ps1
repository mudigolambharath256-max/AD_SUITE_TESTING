...
Write-Host "=== DCONF-011: SMB Signing Configuration ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'DCONF-011'}}, `
            @{N='CheckName'; E={'SMB Signing Configuration'}}, `
...
if (-not $useADModule) {
    ...
    try {
        ...
        $results = $adsiResults | ForEach-Object {
            ...
            [PSCustomObject]@{
                CheckID           = 'DCONF-011'
                CheckName         = 'SMB Signing Configuration'
                Method            = 'ADSI'
                Domain            = $domainNC
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'DCONF-011'
                adSuiteCheckName  = 'SMB_Signing_Configuration'
    ...
    $bhFile = Join-Path $bhDir "DCONF-011_$bhTs.json"
    @{
