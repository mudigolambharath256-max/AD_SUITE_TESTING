...
Write-Host "=== DCONF-030: FSMO Role Holders ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'DCONF-030'}}, `
            @{N='CheckName'; E={'FSMO Role Holders'}}, `
...
try {
    ...
    $bhFile = Join-Path $bhDir "DCONF-030_$bhTs.json"
    @{
