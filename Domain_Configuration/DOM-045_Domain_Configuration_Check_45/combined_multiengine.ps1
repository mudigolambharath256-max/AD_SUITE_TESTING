...
# Check: Domain Configuration Check 45
...
Write-Host "=== Domain Configuration Check 45 ===" -ForegroundColor Cyan
...
try {
    ...
    Write-Warning "DOM-045 PowerShell engine: $_"   # Fix R10: no silent catch
}
...
try {
    ...
    Write-Warning "DOM-045 ADSI engine: $_"          # Fix R10
}
...
try {
    ...
    Write-Warning "DOM-045 C# engine: $_"             # Fix R10
}
...
Write-Host ""
Write-Host "=== DOM-045: $($uniqueResults.Count) unique findings ===" -ForegroundColor Cyan
$uniqueResults | Format-List

# Fix R14: Fixed 5-field schema enforced before CSV export
$csvPath = Join-Path $env:TEMP "DOM-045_results.csv"
$uniqueResults |
