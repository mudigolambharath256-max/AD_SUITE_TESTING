...
Write-Host "=== DCONF-027: Schema Version Check ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        # Execute query
        $adObjects = Get-ADObject -LDAPFilter '(objectClass=dMD)' `
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'DCONF-027'}}, `
            @{N='CheckName'; E={'Schema Version Check'}}, `
...
if (-not $useADModule) {
    ...
    try {
        ...
        $results = $adsiResults | ForEach-Object {
            ...
            [PSCustomObject]@{
                CheckID           = 'DCONF-027'
                CheckName         = 'Schema Version Check'
                Method            = 'ADSI'
                Domain            = $domainNC
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'DCONF-027'
                adSuiteCheckName  = 'Schema_Version_Check'
    ...
    $bhFile = Join-Path $bhDir "DCONF-027_$bhTs.json"
    @{
