...
Write-Host "=== DCONF-003: ms-DS-MachineAccountQuota Setting ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'DCONF-003'}}, `
            @{N='CheckName'; E={'ms-DS-MachineAccountQuota Setting'}}, `
...
if (-not $useADModule) {
    ...
    try {
        ...
        $results = $adsiResults | ForEach-Object {
            ...
            [PSCustomObject]@{
                CheckID           = 'DCONF-003'
                CheckName         = 'ms-DS-MachineAccountQuota Setting'
                Method            = 'ADSI'
                Domain            = $domainNC
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'DCONF-003'
                adSuiteCheckName  = 'ms-DS-MachineAccountQuota_Setting'
    ...
    $bhFile = Join-Path $bhDir "DCONF-003_$bhTs.json"
    @{
