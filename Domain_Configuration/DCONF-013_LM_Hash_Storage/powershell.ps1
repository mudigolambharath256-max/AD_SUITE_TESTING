...
Write-Host "Executing security check: LM Hash Storage" -ForegroundColor Yellow
Write-Host "Check ID: DCONF-013" -ForegroundColor Gray
...
$output = $results | Select-Object `
    @{N='CheckID'; E={'DCONF-013'}}, `
    @{N='CheckName'; E={'LM Hash Storage'}}, `
...
try {
    ...
    foreach ($r in $output) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'DCONF-013'
                adSuiteCheckName  = 'LM_Hash_Storage'
    ...
    $bhFile = Join-Path $bhDir "DCONF-013_$bhTs.json"
    @{
