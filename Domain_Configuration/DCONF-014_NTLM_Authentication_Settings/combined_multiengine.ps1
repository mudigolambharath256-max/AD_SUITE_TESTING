...
Write-Host "=== DCONF-014: NTLM Authentication Settings ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'DCONF-014'}}, `
            @{N='CheckName'; E={'NTLM Authentication Settings'}}, `
...
if (-not $useADModule) {
    ...
    try {
        ...
        $results = $adsiResults | ForEach-Object {
            ...
            [PSCustomObject]@{
                CheckID           = 'DCONF-014'
                CheckName         = 'NTLM Authentication Settings'
                Method            = 'ADSI'
                Domain            = $domainNC
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'DCONF-014'
                adSuiteCheckName  = 'NTLM_Authentication_Settings'
    ...
    $bhFile = Join-Path $bhDir "DCONF-014_$bhTs.json"
    @{
