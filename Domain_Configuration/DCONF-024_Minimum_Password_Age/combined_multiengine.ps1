...
Write-Host "=== DCONF-024: Minimum Password Age ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'DCONF-024'}}, `
            @{N='CheckName'; E={'Minimum Password Age'}}, `
...
if (-not $useADModule) {
    ...
    try {
        ...
        $results = $adsiResults | ForEach-Object {
            ...
            [PSCustomObject]@{
                CheckID           = 'DCONF-024'
                CheckName         = 'Minimum Password Age'
                Method            = 'ADSI'
                Domain            = $domainNC
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'DCONF-024'
                adSuiteCheckName  = 'Minimum_Password_Age'
    ...
    $bhFile = Join-Path $bhDir "DCONF-024_$bhTs.json"
    @{
