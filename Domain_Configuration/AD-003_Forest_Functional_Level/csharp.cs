...
class Program
{
    static void Main()
    {
        string filter = "(objectClass=crossRefContainer)";
        ...
        string[] props = new string[] { "name", "distinguishedName", "msDS-Behavior-Version" };

        using (DirectoryEntry rootDSE = new DirectoryEntry("LDAP://RootDSE"))
        {
            string configNC = rootDSE.Properties["configurationNamingContext"].Value.ToString();

            using (DirectoryEntry config = new DirectoryEntry("LDAP://" + configNC))
            using (DirectorySearcher searcher = new DirectorySearcher(config))
            {
                ...
                foreach (SearchResult r in searcher.FindAll())
                {
                    ...
                    string behavior = r.Properties.Contains("msDS-Behavior-Version") && r.Properties["msDS-Behavior-Version"].Count > 0
                        ? r.Properties["msDS-Behavior-Version"][0].ToString()
                        : "(not set)";

                    Console.WriteLine("Name: " + name);
                    Console.WriteLine("Forest Functional Level (msDS-Behavior-Version): " + behavior);
                    Console.WriteLine("--------------------------------------------");
                }
            }
        }
