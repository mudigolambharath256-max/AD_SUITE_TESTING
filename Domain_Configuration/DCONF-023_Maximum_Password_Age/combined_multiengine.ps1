...
Write-Host "=== DCONF-023: Maximum Password Age ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'DCONF-023'}}, `
            @{N='CheckName'; E={'Maximum Password Age'}}, `
...
if (-not $useADModule) {
    ...
    try {
        ...
        $results = $adsiResults | ForEach-Object {
            ...
            [PSCustomObject]@{
                CheckID           = 'DCONF-023'
                CheckName         = 'Maximum Password Age'
                Method            = 'ADSI'
                Domain            = $domainNC
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'DCONF-023'
                adSuiteCheckName  = 'Maximum_Password_Age'
    ...
    $bhFile = Join-Path $bhDir "DCONF-023_$bhTs.json"
    @{
