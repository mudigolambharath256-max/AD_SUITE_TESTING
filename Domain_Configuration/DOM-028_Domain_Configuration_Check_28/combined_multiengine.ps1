...
# Check: Domain Configuration Check 28
...
Write-Host "=== Domain Configuration Check 28 ===" -ForegroundColor Cyan
...
try {
    ...
    Write-Warning "DOM-028 PowerShell engine: $_"   # Fix R10: no silent catch
}
...
try {
    ...
    Write-Warning "DOM-028 ADSI engine: $_"          # Fix R10
}
...
try {
    ...
    Write-Warning "DOM-028 C# engine: $_"             # Fix R10
}
...
Write-Host ""
Write-Host "=== DOM-028: $($uniqueResults.Count) unique findings ===" -ForegroundColor Cyan
$uniqueResults | Format-List

# Fix R14: Fixed 5-field schema enforced before CSV export
$csvPath = Join-Path $env:TEMP "DOM-028_results.csv"
$uniqueResults |
