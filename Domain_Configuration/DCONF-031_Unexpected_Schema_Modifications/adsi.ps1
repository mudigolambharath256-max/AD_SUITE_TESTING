...
Write-Host "Found $($results.Count) objects for check: Unexpected Schema Modifications" -ForegroundColor Cyan

$results | ForEach-Object {
    $props = $_.Properties
    [PSCustomObject]@{
        CheckID = 'DCONF-031'
        CheckName = 'Unexpected Schema Modifications'
...
try {
    ...
    if ($results -and $results.Count -gt 0) {
        ...
        $bhFile = Join-Path $bhDir "DCONF-031_nodes.json"
