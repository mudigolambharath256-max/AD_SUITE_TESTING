...
Write-Host "Executing security check: Password Complexity Requirements" -ForegroundColor Yellow
Write-Host "Check ID: DCONF-021" -ForegroundColor Gray
...
$output = $results | Select-Object `
    @{N='CheckID'; E={'DCONF-021'}}, `
    @{N='CheckName'; E={'Password Complexity Requirements'}}, `
...
try {
    ...
    foreach ($r in $output) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'DCONF-021'
                adSuiteCheckName  = 'Password_Complexity_Requirements'
    ...
    $bhFile = Join-Path $bhDir "DCONF-021_$bhTs.json"
    @{
