...
Write-Host "=== DCONF-004: Default Domain Password Policy ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'DCONF-004'}}, `
            @{N='CheckName'; E={'Default Domain Password Policy'}}, `
...
if (-not $useADModule) {
    ...
    try {
        ...
        $results = $adsiResults | ForEach-Object {
            ...
            [PSCustomObject]@{
                CheckID           = 'DCONF-004'
                CheckName         = 'Default Domain Password Policy'
                Method            = 'ADSI'
                Domain            = $domainNC
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'DCONF-004'
                adSuiteCheckName  = 'Default_Domain_Password_Policy'
    ...
    $bhFile = Join-Path $bhDir "DCONF-004_$bhTs.json"
    @{
