...
# Check: Domain Configuration Check 7
...
Write-Host "=== Domain Configuration Check 7 ===" -ForegroundColor Cyan
...
try {
    ...
    Write-Warning "DOM-007 PowerShell engine: $_"   # Fix R10: no silent catch
}
...
try {
    ...
    Write-Warning "DOM-007 ADSI engine: $_"          # Fix R10
}
...
try {
    ...
    Write-Warning "DOM-007 C# engine: $_"             # Fix R10
}
...
Write-Host ""
Write-Host "=== DOM-007: $($uniqueResults.Count) unique findings ===" -ForegroundColor Cyan
$uniqueResults | Format-List

# Fix R14: Fixed 5-field schema enforced before CSV export
$csvPath = Join-Path $env:TEMP "DOM-007_results.csv"
$uniqueResults |
