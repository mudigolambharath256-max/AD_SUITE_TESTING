...
$ldapFilter = '(objectClass=nTDSService)'

# Define properties to retrieve
$properties = @('name', 'distinguishedName', 'objectClass', 'whenCreated', 'whenChanged', 'objectSid')

Write-Host "Executing security check: Tombstone Lifetime" -ForegroundColor Yellow
Write-Host "Check ID: DCONF-005" -ForegroundColor Gray
...
$output = $results | Select-Object `
    @{N='CheckID'; E={'DCONF-005'}}, `
    @{N='CheckName'; E={'Tombstone Lifetime'}}, `
...
try {
    ...
    foreach ($r in $output) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'DCONF-005'
                adSuiteCheckName  = 'Tombstone_Lifetime'
    ...
    $bhFile = Join-Path $bhDir "DCONF-005_$bhTs.json"
    @{
