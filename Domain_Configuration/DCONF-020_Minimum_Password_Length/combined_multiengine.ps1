...
Write-Host "=== DCONF-020: Minimum Password Length ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'DCONF-020'}}, `
            @{N='CheckName'; E={'Minimum Password Length'}}, `
...
if (-not $useADModule) {
    ...
    try {
        ...
        $results = $adsiResults | ForEach-Object {
            ...
            [PSCustomObject]@{
                CheckID           = 'DCONF-020'
                CheckName         = 'Minimum Password Length'
                Method            = 'ADSI'
                Domain            = $domainNC
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'DCONF-020'
                adSuiteCheckName  = 'Minimum_Password_Length'
    ...
    $bhFile = Join-Path $bhDir "DCONF-020_$bhTs.json"
    @{
