...
Write-Host "Executing security check: Kerberos Ticket Lifetime" -ForegroundColor Yellow
Write-Host "Check ID: DCONF-015" -ForegroundColor Gray
...
$output = $results | Select-Object `
    @{N='CheckID'; E={'DCONF-015'}}, `
    @{N='CheckName'; E={'Kerberos Ticket Lifetime'}}, `
...
try {
    ...
    foreach ($r in $output) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'DCONF-015'
                adSuiteCheckName  = 'Kerberos_Ticket_Lifetime'
    ...
    $bhFile = Join-Path $bhDir "DCONF-015_$bhTs.json"
    @{
