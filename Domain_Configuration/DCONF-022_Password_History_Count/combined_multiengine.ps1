...
Write-Host "=== DCONF-022: Password History Count ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'DCONF-022'}}, `
            @{N='CheckName'; E={'Password History Count'}}, `
...
if (-not $useADModule) {
    ...
    try {
        ...
        $results = $adsiResults | ForEach-Object {
            ...
            [PSCustomObject]@{
                CheckID           = 'DCONF-022'
                CheckName         = 'Password History Count'
                Method            = 'ADSI'
                Domain            = $domainNC
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'DCONF-022'
                adSuiteCheckName  = 'Password_History_Count'
    ...
    $bhFile = Join-Path $bhDir "DCONF-022_$bhTs.json"
    @{
