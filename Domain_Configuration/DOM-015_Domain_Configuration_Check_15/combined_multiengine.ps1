...
# Check: Domain Configuration Check 15
...
Write-Host "=== Domain Configuration Check 15 ===" -ForegroundColor Cyan
...
try {
    ...
    Get-ADObject -LDAPFilter '(&(objectClass=domainDNS))' `
    ...
    Write-Warning "DOM-015 PowerShell engine: $_"   # Fix R10: no silent catch
}
...
try {
    ...
    $adsiS.Filter   = '(&(objectClass=domainDNS))'
    $adsiS.PageSize = 1000
    ...
    Write-Warning "DOM-015 ADSI engine: $_"          # Fix R10
}
...
try {
    $csFilter  = '(&(objectClass=domainDNS))'
    ...
    Write-Warning "DOM-015 C# engine: $_"             # Fix R10
}
...
Write-Host ""
Write-Host "=== DOM-015: $($uniqueResults.Count) unique findings ===" -ForegroundColor Cyan
$uniqueResults | Format-List

# Fix R14: Fixed 5-field schema enforced before CSV export
$csvPath = Join-Path $env:TEMP "DOM-015_results.csv"
$uniqueResults |
