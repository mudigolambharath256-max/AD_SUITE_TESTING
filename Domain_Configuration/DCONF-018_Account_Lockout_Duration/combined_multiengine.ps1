...
Write-Host "=== DCONF-018: Account Lockout Duration ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'DCONF-018'}}, `
            @{N='CheckName'; E={'Account Lockout Duration'}}, `
...
if (-not $useADModule) {
    ...
    try {
        ...
        $results = $adsiResults | ForEach-Object {
            ...
            [PSCustomObject]@{
                CheckID           = 'DCONF-018'
                CheckName         = 'Account Lockout Duration'
                Method            = 'ADSI'
                Domain            = $domainNC
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'DCONF-018'
                adSuiteCheckName  = 'Account_Lockout_Duration'
    ...
    $bhFile = Join-Path $bhDir "DCONF-018_$bhTs.json"
    @{
