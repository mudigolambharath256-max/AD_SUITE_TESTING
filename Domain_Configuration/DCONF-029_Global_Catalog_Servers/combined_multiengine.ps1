...
Write-Host "=== DCONF-029: Global Catalog Servers ===" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    try {
        ...
        # Execute query
        $adObjects = Get-ADObject -LDAPFilter '(options:1.2.840.113556.1.4.803:=1)' `
        ...
        $results = $adObjects | Select-Object `
            @{N='CheckID'; E={'DCONF-029'}}, `
            @{N='CheckName'; E={'Global Catalog Servers'}}, `
...
if (-not $useADModule) {
    ...
    try {
        ...
        $searcher.Filter = '(options:1.2.840.113556.1.4.803:=1)'
        ...
        $results = $adsiResults | ForEach-Object {
            ...
            [PSCustomObject]@{
                CheckID           = 'DCONF-029'
                CheckName         = 'Global Catalog Servers'
                Method            = 'ADSI'
                Domain            = $domainNC
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'DCONF-029'
                adSuiteCheckName  = 'Global_Catalog_Servers'
    ...
    $bhFile = Join-Path $bhDir "DCONF-029_$bhTs.json"
    @{
