...
Write-Host "=== Multi-Engine Execution: Kerberoastable Accounts with Old Password ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'KRB-019'
                adSuiteCheckName  = 'Kerberoastable_Accounts_with_Old_Password'
    ...
    $bhFile = Join-Path $bhDir "KRB-019_$bhTs.json"
    @{
