...
# Check: Constrained Delegation (Users)
...
Write-Host "=== Multi-Engine Execution: Constrained Delegation (Users) ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        Get-ADObject -LDAPFilter $ldapFilter -Properties $props,userAccountControl,msDS-AllowedToDelegateTo,samAccountName -ErrorAction Stop |
...
public class Program
{
  ...
  {
    string filter = @"(&(objectCategory=person)(objectClass=user)(!(userAccountControl:1.2.840.113556.1.4.803:=2))(msDS-AllowedToDelegateTo=*))";
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'KRB-006'
                adSuiteCheckName  = 'Constrained_Delegation_Users'
    ...
    $bhFile = Join-Path $bhDir "KRB-006_$bhTs.json"
    @{
