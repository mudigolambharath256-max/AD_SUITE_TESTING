...
# Check: Sensitive Accounts Without AES
...
Write-Host "=== Multi-Engine Execution: Sensitive Accounts Without AES ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        $ldapFilter = '(&(objectCategory=person)(objectClass=user)(!(userAccountControl:1.2.840.113556.1.4.803:=2))(adminCount=1)(!(msDS-SupportedEncryptionTypes:1.2.840.113556.1.4.803:=24)))'
        $props = @('name', 'distinguishedName', 'samAccountName', 'msDS-SupportedEncryptionTypes')

        Get-ADObject -LDAPFilter $ldapFilter -Properties $props,adminCount,userAccountControl,msDS-SupportedEncryptionTypes,samAccountName -ErrorAction Stop |
          Select-Object name, distinguishedName, samAccountName, msDS-SupportedEncryptionTypes |
          Sort-Object name |
          ForEach-Object { $_ }
...
try {
    $adsiResults = @(
        ...
        $searcher = [ADSISearcher]'(&(objectCategory=person)(objectClass=user)(!(userAccountControl:1.2.840.113556.1.4.803:=2))(adminCount=1)(!(msDS-SupportedEncryptionTypes:1.2.840.113556.1.4.803:=24)))'
        $searcher.PageSize = 1000
        $searcher.PropertiesToLoad.Clear()
        (@('name', 'distinguishedName', 'samAccountName', 'msDS-SupportedEncryptionTypes', 'adminCount', 'userAccountControl') | ForEach-Object { [void]$searcher.PropertiesToLoad.Add($_) }

        $searcher.FindAll() | ForEach-Object {
          $p = $_.Properties
          [PSCustomObject]@{
            Label = 'Sensitive Accounts Without AES'
            Name = $p['name'][0]
...
public class Program
{
  ...
  {
    string filter = @"(&(objectCategory=person)(objectClass=user)(!(userAccountControl:1.2.840.113556.1.4.803:=2))(adminCount=1)(!(msDS-SupportedEncryptionTypes:1.2.840.113556.1.4.803:=24)))";
    string[] props = new string[] { "name", "distinguishedName", "samAccountName", "msDS-SupportedEncryptionTypes" };
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'KRB-027'
                adSuiteCheckName  = 'Sensitive_Accounts_Without_AES'
    ...
    $bhFile = Join-Path $bhDir "KRB-027_$bhTs.json"
    @{
