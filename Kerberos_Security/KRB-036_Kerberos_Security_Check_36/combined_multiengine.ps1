...
# Check: Kerberos Security Check 36
...
Write-Host "=== Kerberos Security Check 36 ===" -ForegroundColor Cyan
...
try {
    ...
    Write-Warning "KRB-036 PowerShell engine: $_"   # Fix R10: no silent catch
}
...
try {
    ...
    Write-Warning "KRB-036 ADSI engine: $_"          # Fix R10
}
...
try {
    ...
    Write-Warning "KRB-036 C# engine: $_"             # Fix R10
}
...
Write-Host ""
Write-Host "=== KRB-036: $($uniqueResults.Count) unique findings ===" -ForegroundColor Cyan
$uniqueResults | Format-List

# Fix R14: Fixed 5-field schema enforced before CSV export
$csvPath = Join-Path $env:TEMP "KRB-036_results.csv"
$uniqueResults |
