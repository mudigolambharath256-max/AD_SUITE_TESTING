...
Write-Host "=== Multi-Engine Execution: DES-Only Kerberos (Computers) ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        $props = @('name', 'distinguishedName', 'samAccountName', 'userAccountControl', 'operatingSystem')

        Get-ADObject -LDAPFilter $ldapFilter -Properties $props,userAccountControl -ErrorAction Stop |
          Select-Object name, distinguishedName, samAccountName, userAccountControl, operatingSystem |
          Sort-Object name |
          ForEach-Object { $_ }
...
public class Program
{
  ...
  {
    string filter = @"(&(objectCategory=computer)(!(userAccountControl:1.2.840.113556.1.4.803:=2))(userAccountControl:1.2.840.113556.1.4.803:=2097152))";
    string[] props = new string[] { "name", "distinguishedName", "samAccountName", "userAccountControl", "operatingSystem" };
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'KRB-010'
                adSuiteCheckName  = 'DES-Only_Kerberos_Computers'
    ...
    $bhFile = Join-Path $bhDir "KRB-010_$bhTs.json"
    @{
