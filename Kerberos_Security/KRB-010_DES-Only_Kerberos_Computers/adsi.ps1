...
@('name', 'distinguishedName', 'samAccountName', 'userAccountControl', 'operatingSystem', 'objectSid') | ForEach-Object { [void]$searcher.PropertiesToLoad.Add($_) }
...
try {
    ...
    if ($results -and $results.Count -gt 0) {
        ...
        $bhFile = Join-Path $bhDir "KRB-010_nodes.json"
    ...
    Write-Warning "[BloodHound] Export failed: # Check: DES-Only Kerberos (Computers)
# Category: Kerberos Security
# Severity: critical
# ID: KRB-010
# Requirements: None
