# ============================================================================
# KRB-035: S4U2Proxy Abuse Detection - Multi-Engine
# ============================================================================

[CmdletBinding()]
param([Parameter()][string]$SearchBase, [Parameter()][string]$ExportPath)

Write-Host "[KRB-035] S4U2Proxy Abuse Detection" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    $results = Get-ADObject -LDAPFilter '(msDS-AllowedToDelegateTo=*)' -Properties name,distinguishedName,msDS-AllowedToDelegateTo -SearchBase $SearchBase -SearchScope Subtree -ResultSetSize $null -ErrorAction SilentlyContinue
} else {
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                adSuiteCheckName  = 'S4U2Proxy_Abuse_Detection'
    ...
    $bhFile = Join-Path $bhDir "KRB-035_$bhTs.json"
    @{
