...
class Program
{
    ...
    static void Main()
    {
        string filter = @"(&(objectCategory=computer)(servicePrincipalName=*))";
        ...
        try
        {
            using (DirectoryEntry rootEntry = new DirectoryEntry("LDAP://RootDSE"))
            {
                ...
                using (DirectorySearcher searcher = new DirectorySearcher(searchEntry))
                {
                    ...
                    {
                        Console.WriteLine("KRB-005: found " + results.Count + " objects");
        ...
        {
            Console.Error.WriteLine("KRB-005 error: " + ex.Message);
            Environment.Exit(1);
        }
    }
}
