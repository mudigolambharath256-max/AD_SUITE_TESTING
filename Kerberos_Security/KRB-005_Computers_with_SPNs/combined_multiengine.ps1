...
Write-Host "=== Computers with SPNs ===" -ForegroundColor Cyan
...
try {
    ...
    Get-ADObject -LDAPFilter '(&(objectCategory=computer)(servicePrincipalName=*))' `
                 -Properties @('name','distinguishedName','samAccountName','servicePrincipalName', 'objectSid') `
                 -SearchBase $targetNC `
                 -ErrorAction Stop |
    ForEach-Object {
    ...
    Write-Warning "KRB-005 PowerShell engine: $_"   # Fix R10: no silent catch
}
...
try {
    ...
    $adsiS.Filter   = '(&(objectCategory=computer)(servicePrincipalName=*))'
    $adsiS.PageSize = 1000
    $adsiS.PropertiesToLoad.Clear()
    @('name','distinguishedName','samAccountName','servicePrincipalName', 'objectSid') | ForEach-Object { [void]$adsiS.PropertiesToLoad.Add($_) }
    ...
    Write-Warning "KRB-005 ADSI engine: $_"          # Fix R10
}
...
try {
    $csFilter  = '(&(objectCategory=computer)(servicePrincipalName=*))'
    $csProps   = @("name","distinguishedName","samAccountName","servicePrincipalName")   # PS array of double-quoted strings
    $csCheckId = 'KRB-005'
    ...
    Write-Warning "KRB-005 C# engine: $_"             # Fix R10
}
...
Write-Host ""
Write-Host "=== KRB-005: $($uniqueResults.Count) unique findings ===" -ForegroundColor Cyan
$uniqueResults | Format-List

# Fix R14: Fixed 5-field schema enforced before CSV export
$csvPath = Join-Path $env:TEMP "KRB-005_results.csv"
$uniqueResults |
