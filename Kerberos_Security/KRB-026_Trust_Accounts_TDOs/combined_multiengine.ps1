...
Write-Host "=== Multi-Engine Execution: Trust Accounts (TDOs) ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        $props = @('name', 'distinguishedName', 'cn', 'trustDirection', 'trustType', 'trustAttributes', 'msDS-SupportedEncryptionTypes')

        Get-ADObject -LDAPFilter $ldapFilter -Properties $props -ErrorAction Stop |
          Select-Object name, distinguishedName, cn, trustDirection, trustType, trustAttributes, msDS-SupportedEncryptionTypes |
          Sort-Object name |
          ForEach-Object { $_ }
...
public class Program
{
  public static void Run()
  {
    ...
    string[] props = new string[] { "name", "distinguishedName", "cn", "trustDirection", "trustType", "trustAttributes", "msDS-SupportedEncryptionTypes" };
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'KRB-026'
                adSuiteCheckName  = 'Trust_Accounts_TDOs'
    ...
    $bhFile = Join-Path $bhDir "KRB-026_$bhTs.json"
    @{
