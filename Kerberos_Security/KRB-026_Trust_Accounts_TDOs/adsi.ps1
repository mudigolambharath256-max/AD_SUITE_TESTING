...
@('name', 'distinguishedName', 'cn', 'trustDirection', 'trustType', 'trustAttributes', 'msDS-SupportedEncryptionTypes', 'objectSid', 'samAccountName') | ForEach-Object { [void]$searcher.PropertiesToLoad.Add($_) }
...
try {
    ...
    if ($results -and $results.Count -gt 0) {
        ...
        $bhFile = Join-Path $bhDir "KRB-026_nodes.json"
    ...
    Write-Warning "[BloodHound] Export failed: # Check: Trust Accounts (TDOs)
# Category: Kerberos Security
# Severity: info
# ID: KRB-026
# Requirements: None
