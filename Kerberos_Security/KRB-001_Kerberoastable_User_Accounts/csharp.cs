...
class Program
{
    ...
    static void Main()
    {
        string filter = @"(&(objectClass=user)(!(userAccountControl:1.2.840.113556.1.4.803:=2))(servicePrincipalName=*))";
        ...
        try
        {
            using (DirectoryEntry rootEntry = new DirectoryEntry("LDAP://RootDSE"))
            {
                ...
                using (DirectorySearcher searcher = new DirectorySearcher(searchEntry))
                {
                    ...
                    {
                        Console.WriteLine("KRB-001: found " + results.Count + " objects");
        ...
        {
            Console.Error.WriteLine("KRB-001 error: " + ex.Message);
            Environment.Exit(1);
        }
    }
}
