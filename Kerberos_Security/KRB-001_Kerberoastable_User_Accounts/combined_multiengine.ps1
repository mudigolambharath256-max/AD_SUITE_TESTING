...
Write-Host "=== Kerberoastable User Accounts ===" -ForegroundColor Cyan
...
try {
    ...
    Get-ADObject -LDAPFilter '(&(objectClass=user)(!(userAccountControl:1.2.840.113556.1.4.803:=2))(servicePrincipalName=*))' `
                 -Properties @('name','distinguishedName','samAccountName','servicePrincipalName','pwdLastSet', 'objectSid') `
                 -SearchBase $targetNC `
                 -ErrorAction Stop |
    ForEach-Object {
    ...
    Write-Warning "KRB-001 PowerShell engine: $_"   # Fix R10: no silent catch
}
...
try {
    ...
    $adsiS.Filter   = '(&(objectClass=user)(!(userAccountControl:1.2.840.113556.1.4.803:=2))(servicePrincipalName=*))'
    $adsiS.PageSize = 1000
    $adsiS.PropertiesToLoad.Clear()
    @('name','distinguishedName','samAccountName','servicePrincipalName','pwdLastSet', 'objectSid') | ForEach-Object { [void]$adsiS.PropertiesToLoad.Add($_) }
    ...
    Write-Warning "KRB-001 ADSI engine: $_"          # Fix R10
}
...
try {
    $csFilter  = '(&(objectClass=user)(!(userAccountControl:1.2.840.113556.1.4.803:=2))(servicePrincipalName=*))'
    $csProps   = @("name","distinguishedName","samAccountName","servicePrincipalName","pwdLastSet")   # PS array of double-quoted strings
    $csCheckId = 'KRB-001'
    ...
    Write-Warning "KRB-001 C# engine: $_"             # Fix R10
}
...
Write-Host ""
Write-Host "=== KRB-001: $($uniqueResults.Count) unique findings ===" -ForegroundColor Cyan
$uniqueResults | Format-List

# Fix R14: Fixed 5-field schema enforced before CSV export
$csvPath = Join-Path $env:TEMP "KRB-001_results.csv"
$uniqueResults |
