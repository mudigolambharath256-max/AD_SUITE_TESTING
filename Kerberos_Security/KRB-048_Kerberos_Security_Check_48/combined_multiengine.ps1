...
# Check: Kerberos Security Check 48
...
Write-Host "=== Kerberos Security Check 48 ===" -ForegroundColor Cyan
...
try {
    ...
    Write-Warning "KRB-048 PowerShell engine: $_"   # Fix R10: no silent catch
}
...
try {
    ...
    Write-Warning "KRB-048 ADSI engine: $_"          # Fix R10
}
...
try {
    ...
    Write-Warning "KRB-048 C# engine: $_"             # Fix R10
}
...
Write-Host ""
Write-Host "=== KRB-048: $($uniqueResults.Count) unique findings ===" -ForegroundColor Cyan
$uniqueResults | Format-List

# Fix R14: Fixed 5-field schema enforced before CSV export
$csvPath = Join-Path $env:TEMP "KRB-048_results.csv"
$uniqueResults |
