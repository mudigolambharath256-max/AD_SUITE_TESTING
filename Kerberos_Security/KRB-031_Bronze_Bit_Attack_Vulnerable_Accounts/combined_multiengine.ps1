# ============================================================================
# KRB-031: Bronze Bit Attack Vulnerable Accounts - Multi-Engine
# ============================================================================

[CmdletBinding()]
param([Parameter()][string]$SearchBase, [Parameter()][string]$ExportPath)

Write-Host "[KRB-031] Bronze Bit Attack Vulnerable Accounts" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    $results = Get-ADObject -LDAPFilter '(&(objectCategory=person)(objectClass=user)(!(userAccountControl:1.2.840.113556.1.4.803:=2)))' -Properties name,distinguishedName,userAccountControl,samAccountName -SearchBase $SearchBase -SearchScope Subtree -ResultSetSize $null -ErrorAction SilentlyContinue
} else {
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'KRB-031'
                adSuiteCheckName  = 'Bronze_Bit_Attack_Vulnerable_Accounts'
    ...
    $bhFile = Join-Path $bhDir "KRB-031_$bhTs.json"
    @{
