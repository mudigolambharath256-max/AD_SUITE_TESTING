...
Write-Host "KRB-003: found $($results.Count) objects"
...
try {
    ...
    $bhS.Filter   = '(&(objectClass=user)(userAccountControl:1.2.840.113556.1.4.803:=4194304))'
    $bhS.PageSize = 1000
    ...
    foreach ($r in $bhRaw) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                adSuiteCheckName  = 'Users without Pre-Authentication'
    ...
    @{ data = $bhNodes.ToArray()
       meta = @{ type = 'users'; count = $bhNodes.Count; version = 5; methods = 0 }
        ...
        Out-File -FilePath (Join-Path $bhDir "KRB-003_$bhTs.json") -Encoding UTF8 -Force

# Fix R10: No silent catch — warn on failure
} catch { Write-Warning "KRB-003 BloodHound export error: $_" }
