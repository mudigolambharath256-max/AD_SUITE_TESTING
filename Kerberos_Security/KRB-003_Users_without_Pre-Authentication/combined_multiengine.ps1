...
# Check: Users without Pre-Authentication
...
Write-Host "=== Users without Pre-Authentication ===" -ForegroundColor Cyan
...
try {
    ...
    Get-ADObject -LDAPFilter '(&(objectClass=user)(userAccountControl:1.2.840.113556.1.4.803:=4194304))' `
    ...
    Write-Warning "KRB-003 PowerShell engine: $_"   # Fix R10: no silent catch
}
...
try {
    ...
    $adsiS.Filter   = '(&(objectClass=user)(userAccountControl:1.2.840.113556.1.4.803:=4194304))'
    $adsiS.PageSize = 1000
    ...
    Write-Warning "KRB-003 ADSI engine: $_"          # Fix R10
}
...
try {
    $csFilter  = '(&(objectClass=user)(userAccountControl:1.2.840.113556.1.4.803:=4194304))'
    ...
    Write-Warning "KRB-003 C# engine: $_"             # Fix R10
}
...
Write-Host ""
Write-Host "=== KRB-003: $($uniqueResults.Count) unique findings ===" -ForegroundColor Cyan
$uniqueResults | Format-List

# Fix R14: Fixed 5-field schema enforced before CSV export
$csvPath = Join-Path $env:TEMP "KRB-003_results.csv"
$uniqueResults |
