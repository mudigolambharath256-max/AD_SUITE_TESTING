...
# Check: Kerberos Security Check 40
...
Write-Host "=== Kerberos Security Check 40 ===" -ForegroundColor Cyan
...
try {
    ...
    Write-Warning "KRB-040 PowerShell engine: $_"   # Fix R10: no silent catch
}
...
try {
    ...
    Write-Warning "KRB-040 ADSI engine: $_"          # Fix R10
}
...
try {
    ...
    Write-Warning "KRB-040 C# engine: $_"             # Fix R10
}
...
Write-Host ""
Write-Host "=== KRB-040: $($uniqueResults.Count) unique findings ===" -ForegroundColor Cyan
$uniqueResults | Format-List

# Fix R14: Fixed 5-field schema enforced before CSV export
$csvPath = Join-Path $env:TEMP "KRB-040_results.csv"
$uniqueResults |
