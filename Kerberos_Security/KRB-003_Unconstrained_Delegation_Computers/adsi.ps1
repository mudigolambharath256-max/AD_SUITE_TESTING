...
(@('name', 'distinguishedName', 'samAccountName', 'operatingSystem', 'userAccountControl', 'primaryGroupID', 'objectSid') | ForEach-Object { [void]$searcher.PropertiesToLoad.Add($_) }
...
try {
    ...
    if ($results -and $results.Count -gt 0) {
        ...
        $bhFile = Join-Path $bhDir "KRB-003_nodes.json"
    ...
    Write-Warning "[BloodHound] Export failed: # Check: Unconstrained Delegation (Computers)
# Category: Kerberos Security
# Severity: critical
# ID: KRB-003
# Requirements: None
