...
# Check: Unconstrained Delegation (Computers)
...
Write-Host "=== Multi-Engine Execution: Unconstrained Delegation (Computers) ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        Get-ADObject -LDAPFilter $ldapFilter -Properties $props,primaryGroupID,userAccountControl -ErrorAction Stop |
...
try {
    $adsiResults = @(
        ...
        $searcher.FindAll() | ForEach-Object {
          $p = $_.Properties
          [PSCustomObject]@{
            Label = 'Unconstrained Delegation (Computers)'
            Name = $p['name'][0]
...
public class Program
{
  ...
  {
    string filter = @"(&(objectCategory=computer)(!(userAccountControl:1.2.840.113556.1.4.803:=2))(userAccountControl:1.2.840.113556.1.4.803:=524288)(!(primaryGroupID=516)))";
    string[] props = new string[] { "name", "distinguishedName", "samAccountName", "operatingSystem", "userAccountControl" };
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'KRB-003'
                adSuiteCheckName  = 'Unconstrained_Delegation_Computers'
    ...
    $bhFile = Join-Path $bhDir "KRB-003_$bhTs.json"
    @{
