...
# Check: Users with DES Encryption
...
Write-Host "=== Users with DES Encryption ===" -ForegroundColor Cyan
...
try {
    ...
    Get-ADObject -LDAPFilter '(&(objectClass=user)(userAccountControl:1.2.840.113556.1.4.803:=2097152))' `
    ...
    Write-Warning "KRB-002 PowerShell engine: $_"   # Fix R10: no silent catch
}
...
try {
    ...
    $adsiS.Filter   = '(&(objectClass=user)(userAccountControl:1.2.840.113556.1.4.803:=2097152))'
    $adsiS.PageSize = 1000
    ...
    Write-Warning "KRB-002 ADSI engine: $_"          # Fix R10
}
...
try {
    $csFilter  = '(&(objectClass=user)(userAccountControl:1.2.840.113556.1.4.803:=2097152))'
    ...
    Write-Warning "KRB-002 C# engine: $_"             # Fix R10
}
...
Write-Host ""
Write-Host "=== KRB-002: $($uniqueResults.Count) unique findings ===" -ForegroundColor Cyan
$uniqueResults | Format-List

# Fix R14: Fixed 5-field schema enforced before CSV export
$csvPath = Join-Path $env:TEMP "KRB-002_results.csv"
$uniqueResults |
