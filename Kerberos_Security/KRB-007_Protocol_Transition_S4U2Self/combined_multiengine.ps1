...
Write-Host "=== Multi-Engine Execution: Protocol Transition (S4U2Self) ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        $ldapFilter = '(&(!(userAccountControl:1.2.840.113556.1.4.803:=2))(userAccountControl:1.2.840.113556.1.4.803:=16777216))'
        $props = @('name', 'distinguishedName', 'samAccountName', 'objectClass', 'msDS-AllowedToDelegateTo')

        Get-ADObject -LDAPFilter $ldapFilter -Properties $props,userAccountControl -ErrorAction Stop |
          Select-Object name, distinguishedName, samAccountName, objectClass, msDS-AllowedToDelegateTo |
          Sort-Object name |
          ForEach-Object { $_ }
...
public class Program
{
  ...
  {
    string filter = @"(&(!(userAccountControl:1.2.840.113556.1.4.803:=2))(userAccountControl:1.2.840.113556.1.4.803:=16777216))";
    string[] props = new string[] { "name", "distinguishedName", "samAccountName", "objectClass", "msDS-AllowedToDelegateTo" };
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'KRB-007'
                adSuiteCheckName  = 'Protocol_Transition_S4U2Self'
    ...
    $bhFile = Join-Path $bhDir "KRB-007_$bhTs.json"
    @{
