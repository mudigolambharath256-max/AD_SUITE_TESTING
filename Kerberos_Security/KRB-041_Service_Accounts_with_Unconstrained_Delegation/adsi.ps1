...
(@('name', 'distinguishedName', 'whenCreated', 'whenChanged', 'servicePrincipalName', 'userAccountControl', 'samAccountName', 'objectSid') | ForEach-Object { [void]$searcher.PropertiesToLoad.Add($_)
}

Write-Host "[KRB-041] Service Accounts with Unconstrained Delegation" -ForegroundColor Cyan
Write-Host "Severity: CRITICAL | Risk: 10/10" -ForegroundColor Gray
...
$results | ForEach-Object {
    ...
    [PSCustomObject]@{
        CheckID = 'KRB-041'
        CheckName = 'Service Accounts with Unconstrained Delegation'
...
try {
    ...
    if ($results -and $results.Count -gt 0) {
        ...
        $bhFile = Join-Path $bhDir "KRB-041_nodes.json"
