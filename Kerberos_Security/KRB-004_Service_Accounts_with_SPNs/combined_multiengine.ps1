...
try {
    ...
    Get-ADObject -LDAPFilter '(&(objectClass=user)(servicePrincipalName=*)(!(cn=krbtgt)))' `
    ...
    Write-Warning "KRB-004 PowerShell engine: $_"   # Fix R10: no silent catch
}
...
try {
    ...
    $adsiS.Filter   = '(&(objectClass=user)(servicePrincipalName=*)(!(cn=krbtgt)))'
    $adsiS.PageSize = 1000
    ...
    Write-Warning "KRB-004 ADSI engine: $_"          # Fix R10
}
...
try {
    $csFilter  = '(&(objectClass=user)(servicePrincipalName=*)(!(cn=krbtgt)))'
    ...
    Write-Warning "KRB-004 C# engine: $_"             # Fix R10
}
...
Write-Host ""
Write-Host "=== KRB-004: $($uniqueResults.Count) unique findings ===" -ForegroundColor Cyan
$uniqueResults | Format-List

# Fix R14: Fixed 5-field schema enforced before CSV export
$csvPath = Join-Path $env:TEMP "KRB-004_results.csv"
$uniqueResults |
