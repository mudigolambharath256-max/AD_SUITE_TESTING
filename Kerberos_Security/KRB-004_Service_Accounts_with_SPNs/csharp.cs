...
class Program
{
    ...
    static void Main()
    {
        string filter = @"(&(objectClass=user)(servicePrincipalName=*)(!(cn=krbtgt)))";
        ...
        try
        {
            using (DirectoryEntry rootEntry = new DirectoryEntry("LDAP://RootDSE"))
            {
                ...
                using (DirectorySearcher searcher = new DirectorySearcher(searchEntry))
                {
                    ...
                    {
                        Console.WriteLine("KRB-004: found " + results.Count + " objects");
        ...
        {
            Console.Error.WriteLine("KRB-004 error: " + ex.Message);
            Environment.Exit(1);
        }
    }
}
