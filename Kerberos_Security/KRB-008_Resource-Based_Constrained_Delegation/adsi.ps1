...
(@('name', 'distinguishedName', 'samAccountName', 'objectClass', 'msDS-AllowedToActOnBehalfOfOtherIdentity', 'userAccountControl', 'objectSid') | ForEach-Object { [void]$searcher.PropertiesToLoad.Add($_) }
...
try {
    ...
    if ($results -and $results.Count -gt 0) {
        ...
        $bhFile = Join-Path $bhDir "KRB-008_nodes.json"
    ...
    Write-Warning "[BloodHound] Export failed: # Check: Resource-Based Constrained Delegation
# Category: Kerberos Security
# Severity: high
# ID: KRB-008
# Requirements: None
