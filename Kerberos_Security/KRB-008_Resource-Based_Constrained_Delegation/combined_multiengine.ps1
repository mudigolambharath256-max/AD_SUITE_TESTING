...
Write-Host "=== Multi-Engine Execution: Resource-Based Constrained Delegation ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        $ldapFilter = '(&(!(userAccountControl:1.2.840.113556.1.4.803:=2))(msDS-AllowedToActOnBehalfOfOtherIdentity=*))'
        $props = @('name', 'distinguishedName', 'samAccountName', 'objectClass', 'msDS-AllowedToActOnBehalfOfOtherIdentity')

        Get-ADObject -LDAPFilter $ldapFilter -Properties $props,userAccountControl,msDS-AllowedToActOnBehalfOfOtherIdentity -ErrorAction Stop |
          Select-Object name, distinguishedName, samAccountName, objectClass, msDS-AllowedToActOnBehalfOfOtherIdentity |
          Sort-Object name |
          ForEach-Object { $_ }
...
try {
    $adsiResults = @(
        ...
        $searcher = [ADSISearcher]'(&(!(userAccountControl:1.2.840.113556.1.4.803:=2))(msDS-AllowedToActOnBehalfOfOtherIdentity=*))'
        $searcher.PageSize = 1000
        $searcher.PropertiesToLoad.Clear()
        (@('name', 'distinguishedName', 'samAccountName', 'objectClass', 'msDS-AllowedToActOnBehalfOfOtherIdentity', 'userAccountControl') | ForEach-Object { [void]$searcher.PropertiesToLoad.Add($_) }
...
public class Program
{
  ...
  {
    string filter = @"(&(!(userAccountControl:1.2.840.113556.1.4.803:=2))(msDS-AllowedToActOnBehalfOfOtherIdentity=*))";
    string[] props = new string[] { "name", "distinguishedName", "samAccountName", "objectClass", "msDS-AllowedToActOnBehalfOfOtherIdentity" };
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'KRB-008'
                adSuiteCheckName  = 'Resource-Based_Constrained_Delegation'
    ...
    $bhFile = Join-Path $bhDir "KRB-008_$bhTs.json"
    @{
