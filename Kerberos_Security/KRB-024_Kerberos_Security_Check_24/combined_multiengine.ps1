...
# Check: Kerberos Security Check 24
...
Write-Host "=== Kerberos Security Check 24 ===" -ForegroundColor Cyan
...
try {
    ...
    Write-Warning "KRB-024 PowerShell engine: $_"   # Fix R10: no silent catch
}
...
try {
    ...
    Write-Warning "KRB-024 ADSI engine: $_"          # Fix R10
}
...
try {
    ...
    Write-Warning "KRB-024 C# engine: $_"             # Fix R10
}
...
Write-Host ""
Write-Host "=== KRB-024: $($uniqueResults.Count) unique findings ===" -ForegroundColor Cyan
$uniqueResults | Format-List

# Fix R14: Fixed 5-field schema enforced before CSV export
$csvPath = Join-Path $env:TEMP "KRB-024_results.csv"
$uniqueResults |
