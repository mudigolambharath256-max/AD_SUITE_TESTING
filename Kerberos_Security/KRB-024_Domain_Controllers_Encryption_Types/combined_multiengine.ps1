...
Write-Host "=== Multi-Engine Execution: Domain Controllers Encryption Types ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        $props = @('name', 'distinguishedName', 'samAccountName', 'msDS-SupportedEncryptionTypes', 'dNSHostName')

        Get-ADObject -LDAPFilter $ldapFilter -Properties $props,userAccountControl -ErrorAction Stop |
          Select-Object name, distinguishedName, samAccountName, msDS-SupportedEncryptionTypes, dNSHostName |
          Sort-Object name |
          ForEach-Object { $_ }
...
public class Program
{
  public static void Run()
  {
    ...
    string[] props = new string[] { "name", "distinguishedName", "samAccountName", "msDS-SupportedEncryptionTypes", "dNSHostName" };
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'KRB-024'
                adSuiteCheckName  = 'Domain_Controllers_Encryption_Types'
    ...
    $bhFile = Join-Path $bhDir "KRB-024_$bhTs.json"
    @{
