...
(@('name', 'distinguishedName', 'samAccountName', 'msDS-SupportedEncryptionTypes', 'dNSHostName', 'userAccountControl', 'objectSid') | ForEach-Object { [void]$searcher.PropertiesToLoad.Add($_) }
...
try {
    ...
    if ($results -and $results.Count -gt 0) {
        ...
        $bhFile = Join-Path $bhDir "KRB-024_nodes.json"
    ...
    Write-Warning "[BloodHound] Export failed: # Check: Domain Controllers Encryption Types
# Category: Kerberos Security
# Severity: info
# ID: KRB-024
# Requirements: None
