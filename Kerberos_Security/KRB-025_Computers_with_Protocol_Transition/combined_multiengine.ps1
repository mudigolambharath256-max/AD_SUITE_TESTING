...
Write-Host "=== Multi-Engine Execution: Computers with Protocol Transition ===" -ForegroundColor Cyan
Write-Host ""
...
public class Program
{
  ...
  {
    string filter = @"(&(objectCategory=computer)(!(userAccountControl:1.2.840.113556.1.4.803:=2))(userAccountControl:1.2.840.113556.1.4.803:=16777216))";
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'KRB-025'
                adSuiteCheckName  = 'Computers_with_Protocol_Transition'
    ...
    $bhFile = Join-Path $bhDir "KRB-025_$bhTs.json"
    @{
