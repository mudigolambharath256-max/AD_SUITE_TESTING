...
# Check: Accounts Not Delegated (Privileged)
...
Write-Host "=== Multi-Engine Execution: Accounts Not Delegated (Privileged) ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    $adsiResults = @(
        ...
        $searcher.FindAll() | ForEach-Object {
          $p = $_.Properties
          [PSCustomObject]@{
            Label = 'Accounts Not Delegated (Privileged)'
            Name = $p['name'][0]
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'KRB-015'
                adSuiteCheckName  = 'Accounts_Not_Delegated_Privileged'
    ...
    $bhFile = Join-Path $bhDir "KRB-015_$bhTs.json"
    @{
