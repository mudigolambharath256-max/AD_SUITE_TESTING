...
@('name', 'distinguishedName', 'maxPwdAge', 'lockoutDuration', 'lockoutThreshold', 'msDS-SupportedEncryptionTypes', 'objectSid', 'samAccountName') | ForEach-Object { [void]$searcher.PropertiesToLoad.Add($_) }
...
try {
    ...
    if ($results -and $results.Count -gt 0) {
        ...
        $bhFile = Join-Path $bhDir "KRB-014_nodes.json"
    ...
    Write-Warning "[BloodHound] Export failed: # Check: Domain Kerberos Policy
# Category: Kerberos Security
# Severity: info
# ID: KRB-014
# Requirements: None
