...
Write-Host "=== Multi-Engine Execution: Domain Kerberos Policy ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        $props = @('name', 'distinguishedName', 'maxPwdAge', 'lockoutDuration', 'lockoutThreshold', 'msDS-SupportedEncryptionTypes')

        Get-ADObject -LDAPFilter $ldapFilter -Properties $props -ErrorAction Stop |
          Select-Object name, distinguishedName, maxPwdAge, lockoutDuration, lockoutThreshold, msDS-SupportedEncryptionTypes |
          Sort-Object name |
          ForEach-Object { $_ }
...
public class Program
{
  public static void Run()
  {
    ...
    string[] props = new string[] { "name", "distinguishedName", "maxPwdAge", "lockoutDuration", "lockoutThreshold", "msDS-SupportedEncryptionTypes" };
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'KRB-014'
                adSuiteCheckName  = 'Domain_Kerberos_Policy'
    ...
    $bhFile = Join-Path $bhDir "KRB-014_$bhTs.json"
    @{
