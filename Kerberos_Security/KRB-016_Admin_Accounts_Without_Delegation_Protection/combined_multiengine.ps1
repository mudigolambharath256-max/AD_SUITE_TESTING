...
# Check: Admin Accounts Without Delegation Protection
...
Write-Host "=== Multi-Engine Execution: Admin Accounts Without Delegation Protection ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        $ldapFilter = '(&(objectCategory=person)(objectClass=user)(!(userAccountControl:1.2.840.113556.1.4.803:=2))(adminCount=1)(!(userAccountControl:1.2.840.113556.1.4.803:=1048576)))'
        $props = @('name', 'distinguishedName', 'samAccountName', 'userAccountControl', 'memberOf')

        Get-ADObject -LDAPFilter $ldapFilter -Properties $props,adminCount,userAccountControl,samAccountName -ErrorAction Stop |
          Select-Object name, distinguishedName, samAccountName, userAccountControl, memberOf |
          Sort-Object name |
          ForEach-Object { $_ }
...
try {
    $adsiResults = @(
        ...
        $searcher = [ADSISearcher]'(&(objectCategory=person)(objectClass=user)(!(userAccountControl:1.2.840.113556.1.4.803:=2))(adminCount=1)(!(userAccountControl:1.2.840.113556.1.4.803:=1048576)))'
        $searcher.PageSize = 1000
        $searcher.PropertiesToLoad.Clear()
        (@('name', 'distinguishedName', 'samAccountName', 'userAccountControl', 'memberOf', 'adminCount') | ForEach-Object { [void]$searcher.PropertiesToLoad.Add($_) }

        $searcher.FindAll() | ForEach-Object {
          $p = $_.Properties
          [PSCustomObject]@{
            Label = 'Admin Accounts Without Delegation Protection'
            Name = $p['name'][0]
...
public class Program
{
  ...
  {
    string filter = @"(&(objectCategory=person)(objectClass=user)(!(userAccountControl:1.2.840.113556.1.4.803:=2))(adminCount=1)(!(userAccountControl:1.2.840.113556.1.4.803:=1048576)))";
    string[] props = new string[] { "name", "distinguishedName", "samAccountName", "userAccountControl", "memberOf" };
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'KRB-016'
                adSuiteCheckName  = 'Admin_Accounts_Without_Delegation_Protection'
    ...
    $bhFile = Join-Path $bhDir "KRB-016_$bhTs.json"
    @{
