...
# Check: Unconstrained Delegation (Users)
...
Write-Host "=== Multi-Engine Execution: Unconstrained Delegation (Users) ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    $adsiResults = @(
        ...
        $searcher.FindAll() | ForEach-Object {
          $p = $_.Properties
          [PSCustomObject]@{
            Label = 'Unconstrained Delegation (Users)'
            Name = $p['name'][0]
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'KRB-004'
                adSuiteCheckName  = 'Unconstrained_Delegation_Users'
    ...
    $bhFile = Join-Path $bhDir "KRB-004_$bhTs.json"
    @{
