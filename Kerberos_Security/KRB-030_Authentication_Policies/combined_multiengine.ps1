...
Write-Host "=== Multi-Engine Execution: Authentication Policies ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        $ldapFilter = '(objectClass=msDS-AuthNPolicy)'
        $props = @('name', 'distinguishedName', 'cn', 'msDS-UserAllowedToAuthenticateTo')

        Get-ADObject -LDAPFilter $ldapFilter -Properties $props -ErrorAction Stop |
          Select-Object name, distinguishedName, cn, msDS-UserAllowedToAuthenticateTo |
          Sort-Object name |
          ForEach-Object { $_ }
...
public class Program
{
  ...
  {
    string filter = @"(objectClass=msDS-AuthNPolicy)";
    string[] props = new string[] { "name", "distinguishedName", "cn", "msDS-UserAllowedToAuthenticateTo" };
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'KRB-030'
                adSuiteCheckName  = 'Authentication_Policies'
    ...
    $bhFile = Join-Path $bhDir "KRB-030_$bhTs.json"
    @{
