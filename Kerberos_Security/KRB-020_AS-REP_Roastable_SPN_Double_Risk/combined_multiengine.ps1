...
# Check: AS-REP Roastable + SPN (Double Risk)
...
Write-Host "=== Multi-Engine Execution: AS-REP Roastable + SPN (Double Risk) ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        $ldapFilter = '(&(objectCategory=person)(objectClass=user)(!(userAccountControl:1.2.840.113556.1.4.803:=2))(servicePrincipalName=*)(userAccountControl:1.2.840.113556.1.4.803:=4194304))'
        $props = @('name', 'distinguishedName', 'samAccountName', 'servicePrincipalName', 'userAccountControl')
        ...
        Get-ADObject -LDAPFilter $ldapFilter -Properties $props,servicePrincipalName,userAccountControl,samAccountName -ErrorAction Stop |
          Select-Object name, distinguishedName, samAccountName, servicePrincipalName, userAccountControl |
          Sort-Object name |
          ForEach-Object { $_ }
...
try {
    $adsiResults = @(
        ...
        $searcher = [ADSISearcher]'(&(objectCategory=person)(objectClass=user)(!(userAccountControl:1.2.840.113556.1.4.803:=2))(servicePrincipalName=*)(userAccountControl:1.2.840.113556.1.4.803:=4194304))'
        ...
        $searcher.FindAll() | ForEach-Object {
          $p = $_.Properties
          [PSCustomObject]@{
            Label = 'AS-REP Roastable + SPN (Double Risk)'
            Name = $p['name'][0]
...
public class Program
{
  ...
  {
    string filter = @"(&(objectCategory=person)(objectClass=user)(!(userAccountControl:1.2.840.113556.1.4.803:=2))(servicePrincipalName=*)(userAccountControl:1.2.840.113556.1.4.803:=4194304))";
    string[] props = new string[] { "name", "distinguishedName", "samAccountName", "servicePrincipalName", "userAccountControl" };
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'KRB-020'
                adSuiteCheckName  = 'AS-REP_Roastable_SPN_Double_Risk'
    ...
    $bhFile = Join-Path $bhDir "KRB-020_$bhTs.json"
    @{
