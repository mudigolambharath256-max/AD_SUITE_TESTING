...
# Check: Kerberos Security Check 20
...
Write-Host "=== Kerberos Security Check 20 ===" -ForegroundColor Cyan
...
try {
    ...
    Write-Warning "KRB-020 PowerShell engine: $_"   # Fix R10: no silent catch
}
...
try {
    ...
    $adsiS.Filter   = '(&(objectClass=user)(servicePrincipalName=*))'
    $adsiS.PageSize = 1000
    ...
    Write-Warning "KRB-020 ADSI engine: $_"          # Fix R10
}
...
try {
    $csFilter  = '(&(objectClass=user)(servicePrincipalName=*))'
    ...
    Write-Warning "KRB-020 C# engine: $_"             # Fix R10
}
...
Write-Host ""
Write-Host "=== KRB-020: $($uniqueResults.Count) unique findings ===" -ForegroundColor Cyan
$uniqueResults | Format-List

# Fix R14: Fixed 5-field schema enforced before CSV export
$csvPath = Join-Path $env:TEMP "KRB-020_results.csv"
$uniqueResults |
