...
(@('name', 'distinguishedName', 'samAccountName', 'objectClass', 'userAccountControl', 'msDS-SupportedEncryptionTypes', 'objectSid') | ForEach-Object { [void]$searcher.PropertiesToLoad.Add($_) }
...
try {
    ...
    if ($results -and $results.Count -gt 0) {
        ...
        $bhFile = Join-Path $bhDir "KRB-012_nodes.json"
    ...
    Write-Warning "[BloodHound] Export failed: # Check: Accounts Without Explicit Encryption Types
# Category: Kerberos Security
# Severity: medium
# ID: KRB-012
# Requirements: None
