...
Write-Host "=== Multi-Engine Execution: Accounts Without Explicit Encryption Types ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        $ldapFilter = '(&(objectCategory=person)(objectClass=user)(!(userAccountControl:1.2.840.113556.1.4.803:=2))(!(msDS-SupportedEncryptionTypes=*)))'
        $props = @('name', 'distinguishedName', 'samAccountName', 'objectClass')

        Get-ADObject -LDAPFilter $ldapFilter -Properties $props,userAccountControl,msDS-SupportedEncryptionTypes,samAccountName -ErrorAction Stop |
          Select-Object name, distinguishedName, samAccountName, objectClass |
          Sort-Object name |
          ForEach-Object { $_ }
...
public class Program
{
  ...
  {
    string filter = @"(&(objectCategory=person)(objectClass=user)(!(userAccountControl:1.2.840.113556.1.4.803:=2))(!(msDS-SupportedEncryptionTypes=*)))";
    string[] props = new string[] { "name", "distinguishedName", "samAccountName", "objectClass" };
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'KRB-012'
                adSuiteCheckName  = 'Accounts_Without_Explicit_Encryption_Types'
    ...
    $bhFile = Join-Path $bhDir "KRB-012_$bhTs.json"
    @{
