...
Write-Host "=== Multi-Engine Execution: Accounts Supporting DES Encryption ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        $ldapFilter = '(&(!(userAccountControl:1.2.840.113556.1.4.803:=2))(msDS-SupportedEncryptionTypes:1.2.840.113556.1.4.803:=3))'
        $props = @('name', 'distinguishedName', 'samAccountName', 'msDS-SupportedEncryptionTypes')

        Get-ADObject -LDAPFilter $ldapFilter -Properties $props,userAccountControl,msDS-SupportedEncryptionTypes -ErrorAction Stop |
...
public class Program
{
  ...
  {
    string filter = @"(&(!(userAccountControl:1.2.840.113556.1.4.803:=2))(msDS-SupportedEncryptionTypes:1.2.840.113556.1.4.803:=3))";
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'KRB-011'
                adSuiteCheckName  = 'Accounts_Supporting_DES_Encryption'
    ...
    $bhFile = Join-Path $bhDir "KRB-011_$bhTs.json"
    @{
