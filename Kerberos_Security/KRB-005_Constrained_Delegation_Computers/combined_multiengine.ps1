...
# Check: Constrained Delegation (Computers)
...
Write-Host "=== Multi-Engine Execution: Constrained Delegation (Computers) ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $psResults = @(
        ...
        Get-ADObject -LDAPFilter $ldapFilter -Properties $props,userAccountControl,msDS-AllowedToDelegateTo -ErrorAction Stop |
...
try {
    $adsiResults = @(
        ...
        $searcher.FindAll() | ForEach-Object {
          $p = $_.Properties
          [PSCustomObject]@{
            Label = 'Constrained Delegation (Computers)'
            Name = $p['name'][0]
...
public class Program
{
  ...
  {
    string filter = @"(&(objectCategory=computer)(!(userAccountControl:1.2.840.113556.1.4.803:=2))(msDS-AllowedToDelegateTo=*))";
    string[] props = new string[] { "name", "distinguishedName", "samAccountName", "msDS-AllowedToDelegateTo", "operatingSystem" };
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'KRB-005'
                adSuiteCheckName  = 'Constrained_Delegation_Computers'
    ...
    $bhFile = Join-Path $bhDir "KRB-005_$bhTs.json"
    @{
