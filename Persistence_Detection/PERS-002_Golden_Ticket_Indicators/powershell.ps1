...
Write-Host "[PERS-002] Golden Ticket Indicators" -ForegroundColor Cyan
Write-Host "Severity: CRITICAL | Risk: 10/10 | MITRE: T1558.001" -ForegroundColor Gray

$results = Get-ADObject -LDAPFilter '(samAccountName=krbtgt)' `
...
$output = $results | Select-Object `
    @{N='CheckID'; E={'PERS-002'}}, `
    @{N='CheckName'; E={'Golden Ticket Indicators'}}, `
    @{N='RiskScore'; E={10}}, `
...
try {
    ...
    foreach ($r in $output) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'PERS-002'
                adSuiteCheckName  = 'Golden_Ticket_Indicators'
    ...
    $bhFile = Join-Path $bhDir "PERS-002_$bhTs.json"
    @{
