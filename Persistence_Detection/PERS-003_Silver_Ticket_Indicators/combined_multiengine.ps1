# ============================================================================
# PERS-003: Silver Ticket Indicators - Multi-Engine
# ============================================================================

[CmdletBinding()]
param([Parameter()][string]$SearchBase, [Parameter()][string]$ExportPath)

Write-Host "[PERS-003] Silver Ticket Indicators" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    $results = Get-ADObject -LDAPFilter '(servicePrincipalName=*)' -Properties name,distinguishedName,servicePrincipalName -SearchBase $SearchBase -SearchScope Subtree -ResultSetSize $null -ErrorAction SilentlyContinue
} else {
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'PERS-003'
                adSuiteCheckName  = 'Silver_Ticket_Indicators'
    ...
    $bhFile = Join-Path $bhDir "PERS-003_$bhTs.json"
    @{
