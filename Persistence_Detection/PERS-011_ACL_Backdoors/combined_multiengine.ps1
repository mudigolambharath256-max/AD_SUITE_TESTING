# ============================================================================
# PERS-011: ACL Backdoors - Multi-Engine
# ============================================================================

[CmdletBinding()]
param([Parameter()][string]$SearchBase, [Parameter()][string]$ExportPath)

Write-Host "[PERS-011] ACL Backdoors" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    $results = Get-ADObject -LDAPFilter '(|(objectClass=user)(objectClass=group)(objectClass=computer))' -Properties name,distinguishedName,samAccountName -SearchBase $SearchBase -SearchScope Subtree -ResultSetSize $null -ErrorAction SilentlyContinue
} else {
    $root = [ADSI]'LDAP://RootDSE'
    $searcher = New-Object System.DirectoryServices.DirectorySearcher
    $searcher.SearchRoot = [ADSI]"LDAP://$($root.defaultNamingContext)"
    $searcher.Filter = '(|(objectClass=user)(objectClass=group)(objectClass=computer))'
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'PERS-011'
                adSuiteCheckName  = 'ACL_Backdoors'
    ...
    $bhFile = Join-Path $bhDir "PERS-011_$bhTs.json"
    @{
