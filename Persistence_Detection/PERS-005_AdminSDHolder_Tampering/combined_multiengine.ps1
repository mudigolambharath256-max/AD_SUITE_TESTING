# ============================================================================
# PERS-005: AdminSDHolder Tampering - Multi-Engine
# ============================================================================

[CmdletBinding()]
param([Parameter()][string]$SearchBase, [Parameter()][string]$ExportPath)

Write-Host "[PERS-005] AdminSDHolder Tampering" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    $results = Get-ADObject -LDAPFilter '(distinguishedName=CN=AdminSDHolder,CN=System,*)' -Properties name,distinguishedName -SearchBase $SearchBase -SearchScope Subtree -ResultSetSize $null -ErrorAction SilentlyContinue
} else {
    $root = [ADSI]'LDAP://RootDSE'
    $searcher = New-Object System.DirectoryServices.DirectorySearcher
    $searcher.SearchRoot = [ADSI]"LDAP://$($root.defaultNamingContext)"
    $searcher.Filter = '(distinguishedName=CN=AdminSDHolder,CN=System,*)'
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'PERS-005'
                adSuiteCheckName  = 'AdminSDHolder_Tampering'
    ...
    $bhFile = Join-Path $bhDir "PERS-005_$bhTs.json"
    @{
