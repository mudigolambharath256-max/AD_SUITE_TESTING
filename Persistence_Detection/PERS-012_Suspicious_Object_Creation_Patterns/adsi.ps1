...
(@('name', 'distinguishedName', 'whenCreated', 'whenChanged', 'samAccountName', 'objectSid') | ForEach-Object { [void]$searcher.PropertiesToLoad.Add($_)
}

Write-Host "[PERS-012] Suspicious Object Creation Patterns" -ForegroundColor Cyan
Write-Host "Severity: MEDIUM | Risk: 6/10" -ForegroundColor Gray
...
$results | ForEach-Object {
    ...
    [PSCustomObject]@{
        CheckID = 'PERS-012'
        CheckName = 'Suspicious Object Creation Patterns'
...
try {
    ...
    if ($results -and $results.Count -gt 0) {
        ...
        $bhFile = Join-Path $bhDir "PERS-012_nodes.json"
