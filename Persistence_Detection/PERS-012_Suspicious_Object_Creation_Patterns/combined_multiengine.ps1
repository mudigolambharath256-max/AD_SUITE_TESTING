# ============================================================================
# PERS-012: Suspicious Object Creation Patterns - Multi-Engine
# ============================================================================

[CmdletBinding()]
param([Parameter()][string]$SearchBase, [Parameter()][string]$ExportPath)

Write-Host "[PERS-012] Suspicious Object Creation Patterns" -ForegroundColor Cyan
...
if ($useADModule) {
    ...
    $results = Get-ADObject -LDAPFilter '(&(objectCategory=person)(objectClass=user)(whenCreated>=20000101000000.0Z))' -Properties name,distinguishedName,samAccountName -SearchBase $SearchBase -SearchScope Subtree -ResultSetSize $null -ErrorAction SilentlyContinue
} else {
    $root = [ADSI]'LDAP://RootDSE'
    $searcher = New-Object System.DirectoryServices.DirectorySearcher
    $searcher.SearchRoot = [ADSI]"LDAP://$($root.defaultNamingContext)"
    $searcher.Filter = '(&(objectCategory=person)(objectClass=user)(whenCreated>=20000101000000.0Z))'
...
try {
    ...
    foreach ($r in $uniqueResults) {
        ...
        $bhNodes.Add(@{
            ...
            Properties       = @{
                ...
                enabled           = $true
                adSuiteCheckId    = 'PERS-012'
                adSuiteCheckName  = 'Suspicious_Object_Creation_Patterns'
    ...
    $bhFile = Join-Path $bhDir "PERS-012_$bhTs.json"
    @{
